...
OpenXML requires strict element order in `RunProperties`. Wrong order causes Schema validation errors.
...
new Paragraph(
    new ParagraphProperties(
        new BookmarkStart { Id = "100", Name = "Figure1" }  // Wrong!
    ),
...
**Use `Folio.cs` and `Blueprint.cs` as API pattern references.** These files demonstrate correct OpenXML API usage (class names, property names, constructor patterns). When writing code:

✓ Correct approach:

1. Find the corresponding API pattern in blueprint files
2. Reference its API call structure (class names, property assignments, element ordering)
3. Adapt content and **document structure** to match user requirements — blueprints are API cookbooks, not mandatory document templates

❌ Wrong approach:

- Recall API names from memory and write directly
- Infer property names from "common sense"
- Use properties not found in any blueprint example

**Document structure is flexible.** Each function in the blueprints (cover, TOC, body, table, header/footer) is an independent building block. Select and combine them based on the document's actual needs — skip any that don't apply.

### Table Creation Checklist

- [ ] Based on `ComposeFinancialTable()` or `ComposeKpiTable()` pattern?
...
**For comprehensive best practices, see `blueprints/Folio.cs` header comments:**

- **Unit Conversions**: pt/Twips/EMU/cm formulas
- **Paper Sizes**: A3/A4/A5/Letter/Legal (Twips & EMU)
- **Table Management**: Width types, cell merging, fixed layout
- **TOC & Outline**: Field codes, OutlineLevel, bookmarks
- **Cross-References**: REF/PAGEREF fields, footnotes
- **Fonts**: RunFonts properties, Chinese fonts
- **Paragraphs**: Spacing, indentation, heading properties
- **Sections**: Break types, columns, page orientation

**Helper methods available in Folio.cs:**
`ComposePortraitPageSize`, `ComposeLandscapePageSize`, `ComposePercentTableWidth`, `ComposeTocFieldStart`, `ComposeFieldCode`, `ComposeFonts`, `ComposeFontSize`, `ComposeRunProps`, `ComposeSpacing`, `ComposeIndent`, `ComposePageBreak`, `ComposeSectionProps`, `ComposePageMargin`, `PtToTwips`, `TwipsToEmu`, `CmToTwips`
