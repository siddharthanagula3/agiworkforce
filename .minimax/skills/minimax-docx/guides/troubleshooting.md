...
**禁止用超链接列表模拟目录。** 必须用 `FieldChar(Begin) → FieldCode(" TOC \\o \"1-3\" ") → FieldChar(Separate) → ... → FieldChar(End)` 结构，见 `Blueprint.cs:ComposeTocSegment()`。
...
**`python3 engine/forge.py produce` reads from `{WORKSPACE}/.foundry/` directory.**

Paths are dynamically resolved. Run `python3 engine/forge.py status` to see current paths.

```bash
# Standard workflow
1. Edit file: {WORKSPACE}/.foundry/Launcher.cs
2. Build: python3 engine/forge.py produce output/doc.docx
3. Verify: pandoc {WORKSPACE}/output/doc.docx -t plain

# Backup if needed (optional)
cp {WORKSPACE}/.foundry/Launcher.cs {WORKSPACE}/backup/
```
...
**Reference:** See `Blueprint.cs` and `Folio.cs` for examples using Continuous section breaks with different column configurations.
...
// ❌ Wrong: NextPage creates a new page
new SectionProperties(
...
| CS1501     | Wrong argument count        | Check correct signature in Blueprint.cs    |
...
**See `blueprints/Palettes.cs` for all colour scheme definitions.**

---

## 9. Schema Auto-Fix Limitations

The `python3 engine/forge.py produce` auto-fixes element order issues, but **cannot fix**:
