...
- [x] **P2 one TS ai-client — DONE 2026-07-09:** gateway `llm.ts`+`cloudChat.ts` on `packages/providers`; 6 new OpenAI-compat adapters + all 11 wired; one shared `streamFromProvider`; retired `/api/llm/v2/chat`+`/completion`; web v1 route + tool-loop + agents/execute fully on the `ADAPTER_PROVIDERS` table for all 12 providers, byte-stable; `apps/web/lib/llm-providers` (4,721 LOC) DELETED with zero importers. Only the desktop Rust `core/llm` engine remains (Wave 5 c2, live-gated). Migration surfaced+fixed ~11 latent bugs.
- [x] **P3 UI layering — mechanical migration DONE 2026-07-09:** web on `@agiworkforce/ui`; `unified-chat` on `ui`+`design-tokens`; one renderer set + BYOK dialog; 18 shadcn primitives ported; web's `@shared/ui` fork migrated onto `@agiworkforce/ui` (25 forks repointed+deleted, ~99 importer files, −1,702 lines) + 16 dead leaf forks deleted + barrel cleaned; full web suite 3733/3733. RESIDUAL RESOLVED 2026-07-09: `packages/ui` gained the a11y/error props additively (Button `isLoading`, Input/Textarea `hasError`+`errorMessageId`, Card polymorphic `as`, Alert success/warning + aria-live, Slider `valueLabel`, Dialog `hideCloseButton`+aria-modal) with pkg tests; the 8 divergent web forks deleted and ~200 deep imports across 101 files repointed; desktop typecheck 0 proves backward-compat; one real form-submit regression (`System.tsx` untyped in-form Button now `type="submit"` by pkg default) audited and fixed. STILL WEB-LOCAL BY DESIGN: `form.tsx` react-hook-form integration, `sidebar` island, bespoke/aceternity components, legacy imperative toast (0 real call sites, sonner already canonical/mounted — entangled type-coupled subsystem left for a standalone deletion pass). Queued: human visual-QA pass on the canonical-look deltas (button/input sizing, glass dialog, Tailwind v4 `outline-hidden`/`shadow-xs`, untyped in-button lucide icons render 24px vs 16px).
- [x] **P4/Wave 5 Rust engine — all six CLI-side stages DONE 2026-07-09** (`docs/plans/rust-engine-extraction-2026-07-09.md`): [x] (f) sandbox-policy renamed; [x] (b1) ts-rs codegen wired (`check:protocol-types`); [x] (c1) `agiworkforce-llm` crate + CLI facade (byte-identical JSONL); [x] (a) desktop adopts `agiworkforce-execpolicy` (338-eval same-or-stricter corpus); [x] (d1) `agiworkforce-mcp` crate + CLI facade (sim harness); [x] (e1) `agiworkforce-agent-core` crate + CLI TurnHostAdapter (JSONL byte-identical). REMAINING (desktop-side adoption of the shared crates + b2, gated on live-provider + desktop-device smoke): (c2/c3/c4) desktop onto agiworkforce-llm runners; (d2) desktop onto agiworkforce-mcp; (e2) desktop local-chat loop onto agiworkforce-agent-core; (b2) Op TS derive + first generated-type adoption. Verification boundary for e1's verbatim-moved bulk: `RUST-AGENTCORE-LIVE-TURN-VERIFY-01`.
- [x] **P5 data (done + verified 2026-07-09):** web row types derive from cloud-contracts; gateway real RLS via data-layer for policied tables (`SVC-GATEWAY-RLS-NOOP-01` → partially fixed, deploy probe recorded); shared sync-apply engine + cross-language golden fixtures (desktop Rust replay green).
