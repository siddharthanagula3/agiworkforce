...
- [ ] **P4/Wave 5 Rust engine:** [x] `sandbox-policy` renamed; [x] ts-rs codegen wired (`check:protocol-types`); [~] desktop adopts `agiworkforce-execpolicy` (in flight). REMAINING (staged in `docs/plans/rust-engine-extraction-2026-07-09.md`, gated on live-provider + desktop-device smoke): extract `agiworkforce-llm` (stage c), `agiworkforce-mcp` (d), `agiworkforce-agent-core` (e); desktop links `protocol` transitively via mcp; add `Op` envelope TS derive + first generated-type adoption shim (b2).
