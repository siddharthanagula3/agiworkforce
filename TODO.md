...
- [ ] **F01 - packages/types/src/models.json:39** - Hallucination / fake implementation. The OpenAI provider default and routing use `gpt-5.5`; the same file defines `gpt-5.5` as `apiModelId` at `packages/types/src/models.json:647`. Official OpenAI model docs checked on 2026-06-03 list GPT-5.2, GPT-5.1, GPT-5, GPT-5 mini/nano, and Codex variants, but not `gpt-5.5`. Because `apps/cli/src/model_catalog.rs:32` includes this shared catalog, the unsupported ID can leak into CLI/provider calls. Replace with verified provider IDs or make `gpt-5.5` an internal display alias mapped before API calls.
