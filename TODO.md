...
- [x] **P3 UI layering (packages side done 2026-07-09):** web on `@agiworkforce/ui`; `unified-chat` on `ui`+`design-tokens`; one renderer set + BYOK dialog; 18 shadcn primitives ported. REMAINING (deferred follow-on, never big-bang): migrate web's `@shared/ui` 116-importer + `@shared/components/ui` 14-importer trees onto `packages/ui` batch-by-batch; web adopts the newly-ported primitives; decide `form.tsx` FormField collision + legacy imperative toast retirement.
