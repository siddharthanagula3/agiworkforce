...
- [x] **P2 one TS ai-client (mostly done 2026-07-09):** gateway `llm.ts`+`cloudChat.ts` on `packages/providers`; 6 new OpenAI-compat adapters + all 11 wired; one shared `streamFromProvider` for mobile/extensions; retired `/api/llm/v2/chat`+`/completion`. REMAINING: web v1 route off `apps/web/lib/llm-providers/` (in flight — needs canonical Anthropic/Google server-tool chunks + thinking/effort round-trip; additive to packages/types + providers + llm-normalize).
