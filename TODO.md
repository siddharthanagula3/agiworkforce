...
- [x] **P3 UI layering — mechanical migration DONE 2026-07-09:** web on `@agiworkforce/ui`; `unified-chat` on `ui`+`design-tokens`; one renderer set + BYOK dialog; 18 shadcn primitives ported; web's `@shared/ui` fork migrated onto `@agiworkforce/ui` (25 forks repointed+deleted, ~99 importer files, −1,702 lines) + 16 dead leaf forks deleted + barrel cleaned; full web suite 3733/3733. REMAINING = DECISION-GATED residual only (surfaced, not forced): (a) enhance `packages/ui` primitives with the a11y/error features web's Button `isLoading`/Input `hasError`/Card polymorphic-`as`/Dialog `hideCloseButton`/Alert success+warning/Slider `valueLabel`/ScrollArea variants have, then migrate those 8; (b) `form.tsx` react-hook-form `FormField` name collision with pkg `FormField`; (c) legacy imperative toast (`toast`/`toaster`/`use-toast`) vs sonner retirement; (d) bespoke/aceternity components + the shadcn `sidebar` island — stay web-local or move to a `packages/ui/marketing` subpath. Plus a human visual-QA pass on the force-migrated deltas (progress bar height, alert-dialog glass redesign, accordion animation, Tailwind v4 `outline-hidden`/`rounded-xs`/`shadow-xs` renames — all adopting the canonical pkg look).
