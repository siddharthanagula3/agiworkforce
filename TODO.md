...
These are called by frontend but not registered in current Tauri `invoke_handler` sweep:

- [x] `google_batch_calculate_cost`
- [x] `google_batch_cancel`
- [x] `google_batch_create`
- [x] `google_batch_create_embeddings`
- [x] `google_batch_create_images`
- [x] `google_batch_create_jsonl`
- [x] `google_batch_delete`
- [x] `google_batch_get`
- [x] `google_batch_get_embeddings`
- [x] `google_batch_get_results`
- [x] `google_batch_list`
- [ ] `write_file` (currently appears only as doc/example reference in frontend utility comments)

All google_batch commands have been implemented and registered in the Tauri invoke handler.
Action required for write_file:
- Either implement + register each command end-to-end, or remove/disable frontend callsites with explicit user-facing fallback errors (no silent failures).

Evidence for google_batch:
- `apps/desktop/src-tauri/src/sys/commands/google_batch.rs` (new file)
- `apps/desktop/src-tauri/src/sys/commands/mod.rs` (registration)

### 3.2 Email tool completeness

- [x] Implement backend support for email move operation (move/copy between folders), then wire frontend `moveMessage` to real command.
- [ ] Add tests for move + delete + mark read command parity.
...
- [ ] `AUDIT-STREAM-021` Stop/cancel gap: `chat_stop_generation` sets a global stop flag but active tool execution in `execute_chat_tool_with_timeout` does not observe that flag while waiting.
...
- [ ] `AUDIT-STREAM-022` Dual status channels are not unified: chat tool UI uses `chat:tool-*` artifact path, while cancellation logic in reset relies on `activeToolStreams` populated from `agi:tool_stream`.
...
- [ ] `AUDIT-UI-023` File read inline renderer mismatch: `file_read` is mapped to `InlineCodeDiff`, but normalization does not map `{ path, content }` into diff/read shape.
...
- [ ] `AUDIT-UI-024` No dedicated inline renderer mapping for `file_list` / MCP filesystem list tools.
...
- [ ] `AUDIT-STREAM-027` Streaming tool-loop fallback text is appended server-side without emitting a matching `chat:stream-chunk`.
...
- [ ] `AUDIT-STREAM-032` Stream target resolution falls back to “last assistant/current streaming” when conversation/message match is missing.
...
- [ ] `AUDIT-STREAM-033` `chat:stream-end` and `chat:stream-error` clear global loading/tool timeouts even when no target message is resolved.
...
- [ ] `AUDIT-UI-034` Chat tool cards can stay visually stuck in running state because message metadata status is set to `running` on `chat:tool-calls` but not transitioned on `chat:tool-result`.
...
- [ ] `AUDIT-UI-035` Tool card/store linkage gap: chat tool metadata writes `tool_call` but not `actionId`, while `MessageBubble` resolves live tool state from `metadata.actionId`.
...
- [ ] `AUDIT-UI-036` Preview sidecar uses a snapshot of artifact data at click-time (`payload.artifact`) and does not subscribe to later artifact updates.
...
- [ ] `AUDIT-STREAM-038` Stop flag is process-global (`STOP_GENERATION`), not scoped per conversation/message.
...
- [ ] `AUDIT-CAPTURE-039` Native desktop picker activation on macOS is gated by fragile frontend runtime heuristics.
...
- [ ] `AUDIT-CAPTURE-040` macOS capture handlers ignore frontend-provided region/window targets and always use interactive `screencapture` flows.
...
- [ ] `AUDIT-CAPTURE-041` Main-window hide/show during macOS capture is best-effort with ignored errors and no recovery signal.
...
- [ ] `AUDIT-VOICE-042` Voice hook timeout helper does not clear timer handles after promise resolution.
...
- [ ] `AUDIT-VOICE-043` Mode naming/semantics are contradictory (`preferLocalWhisper` drives Whisper Cloud branch).
...
- [ ] `AUDIT-STREAM-045` Core chat send path bypasses hardened IPC wrapper and directly calls `lib/tauri-mock` invoke.
...
- [ ] `AUDIT-APPROVAL-046` MCP/tool confirmation response path is wired to the wrong backend command in the active approval UI.
...
- [ ] `AUDIT-APPROVAL-047` Dead/duplicated tool-confirmation event contracts exist in frontend.
...
- [ ] `AUDIT-TOOLS-048` Chat tool schema source bypasses registry and uses static built-ins, creating schema/runtime drift.
...
- [ ] `AUDIT-APPROVAL-049` Risk-level normalization drops unsupported values into UI model without canonical mapping.
...
- [ ] `AUDIT-MCP-050` Folder selection and MCP filesystem server root are not coupled, producing false-positive “allowed” UX.
...
- [ ] `AUDIT-CONFIG-051` Project-folder permission widening is memory-only in `project_context_set_folder`.
...
- [ ] `AUDIT-UI-052` Tool-call card controls emit unhandled frontend events instead of invoking backend control commands.
...
- [ ] `AUDIT-UI-053` Tool-stream cleanup policy can hide completed/error stream state before message metadata is reconciled.
...
- [ ] `AUDIT-STREAM-056` Tool stream IDs are inconsistent across start/progress/result phases for several tools.
...
- [ ] `AUDIT-EVENT-057` Agent event naming is split between legacy and active contracts in frontend listeners.
...
- [ ] `AUDIT-APPROVAL-058` Approval request `type` contract is drifted (`tool_execution` not part of frontend union).
...
- [ ] `AUDIT-STREAM-059` Chat send flow has no global stream watchdog and intentionally skips `finally` cleanup.
...
- [ ] `AUDIT-CANCEL-060` Tool cancel path is cooperative-only and can miss active subprocess termination for long-running tools.
...
- [ ] `AUDIT-CAPTURE-063` Conversation association for screen capture is wired through a non-existent store method in chat input.
...
- [ ] `AUDIT-ENV-064` Tauri-runtime detection is inconsistent across chat/capture paths.
...
- [ ] `AUDIT-TOOLS-067` `file_list` is exposed to the model but not registered in the production `ToolRegistry`.
...
- [ ] `AUDIT-SIDECAR-082` Preview sidecar binds to a stale artifact snapshot instead of live store/message updates.
...
- [ ] `AUDIT-CAPTURE-089` macOS capture commands ignore frontend-selected region/window arguments and re-enter interactive picker flow.
...
- [ ] `AUDIT-CAPTURE-090` macOS capture flow lacks panic-safe restoration when hiding the main window.
...
- [ ] Not committed yet.
- [ ] Not pushed yet.
- [ ] Commit only after section 3 and section 4 are complete or explicitly deferred by owner.
