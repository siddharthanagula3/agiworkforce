...
} from '@/lib/free-trial-config';
import { FREE_TRIAL_MODELS, FREE_TRIAL_PROMPT_LIMIT } from '@/lib/free-trial-config';
...
export async function reserveFreeTrialPrompt(params: {
  userId: string;
  requestId: string;
  ...
  const rows = await db.query<{ prompt_count: number }>(
    ...
    [params.userId, FREE_TRIAL_PROMPT_LIMIT],
  );
...
export async function refundFreeTrialPrompt(params: {
  userId: string;
  requestId: string;
  reason: string;
}): Promise<void> {
