/**
 * @file THE ACTION ALLOWLIST.
 *
 * A frozen record keyed by a closed union. Nothing acts by inference: an id
 * that is not a key here has no execution path at all, and the executors are
 * reached only through `SUPPORT_ACTIONS[id]`.
 *
 * Every entry is SAFE and REVERSIBLE:
 *   revoke_connector      — reconnect from settings; nothing is deleted
 *   regenerate_api_key    — old key revoked, new key issued with the SAME
 *                           name and the SAME scopes (never escalated)
 *   export_account_data   — a read; produces a download
 *   open_billing_portal   — opens Stripe's portal; changes nothing by itself
 *   resend_verification_email — registered, currently unavailable (see availability.ts)
 *
 * Destructive/irreversible operations are NOT here and are not one flag away
 * from being here — see `excluded.ts`.
 *
 * `describe()` is what the user actually confirms against. It is generated by
 * the SERVER from server-normalized params. The model can only name an
 * `actionId`; it never writes the sentence the user agrees to.
 */
...
export interface SupportActionDefinition<TParams = unknown> {
  id: SupportActionId;
  title: string;
  /** Shown in the action list, before any target is chosen. */
  description: string;
  ...
  /** Per-day ceiling counted over this user's own proposal rows. */
  perDayLimit: number;
  audit: SupportActionAuditBinding;
  /**
   * Only for `execution: 'handoff'`. A CONSTANT — never derived from the model,
   * the client, or a request body.
   */
...
/**
 * Connector ids are validated for SHAPE here and for OWNERSHIP at propose and
 * again at confirm, against the caller's own resolved connector list. A shape
 * check is not an authorization check and is not treated as one.
 */
...
export const SUPPORT_ACTIONS: Readonly<{
  ...
  export_account_data: Object.freeze({
    ...
    // CONSTANT. The existing, already-audited route owns the effect; we do not
    // duplicate it and we do not modify it (it is owned by another workflow).
