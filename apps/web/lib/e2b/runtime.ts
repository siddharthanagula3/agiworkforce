...
export const getE2BExecutor = tracedCodeAction(
  'sandbox_provision',
  async function getE2BExecutor(
    scope?: E2BSessionScope,
    ...
    const provisioningRate = getSandboxComputeMicrousdPerSecond(
      await templateComputeShape(template),
    );
    ...
    const executor: E2BExecutor = {
      ...
      git: {
        ...
        async currentBranch({ path }): Promise<CommandExecutionResult> {
          try {
            const result = await sandbox.commands.run(GIT_CURRENT_BRANCH_COMMAND, {
              cwd: path,
              timeoutMs: E2B_COMMAND_TIMEOUT_MS,
            });
            return commandResult(result.stdout, result.stderr, result.exitCode, result.error);
