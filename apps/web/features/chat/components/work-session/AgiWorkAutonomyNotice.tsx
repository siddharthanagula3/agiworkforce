  ...
  useEffect(() => {
    ...
    void fetchPreferenceNamespace<ToolApprovalPreferences>(
      ...
      .then(async (preferences) => {
        const allowed =
          preferences.defaultPolicy === 'autonomous'
            ? await readAutonomousToolApprovalsAllowed()
            : preferences.defaultPolicy === 'auto_approve_read_only';
        if (!cancelled) setAutoApproves(allowed);
      })
