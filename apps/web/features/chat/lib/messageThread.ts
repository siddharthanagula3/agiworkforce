 ...
 */
export function compareThreadOrder(left: Message, right: Message): number {
  if (left.createdAt !== right.createdAt) return left.createdAt < right.createdAt ? -1 : 1;
  if (left.id === right.id) return 0;
  return left.id < right.id ? -1 : 1;
}

function indexById(rows: readonly Message[]): Map<string, Message> {
  const byId = new Map<string, Message>();
  for (const row of rows) byId.set(row.id, row);
  return byId;
}

function newestChildByParent(rows: readonly Message[]): Map<string | null, Message> {
  const newest = new Map<string | null, Message>();
 ...
 */
export function deepestDescendant(rows: readonly Message[], id: string): string {
 ...
 */
export function resolveLeafForSibling(rows: readonly Message[], siblingId: string): string {
 ...
 */
function deepestLatestChain(rows: readonly Message[]): Message[] {
...
export function resolveVisibleThread(
  ...
  if (!activeLeafId) return rows as Message[];
 ...
 */
export function siblingGroup(rows: readonly Message[], message: Message): SiblingGroup {
 ...
 * `messageRowPropsEqual` from re-rendering the whole transcript per frame.
 */
 ...
 */
export function stampLinearParents(rows: readonly Message[]): Message[] {
  if (rows.length === 0) return rows as Message[];
  ...
  if (assigned.size === 0) return rows as Message[];
  return rows.map((row) => {
...
export function linearTail(rows: readonly Message[]): string | null {
