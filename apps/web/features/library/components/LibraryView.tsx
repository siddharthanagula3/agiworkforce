...
/**
 * LibraryView — the `/library` page body: browse the user's cataloged files
 ...
 *
 * Downloads and image previews go through the authed same-origin
 * `/api/files/{id}` route with session cookies — no public URLs. Failures
...
export function LibraryView() {
  ...
  const handleDownload = useCallback(async (item: LibraryItem) => {
    try {
      const res = await fetch(item.uri, { credentials: 'same-origin' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6" data-testid="library-view">
      <header className="flex flex-col gap-1">
        <h1 className="font-[var(--serif)] text-[28px] font-medium text-[var(--chat-text-primary)]">
          Library
        </h1>
