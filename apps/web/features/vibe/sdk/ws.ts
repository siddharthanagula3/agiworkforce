/**
 * WebSocket connection handler from VibeSDK
 *
 * Copied directly from: https://github.com/nichochar/vibing-ai/blob/main/sdk/src/ws.ts
 */
...
function toWsCloseEvent(ev: CloseEvent | { code?: number; reason?: string }): {
  code: number;
  reason: string;
} {
  return {
  ...
  function normalizeServerPayload(raw: unknown): AgentWsServerMessage | null {
    ...
    if (looksLikeAgentState(state)) {
      return {
        type: 'cf_agent_state',
        state,
      } as unknown as AgentWsServerMessage;
    }
  ...
  function send(msg: AgentWsClientMessage): void {
    ...
    if (pendingSends.length > maxPendingSends) {
      ...
      emitter.emit('ws:error', {
        error: new Error(
          `Message queue overflow: dropped oldest message (queue size: ${maxPendingSends})`,
        ),
      });
    }
  }
