/**
 * SessionStateStore - State management from VibeSDK
 *
 * Copied directly from: https://github.com/nichochar/vibing-ai/blob/main/sdk/src/state.ts
 */
...
function extractPhaseInfo(msg: unknown): {
  name?: string;
  description?: string;
} {
  ...
  applyWsMessage(msg: AgentWsServerMessage): void {
    switch (msg.type) {
      ...
      case 'phase_generating': {
        ...
        this.setState({
          phase: { status: 'generating', ...extractPhaseInfo(m) },
        });
        break;
      }
      case 'phase_generated': {
        ...
        this.setState({
          phase: { status: 'generated', ...extractPhaseInfo(m) },
        });
        break;
      }
      case 'phase_implementing': {
        ...
        this.setState({
          phase: { status: 'implementing', ...extractPhaseInfo(m) },
        });
        break;
      }
      case 'phase_implemented': {
        ...
        this.setState({
          phase: { status: 'implemented', ...extractPhaseInfo(m) },
        });
        break;
      }
      case 'phase_validating': {
        ...
        this.setState({
          phase: { status: 'validating', ...extractPhaseInfo(m) },
        });
        break;
      }
      case 'phase_validated': {
        ...
        this.setState({
          phase: { status: 'validated', ...extractPhaseInfo(m) },
        });
        break;
      }
      ...
      case 'cloudflare_deployment_started': {
        ...
        this.setState({
          cloudflare: { status: 'running', instanceId: m.instanceId },
        });
        break;
      }
