/**
 * BuildSession - Session management from VibeSDK
 *
 * Copied directly from: https://github.com/nichochar/vibing-ai/blob/main/sdk/src/session.ts
 */
...
export class BuildSession {
  ...
  async waitForGenerationStarted(
    options: WaitOptions = {},
  ): Promise<WsMessageOf<'generation_started'>> {
  ...
  async waitForGenerationComplete(
    options: WaitOptions = {},
  ): Promise<WsMessageOf<'generation_complete'>> {
    return await this.waitForMessageType(
      'generation_complete',
      options.timeoutMs ?? this.getDefaultTimeoutMs(),
    );
  ...
  async waitForPreviewDeployed(
    options: WaitOptions = {},
  ): Promise<WsMessageOf<'deployment_completed'>> {
    const timeoutMs = options.timeoutMs ?? this.getDefaultTimeoutMs();
