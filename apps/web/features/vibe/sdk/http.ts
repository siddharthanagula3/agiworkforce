/**
 * HTTP client from VibeSDK
 *
 * Copied directly from: https://github.com/nichochar/vibing-ai/blob/main/sdk/src/http.ts
 */
...
export class HttpClient {
  ...
  private async ensureAccessToken(): Promise<string | undefined> {
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        `HTTP ${resp.status} for /api/auth/exchange-api-key: ${text || resp.statusText}`,
      );
    }
