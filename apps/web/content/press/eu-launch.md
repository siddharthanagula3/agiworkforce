...
3. EU AI Act compliant, privacy-by-architecture, free: AGI Mobile launches on iOS and Android 2026-08-16
...
AGI Mobile covers all of them. The path: Gemma + LiteRT (Google's own canonical local inference stack, recommended by Google for non-Nano Android devices) as primary, Qwen3-4B as a universal fallback for devices with 4 GB+ RAM. Both paths run entirely on-device. No cloud call during inference.
...
- **No inference data transmitted.** The legal basis question for inference data does not arise because inference happens on-device. There is no controller-to-processor transfer during a conversation.
...
- **EU representative.** AGI Automation LLC will appoint an EU representative per Art. 27 GDPR before the 2026-08-16 launch. Name and contact to be published at agiworkforce.com/legal/eu-representative.

---

## The practical privacy advantage for EU users

Most AI assistants — cloud-based — require the user to accept that conversations transit servers in the United States or elsewhere, triggering GDPR Chapter V transfer obligations and creating legal uncertainty post-Schrems III. AGI Mobile v1 avoids this entirely by design: the model runs on the user's handset. There is no inference-time network call. The EU user's conversation is never a packet on the wire.
...
AGI Automation LLC is a Delaware-registered software company building a multi-surface AI agent platform. The platform wraps 10+ AI providers (cloud and local) into a single chat layer across six surfaces: iOS, Android, Desktop (macOS, Windows, Linux), Web, CLI, VS Code extension, and Chrome extension. The company's first product, AGI Mobile, launches globally on 2026-08-16.
...
_This press kit is embargoed until 2026-08-16 00:00 CET. Do not publish before the embargo lifts._
