...
import { isImageJobAttemptDue, runImageGenerationJobAttempt } from './image-job-executor';
...
export async function driveImageGenerationJob(
  ...
  if (job.cancelRequestedAt !== null) return { jobId, status: job.status, skipped: 'cancelled' };
