...
async function handleCreateSchedule(request: NextRequest) {
  ...
  const recurrence = typeof body.recurrence === 'string' && VALID_RECURRENCES.includes(body.recurrence)
    ? body.recurrence
    : 'once';
  ...
  const timeOfDay = typeof body.timeOfDay === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(body.timeOfDay)
    ? body.timeOfDay
    : '09:00';

  // Validate timezone format (basic IANA check)
  const timezone = typeof body.timezone === 'string' && body.timezone.length <= 50 && /^[\w/+-]+$/.test(body.timezone)
    ? body.timezone
    : 'UTC';
