...
async function handleUpdateSchedule(request: NextRequest, context: RouteContext) {
  ...
  if (typeof body.model === 'string' && body.model.length <= 100) updates.model = body.model;
  if (typeof body.recurrence === 'string' && VALID_RECURRENCES.includes(body.recurrence)) {
    updates.recurrence = body.recurrence;
  }
  if (typeof body.cronExpression === 'string') updates.cron_expression = body.cronExpression;
  ...
  if (Array.isArray(body.daysOfWeek)) {
    const validDays = body.daysOfWeek.filter(
  ...
  if (typeof body.timeOfDay === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(body.timeOfDay)) updates.time_of_day = body.timeOfDay;
  if (typeof body.timezone === 'string' && body.timezone.length <= 50 && /^[\w/+-]+$/.test(body.timezone)) updates.timezone = body.timezone;
