...
describe('budget admission control runs before any provider work', () => {
  ...
  it('denies deterministically when the reservation declines the spend', async () => {
    ...
    const result = await processRequest(chatRequest('admission-declined'), {
      ok: true,
      userId: 'user-pro',
      token: 'session-token',
      subscription: proSubscription,
  ...
  it('denies deterministically when a rolling window is exhausted', async () => {
    const { ManagedUsageRequestError } =
      await import('@/lib/services/managed-usage-request-service');
    mocks.reserveManagedUsage.mockRejectedValue(
      new ManagedUsageRequestError(
        'Your rolling weekly usage limit is reached.',
        429,
        'rolling_weekly_limit_reached',
      ),
    );

    const result = await processRequest(chatRequest('admission-weekly'), {
      ok: true,
      userId: 'user-pro',
      token: 'session-token',
      subscription: proSubscription,
    ...
    await expect(result.response.json()).resolves.toMatchObject({
      error: { code: 'rolling_weekly_limit_reached' },
    });
  });

  it('holds a reservation covering the estimate before the request is allowed through', async () => {
    const result = await processRequest(chatRequest('admission-reserved'), {
      ok: true,
      userId: 'user-pro',
      token: 'session-token',
      subscription: proSubscription,
