...
import {
  ...
  releaseCloudAgentInputCheckpoint,
  ...
  type ClaimedCloudAgentInputCheckpoint,
...
function jsonError(message: string, status: number): NextResponse {
  return NextResponse.json(
    { error: { message, type: 'invalid_request_error', code: 'tool_input_invalid' } },
    { status, headers: getSecurityHeaders() },
  );
}

function publicCheckpointMessages(
  messages: ClaimedCloudAgentInputCheckpoint['checkpoint']['messages'],
...
async function releaseClaim(
  ...
  await releaseCloudAgentInputCheckpoint(db, {
    userId,
    ...
    logger.error(
      ...
      'Cloud agent input lease could not be released',
    );
  });
}

function checkpointError(error: unknown): NextResponse | null {
  if (error instanceof CloudAgentInputResponseError) {
    return jsonError('Input responses do not match the paused tool calls.', 400);
  }
  if (error instanceof CloudAgentApprovalCheckpointExpiredError) {
    return jsonError('This input request expired and can no longer be resumed.', 410);
  }
  if (error instanceof CloudAgentApprovalCheckpointNotFoundError) {
    return jsonError('Pending input request not found.', 404);
  }
  if (error instanceof CloudAgentApprovalCheckpointConflictError) {
    return jsonError('This input request is already being resumed.', 409);
  }
  return null;
}

async function handleToolInputResume(request: NextRequest) {
  ...
  let resumeFields;
  try {
    const parsed = ToolInputResumeRequestSchema.safeParse(await request.json());
    if (!parsed.success) return jsonError('Invalid input resume request.', 400);
  ...
  let claim: ClaimedCloudAgentInputCheckpoint;
  try {
    claim = await claimCloudAgentInputCheckpoint(db, {
      userId,
      runId: resumeFields.run_id,
      inputs: resumeFields.tool_inputs.map((entry) => ({
        toolCallId: entry.tool_call_id,
        inputResponses: entry.input_responses,
      })),
      leaseSeconds: 86_400,
    });
  } catch (error) {
      ...
      try {
        ...
        if (processed.managedUsage) {
          await finalizeManagedUsageRequest({
            ...
            logger.error(
              {
                event: 'tool_input_resume_discovery_release_unrecorded',
                error: settlementError,
                userId,
                requestId: processed.requestId,
                runId: claim.checkpoint.runId,
              },
              'Managed input-resume reservation release could not be persisted',
            );
          });
        }
        await releaseClaim(db, userId, claim);
  ...
  // A tool blocked in the permission store since the pause must not run on
  // resume. The tool loop's resume path settles a denied call as an error;
  // logging it here makes the enforcement observable.
  ...
  if (blockedToolCallIds.size > 0) {
    logger.warn(
      { userId, runId: claim.checkpoint.runId, blockedToolCalls: blockedToolCallIds.size },
      'Input resume: a paused tool is now blocked by the user permission store',
    );
  }
  ...
  // claim that proves this user owns the paused run, and none of them are
  // reachable from here. What changed (AGI-39) is that a Workflow-platform
  // outage, or an engaged AGI_DURABLE_INITIAL_TURNS kill-switch, no longer turns
  // an authorized input resume into a 503: the same resume runs request-scoped
  // instead, and only detachability is lost.
  let turn;
  try {
    turn = await runCloudAgentTurn({
      ...
      continuation: {
        ...
        resume: {
          inputResponses: claim.resumptions.map((resumption) => ({
            toolCallId: resumption.toolCallId,
            inputResponses: resumption.inputResponses,
            ...(resumption.requestState ? { requestState: resumption.requestState } : {}),
            round: resumption.round,
          })),
    ...
    // failure. The lease is released so the pause stays claimable and the user
    // can retry rather than being stranded.
    logger.error(
      { error, userId, requestId: processed.requestId, runId: claim.checkpoint.runId },
      'Input continuation could not be started on either transport',
    );
    return NextResponse.json(
      {
        error: {
...
export const POST = withCorsRoute(withErrorHandler(handleToolInputResume));
