...
export function createFailoverPlan(
  ...
  const nextAdmissibleCandidate = (): FailoverAttempt | null => {
    while (remaining.length > 0) {
      ...
      if (options.isCredentialCooling?.({ modelKey: candidate, provider })) {
        logger.warn(
          { requestId: processed.requestId, model: candidate, provider },
          'Managed failover candidate skipped: credential is cooling down',
        );
        continue;
      }
  ...
  const nextFreeLaneCandidate = (): FailoverAttempt | null => {
    ...
    for (;;) {
      ...
      if (options.isCredentialCooling?.({ modelKey: route.modelKey, provider: route.provider })) {
        logger.warn(
          { requestId: processed.requestId, routeId: route.routeId, provider: route.provider },
          'Free-lane failover candidate skipped: credential is cooling down',
        );
        continue;
      }
  ...
  return {
    next: (error: unknown, context?: FailoverStepContext): FailoverAttempt | null => {
      ...
      const resilienceScope = resilienceScopeForCategory(category);
      if (resilienceScope) {
        options.onResilienceObservation?.({
          scope: resilienceScope,
          provider: latestView.provider,
          model: latestView.chatRequest.model,
          routeId: latestRouteId,
