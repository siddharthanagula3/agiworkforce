/**
 * NON-mocked-pipeline integration test for the Deep Research loop against a
 * RECORDED REAL google wire (release-blocker regression, 2026-07-10).
 *
 * The unit tests in research-loop.test.ts mock buildToolLoopStream with
 * hand-built OpenAI-shaped SSE, so they could never catch a bug in the real
 * google translation pipeline -- and one shipped: the live Gemini `alt=sse`
 * wire is CRLF-framed, parseGeminiStream only split LF frames, and every
 * research report came out EMPTY (and unpersisted) despite a working
 * activity feed.
 *
 * This test feeds the loop through the REAL pipeline instead:
 *   recorded live Gemini bytes (CRLF framing preserved byte-for-byte)
 *     -> real parseGeminiStream -> real translateGeminiStream
 *     -> real chunksToOpenAiSse (legacy-web wireMode, real OpenAIWireAssembler)
 *     -> runResearchLoop's collector.
 * Only the network call itself is replaced by the recording; every
 * translation layer between provider bytes and the research loop is real.
 */
...
function recordedBytesStream(): ReadableStream<Uint8Array> {
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      // Deliver in small chunks so CRLF separators split across reads.
      const step = 64;
...
describe('research loop over recorded REAL google wire (CRLF framing)', () => {
  it('streams the synthesized report as delta.content and surfaces the real grounded source', async () => {
    const run = await collectRun(
      runResearchLoop(
        ...
        // 1 gathering round + synthesis; both turns replay the recording.
        { maxIterations: 2, maxSearches: 12 },
      ),
    );

    // The report body MUST come through as normal content deltas -- this is
    // exactly what regressed (empty message body) before the CRLF fix.
    ...
    // The real grounded source flows into the cumulative x_search_results.
    ...
    // Run completes (not error) and terminates the stream exactly once.
  ...
  it('never ends as a silent empty message when synthesis yields no text', async () => {
    // Replay only the STOP frame (no text deltas) for every turn: the loop
    // must emit an honest error body instead of an empty message.
    const stopOnly =
