...
describe('runToolLoop Anthropic dispatch (mocked adapter)', () => {
  ...
  it('surfaces an adapter-level error as inline SSE content and stops -- no [DONE], no second call', async () => {
    ...
    // Matches the pre-existing catch-block UX exactly (lines 552-567): a
    // `break`, not the normal-completion branch -- no [DONE] is emitted, and
    // the loop does not re-invoke the provider for a "step 2".
