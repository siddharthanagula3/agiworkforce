 ...
 * be replaced by it.
 *
 * It is not the only writer that resolves: the client's save merges into the
 * same row and a slow or retried one can land after this, so the message write
 * path resolves too. Whichever of the two writes lands last stores publisher
 * URLs.
 *
...
export async function patchAssistantTurnSourceUrls(params: {
  ...
  try {
    ...
    const patch = await resolveSourceUrlMetadataRedirects(stored, overrides ?? {});
    if (!patch) return;
