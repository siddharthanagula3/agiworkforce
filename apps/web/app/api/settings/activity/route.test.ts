...
describe('GET /api/settings/activity?limit=50', () => {
  it('reads the audit trail through the rls scoped handle, not the schema owner pool', async () => {
    const request = new NextRequest('http://localhost/api/settings/activity?limit=50');
  ...
  it('keeps the caller constrained to their own rows', async () => {
    await GET(new NextRequest('http://localhost/api/settings/activity?limit=50'));
