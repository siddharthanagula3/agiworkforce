-- Down for 0119: restore 0118's static-allowance metering.
--
-- Recreates the `v_overage_used + estimate <= headroom` comparison. Note this
-- reinstates the double-count 0119 removed, so a caller passing the REMAINING
-- balance will refuse users at roughly half their purchased credit. Only roll
-- back alongside a caller that passes a static per-window allowance.
...
begin
  ...
  if exists (
    ...
    from public.reserve_managed_usage_request(
      p_user_id, p_idempotency_key, p_request_hash, p_provider, p_model,
      p_estimated_cost_cents, p_lease_token, p_lease_seconds
    ) legacy;
    return;
  end if;
  ...
  if p_session_cap_cents > 0
    ...
    if v_headroom > 0 and v_overage_used + p_estimated_cost_cents <= v_headroom then
      ...
      return query select 'session_limit'::text, 'declined'::text, null::text,
        p_estimated_cost_cents, null::text, 'ROLLING_FIVE_HOUR_LIMIT_REACHED'::text;
      return;
    end if;
  end if;
  ...
  if p_weekly_cap_cents > 0
    ...
    if v_headroom > 0 and v_overage_used + p_estimated_cost_cents <= v_headroom then
      ...
      return query select 'weekly_limit'::text, 'declined'::text, null::text,
        p_estimated_cost_cents, null::text, 'ROLLING_WEEKLY_LIMIT_REACHED'::text;
      return;
    end if;
  end if;
  ...
  if p_is_flagship
    ...
    if v_headroom > 0 and v_overage_used + p_estimated_cost_cents <= v_headroom then
      ...
      return query select 'flagship_weekly_limit'::text, 'declined'::text, null::text,
        p_estimated_cost_cents, null::text, 'FLAGSHIP_WEEKLY_LIMIT_REACHED'::text;
      return;
    end if;
  end if;
  ...
  if v_reservation.reservation_decision = 'acquired' then
    ...
    set is_flagship = p_is_flagship, updated_at = now()
    where request_row.user_id = p_user_id
      and request_row.idempotency_key = p_idempotency_key
...
delete from public.schema_migrations where filename = '0119_overage_headroom_semantics.sql';
