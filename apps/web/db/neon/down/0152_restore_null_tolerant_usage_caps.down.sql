-- Down for 0152: restore 0119's cap semantics.
--
-- NOT YET APPLIED — draft only, pending explicit approval before running.
--
-- Reinstates BOTH defects 0152 removed: `p_*_cap_cents is null` raises `22023`,
-- so every managed turn from a plan `isPlanUsageUncapped` recognises answers
-- 503 again; and each rolling ceiling is gated on `> 0`, so a cap of exactly
-- zero disables enforcement instead of denying. Roll back only alongside a
-- caller that never passes NULL and has no zero-budget tier — which the
-- TypeScript in `lib/server/managed-usage-policy.ts` is not.
...
delete from public.schema_migrations where filename = '0152_restore_null_tolerant_usage_caps.sql';
