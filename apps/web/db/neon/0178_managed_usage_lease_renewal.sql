-- 0178 : renew a managed usage lease while its turn is still running.
--
-- NOT YET APPLIED : draft only, pending explicit approval before running.
--
-- resolveManagedUsageLeaseSeconds asks for 86400 seconds for any turn that can
-- enter a tool loop, reserve_managed_usage_request clamped every lease to one
-- hour, and no function anywhere updated lease_expires_at after the insert.
-- The only `set lease_expires_at` in the tree before this migration is in 0105,
-- on a different table. A turn that ran past the clamp was therefore reclaimed
-- by recover_stale_managed_usage_requests, refunded to the user as
-- outcome_unknown, and when the real completion landed
-- finalize_managed_usage_request applied first-terminal-wins and returned
-- 'already_finalized'. None of the six finalize call sites branch on that, so
-- nothing surfaced: the provider was paid and the user was not billed, on
-- exactly the long agentic turns the 86400 was written for.
--
-- Two changes, one policy:
--
--   * The initial window may now be up to 6 hours rather than 1, which is what
--     a single long provider call needs before any step can renew anything.
--     Callers still ask for what they want; this is the ceiling on the ask.
--   * Every provider step renews the lease to at least an hour out, bounded
--     absolutely by 24 hours from creation. A live turn keeps its lease for as
--     long as it keeps making provider calls; a wedged one still expires.
--
-- WHY NOT SIMPLY ADMIT THE 86400 ASK: a lease is a promise that nobody else
-- will reclaim the row, and a 24 hour promise taken at reservation time cannot
-- be withdrawn when the turn dies two seconds later. Renewal makes the promise
-- proportional to observed progress, which is the property recovery needs.
-- This is also the ordering constraint on AGI-2: raising recovery cadence
-- before this landed would have turned a once-daily misfire into a frequent
-- one.
--
-- Both function bodies are reproduced whole because Postgres has no partial
-- redefinition. Only the marked hunks differ from 0056 and 0070.
...
comment on function public.reserve_managed_usage_request(
  text, text, text, text, text, integer, text, integer
) is
  'Reserves a managed usage request under an idempotency key, with an initial lease window of up to six hours that every provider step renews.';
...
comment on function public.extend_managed_usage_request_provider_step(
  ...
  'Idempotently reserves each provider operation under the original tenant, request, lease, rolling caps, and billing-period balance, and renews the lease so a turn that is still running is not reclaimed as stale.';
