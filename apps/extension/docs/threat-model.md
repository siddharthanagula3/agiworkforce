...
| Background service worker               | Privileged Chrome owner for tabs, scripting, debugger, alarms, network, and native messaging.                                                                | [`background.ts`](src/background.ts) validates message shape, sender class, tab ownership, origins, request bounds, and action plans before dispatch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `cookies`                                 | The Clerk sync-host handshake reads the user's agiworkforce.com session so the extension acts as the account already signed in on the web. | Session-cookie confidentiality on the configured Clerk and web origins only; no extension code calls `chrome.cookies`, and nothing sets a cookie.                                                                                                                                                                                                                         |
...
| Cross-tab screenshot or DOM exfiltration.                                           | Content-script screenshots bind to the sender tab; DOM mutations reject a different target tab; extension-page captures resolve an explicit/active tab. The `capture_page` command reaches the worker without passing the message router, so it resolves the visible tab through `authorizeBrowserToolTab` ([`tabAuthority.ts`](src/features/browser-tools/tabAuthority.ts)) against the same approved-origin list and reports the refusal rather than capturing. | Screenshots contain raw pixels and extension pages hold broad tab privileges. UI/source confusion is still possible if the user changes tabs.                                                                                                                                                                                                            |
