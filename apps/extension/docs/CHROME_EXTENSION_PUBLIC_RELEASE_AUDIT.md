...
- Bundle composition and the all-sites content script (~140 KB parsed on every page, most of it for features that only run on approved sites).
...
| Service worker  | `src/background.ts` (single ES-module bundle, 143 KB) plus a shared chunk. Owns message dispatch, managed-cloud streaming, native messaging, context menus, alarms, scheduled tasks, computer use (CDP), page watch, downloads, tab groups.                                                                                                                                                                                 |
| Popup           | None. `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })`: the toolbar action opens the side panel.                                                                                                                                                                                                                                                                                                       |
| Side panel      | `src/side_panel.html` + `src/side_panel.ts` (415 KB built). One panel document per Chrome window; the conversation is scoped to the window (`window:<id>`), not to a tab or page. Header (model picker, history, new chat, quota, menu), composer with page chip, slash commands, attachments, autonomy chip, effort/quick controls; drawer with history, automation launchers, tools, pairing, allowlist, memory, account. |
| Content scripts | `src/content.js` (140 KB IIFE) on `http://*/*`, `https://*/*`, top frame only, `document_idle`. On an origin that is not on the approved-sites list it registers the message listener and does nothing else (measured: no DOM injection, no runtime message).                                                                                                                                                               |
...
| Per-site allow / always / decline for actions (B) | Yes, four-way                                         | Yes, plus an operating mode                                                  | Approved sites + per-site Chrome host grant + ask-before-acting                      | Yes       | Two allowlist editors with different semantics (EXT-014).                       |
...
| ~~`cookies`~~                               | Removed in this pass: its only handler had no caller                                      | ,                                                    | ,                                                                                                  | ,                                                             | Not needed    | Removed                                                                                            |
...
| Content bundle                                                                     | 139.6 KB raw / 36.1 KB gzip, IIFE, no DOMPurify, no Sentry                                                                                                                 | MEASURED     |
| Service worker bundle                                                              | 142.9 KB + shared chunk 2,612 KB (Clerk 807 KB source, zod 273 KB, contracts/types 258 KB, cloud-contracts 183 KB)                                                         | MEASURED     |
...
| Permissions              | Every remaining permission has a written justification in `docs/chrome-web-store-listing.md`, frozen by `manifest-contract.test.ts`. `cookies` removed. The all-sites content script is now disclosed as the install warning it produces.                                               |
...
### EXT-014: Two allowlist editors with different semantics

Severity: P2
Area: Consent UX
Surface: Side Panel / Options
Evidence: the drawer writes only `agi_site_allowlist` (`side_panel.ts` `drawerWriteAllowlist`); the options page additionally requests the per-origin Chrome host permission and records browser-control consent, and revokes both on removal. Drawer copy corrected in this pass; the split remains.
Expected: one place to approve a site, or the drawer clearly read-only with a link to Settings.
Actual: a site approved from the drawer cannot run computer use until the user finds Options; a site removed from the drawer keeps its consent record and host grant.
Root Cause: Verified.
Recommended Correction: make the drawer allowlist read-only with "Manage in Settings", or have it call the same `requestBrowserControlHostPermission` flow inside the click gesture.
Verification: approve from the drawer, start computer use, no refusal; remove from the drawer, `chrome://extensions` shows the origin gone.
...
| ------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| EXT-101 | P1       | `cookies` permission with no caller; store justification described a UI that did not exist                                                               | Permission, handler, blocklist, disclosure and listing row removed; dead tab-control handlers removed   |
...
| EXT-109 | P2       | Restricted-page copy told users to add the site to Approved sites (grants no host permission); Web Store not detected                                    | `isRestrictedPageUrl` covers Web Store, `view-source:`, `chrome-untrusted:`, `devtools:`; accurate copy |
...
| EXT-132 | P3       | Quick-mode toggle exposed no state to assistive tech; decorative icons announced                                                                         | `aria-pressed`; `aria-hidden` on `renderIcon`                                                           |
