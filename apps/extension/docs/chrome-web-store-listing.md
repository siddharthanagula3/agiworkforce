...
| `debugger`        | Browser control on approved sites. The DevTools Protocol is what lets the assistant click, type, scroll and navigate in a real page the way a person does, including inside pages that ordinary script injection cannot drive. The same attachment carries the console-message and network-request capture the user turns on from the side panel: message text, request URL, method, status, resource type and timing only, never a request or response body and never a request header. It is used only after the user adds a site AND separately confirms full browser control for that site AND Chrome grants host access for that one origin, and only while Chrome's own debugging bar is showing. Dismissing that bar stops the run and the capture. |
