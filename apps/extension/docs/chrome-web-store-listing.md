...
| `cookies`         | Reading the user's own agiworkforce.com sign-in session so the side panel can use the account they already signed into on the web (the Clerk sync-host handshake). The extension never sets a cookie and never reads cookies from any other site.                                                                                                                                                                                                                                |
| `tabGroups`       | Grouping tabs the assistant opens on the user's behalf so they stay together and are easy to close.                                                                                                                                                                                                                                                                                                                                                                              |
| `downloads`       | Saving a file the user asks the assistant to download from a page they have approved. A download only ever starts from an explicit request, only from the approved page's own origin or another origin already on the user's allowlist, and Chrome picks the filename. The extension reads back only the downloads it started, to show progress and to open the containing folder.                                                                                               |
...
| `content_scripts` match: `http://*/*`, `https://*/*`           | The single install-time warning Chrome shows, "read and change all your data on all websites", comes from this match, not from the host permissions above. One content script loads on every ordinary page so the assistant can read the page the user points it at and, on an approved site, show the in-page panel and run page tools and automation. On every other origin the script adds no UI, sends no startup message and answers no privileged message. |
...
| Website content         | Yes       | Up to 5,000 characters of a page's visible text, with secret-like values redacted, sent to the AGI service only when the user attaches the page, runs a page slash command or the Summarize menu item, or runs an action on an approved site. |
