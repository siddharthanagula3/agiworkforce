...
// ---------------------------------------------------------------------------
// JSON-RPC protocol layer (Google A2A-inspired, minimal)
// ---------------------------------------------------------------------------
//
// This is the thin protocol surface used by the multi-agent coordination
// protocol. Transports (stdio / WebSocket) drive `handle_request`; the
// HTTP server above wraps this for localhost delivery.
//
// Types kept separate from the HTTP-server types above so the protocol
// surface can be tested and evolved independently.
pub mod jsonrpc {
    use serde::{Deserialize, Serialize};
    use std::collections::HashMap;
