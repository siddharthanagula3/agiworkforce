...
impl JsonRpcResponse {
    ...
    fn err(id: Option<serde_json::Value>, code: i32, msg: String) -> Self {
        Self {
            jsonrpc: "2.0".into(),
            id,
            result: None,
    ...
    async fn process(&self, req: JsonRpcRequest) -> JsonRpcResponse {
        match req.method.as_str() {
            "initialize" => JsonRpcResponse::ok(
                req.id,
                serde_json::json!({"capabilities": {"tools": true, "streaming": true}, "serverInfo": {"name": "agiworkforce-app-server", "version": env!("CARGO_PKG_VERSION")}}),
            ),
            "tools/list" => JsonRpcResponse::ok(
                req.id,
                serde_json::json!({"tools": ["read_file","write_file","edit_file","run_command","search_files","list_directory","web_search","web_fetch","apply_patch","grep_files","tool_search","task"]}),
            ),
...
pub async fn run_app_server(config: AppServerConfig) -> Result<()> {
    match config.transport {
        AppServerTransport::Stdio => run_stdio(config).await,
        AppServerTransport::WebSocket { .. } => run_ws(config).await,
    }
}

async fn run_ws(config: AppServerConfig) -> Result<()> {
    let addr = match config.transport {
        AppServerTransport::WebSocket { addr } => addr,
        _ => "127.0.0.1:8787".parse()?,
    };
    ...
    loop {
        ...
        let resp = match method {
            "initialize" => {
                ...
                JsonRpcResponse::ok(
                    id,
                    serde_json::json!({"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "agiworkforce", "version": env!("CARGO_PKG_VERSION")}}),
                )
            }
