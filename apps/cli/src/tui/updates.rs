...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, AGIWORKFORCE_CLI_VERSION).unwrap_or(false) {
...
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/agiworkforce.json";
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/agiworkforce/agiworkforce/releases/latest";
...
fn version_filepath(config: &Config) -> PathBuf {
    config.agiworkforce_home.join(VERSION_FILENAME)
}
...
mod tests {
    ...
    fn extract_version_from_brew_api_json() {
        //
        // https://formulae.brew.sh/api/cask/agiworkforce.json
        let cask_json = r#"{
            "token": "agiworkforce",
            "full_token": "agiworkforce",
            "tap": "homebrew/cask",
            "version": "0.96.0",
        }"#;
