...
use agiworkforce_core::default_client::get_agi_user_agent;
...
fn send_realtime_audio_chunk(
    ...
    tx.send(AppEvent::AgiWorkforceOp(Op::RealtimeConversationAudio(
...
async fn resolve_auth() -> Result<TranscriptionAuthContext, String> {
    ...
    let auth = AgiWorkforceAuth::from_auth_storage(&agiworkforce_home, AuthCredentialsStoreMode::Auto)
        ...
        .ok_or_else(|| "No auth is configured; please run `agi login`".to_string())?;
    ...
    let (endpoint, request) =
        if matches!(auth.mode, AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens) {
            ...
            let mut req = client
                ...
                .multipart(form)
                .header("User-Agent", get_agi_user_agent());
