...
const TUI_NOTIFY_CLIENT: &str = "agiworkforce-tui";

async fn initialize_app_server_client_name(thread: &AgiWorkforceThread) {
...
pub(crate) fn spawn_agent(
    ...
    tokio::spawn(async move {
        let NewThread {
            ...
            Ok(v) => v,
            Err(err) => {
                let message = format!("Failed to initialize agiworkforce: {err}");
                ...
                tracing::error!("failed to initialize agiworkforce: {err}");
                return;
            }
        };
        ...
        let ev = agiworkforce_protocol::protocol::Event {
            // The `id` does not matter for rendering, so we can use a fake value.
            id: "".to_string(),
            msg: agiworkforce_protocol::protocol::EventMsg::SessionConfigured(session_configured),
        };
        app_event_tx_clone.send(AppEvent::CodexEvent(ev));

        let thread_clone = thread.clone();
        tokio::spawn(async move {
            while let Some(op) = agi_op_rx.recv().await {
        ...
        while let Ok(event) = thread.next_event().await {
            ...
            if is_shutdown_complete {
                ...
                // the Arc<AgiWorkforceThread> can be released and thread resources can clean up.
                break;
            }
        }
    });
