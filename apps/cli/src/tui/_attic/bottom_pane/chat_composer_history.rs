...
impl ChatComposerHistory {
    ...
    fn populate_history_at_index(
        ...
        if global_idx >= self.history_entry_count {
            ...
            app_event_tx.send(AppEvent::AgiWorkforceOp(op));
        }
        None
    }
}
...
mod tests {
    ...
    fn navigation_with_async_fetch() {
        ...
        // Verify that an AppEvent::AgiWorkforceOp with the correct GetHistoryEntryRequest was sent.
        let event = rx.try_recv().expect("expected AppEvent to be sent");
        let AppEvent::AgiWorkforceOp(history_request1) = event else {
        ...
        // Verify second AgiWorkforceOp event for offset 1.
        let event2 = rx.try_recv().expect("expected second event");
        let AppEvent::AgiWorkforceOp(history_request_2) = event2 else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
