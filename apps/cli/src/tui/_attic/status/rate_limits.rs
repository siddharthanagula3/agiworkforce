...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `agiworkforce` or `agi_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "agiworkforce".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("agiworkforce");
        ...
        let combine_non_agi_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_agi_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_agi_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_agi_single_limit_renders_combined_row() {
        let now = Local::now();
        let agi_primary = RateLimitSnapshotDisplay {
            limit_name: "agiworkforce".to_string(),
            captured_at: now,
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "agiworkforce-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[agi_primary, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "agiworkforce-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_primary_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "agiworkforce-other".to_string(),
            captured_at: now,
        ...
        assert_eq!(
            labels,
            vec![
                "agiworkforce-other limit".to_string(),
                "1h limit".to_string(),
