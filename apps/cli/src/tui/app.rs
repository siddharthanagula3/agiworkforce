...
use agiworkforce_core::models_manager::manager::RefreshStrategy;
#[cfg(target_os = "windows")]
...
use self::model_migration::MODEL_AVAILABILITY_NUX_MAX_SHOW_COUNT;
use self::model_migration::StartupTooltipOverride;
use self::model_migration::handle_model_migration_prompt_if_needed;
use self::model_migration::migration_prompt_hidden;
use self::model_migration::prepare_startup_tooltip_override;
use self::model_migration::select_model_availability_nux;
use self::model_migration::should_show_model_migration_prompt;
use self::model_migration::target_preset_for_upgrade;
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_agi_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_agi_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
async fn emit_custom_prompt_deprecation_notice(
    ...
    let details = format!(
        "Detected {prompt_count} custom {prompt_label} in `$AGIWORKFORCE_HOME/prompts`. Use the `$skill-creator` skill to convert each custom prompt into a skill."
    );
...
impl App {
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let agi_op_tx = if let Some(thread) = live_thread {
            crate::chatwidget::spawn_op_forwarder(thread)
        } else {
            let (tx, _rx) = unbounded_channel();
            tx
        };
        self.replace_chat_widget(ChatWidget::new_with_op_sender(init, agi_op_tx));
    ...
    async fn drain_active_thread_events(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        loop {
            match rx.try_recv() {
                Ok(event) => self.handle_agi_event_now(event),
    ...
    fn replay_thread_snapshot(
        ...
        if let Some(event) = snapshot.session_configured {
            self.handle_agi_event_replay(event);
        }
        self.chat_widget
    ...
    pub async fn run(
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, &config.agiworkforce_home).await;
        ...
        let feedback_audience = if auth_ref
            .and_then(AgiWorkforceAuth::get_account_email)
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            model.as_str(),
            model.as_str(),
            auth_ref.and_then(AgiWorkforceAuth::get_account_id),
            auth_ref.and_then(AgiWorkforceAuth::get_account_email),
            auth_mode,
        ...
        {
            session_telemetry.counter("agiworkforce.status_line", /*inc*/ 1, &[]);
        }
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "agiworkforce.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "agiworkforce.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "agiworkforce.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
            ...
            AppEvent::TerminalTitleSetup { items } => {
                let ids = items.iter().map(ToString::to_string).collect::<Vec<_>>();
                let edit = agiworkforce_core::config::edit::terminal_title_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.agiworkforce_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn handle_agi_event_now(&mut self, event: Event) {
        ...
        self.chat_widget.handle_agi_event(event);
    ...
    fn handle_agi_event_replay(&mut self, event: Event) {
        self.chat_widget.handle_agi_event_replay(event);
    }
    ...
    async fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
        ...
        self.handle_agi_event_now(event);
        if self.backtrack_render_pending {
            tui.frame_requester().schedule_frame();
        }
        Ok(())
...
mod tests {
    ...
    async fn startup_custom_prompt_deprecation_notice_emits_when_prompts_exist() -> Result<()> {
        ...
        let prompts_dir = agiworkforce_home.path().join("prompts");
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, agiworkforce_home.path()).await;
    ...
    async fn startup_custom_prompt_deprecation_notice_skips_empty_prompts_dir() -> Result<()> {
        let agiworkforce_home = tempdir()?;
        std::fs::create_dir_all(agiworkforce_home.path().join("prompts"))?;
        let (tx_raw, mut rx) = unbounded_channel();
        let app_event_tx = AppEventSender::new(tx_raw);
    ...
    -> Result<()> {
        ...
        let approval_event = Event {
            ...
            msg: EventMsg::ExecApprovalRequest(
                agiworkforce_protocol::protocol::ExecApprovalRequestEvent {
        ...
        app.handle_agi_event_now(first_event);
        app.handle_agi_event_now(second_event);
        app.chat_widget
    ...
    async fn replayed_turn_complete_submits_restored_queued_follow_up() {
        ...
        let (chat_widget, _app_event_tx, _rx, mut new_op_rx) =
            make_chatwidget_manual_with_sender().await;
        app.chat_widget = chat_widget;
        app.chat_widget.handle_agi_event(session_configured);
    ...
    async fn replace_chat_widget_keeps_replacement_terminal_title_cache_when_present() {
        ...
        replacement.last_terminal_title = Some("agiworkforce".to_string());

        app.replace_chat_widget(replacement);

        assert_eq!(
            app.chat_widget.last_terminal_title,
            Some("agiworkforce".to_string())
        );
    }
    ...
    async fn live_turn_started_refreshes_status_line_with_runtime_context_window() {
        ...
        app.handle_agi_event_now(Event {
            id: "turn-started".to_string(),
            msg: EventMsg::TurnStarted(TurnStartedEvent {
                turn_id: "turn-1".to_string(),
...
"#;
    ...
    async fn model_migration_prompt_skips_when_target_missing_or_hidden() {
        ...
        available.retain(|preset| preset.model != "claude-opus-4-6");
        ...
        assert!(target_preset_for_upgrade(&with_hidden_target, "claude-opus-4-6").is_none());
    }
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        ...
        let current = available_models
            ...
            .find(|preset| preset.model == "claude-opus-4-6")
            .cloned()
            .expect("claude-opus-4-6 preset present");
        assert!(
            !current.show_in_picker,
            "expected claude-opus-4-6 to be hidden from picker for this test"
        );
    ...
    async fn replayed_initial_messages_apply_rollback_in_queue_order() {
        ...
        app.handle_agi_event_replay(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id,
                forked_from_id: None,
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("agiworkforce resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("agiworkforce resume my-session".to_string())
        );
    }
}
