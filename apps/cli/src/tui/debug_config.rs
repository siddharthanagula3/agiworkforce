...
use agiworkforce_core::config_loader::ConfigLayerEntry;
use agiworkforce_core::config_loader::ConfigLayerStack;
use agiworkforce_core::config_loader::ConfigLayerStackOrdering;
use agiworkforce_core::config_loader::NetworkConstraints;
use agiworkforce_core::config_loader::RequirementSource;
use agiworkforce_core::config_loader::ResidencyRequirement;
use agiworkforce_core::config_loader::SandboxModeRequirement;
use agiworkforce_core::config_loader::WebSearchModeRequirement;
    ...
    if let Some(proxy) = session_network_proxy {
        ...
        let all_proxy = session_all_proxy_url(
            ...
            config
                ...
                .network
                .as_ref()
                .is_some_and(agiworkforce_core::config::NetworkProxySpec::socks_enabled),
        );
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_agi_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_agi_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use agiworkforce_core::config_loader::ConstrainedWithSource;
    use agiworkforce_core::config_loader::McpServerIdentity;
    use agiworkforce_core::config_loader::McpServerRequirement;
    use agiworkforce_core::config_loader::NetworkConstraints;
    use agiworkforce_core::config_loader::RequirementSource;
    use agiworkforce_core::config_loader::ResidencyRequirement;
    use agiworkforce_core::config_loader::SandboxModeRequirement;
    use agiworkforce_core::config_loader::Sourced;
    use agiworkforce_core::config_loader::WebSearchModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\agiworkforce\\config.toml")
        } else {
            absolute_path("/etc/agiworkforce/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.agiworkforce")
        } else {
            absolute_path("/repo/.agiworkforce")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement {
                        identity: McpServerIdentity::Command {
                            command: "agiworkforce-mcp".to_string(),
                        },
                    },
                )]),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.agiworkforce\\config.toml")
        } else {
            absolute_path("/home/alice/.agiworkforce/config.toml")
        };
        let stack = ConfigLayerStack::new(
