...
use agiworkforce_feedback::feedback_diagnostics::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use agiworkforce_feedback::feedback_diagnostics::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/agiworkforce/agiworkforce/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const AGIWORKFORCE_FEEDBACK_INTERNAL_URL: &str = "http://go/agiworkforce-feedback-internal";
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #agiworkforce-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("https://go/agiworkforce-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    AGIWORKFORCE_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "agiworkforce-logs.log".into()]).into(),
    ];
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use agiworkforce_feedback::feedback_diagnostics::FeedbackDiagnostic;
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = agiworkforce_feedback::AgiWorkforceFeedback::new().snapshot(None);
    ...
    fn feedback_view_with_connectivity_diagnostics() {
        ...
        let snapshot = agiworkforce_feedback::AgiWorkforceFeedback::new()
            .snapshot(None)
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/agiworkforce-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/agiworkforce/agiworkforce/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
