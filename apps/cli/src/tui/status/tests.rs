...
async fn test_config(temp_home: &TempDir) -> Config {
    ConfigBuilder::default()
        .agiworkforce_home(temp_home.path().to_path_buf())
        .build()
        .await
        .expect("load config")
}
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("claude-opus-4-6-max".to_string());
    ...
    let model_slug = agiworkforce_core::test_support::get_model_offline(config.model.as_deref());
