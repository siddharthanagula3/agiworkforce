//! Transcript/history cells for the AGI Workforce TUI.
//!
...
use agiworkforce_core::web_search::web_search_detail;
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let content = text![
            ...
            "https://github.com/agiworkforce/agiworkforce/releases/latest"
                .cyan()
                .underlined(),
        ];
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " agiworkforce to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
...
pub fn new_guardian_denied_patch_request(
    ...
    let mut summary = vec![
        "Request ".into(),
        "denied".bold(),
        " for agiworkforce to apply ".into(),
    ];
    if files.len() == 1 {
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for AGI Workforce".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what AGI Workforce is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/resume".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ AGI Workforce (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("AGI Workforce").bold(),
            Span::from(" ").dim(),
...
mod tests {
    ...
    use agiworkforce_core::config::types::McpServerConfig;
    use agiworkforce_core::config::types::McpServerTransportConfig;
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved agiworkforce to".to_string(),
