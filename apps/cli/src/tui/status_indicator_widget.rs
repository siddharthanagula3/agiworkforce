...
mod tests {
    ...
    fn details_args_can_disable_capitalization_and_limit_lines() {
        ...
        let mut w = StatusIndicatorWidget::new(tx, crate::tui::FrameRequester::test_dummy(), true);
        w.update_details(
            Some("cargo test -p agiworkforce-core and then cargo test -p agiworkforce-tui".to_string()),
        ...
        assert_eq!(
            w.details(),
            Some("cargo test -p agiworkforce-core and then cargo test -p agiworkforce-tui")
        );
