//! Connector (Apps) popup rendering helpers for `ChatWidget`.
//!
//! Owns the selection-view params builders and display-label helpers that drive
//! the `/apps` slash-command UI panel.

use super::*;

impl ChatWidget {
    pub(super) fn open_connectors_loading_popup(&mut self) {
    ...
    pub(super) fn open_connectors_popup(&mut self, connectors: &[connectors::AppInfo]) {
        self.bottom_pane.show_selection_view(
            self.connectors_popup_params(connectors, /*selected_connector_id*/ None),
        );
    }

    pub(super) fn connectors_loading_popup_params(&self) -> SelectionViewParams {
    ...
    pub(super) fn connectors_popup_params(
        &self,
    ...
    pub(super) fn refresh_connectors_popup_if_open(&mut self, connectors: &[connectors::AppInfo]) {
    ...
    pub(super) fn connectors_popup_hint_line() -> Line<'static> {
        Line::from(vec![
            "Press ".into(),
    ...
    pub(super) fn connector_brief_description(connector: &connectors::AppInfo) -> String {
        let status_label = Self::connector_status_label(connector);
    ...
    pub(super) fn connector_status_label(connector: &connectors::AppInfo) -> &'static str {
    ...
    pub(super) fn connector_description(connector: &connectors::AppInfo) -> Option<String> {
        connector
            .description
            .as_deref()
            .map(str::trim)
