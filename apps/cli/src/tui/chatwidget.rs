//! The main AGI Workforce TUI chat surface.
//!
...
use agiworkforce_core::models_manager::manager::ModelsManager;
...
use agiworkforce_protocol::protocol::ListCustomPromptsResponseEvent;
...
use self::notifications::extract_first_bold;
use self::notifications::hook_event_label;
mod rate_limit;
use self::rate_limit::RateLimitErrorKind;
use self::rate_limit::RateLimitSwitchPromptState;
use self::rate_limit::RateLimitWarningState;
use self::rate_limit::RATE_LIMIT_SWITCH_PROMPT_THRESHOLD;
use self::rate_limit::nudge_model_slug;
use self::rate_limit::rate_limit_error_kind;
pub(crate) use self::rate_limit::get_limits_duration;
mod message_merge;
use self::message_merge::append_text_with_rebased_elements;
use self::message_merge::merge_user_messages;
use self::message_merge::remap_placeholders_for_message;
mod exec;
use self::exec::RunningCommand;
use self::exec::UnifiedExecProcessSummary;
use self::exec::UnifiedExecWaitState;
use self::exec::UnifiedExecWaitStreak;
use self::exec::is_standard_tool_call;
use self::exec::is_unified_exec_source;
#[cfg(test)]
...
pub(crate) struct ChatWidget {
    ...
    // Latest completed user-visible output that `/copy` should place on the clipboard.
...
impl ChatWidget {
    ...
    fn on_session_configured(
        ...
        // Ask agiworkforce-core to enumerate custom prompts for this session.
    ...
    fn handle_steer_rejected_error(&mut self, agi_error_info: &AgiWorkforceErrorInfo) -> bool {
        matches!(
            agi_error_info,
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_agi_limit = limit_id.eq_ignore_ascii_case("agiworkforce");
            let warnings = if is_agi_limit {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let agi_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<agiworkforce_core::AgiWorkforceThread>,
        session_configured: agiworkforce_protocol::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
        ...
        match cmd {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("agiworkforce.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "agiworkforce.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
            ...
            SlashCommand::Copy => {
                ...
                match copy_result {
                    Ok(()) => {
                        ...
                        self.add_info_message(
                            "Copied latest output to clipboard.".to_string(),
                            hint,
                        );
                    }
                    Err(err) => {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                self.request_redraw();
                self.app_event_tx
                    .send(AppEvent::AgiWorkforceOp(Op::SetThreadName { name }));
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                ...
                tx.send(AppEvent::AgiWorkforceOp(Op::SetThreadName { name }));
            }),
        );
    ...
    pub(crate) fn handle_agi_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    pub(crate) fn handle_agi_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_agi_event: {:?}", msg);
            }
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::AgiWorkforceOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for AGI Workforce.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running agiworkforce -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When AGI Workforce runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        if !agiworkforce_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    ...
                    line!["Learn more: https://agiworkforce.com/docs/windows"],
                ])
        ...
        self.session_telemetry.counter(
            "agiworkforce.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the agent sandbox to protect your files and control network access. Learn more <https://agiworkforce.com/docs/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "agiworkforce.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "agiworkforce.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "agiworkforce.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "agiworkforce.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                name: "Use AGI Workforce with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "agiworkforce.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "agiworkforce.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = agiworkforce_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("agiworkforce resume {name}"));
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload AGI Workforce."
            };
    ...
    /// Forward an `Op` directly to agiworkforce.
    pub(crate) fn submit_op(&mut self, op: Op) -> bool {
        ...
        if let Err(e) = self.agi_op_tx.send(op) {
...
async fn fetch_rate_limits(base_url: String, auth: AgiWorkforceAuth) -> Vec<RateLimitSnapshot> {
