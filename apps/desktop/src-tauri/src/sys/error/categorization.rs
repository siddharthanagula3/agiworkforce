        ...
        match self {
            ...
            ToolError::FileSystemError(msg) => {
                if msg.contains("not found") {
                    "File not found. Please check your path.".to_string()
...
impl Categorizable for LLMError {
    ...
    fn retry_delay_ms(&self) -> Option<u64> {
        match self {
            LLMError::RateLimitError(_) => Some(10000),
            LLMError::Timeout(_) => Some(2000),
            LLMError::NetworkError(_) => Some(1000),
            LLMError::ApiError(_) => Some(2000),
            _ => None,
        }
    }
}
...
impl Categorizable for ResourceError {
    ...
    fn retry_delay_ms(&self) -> Option<u64> {
        match self {
            ResourceError::MemoryLimitExceeded(_) => Some(5000),
            ...
            ResourceError::StorageLimitExceeded(_) => None,
        }
    }
}
