...
mod knowledge_base_tests {
    ...
    fn test_experience_lessons_extraction() {
        ...
        let lessons: Vec<_> = experiences.iter().map(|e| &e.lessons_learned).collect();
...
mod planner_tests {
    ...
    fn test_topological_sort_simple() {
        ...
        let mut adj_list: std::collections::HashMap<i32, Vec<i32>> =
...
mod memory_tests {
    ...
    fn test_memory_retrieval() {
        let memory = vec![("key1", "value1"), ("key2", "value2"), ("key3", "value3")];

        let result = memory.iter().find(|(k, _)| *k == "key2").map(|(_, v)| *v);
...
mod learning_tests {
    ...
    fn test_outcome_classification() {
        ...
        let success_rate =
            outcomes.iter().filter(|(_, success)| *success).count() as f64 / outcomes.len() as f64;
    ...
    fn test_pattern_recognition() {
        ...
        let matches = experiences.iter().filter(|e| e.contains(pattern)).count();
