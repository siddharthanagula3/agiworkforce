...
impl KnowledgeBase {
    ...
    fn init_database(&self) -> Result<()> {
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_knowledge_documents_projec
             ON knowledge_documents(project_id)",
            [],
        )?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_knowledge_chunks_documen
             ON knowledge_chunks(document_id)",
            [],
        )?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_knowledge_chunks_projec
             ON knowledge_chunks(project_id)",
            [],
        )?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_project_memory_projec
