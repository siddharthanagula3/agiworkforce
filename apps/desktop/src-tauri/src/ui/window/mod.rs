use crate::data::state::{AppState, DockPosition, WindowGeometry};
...
const WINDOW_DEFAULT_HEIGHT: f64 = 850.0;
const WINDOW_MIN_HEIGHT: f64 = 700.0;
...
fn emit_preview(window: &WebviewWindow, preview: Option<DockPosition>) -> Result<()> {
    ...
    window.emit("window:preview", &payload)?;
    Ok(())
}
