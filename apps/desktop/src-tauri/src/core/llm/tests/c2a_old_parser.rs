...
pub(crate) struct SseStreamParser {
    inner: Pin<Box<dyn Stream<Item = Result<bytes::Bytes, Box<dyn Error + Send + Sync>>> + Send>>,
    buffer: Vec<u8>,
...
pub(crate) fn parse_sse_event(
    ...
    match provider {
        ...
        // LM Studio, llama.cpp, and vLLM's servers all stream OpenAI-format SSE chunks.
        crate::core::llm::Provider::LmStudio => parse_openai_sse(event),
        crate::core::llm::Provider::LlamaCpp => parse_openai_sse(event),
        crate::core::llm::Provider::Vllm => parse_openai_sse(event),
    }
}
