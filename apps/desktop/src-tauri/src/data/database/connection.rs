...
mod tests {
    ...
    fn test_connection_string_detection() {
        assert!(matches!(
            ConnectionConfig::detect_db_type("postgres://localhost:5432"),
            Ok(DatabaseType::PostgreSQL)
        ));
        assert!(matches!(
            ConnectionConfig::detect_db_type("mysql://localhost:3306"),
            Ok(DatabaseType::MySQL)
        ));
        assert!(matches!(
            ConnectionConfig::detect_db_type("mongodb://localhost:27017"),
            Ok(DatabaseType::MongoDB)
        ));
        assert!(matches!(
            ConnectionConfig::detect_db_type("redis://localhost:6379"),
            Ok(DatabaseType::Redis)
        ));
    }
