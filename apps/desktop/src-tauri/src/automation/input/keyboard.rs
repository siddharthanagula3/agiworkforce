...
impl KeyboardSimulator {
    ...
    /// Execute a macro (sequence of keyboard actions)
...
impl KeyboardSimulator {
    /// Helper to convert common modifier names to keys
    pub fn modifier_key(name: &str) -> Option<Key> {
        match name.to_lowercase().as_str() {
            "ctrl" | "control" => Some(Key::Control),
            "shift" => Some(Key::Shift),
            "alt" | "option" | "opt" => Some(Key::Alt),
            // Meta isn't supported by enigo 0.6; map command/super to control as a fallback
            "cmd" | "command" | "meta" | "super" | "windows" => Some(Key::Control),
            _ => None,
        }
    }

    /// Helper function to convert virtual key codes to enigo Keys (for compatibility)
    pub fn vk_to_key(vk: u16) -> Option<Key> {
        match vk {
            ...
            0x70..=0x87 => Some(Key::F1), // F1-F24 (simplified)
            _ => None,
        }
    }
}
