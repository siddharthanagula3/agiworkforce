...
mod tests {
    ...
    fn test_rm_rf_malicious_variants() {
        ...
        assert!(any_dangerous("rm -rf /home/user"), "rm -rf should be flagged");
        // Flags in different order
        assert!(any_dangerous("rm -fr /tmp/data"), "rm -fr should be flagged");
        // Uppercase (case-insensitive)
        assert!(any_dangerous("RM -RF /"), "RM -RF should be flagged (case insensitive)");
    }

    #[test]
    fn test_rm_rf_benign_not_flagged() {
        // "rm" alone without -rf flags is not matched
        assert!(!any_dangerous("rm file.txt"), "plain rm should not be flagged");
    }
    ...
    fn test_format_drive_malicious_variants() {
        ...
        assert!(any_dangerous("format D:"), "format D: should be flagged (upper drive letter)");
        // Case-insensitive on the command itself
        assert!(any_dangerous("FORMAT c:"), "FORMAT c: should be flagged (case insensitive)");
    }

    #[test]
    fn test_format_drive_benign_not_flagged() {
        // "format" without a Windows drive letter is not matched
        assert!(!any_dangerous("format --help"), "format --help should not be flagged");
        assert!(!any_dangerous("printf 'format your thoughts'"), "unrelated format string should not be flagged");
    }
    ...
    fn test_del_slash_flag_malicious_variants() {
        assert!(any_dangerous("del /f important.dll"), "del /f should be flagged");
    ...
    fn test_del_slash_flag_benign_not_flagged() {
        ...
        assert!(!any_dangerous("del myfile.txt"), "del without flags should not be flagged");
    }
    ...
    fn test_deltree_malicious_variants() {
        assert!(any_dangerous("deltree C:\\Windows"), "deltree should be flagged");
        assert!(any_dangerous("DELTREE /Y C:\\Users"), "DELTREE should be flagged (case insensitive)");
        assert!(any_dangerous("deltree mydir"), "deltree with any arg should be flagged");
    }
    ...
    fn test_deltree_benign_not_flagged() {
        ...
        assert!(!any_dangerous("cat deltree_notes.md"), "file name with deltree prefix may vary — just ensure unrelated commands are clean");
    }
    ...
    fn test_mkfs_malicious_variants() {
        assert!(any_dangerous("mkfs.ext4 /dev/sda1"), "mkfs.ext4 should be flagged");
        assert!(any_dangerous("mkfs /dev/sdb"), "plain mkfs should be flagged");
        assert!(any_dangerous("MKFS.FAT /dev/sdc1"), "MKFS.FAT should be flagged (case insensitive)");
    }

    #[test]
    fn test_mkfs_benign_not_flagged() {
        assert!(!any_dangerous("mkdir /home/user/projects"), "mkdir should not be flagged");
    ...
    fn test_system32_malicious_variants() {
        assert!(any_dangerous("del C:\\Windows\\system32\\kernel32.dll"), "system32 path should be flagged");
        assert!(any_dangerous("cd %windir%\\System32"), "System32 (mixed case) should be flagged");
        assert!(any_dangerous("SYSTEM32 is the target"), "SYSTEM32 uppercase should be flagged");
    }

    #[test]
    fn test_system32_benign_not_flagged() {
        // Commands that don't mention system32 are safe
        assert!(!any_dangerous("cd C:\\Users\\me\\Documents"), "normal Windows path should not be flagged");
    }
    ...
    fn test_etc_passwd_malicious_variants() {
        assert!(any_dangerous("cat /etc/passwd"), "reading /etc/passwd should be flagged");
        assert!(any_dangerous("curl localhost/../etc/passwd"), "path traversal to /etc/passwd should be flagged");
        assert!(any_dangerous("/ETC/PASSWD"), "/ETC/PASSWD uppercase should be flagged (case insensitive)");
    }

    #[test]
    fn test_etc_passwd_benign_not_flagged() {
        assert!(!any_dangerous("cat /etc/hosts"), "/etc/hosts should not be flagged");
        assert!(!any_dangerous("ls /etc/"), "/etc/ directory listing should not be flagged");
    }
    ...
    fn test_ssh_dir_malicious_variants() {
        assert!(any_dangerous("cat ~/.ssh/id_rsa"), "reading ssh private key should be flagged");
        assert!(any_dangerous("cp ~/.ssh/config /tmp"), "copying .ssh config should be flagged");
        assert!(any_dangerous("~/.SSH/authorized_keys"), "~/.SSH uppercase should be flagged (case insensitive)");
    }

    #[test]
    fn test_ssh_dir_benign_not_flagged() {
        assert!(!any_dangerous("ssh user@host"), "plain ssh command should not be flagged");
        assert!(!any_dangerous("ssh-keygen -t ed25519"), "ssh-keygen should not be flagged");
    }
    ...
    fn test_sensitive_keywords_malicious_variants() {
        assert!(any_dangerous("echo $PASSWORD"), "PASSWORD env var reference should be flagged");
        assert!(any_dangerous("print(api_key)"), "api_key reference should be flagged");
        assert!(any_dangerous("store_secret('abc123')"), "store_secret call should be flagged");
    }
    ...
    fn test_sensitive_keywords_benign_not_flagged() {
        ...
        assert!(!any_dangerous("git status"), "git status should not be flagged");
        assert!(!any_dangerous("cat file.txt"), "cat file.txt should not be flagged");
    }
    ...
    fn test_registry_malicious_variants() {
        assert!(any_dangerous("regedit /s payload.reg"), "regedit should be flagged");
        assert!(any_dangerous("reg delete HKLM\\Software\\App"), "reg delete should be flagged");
        assert!(any_dangerous("REG ADD HKCU\\Run /v mal /d mal.exe"), "REG ADD should be flagged (case insensitive)");
    }
    ...
    fn test_registry_benign_not_flagged() {
        ...
        assert!(!any_dangerous("grep -r foo ."), "grep -r should not be flagged");
        assert!(!any_dangerous("cd /var/log"), "cd command should not be flagged");
    }
    ...
    fn test_sudo_destructive_malicious_variants() {
        assert!(any_dangerous("sudo rm -rf /var/www"), "sudo rm should be flagged");
        assert!(any_dangerous("sudo dd if=/dev/zero of=/dev/sda"), "sudo dd should be flagged");
        assert!(any_dangerous("SUDO RM important_file"), "SUDO RM should be flagged (case insensitive)");
    }

    #[test]
    fn test_sudo_destructive_benign_not_flagged() {
        assert!(!any_dangerous("sudo apt-get update"), "sudo apt-get should not be flagged");
        assert!(!any_dangerous("sudo systemctl restart nginx"), "sudo systemctl should not be flagged");
    }
    ...
    fn test_case_insensitivity_rm() {
        assert!(any_dangerous("RM -RF /tmp"), "RM -RF uppercase should match");
        assert!(any_dangerous("Rm -Rf /tmp"), "Rm -Rf mixed case should match");
        assert!(any_dangerous("rM -rF /tmp"), "rM -rF mixed case should match");
    }

    #[test]
    fn test_case_insensitivity_format() {
        assert!(any_dangerous("Format C:"), "Format C: title case should match");
        assert!(any_dangerous("FORMAT D:"), "FORMAT D: all-caps should match");
        assert!(any_dangerous("fOrMaT e:"), "fOrMaT mixed case should match");
    }

    #[test]
    fn test_case_insensitivity_del() {
        assert!(any_dangerous("DEL /F file.txt"), "DEL /F uppercase should match");
        assert!(any_dangerous("Del /Q file.txt"), "Del /Q title case should match");
        assert!(any_dangerous("dEl /S *.log"), "dEl /S mixed case should match");
    }
