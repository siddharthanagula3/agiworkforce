...
// Local definitions for types/constants missing or hard to find in current bindings
#[repr(C)]
...
// Wrapper for AXUIElementRef to handle CFRelease
#[derive(Debug, Clone)]
...
impl MacAutomationService {
    ...
    // Helper: Register an element in the cache and return its ID (pointer address as hex string)
    pub fn register_element(&self, element_ref: AXUIElementRef) -> Result<String> {
        unsafe {
            // Retain the element because we are storing it in cache
    ...
    pub fn get_element(&self, id: &str) -> Result<AXElement> {
        ...
        if let Some(el) = cache.get(id) {
            // Retain before returning a clone of the wrapper (which releases on drop)
            unsafe {
    ...
    pub fn invoke(&self, element_id: &str) -> Result<()> {
        ...
        unsafe {
            // In a real implementation we should check if action is supported
