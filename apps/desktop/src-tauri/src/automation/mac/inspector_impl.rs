...
impl InspectorService {
    ...
    fn get_detailed_info(&self, element: &AXElement) -> Result<DetailedElementInfo> {
        // Register to get stable ID
        let id = self.service.register_element(element.0)?;

        // Fetch basic properties
        let name = "Mac Element".to_string(); // Placeholder until I implement fetch_string_attr
        ...
        Ok(DetailedElementInfo {
            ...
            class_name,
            control_type,
            bounding_rect: self.service.bounding_rect(&id)?, // This calls the real impl
            properties,
            automation_id: None,
            parent: None,
            children: vec![], // TODO
