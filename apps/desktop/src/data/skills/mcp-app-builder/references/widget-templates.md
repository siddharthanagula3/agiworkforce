...
Display widgets don't call `sendMessage` — they render and sit there. The tool should return a text summary **alongside** the widget so the assistant can keep reasoning while the user sees the visual:
...
- **Put the assistant's reasoning in each item** — a `note` field rendered as a small callout on the card gives users the "why" inline.
- **Silent state via `updateModelContext`** — cart/selection changes should inform the assistant without spamming the chat. Reserve `sendMessage` for terminal actions ("checkout", "done").
