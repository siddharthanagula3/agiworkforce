...
The message appears in chat and The assistant responds to it. Use `role: "user"` — the widget speaks on the user's behalf.

### `app.updateModelContext({ content })`

Update the assistant's context **silently** — no visible message. Use for state that informs but doesn't warrant a chat bubble.
...
Call a tool on your MCP server directly, bypassing the assistant. Returns the tool result.
...
Use for data fetches that don't need the assistant's reasoning — pagination, detail lookups, refreshes.
...
Fires with the arguments the assistant passed to the tool. Useful if the widget needs to know what was asked for (e.g., highlight the search term).
...
1. The assistant calls a tool with `_meta.ui.resourceUri` declared
