...
The assistant drives it. That combination means: **tool inputs are untrusted**, even though they come from an AI the user trusts. A prompt-injected web page can make the assistant call your `delete_file` tool with a path you didn't intend.
...
Split read and write into separate tools. Most workflows only need read. A tool that's read-only can't be weaponized into data loss no matter what the assistant is tricked into calling it with.
...
The assistant will happily ask to read a 4GB log file. Cap everything:
