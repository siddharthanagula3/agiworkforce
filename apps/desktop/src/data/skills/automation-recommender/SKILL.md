...
description: Analyze a codebase and recommend AGI Workforce automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their platform setup, mentions improving workflows, asks how to first set up the platform for a project, or wants to know what automation features they should use.
tools: Read, Glob, Grep, Bash
---

# Automation Recommender

Analyze codebase patterns to recommend tailored platform automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask the assistant separately to help build them.
...
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by the assistant or user via `/skill-name`) |
...
# Check for existing platform config
...
| Code style | **project-conventions** | assistant-only |
...
**Install**: `agiworkforce mcp add context7`
...
**Invocation**: User-only / Both / assistant-only
...
- `user-invocable: false` — assistant-only (for background knowledge)
...
agiworkforce -p "fix lint errors in src/" --allowedTools Edit,Write

# CI pipeline with structured output
agiworkforce -p "<prompt>" --output-format stream-json | your_command
```
