...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.claude/skills/<name>/SKILL.md`. Skills can be invoked by the assistant automatically when relevant, or by users directly with `/skill-name`.
...
user-invocable: false           # Only the assistant can invoke (for background knowledge)
...
| Setting | User | Assistant | Use for |
|---------|------|--------|---------|
...
### Project Conventions (assistant-only)

Background knowledge the assistant applies automatically:
...
The command output replaces the placeholder before the assistant sees the skill content.
