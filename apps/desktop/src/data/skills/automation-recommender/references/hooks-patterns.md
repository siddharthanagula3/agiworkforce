...
Hooks automatically run commands in response to platform events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when the platform sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when the assistant requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when The assistant is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when The assistant needs attention
**Value**: Know when The assistant is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"The assistant is waiting\" with title \"AGI Workforce\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | The assistant needs permission for a tool |
| `idle_prompt` | The assistant waiting for input (60+ seconds) |
