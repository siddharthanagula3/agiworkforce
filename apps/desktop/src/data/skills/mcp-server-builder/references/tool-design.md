# Tool Design — Writing Tools the Assistant Uses Correctly

Tool schemas and descriptions are prompt engineering. They land directly in the assistant's context and determine whether the assistant picks the right tool with the right arguments. Most MCP integration bugs trace back to vague descriptions or loose schemas.

---

## Descriptions

**The description is the contract.** It's the only thing the assistant reads before deciding whether to call the tool. Write it like a one-line manpage entry plus disambiguating hints.
...
The assistant will call this for anything vaguely search-shaped, including things it can't do.
...
**Describe every parameter.** The `.describe()` text shows up in the schema the assistant sees. Omitting it is leaving money on the table.
...
The assistant reads whatever you put in `content[].text`. Make it parseable.

**Do:**
- Return JSON for structured data (`JSON.stringify(result, null, 2)`)
- Return short confirmations for mutations (`"Created issue #123"`)
- Include IDs the assistant will need for follow-up calls
...
- Return bare success with no identifier (`"ok"` after a create — the assistant can't reference what it made)
...
The ceiling isn't a hard protocol limit — it's context-window economics. Every tool schema is tokens the assistant spends *every turn*. Thirty tools with rich schemas can eat 3–5k tokens before the conversation even starts.

---

## Errors

Return MCP tool errors, not exceptions that crash the transport. Include enough detail for the assistant to recover or retry differently.
