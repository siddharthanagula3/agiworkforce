...
One line of config, lands directly in the assistant's system prompt. Use it for tool-use hints that don't fit in individual tool descriptions.
...
This is the highest-leverage one-liner in the spec. If the assistant keeps misusing your tools, put the fix here.
...
**Requires client support** — check `clientCapabilities.sampling` first. Model preference hints are substring-matched (`"claude-3-5"` matches any variant).
...
| Elicitation | — | `elicitation: {}` | return text, ask the assistant to relay |
