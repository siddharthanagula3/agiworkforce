...
MCP defines three server-side primitives. Tools are model-controlled (the assistant decides when to call them). The other two are different:
...
Most servers only need tools. Reach for these when the shape of your integration doesn't fit "the assistant calls a function."
...
- Large reference data (docs, schemas, configs) that the assistant should be able to browse
- Content that changes independently of conversation (log files, live data)
- Anything where "the assistant decides to fetch" is the wrong mental model

**When a tool is better:**
- The operation has side effects
- The result depends on parameters the assistant chooses
- You want the assistant (not the host UI) to decide when to pull it in
...
| You want to... | Use |
|---|---|
| Let the assistant fetch something on demand, with parameters | **Tool** |
