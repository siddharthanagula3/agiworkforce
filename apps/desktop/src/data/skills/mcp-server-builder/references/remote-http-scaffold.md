...
FastMCP derives the JSON schema from type hints and the docstring becomes the tool description. Keep docstrings terse and action-oriented — they land in the assistant's context window verbatim.
...
| Surface | How |
|---|---|
| **AGI Workforce** | `agiworkforce mcp add --transport http <name> <url>` (add `--scope user` for global, `--header "Authorization: Bearer ..."` for auth) |
| **Desktop / Web** | Settings → Connectors → Add custom connector. **Not** the desktop config — remote servers configured there are ignored. |
