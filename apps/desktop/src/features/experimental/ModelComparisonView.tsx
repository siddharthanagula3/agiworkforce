 ...
 *
 * Uses invoke('chat_send_message', ...) via the existing chat infrastructure.
 */
...
// Response shape from the chat_send_message Tauri command (non-streaming path).
// Mirrors ChatSendMessageResponse on the Rust side.
    ...
    await Promise.allSettled(
      selectedModels.map(async (modelId) => {
        ...
        try {
          ...
          const inputTokens = response?.prompt_tokens;
          const outputTokens = response?.completion_tokens;
          const costUsd = response?.cost;
