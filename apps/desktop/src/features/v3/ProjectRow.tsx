...
export function ProjectRow({
  ...
  const name = project.name || t('common.untitled');
  const showActions = hovered || menuOpen || active;
  const emoji = project.iconEmoji ?? null;
    ...
    >
      {editing ? (
        ...
        <button
          ...
          style={{
            ...
            gap: 8,
            padding: '5px 10px',
            paddingRight: showActions ? 52 : 10,
            borderRadius: 6,
            border: 'none',
        ...
        >
          {emoji ? (
            ...
            <Folder size={14} style={{ flexShrink: 0, color: accentColor }} />
          )}
          <span
            style={{
              flex: 1,
      ...
      {!editing && showActions && (
        ...
        >
          <button
            type="button"
            aria-label={t('sidebar.projects.newChat')}
            title={t('sidebar.projects.newChat')}
            onClick={(e) => {
              e.stopPropagation();
