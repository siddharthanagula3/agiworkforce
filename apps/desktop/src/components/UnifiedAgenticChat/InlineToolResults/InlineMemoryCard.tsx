...
const IMPORTANCE_CONFIG = {
  low: { label: 'Low', color: 'text-zinc-400', bar: 'bg-zinc-500', width: 'w-1/3' },
...
export function InlineMemoryCard({ result, status }: ToolResultProps) {
  ...
  if (status === 'running') {
    return (
      <div className="mt-3 flex items-center gap-2 p-3 rounded-lg bg-zinc-900/80 border border-white/10">
        ...
        <span className="text-sm text-zinc-400">Accessing memory...</span>
      </div>
    );
  }
  ...
  const badgeClass = TYPE_BADGE[memoryType] ?? 'bg-zinc-500/20 text-zinc-300';
  ...
  return (
    <div className="mt-3 rounded-lg bg-zinc-900/80 border border-white/10 overflow-hidden">
      ...
      <div className="p-3 space-y-3">
        {content && <p className="text-xs text-zinc-400 leading-relaxed line-clamp-3">{content}</p>}
