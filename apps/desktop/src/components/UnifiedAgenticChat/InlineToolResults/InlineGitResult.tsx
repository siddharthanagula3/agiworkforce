  ...
  if (status === 'running') {
    return (
      <div className="mt-3 flex items-center gap-2 p-3 rounded-lg bg-card/80 border border-white/10">
        ...
        <span className="text-sm text-muted-foreground">Running git operation...</span>
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 rounded-lg bg-card/80 border border-white/10 overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2.5 bg-muted/60 border-b border-white/10">
        ...
        <span className="text-xs font-mono font-medium text-foreground capitalize">
          {operation}
        </span>
        {branch && (
      ...
      <div className="p-3 space-y-2">
        {commit && (
          <div className="space-y-1">
            ...
            {commit.message && (
              <p className="text-xs text-foreground line-clamp-2">{commit.message}</p>
            )}
            {commit.author && <p className="text-xs text-muted-foreground">by {commit.author}</p>}
          </div>
        )}
              ...
              <li key={i} className="flex items-center gap-2 text-xs">
                <span
                  className={cn(
                    ...
                    FILE_STATUS_COLOR[f.status] ?? 'text-muted-foreground',
                  )}
                >
                  {FILE_STATUS_CHAR[f.status] ?? '?'}
                </span>
                <span className="text-muted-foreground truncate font-mono">{f.path}</span>
              </li>
            ))}
            {files.length > 5 && (
              <li className="text-xs text-muted-foreground">+ {files.length - 5} more files</li>
            )}
          </ul>
        )}
        ...
        {diff && diffLines.length > 0 && (
          <div className="rounded bg-background/60 overflow-hidden">
            <div className="px-2 py-1 border-b border-white/5 flex items-center justify-between">
              <span className="text-xs text-muted-foreground font-mono">Diff</span>
              {diffLines.length > 6 && (
                <Button
                  size="xs"
                  variant="ghost"
