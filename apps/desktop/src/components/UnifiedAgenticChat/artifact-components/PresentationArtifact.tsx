...
export function PresentationArtifact({ artifact, className }: PresentationArtifactProps) {
  const slides = useMemo(() => {
    // Split by horizontal rule or specific slide delimiters
    // Assuming markdown format where '---' or '# Slide Title' delimits slides
    const content = artifact.content;

    // First try splitting by '---' if present multiple times
    if (content.split('\n---\n').length > 1) {
      return content.split('\n---\n').map((s) => s.trim());
    }

    // Fallback: Split by H1 or H2 headers
