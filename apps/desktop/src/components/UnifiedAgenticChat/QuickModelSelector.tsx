...
export const QuickModelSelector = ({ className, onClose }: QuickModelSelectorProps) => {
  ...
  const manualSelectionEnabled = useMemo(
    () => canAccessManualModelSelection(userPlanTier),
    [userPlanTier],
  );
  ...
  const modelGroups = useMemo(() => {
    ...
    visibleManagedProviders.forEach((p) => {
      groups[p] = [];
    });

    getManualOverrideModels().forEach((metadata) => {
      if (!visibleManagedProviders.includes(metadata.provider)) {
        return;
      }
      if (metadata.modelType && !['chat', 'multimodal', 'reasoning'].includes(metadata.modelType)) {
        return;
      }
      if (effectiveSearchQuery) {
        const matchesName = metadata.name.toLowerCase().includes(effectiveSearchQuery);
        const matchesId = metadata.id.toLowerCase().includes(effectiveSearchQuery);
        const matchesProvider = metadata.provider.toLowerCase().includes(effectiveSearchQuery);
      ...
      groups[metadata.provider]?.push(metadata);
    });
