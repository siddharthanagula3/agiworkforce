...
function StepCard({ step, isLast, isExpanded, onToggleExpand }: StepCardProps) {
  ...
  return (
    <div className="relative">
      {}
      {!isLast && <div className="absolute left-[11px] top-8 h-full w-0.5 bg-border" />}
      ...
      >
        ...
        <div className="relative shrink-0">
          ...
          >
            <StatusIcon
              className={cn('h-3.5 w-3.5', statusConfig.iconColor, statusConfig.animate)}
            />
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className={cn('text-sm font-medium', statusConfig.textColor)}>{step.description}</p>
            {hasReasoning && (
              <button
                type="button"
