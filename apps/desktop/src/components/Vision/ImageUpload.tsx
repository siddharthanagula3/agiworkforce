...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const updateImageDetail = (id: string, detail: 'low' | 'high' | 'auto') => {
    const updatedImages = images.map((img) => (img.id === id ? { ...img, detail } : img));
    onImagesChange(updatedImages);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {(onCaptureClick || onCaptureFromClipboard) && (
          <div className="mt-4 flex justify-center gap-2">
            {onCaptureClick && (
              <Button onClick={onCaptureClick} variant="outline" size="sm" className="gap-2">
