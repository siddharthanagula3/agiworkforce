        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {images.length > 0 && (
            <Card className="p-6">
              ...
              <VisionAnalysis images={images} onResult={handleAnalysisResult} />
            </Card>
          )}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                          {item.result.model} •{' '}
                          {item.result.cost ? `$${item.result.cost.toFixed(6)}` : 'Free'} •{' '}
                          {item.result.processing_time_ms}ms
                        </div>
                        <p className="text-sm line-clamp-3">{item.result.content}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {images.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                Upload images, capture your screen, or paste from clipboard to get started
              </p>
            </div>
          )}
        </div>
