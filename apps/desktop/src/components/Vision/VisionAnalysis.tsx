...
import { Clock, DollarSign, Eye, FileText, GitCompare, Loader2, Sparkles } from 'lucide-react';
...
type AnalysisMode = 'describe' | 'extract_text' | 'compare' | 'custom';

export const VisionAnalysis: React.FC<VisionAnalysisProps> = ({ images, onResult }) => {
  ...
  const analyzeImages = async () => {
    ...
    try {
      ...
      if (mode === 'describe' && !customPrompt) {
        ...
        prompt = 'Compare these images and describe all differences, similarities, and changes.';
      }
      ...
      if (mode === 'extract_text' && images.length === 1) {
        response = await invoke<VisionResponse>('vision_extract_text', {
      ...
      {result && (
        <Card className="p-6 space-y-4">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
              {result.cost !== undefined && (
                <div className="flex items-center gap-1">
                  <DollarSign className="h-4 w-4" />${result.cost.toFixed(6)}
                </div>
              )}
              <div className="flex items-center gap-1">
          ...
          <div className="prose dark:prose-invert max-w-none">
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Model: {result.model}</p>
          ...
          {result.tokens && (
            <div className="text-xs text-gray-500 dark:text-gray-400 flex gap-4">
              <span>Input: {result.prompt_tokens?.toLocaleString()} tokens</span>
              <span>Output: {result.completion_tokens?.toLocaleString()} tokens</span>
