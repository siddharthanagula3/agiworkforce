...
Define custom node types for different workflow actions:
...
export function TriggerNode({ data }: { data: NodeData }) {
  return (
    <div className="px-4 py-2 rounded-lg bg-blue-500 text-white shadow-lg">
...
export function ActionNode({ data }: { data: NodeData }) {
  return (
    <div className="px-4 py-2 rounded-lg bg-green-500 text-white shadow-lg">
...
export function ConditionNode({ data }: { data: NodeData }) {
  return (
    <div className="px-4 py-2 rounded-lg bg-yellow-500 text-white shadow-lg">
      <Handle type="target" position={Position.Top} />
      <div className="font-bold">{data.label}</div>
...
function WorkflowToolbar() {
  const { saveWorkflow, loadWorkflow, autoLayoutWorkflow, clearWorkflow } = useConfiguratorStore();

  return (
    <div className="flex gap-2 p-4 border-b">
      <button onClick={saveWorkflow}>Save</button>
      <button onClick={loadWorkflow}>Load</button>
      <button onClick={autoLayoutWorkflow}>Auto Layout</button>
