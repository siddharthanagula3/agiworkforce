...
1. **LOCKED: managed cloud stays private-beta** (`AGI_MANAGED_COMPUTE_PRIVATE_BETA=1`; ledgering/abuse/fraud/refunds/retention/deletion must be proven per CLAUDE.md). Cloud-only VS Code MUST ship behind the SAME `buildManagedComputeGateResponse` gate as web/gateway — cannot open paid compute to all VS Code users. **Founder decision:** confirm the extension launches gated (waitlist front door for unentitled accounts) before flipping the default. M1's demo works for an entitled/beta account.
