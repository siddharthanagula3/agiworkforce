...
- Apple Developer documentation updates for the 27 platform releases: https://developer.apple.com/documentation/updates
- Apple WWDC26 iOS guide: https://developer.apple.com/wwdc26/guides/ios/
- Apple newsroom, June 8 2026: https://www.apple.com/newsroom/2026/06/apple-aids-app-development-with-new-intelligence-frameworks-and-advanced-tools/
...
| Xcode and SDK readiness     | New Apple platform APIs are roadmap inputs, not current AGI feature claims.                                              | Before building with the newest SDK, verify Expo/RN launch behavior, scene lifecycle compliance, Device Hub resizing, iPhone Mirroring resizing, visual QA, and store metadata. | Apple 27 platform updates                              |
| Native AI evaluations       | Apple Evaluations and Core AI are future quality and runtime paths.                                                      | Do not claim Apple Evaluations, Core AI, Private Cloud Compute, or Apple Foundation Models support until AGI has implemented, gated, and manually verified those paths.         | Apple 27 platform updates                              |
...
- The 27 platform release path strengthens the long-term Local Mode story, but the current public binary must keep Apple runtime labels hidden unless the runtime is actually available and selected.

## Xcode 27 And Platform 27 Product Notes

- Xcode agent skills, app localization assistance, iPhone Mirroring resize checks, Instruments, MetricKit, and StateReporting are useful for the release workflow, but they do not change current AGI user-facing claims.
- App Intents, Spotlight, Siri, Visual Intelligence, Suggested Actions, and Media Intents are future native entry points. Add them only when each intent maps to a real AGI action with permission, privacy, and Local/Cloud boundary checks.
- Foundation Models, Private Cloud Compute, Core AI, and third-party Language Model protocol packages should be treated as a future Apple-native model layer. They must be capability-gated by OS, device, account, entitlement, and provider terms before appearing in the model picker.
- The Evaluations framework is a future fit for prompt and model-behavior QA. It should not replace manual visual QA for App Store readiness.
- If AGI adopts a newer Apple SDK, verify launch, routing, app state restore, scene lifecycle behavior, resizing, keyboard, safe areas, permissions, and recording indicators on current simulators and physical devices before App Store submission.
- Do not enable App Intents, Siri, Spotlight, or Suggested Actions unless the underlying AGI action already exists, respects Local versus Cloud boundaries, and has App Intents tests or equivalent manual evidence.
- Do not show Apple Foundation Models, Private Cloud Compute, Core AI, Dynamic Profiles, or third-party Language Model protocol providers in the product until the implementation is live behind capability detection and verified on supported devices.
