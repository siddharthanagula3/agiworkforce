...
| Xcode and SDK readiness     | New Apple platform APIs are roadmap inputs, not current AGI feature claims.                                              | Before building with the newest SDK, verify Expo/RN launch behavior, scene lifecycle compatibility, iPhone Mirroring resizing, visual QA, and store metadata.           | Apple 27 platform updates                              |
...
- If AGI adopts a newer Apple SDK, verify launch, routing, app state restore, resizing, keyboard, safe areas, permissions, and recording indicators on current simulators and physical devices before App Store submission.
