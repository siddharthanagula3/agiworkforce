...
| Keywords         | `ai,chat,assistant,local,offline,privacy,llm,dpdp,on-device`                                                                              | 58 / 100           |
...
HealthKit has been removed from the app (no entitlement, no health data
read or advertised) — no Health & Fitness data-type declaration is needed.
...
- `store-listing/screenshots/safeguards/` — compliance evidence screenshots
  (S-01 through S-07; see `SAFEGUARDS-SCREENSHOTS.md` for capture specs)
