...
**OPEN GAP (2026-07-04, not yet resolved):** this build declares the
HealthKit entitlement and reads real step/sleep/workout data for the
optional Apple Health connector (advertised in the What's New copy and
`healthkit` keyword). Apple's App Privacy questionnaire will very likely
require a Health & Fitness data-type entry here, and the Privacy
Manifest (`ios/PrivacyInfo.xcprivacy`, generated by `app.config.js`)
does not yet declare one either. Get an engineering + product decision
on the correct declaration (linked-to-user? tracking? retention?) before
completing this section — do not submit with only Contact Info declared.
...
11. **Upgrade CTA / Guideline 3.1.1 (LIVE LEGAL/POLICY QUESTION — do not
    resolve without founder + legal sign-off)**: As of the 2026-07-04
    founder decision, the chat-paywall CTA (`PaywallBottomSheet.tsx`) no
    longer opens a browser checkout — it shows an informational message
    when native IAP is off. The web-checkout link still exists on the
    Settings → Billing screen ("Upgrade plan" / "Adjust plan" row,
    `cloud-billing/index.tsx`), which opens
    `https://agiworkforce.com/pricing` in the system browser rather than
    a StoreKit/Play Billing purchase. Apple Guideline 3.1.1 and Google
    Play's external offers policy both have region-specific rules on
    whether/how apps may link out to external purchase flows for digital
    subscriptions purchasable in-app. Confirm with legal counsel whether
    this Settings-screen flow is compliant as-is, needs an
    entitlements-reader integration, external-link disclosure copy, or
    must be gated behind Apple's External Purchase Link Entitlement (or
    removed) for iOS before submission. See `KILL-SWITCH.md` for how to
    hide the CTA entirely if the answer is "not yet."
