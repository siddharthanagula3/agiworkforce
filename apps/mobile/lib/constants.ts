...
export const WS_URL =
  process.env.EXPO_PUBLIC_WS_URL ?? 'wss://signaling.agiworkforce.com';

export const SUPABASE_URL = (() => {
  const url = process.env.EXPO_PUBLIC_SUPABASE_URL;
  if (!url) {
    console.error('[constants] EXPO_PUBLIC_SUPABASE_URL is not set — Supabase will not work');
  }
  return url ?? '';
})();
...
export const SUPABASE_ANON_KEY = (() => {
  ...
  if (!key) {
    console.error('[constants] EXPO_PUBLIC_SUPABASE_ANON_KEY is not set — Supabase will not work');
  }
  return key ?? '';
})();
