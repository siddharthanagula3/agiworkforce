  ...
  useEffect(() => {
    ...
    if (!session && !inAuthGroup) {
      ...
      if (!onboardingDone && !inOnboarding) {
        // .expo/types/router.d.ts is gitignored and only regenerated by
        // `expo start`; CI doesn't have the typed entry for this route.
        router.replace('/(public)/onboarding' as never);
      } else {
        router.replace('/(app)');
      }
