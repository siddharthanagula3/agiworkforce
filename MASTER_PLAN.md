...
| Phase B — God-file splits         | 🟡 13/21 shipped       | **Shipped:** cli main.rs (`8cd6f740f`), cli a2a.rs (`dd34923db`), cli tools.rs (`668d06f96`), cli safety.rs (`0b2e6a627`), **cli repl.rs (`8751c8270`, new)**, vscode extension.ts (`e11dc7ea1`), vscode sidebarProvider.ts (`c019dfec2`), vscode agentModeProvider.ts (`9919fa354`+`526f524df`), mobile companion (`0276d541f`), mobile chatStore.ts (`b502947f9`), desktop settingsStore.ts (`8aa20c791`), chrome ext side_panel.ts partial (`2de290670`), **chrome ext background.ts partial (`50b60960a`, new)**. **Remaining (multi-fire per plan's own estimates):** 8 god files — cli tui/chatwidget.rs 9,743 LOC + cli tui/app.rs 8,251 + cli tui/bottom_pane/chat_composer.rs 9,873 (each 12h+) + cli agent.rs 2,773 + cli models.rs 2,616 + desktop chatStore.ts 2,727 + desktop slashCommandHandlers.ts 2,217 + desktop SettingsPanel.tsx 1,995 + desktop mcpStore.ts 1,915 + desktop billingUsage.ts 1,783 + desktop UnifiedAgenticChat/{index,ArtifactRenderer}.tsx 1,716+1,735 + web stripe-webhook 1,725 + web llm-completions 1,725 + web UserSettings 1,661 + web BillingDashboard 1,497. Plus E1 (desktop useAgenticEvents) and E3 (chrome ext side_panel full) which need state-refactor escalations before clean decomposition. |
