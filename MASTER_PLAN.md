...
| Phase A — Critical fixes          | ✅ 18/20 shipped       | Closed: #1 SSRF allowlist (`ceda1ad10`), #2 handoff→HTTP 501 (`a618d13ef`), **#3 /export + /extra-usage verified shipped (`handle_export` at `repl.rs:1274` is a real implementation exporting session as JSON/markdown; `/extra-usage` at `tui_app.rs:1958` shows pricing)**, #4 agent.rs verified false-positive (`c0c012464`), #5 model_catalog.rs panics verified test-only (`fast_completion_model_correct_provider`), #6 models.rs panics verified test-only (`lmstudio_provider_resolved_from_name`), #7 \_attic delete -107K LOC (`0e81d1546`), #8 workspace clippy 33-deny lints (`fceaee92f` + 13 crates inherit `1c1789eaa`) — `unwrap_used`+`expect_used` cleanup of 2,409 sites still pending, #9 mobile TLS pinning guard (`9ca369c03`), #11 chrome ext innerHTML at content.ts:1607 (`0536969c2`), #12 47-site innerHTML sweep + dom-helpers (`069b17bb6`), #13 desktop dispatch_hmac UI verified shipped, #15 web voice/transcribe rate-limit verified, #16 web chat-completions rate-limit verified at line 229, #17 web chat/conversations CSRF verified, #18 google `?key=` verified header-auth, #19 google tool_result.name verified regression-test, #20 browser-tool evaluate verified `allowEvaluate` gated. **Truly remaining (externally blocked):** #10 EAS iOS signingCredentials (needs apple developer secrets from ops), #14 sandbox OS-level Seatbelt/bwrap/AppContainer (multi-day design + Windows CI runner). |
...
| Phase B — God-file splits         | ✅ 3/21 shipped        | (1) cli main.rs 2,385 LOC → lib.rs + 7-LOC entry (8cd6f740f). (2) cli a2a.rs 1,856 LOC → a2a/{mod,protocol,registry,security,server,client,jsonrpc}.rs (dd34923db). (3) vscode extension.ts 1,602 LOC → 253 LOC + lifecycle/{chatSetup,commandSetup,providerSetup}.ts (e11dc7ea1, commit message mislabeled as web). 18 god files remain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Phase C — PNG-grounded features   | ✅ 11/15 shipped       | #C2 desktop adaptive thinking ✓, #C5 web partner-perks ✓ (cb16170b9), #C6 web slash-cmds modal ✓, #C9 mobile offline queue ✓, #C10 mobile theme prefs scaffold ✓, #C11 chrome ext pairing flow ✓, #C12 chrome ext conversation history ✓, #C13 vscode chat-in-editor WebviewPanel ✓, #C14 vscode @mention sidebar ✓, #C8 web homepage verified-shipped. Pending #C1 Cowork tab (3d), #C3 submenu nesting (1d), #C4 Account/Billing tabs (2d), #C7 Show thinking (2d), #C15 marketplace screenshots (needs GUI)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Phase D — Cross-surface polish    | ✅ 8/12 shipped        | Web lint sweep ✓, packages posttest=pnpm build ✓ (19 pkgs), workspace clippy 33-deny lints ✓, vscode TS project refs ✓, `exports` field shipped on all 23 packages, OpenClaw packages 117 tests shipped, 13 utility crates inherit workspace lints ✓, web light-mode token overrides in globals.css ✓ (cb16170b9). Pending: composite workspace-wide, mobile theme component migration + desktop preview persistence, a11y/ARIA audit, insta snapshot tests for cli TUI, domain-first reorg per `REFERENCE_STRUCTURE.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
