...
| Phase B — God-file splits         | 🟡 14/21 shipped       | **Shipped:** cli main.rs, cli a2a.rs, cli tools.rs, cli safety.rs, cli repl.rs, vscode extension.ts, vscode sidebarProvider.ts, vscode agentModeProvider.ts, mobile companion, mobile chatStore.ts, desktop settingsStore.ts, chrome ext side_panel.ts partial, chrome ext background.ts partial, **web BillingDashboard.tsx 1,497 → 420 LOC + `Billing/{types,Subscription,Usage,Topup}.tsx` (web-100's split swept into commit `5df7e1d3b` by lint-staged race — verified live in working tree)**. **Remaining (genuinely multi-fire per plan):** cli tui/chatwidget.rs 9,743 LOC + cli tui/app.rs 8,251 + cli tui/bottom_pane/chat_composer.rs 9,873 (each 12h+) + cli agent.rs 2,773 + cli models.rs 2,616 + desktop chatStore.ts 2,727 + desktop slashCommandHandlers.ts 2,217 + desktop SettingsPanel.tsx 1,995 + desktop mcpStore.ts 1,915 + desktop billingUsage.ts 1,783 + desktop UnifiedAgenticChat/{index,ArtifactRenderer}.tsx 1,716+1,735 + web stripe-webhook 1,725 + web llm-completions 1,725 + web UserSettings 1,661. Plus **E1+E3** (desktop useAgenticEvents, chrome ext side_panel full + background full — same shared-singleton blocker pattern; need a `SharedListenerContext` refactor first). |
