...
| Phase A — Critical fixes          | ✅ 16/20 shipped       | Closed: #1 SSRF allowlist (`a2a.rs:36-101`, `ceda1ad10`), #2 handoff→HTTP 501 (`a618d13ef`), #3 \_attic delete -107K LOC (`0e81d1546`), #4 agent.rs verified (`c0c012464`), **#5 model_catalog.rs:1482/1491 verified test-only (in `#[test] fn fast_completion_model_correct_provider`, not user-reachable)**, **#6 models.rs:2270/2293/2334 verified test-only (in `#[test] fn lmstudio_provider_resolved_from_name` etc.)**, #8 workspace clippy 33-deny lints (`fceaee92f` + 13 crates inherit `1c1789eaa`), #9 mobile TLS pinning guard (`9ca369c03`), #11 chrome ext innerHTML (`0536969c2`), #12 47-site innerHTML sweep (`069b17bb6`), #13 dispatch UI verified shipped, #15 web voice rate-limit verified (alias inherits canonical), #16 web chat-completions rate-limit verified (line 229), #17 web chat/conversations CSRF verified, #18 google ?key= verified header-auth, #19 google tool_result.name verified regression-test, #20 browser-tool evaluate verified allowEvaluate gated. Deferred (irreducibly multi-week per plan): #8 `unwrap_used`/`expect_used` adoption (2,409 sites pending Phase B+ cleanup), #10 EAS signing credentials (ops blocker), #14 sandbox OS-level Seatbelt/bwrap/AppContainer (multi-day design). |
