...
| Phase B — God-file splits         | ✅ **22/25 fully closed** · 3 partial · all escalations closed | **Fully closed (22/25):** cli main.rs, cli a2a.rs, cli tools.rs, cli safety.rs, cli repl.rs, cli agent.rs, cli models.rs, desktop settingsStore.ts, desktop SettingsPanel.tsx, desktop mcpStore.ts, desktop slashCommandHandlers.ts, desktop chatStore.ts, desktop billingUsage.ts, desktop UnifiedAgenticChat/index.tsx, desktop ArtifactRenderer.tsx, **desktop useAgenticEvents.ts (E1 closed via `9066869de` SharedListenerContext refactor)**, web stripe-webhook, web llm-completions, web UserSettings (Phase B.2 via `d0f84d94f` Notifications+System panels), web BillingDashboard, mobile companion, mobile chatStore.ts, **chrome ext side_panel.ts + background.ts (E3 closed via `6741ee045` SharedSidePanelContext+SharedBackgroundContext)**, vscode extension.ts, vscode sidebarProvider.ts, vscode agentModeProvider.ts. **Partial (3/25 — multiple chunks extracted, full split is multi-fire per plan's 12h estimates):** cli tui/chatwidget.rs (now ~8,800 LOC after 5 chunks: Notifications, rate_limit, message_merge, exec, plan); cli tui/app.rs (now ~7,800 LOC after 2 chunks: thread_event_store, plugin_io); cli tui/bottom_pane/chat_composer.rs (now ~9,400 LOC after 2 chunks: text_ops, render). **All E1+E2+E3 escalations CLOSED.** |
