...
### OPEN P0 — 4 OPEN (Wave-8 hunt `wnt2merph`, 2026-05-29) — feed Wave 9
- **A1 (cli)** `onboarding.rs:223` — v1 LOCAL-ONLY first-run impossible (ollama not in `ALL_PROVIDERS` → falls through to cloud login; picker writes a cloud default). Breaks the core local-only thesis. [OPEN → Wave 9]
- **A2 (cli)** `chat.rs:304-365` — PRIVACY-01 hard-lock violation: fallback chain egresses Local context to cloud (`validate_privacy_boundary` runs once, not inside the loop). +A23 continuation loop. [OPEN → Wave 9, sequential after F5/chat.rs]
- **A3 (mobile)** `index.js`→`tier2.ts:76` — `initExecutorch` never called → `tier2LoadModel` throws on clean install → NO real offline generation. **Corrects the 2c "real offline response" claim (that test mocked the module).** +A14 `ET_VERSION_TAG` prefix. [OPEN → Wave 9]
- **A4 (mobile)** `waitlist/service.ts:77` — mobile waitlist POST always 403 (no CSRF token) → no signup row written → breaks 2d funnel rollup. [OPEN → Wave 9]
