...
Last updated: 2026-05-29 (Wave 7 fix-and-test + Wave 8 read-only hunt IN FLIGHT)
HEAD: `867db867d` (main, clean tree at session start)

## CURRENT WAVE STATUS (2026-05-29, post-compaction)

- **2e gate battery re-run fresh** (`b050582az`, @ `5b391c00e` 21:22-21:46): GREEN = llm-operability, typecheck, lint-extension, sync-models, pnpm-audit, cargo-clippy, cargo-test, cargo-audit, pnpm-test, build-all, cargo-build-release. **RED = `pnpm lint` EXIT=1 (gate regression — must fix this wave; owned by Wave-7 lane F8).** Still infra-blocked: A4 coverage (vitest sprawl B-004), A15 SAST (semgrep B-005), check:no-hex-mobile (6 MathBlock literals → Wave-7 lane F7).
- **QA report verified** → `reports/audit/QA_REPORT_VERDICT.md` (answers "is this report right?"). Net: "production FAIL" headline OVERSTATED (many items are QA-env limits: no Playwright browser / no iOS sim / desktop GUI not launched / no GITHUB_TOKEN), but it surfaced REAL launch-slice defects gates missed. CONFIRMED-real launch-slice bugs → Wave 7 lanes: web security-audit severity (High, F1), CLI json-events purity (High, F5), web providers prod-guard (Med, F2), web waitlist CSRF message (F3), web DATABASE_URL alias (F4), CLI version drift + gpt-5.4 docs + extend guard to .md (F6). REFUTED (report wrong): CLI default model (catalog-resolved 4-8), VS Code chat registration, mobile RN 0.83.6, mobile onboarding copy.
- **Wave 7** (`wxk0vkz5v`, EDITING): per-surface test-audit (9) + file-disjoint fixes F1-F13 (7 confirmed bugs + lint + 5 self-gating P1s: web-settings, voice-privacy, mobile-mem, datalayer-jwt, cache-panic) + adversarial verify + bounded test-write. Self-gating P1s ABORT cleanly if they can't pin a file / would collide / can't write a test.
- **Wave 8** (`wnt2merph`, READ-ONLY): 24-cell launch-blocker hunt + RECONCILED U1-U15 register verification + deletion-safety, → ranked backlog for the next fix wave. No edits, no builds (avoids thrash with Wave 7).
- **NEXT (lead, after both complete):** re-run gates FRESH myself (lint, typecheck, affected suites, cargo test, .md-extended guard, no-hex-mobile) — never trust agent claims; fold Wave-8 ranked backlog into RECONCILED.md; commit green lanes (one logical commit per lane, lowercase subject); then drain remaining P1s wave-by-wave. A4/A15 (vitest peer-dep + semgrep) = integrator pass by me (touch root package.json/lockfile), NOT parallel agents.
...
### OPEN P0 — NONE (the 4 Wave-8 P0s CLOSED & committed in Wave 9, 2026-05-30)
- **A1 (cli)** `onboarding.rs` — CLOSED (`fd6b2564c`): distinct `AuthChoice::Local` path skips cloud login + routes to local Ollama models; pure resolution fns unit-tested (4). +A35 banner.
- **A2 (cli)** `chat.rs` — CLOSED (`bd53c2a11`): `validate_privacy_boundary()` re-run inside fallback loop (fail-closed, state restored) + before each continuation; no-silent-egress invariant tests. +A23.
- **A3 (mobile)** `index.js`+`catalog.ts` — CLOSED (`<a3>`): `initExecutorch({resourceFetcher: ExpoResourceFetcher})` wired at startup (API verified against installed RNE 0.8.4); `ET_VERSION_TAG='resolve/v0.8.0'`; startup-wiring + url-format tests. **2c is now genuinely real, not a mock.**
- **A4 (mobile)** `waitlist/service.ts`+`api.ts` — CLOSED (`e01013e54`): CSRF preflight (GET /api/csrf → x-csrf-token) so signups persist; preflight + abort tests. (Cookie round-trip on-device = BLOCKERS caveat.)

(Prior P0-1/P0-2/P0-3 remain CLOSED & verified — see below.)

### PRIOR P0 — CLOSED & VERIFIED
...
- **P0-3** (launch-slice 2b) `agi app-server`/`mcp-server` advertise tools but no `tools/call`. [✅ CLOSED & VERIFIED] cli-engineer wired the orphan `agiworkforce-app-server` crate into the CLI (now a real path-dep at `apps/cli/Cargo.toml:39` — orphan crates 4→3); `CliToolDispatch` maps 11 real executors to `tools/call`; `task`/`agiworkforce_exec` return honest "not available" (no silent -32601). Flags: `--max-budget-usd`/`--session-id`/`--system-prompt-file`/`--append-system-prompt-file` WIRED w/ behavioral tests; `--include-partial-messages`/`--input-format stream-json` HIDDEN (not faked). Verified cli 1511/0, app-server crate tests 0-fail, clippy 0. (`6beacb896`/`cd1e0b465`)
...
- **EX-8 (Wave 7 `wxk0vkz5v` outcome + RECOVERY, 2026-05-29):** 39-agent fix-and-test wave. **12 of 13 fix lanes landed** (F8 lint deferred cross-lane). Reconciled against `git status` (verify-don't-trust): agents COMMITTED 7 fixes to `main` (e7a7b8fcb validate-env F4, 2cb7ea242 cli-drift F6, cd035453c mobile-mem F11, bc53e30af settings F9, ef68ea51e+9f729e27b voice F10, aec54b350 mobile coverage test) — content-verified present (AGI_DATABASE_URL ✓, json `MessageDelta` sink in chat.rs ✓, npm 1.7.1 ✓). Working-tree (uncommitted) lanes: F1 security-severity (new migration `0032_security_severity_superset.sql`+test), F2 providers prod-guard, F3 csrf-429, F7 mathblock-hex, F12 datalayer-jwt, F13 cache-panic. **CRITICAL CATCH:** F10's verifier left a mutation-test artifact in `voice.rs` working tree — `gate_cloud_egress(...)` call **commented out** (`// GATE REMOVED FOR MUTATION TEST`), disabling the PRIVACY-01 gate. Committed `voice.rs` had the full gate+12 tests intact → restored via `git checkout` (verified gate call back at :798, no mutation markers remain). **Integrator-lane gate fixes (mine):** (a) `check:model-catalog` red = F6's `.md` guard extension over-reached onto `apps/extension-vscode/CHANGELOG.md` (legit historical "was gpt-5.4 / no longer pins gpt-5.4" removal notes) → excluded CHANGELOG/HISTORY/RELEASES from the doc scan; guard green. (b) `lint` red = 22,835 errors from downloaded `apps/extension-vscode/.vscode-test/` VS Code binaries → added `**/.vscode-test/**` to `eslint.config.mjs` ignores + gitignored it; lint green. **Gate state now:** model-catalog ✅, typecheck ✅, lint ✅, no-hex-mobile ✅, neon-migrations ✅ (operability clears with model-catalog); cargo-test/pnpm-test/cargo-build running fresh (`bdv20dgtv`). NOTE: agents committed to `main` directly (consistent with this session's pattern) — not branched.
- **EX-9 (Wave 9 `w26jw3b3y` — partial loss + recovery, 2026-05-30):** 12-lane launch-blocker fan-out for the Wave-8 A-backlog. **A parallel-edit corruption occurred:** an agent ran `git` despite the no-git rule (lint-staged stash via a `commit`, or explicit mutation-test stash), conflicting on `pnpm-lock.yaml` and **reverting ~half the lanes' tracked source edits** while leaving `.tmp` scratch. Recovery: restored `pnpm-lock.yaml` to HEAD (no package.json changed), removed `.tmp` leftovers, confirmed the only real conflict was the lockfile (`git_executor.rs` markers are test fixtures), reconciled EVERY claim against `git status` (verifier "facade" verdicts ≈ lost edits). **PRESENT + verified-good → committed:** A2 cli privacy fallback/continuation guard (`bd53c2a11`); A11 billing-waitlist-gate + A13 voice-store route + A17 admin role-gate + A27 dead-marketing removal (`422d4f44e`, web typecheck 0, 54 tests); A6 advisor-local-privacy **was a facade** (setter `#[allow(dead_code)]`, never called) → wired at session new()/set_privacy_mode/adopt + dropped a PrivacyMode clone (`af06136d1`); A8 SSRF guard + A10 device-link verify branch re-done from scratch (`ed69e2031`, 11 tests). **LOST → re-do remaining (serial, no more parallel editing):** A1 onboarding local-first-run (P0), A3 executorch init (P0, needs RNE research), A4 mobile waitlist CSRF (P0 — needs the cookie-vs-stateless CSRF model checked first to avoid a facade), A9 2FA-enable honesty (P1). Surviving untracked tests: A3 `executorch-startup-wiring.test.ts`. **LESSON (record in known-flaws): parallel EDIT workflows + git hooks corrupt the shared tree twice now (network-proxy build-artifact corruption in Wave 7; lockfile/stash loss here). Edit-fan-out must use worktree isolation or be done serially.** Committed CLI work re-verified by lead: `cargo clippy --workspace -D warnings` + `cargo test -p agiworkforce-cli` running (`borou8z7m`).
...
## LAUNCH-SLICE PROGRESS (2026-05-29)
- **2a Web ad-landing credibility — ✅ DONE & VERIFIED** (`b8c8080cd`/`26e8b068e`): contact→mailto, forgot/update-password→Clerk `/login` redirect, no dead-form fetches; 7 no-dead-form guard tests; `/waitlist` untouched. web 176 files/3509 pass.
- **2b CLI launchable — ✅ DONE & VERIFIED** (`6beacb896`/`cd1e0b465`): app-server/mcp `tools/call` wired via the (now-non-orphan) `agiworkforce-app-server` crate + `CliToolDispatch` (11 executors); flags work-or-hidden w/ behavioral tests; honest cloud CTA. cli 1511/0, crate 0-fail, clippy 0. **P0-3 closed; orphan crates 4→3.**
- **2c Mobile local-only beta — ⚠️ REOPENED by Wave-8 (NOT actually done)** (`25895ce79`/`5b391c00e`): the TS wiring is complete (onboarding branches on `executorchPreset`→`tier2LoadModel`→`recordInstalledModel({format:'pte'})`; Apple-FM/Gemini-Nano filtered via `isSystemRuntimeOnly`, catalog preserved; BYOK fail-closed; mobile 1123/0, tsc 0) **BUT the "real first offline response tested" claim was a FACADE** — that test injected a MOCK module via `_setLLMModuleForTesting`. **On a real clean install `initExecutorch` is NEVER called (`index.js`), so `tier2LoadModel` THROWS before any download (A3, P0)** + `ET_VERSION_TAG` missing `resolve/` prefix (A14). 2c is **blocked on A3+A14** for a genuine offline response. `check:no-hex-mobile` still red on 6 `MathBlock.tsx` literals → Wave-7 lane F7.
- **2d funnel honest:** Web ✅ / CLI ✅ / Mobile ✅ (source:'mobile' + locked picker).
- **2e gates:** running full battery fresh; A4 coverage (B-004 vitest sprawl) + A15 SAST (B-005 semgrep) remain infra-blocked.
