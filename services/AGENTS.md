...
- Read-only context: `cloudDb/**`, `packages/contracts/types/**`, and `packages/platform/data-layer/**`.
...
- Shared schemas belong in `packages/contracts/types`; apps and packages must not import service internals.
...
- Managed cloud is in public alpha and open by default (founder decision, 2026-06-27); the private-beta/waitlist launch gate has been removed. `AGI_MANAGED_COMPUTE_PRIVATE_BETA` remains only as an incident-response kill-switch. Ledgering, metering, fraud, refunds, chargebacks, abuse controls, retention, deletion, and provider terms must keep pace with public usage, but they no longer gate access.
