...
- `ZHIPU_MODEL_CATALOG` — curated catalog, sourced from `packages/types/src/models.json`.
- `applyZhipuThinkingMode` — request-param mutator for the thinking-mode quirk below.

## Known Quirks

`open.bigmodel.cn` is not in `@agiworkforce/llm-normalize`'s bundled hostname
