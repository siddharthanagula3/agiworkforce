 ...
 *
 * Anthropic's SDK (`APIError`) exposes `headers: Headers | undefined` on
 * caught errors. We accept either a `Headers` instance or a plain
 * `Record<string, string>` object so this helper also works in tests with
 * stubbed errors.
 */
...
/**
 * Pull the `Retry-After` from an SDK error if present. The Anthropic and
 * OpenAI SDKs both attach the response headers to thrown errors as either
