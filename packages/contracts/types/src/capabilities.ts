 ...
 */
export function surfaceCapabilityGrant(
  platform: SyncedAppSurface,
): ReadonlySet<PlatformCapability> {
  const row = getPlatformCapabilities(platform);
  return new Set(ALL_PLATFORM_CAPABILITIES.filter((capability) => row[capability]));
}
