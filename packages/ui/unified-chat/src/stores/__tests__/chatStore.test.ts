...
describe('useChatStore — misc state actions', () => {
  ...
  it('appends external context without replacing the active unsent draft', () => {
    useChatStore.getState().setDraftContent('Keep my typed request');

    useChatStore.getState().appendDraftContent('Reviewed browser context');

    expect(useChatStore.getState().draftContent).toBe(
      'Keep my typed request\n\nReviewed browser context',
    );
  });

  it('keeps unsent drafts isolated when the active conversation changes', () => {
    useChatStore.getState().setActiveConversation('conv-a');
    useChatStore.getState().setDraftContent('Draft for A');
    useChatStore.getState().setActiveConversation('conv-b');
    useChatStore.getState().setDraftContent('Draft for B');

    useChatStore.getState().setActiveConversation('conv-a');
    expect(useChatStore.getState().draftContent).toBe('Draft for A');

    useChatStore.getState().setActiveConversation('conv-b');
    expect(useChatStore.getState().draftContent).toBe('Draft for B');
  });

  it('clears only the originating conversation draft after a conversation switch', () => {
    useChatStore.getState().setActiveConversation(null);
    useChatStore.getState().setDraftContent('New-chat draft');
    useChatStore.getState().setActiveConversation('created-conversation');
    useChatStore.getState().setDraftContent('Created conversation draft');

    useChatStore.getState().clearDraftContent(null);

    expect(useChatStore.getState().draftContent).toBe('Created conversation draft');
    useChatStore.getState().setActiveConversation(null);
    expect(useChatStore.getState().draftContent).toBe('');
  });
