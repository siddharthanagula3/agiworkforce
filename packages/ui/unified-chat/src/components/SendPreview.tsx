...
export function SendPreview({
    ...
    return (
      ...
      >
        <button
          ...
          className="inline-flex h-6 items-center gap-1 rounded-full px-1.5 text-caption font-medium text-[var(--chat-text-muted)] transition-colors hover:bg-[var(--chat-surface-overlay)] hover:text-[var(--chat-text-secondary)]"
        >
        ...
        {expanded ? (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-caption font-medium uppercase tracking-wide text-[var(--chat-text-secondary)]">
                <Lock className="h-3 w-3" aria-hidden />
                {presentation.privacyShortLabel}
              </span>
            </div>
            <p className="mt-2 text-caption leading-relaxed text-[var(--chat-text-secondary)]">
              {presentation.bannerCopy}
            </p>
            {detailsAvailable ? (
              <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-caption text-[var(--chat-text-secondary)]">
                {presentation.bodyCharLabel ? (
  ...
  return (
    <div
      ...
      data-stays-local={presentation.staysLocal ? 'true' : 'false'}
      className={cn(
        'flex flex-col gap-0.5 rounded-md border px-3 py-1 text-xs',
        accentClass,
        className,
      )}
    >
      <div className="flex items-center gap-2">
        ...
        <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-[var(--chat-border)] bg-[var(--chat-surface-overlay)] px-2 py-0.5 text-caption font-medium uppercase tracking-wide text-[var(--chat-text-secondary)]">
          <Lock className="h-3 w-3" aria-hidden />
          {presentation.privacyShortLabel}
        </span>
      ...
      <button
        ...
        aria-expanded={expanded}
        aria-controls={detailsId}
        className="inline-flex items-center gap-1 self-start text-caption font-medium uppercase tracking-wide text-[var(--chat-text-muted)] hover:text-[var(--chat-text-secondary)]"
      >
        {expanded ? (
          <>
      ...
      {expanded ? (
        <div id={detailsId} data-testid="send-preview-details" className="flex flex-col gap-2">
          <p className="text-caption leading-relaxed text-[var(--chat-text-secondary)]">
            {presentation.bannerCopy}
          </p>
          {detailsAvailable ? (
            <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-caption text-[var(--chat-text-secondary)]">
              {presentation.bodyCharLabel ? (
                <DetailRow term="Message" definition={presentation.bodyCharLabel} />
              ) : null}
              {presentation.attachmentLabel ? (
