...
describe('isMessageContinuable', () => {
  ...
  it('does NOT offer Continue when the partial content is empty', () => {
    expect(
      isMessageContinuable(assistantMessage({ content: '', metadata: { finishReason: 'stopped' } })),
    ).toBe(false);
    expect(
      isMessageContinuable(
  ...
  it('does NOT offer Continue while streaming, on errored turns, or on non-assistant roles', () => {
    ...
    expect(
      isMessageContinuable(assistantMessage({ error: 'boom', metadata: { finishReason: 'stopped' } })),
    ).toBe(false);
    expect(
      isMessageContinuable(assistantMessage({ role: 'user', metadata: { finishReason: 'length' } })),
    ).toBe(false);
