...
const MarkdownImage = ({ src, alt, title }: { src?: string; alt?: string; title?: string }) => {
  ...
  return (
    ...
    >
      ...
      {/* eslint-disable-next-line @next/next/no-img-element -- data:/blob: and
          arbitrary remote sources are not compatible with next/image without
          per-host remotePatterns config; a raw <img> is the robust path here. */}
      <img
        src={src}
        alt={alt || ''}
        title={title}
