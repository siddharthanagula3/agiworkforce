/**
 * SpreadsheetArtifact — inline editable spreadsheet renderer.
 *
 * Surface-agnostic: no Tauri imports, no desktop-specific deps.
 * Parses artifact.content as a JSON array-of-objects and renders
 * a tabular view with optional in-place cell editing.
 */
    ...
    >
      ...
      <div className="flex items-center justify-between px-2 py-1.5 border-b bg-muted/30">
        ...
        <button
          ...
          className="flex items-center gap-1.5 h-7 px-2 text-xs rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
                    ...
                    >
                      {isEditing ? (
                        <input
                          autoFocus
                          type="text"
                          className="w-full h-full px-2 py-1.5 bg-card text-sm focus:outline-none text-foreground"
                          value={editValue || ''}
