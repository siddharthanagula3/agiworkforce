/**
 * Cloud contract — signed **org policy** (enterprise Local, design
 ...
 * lives with the cloud contracts because it is a cross-surface wire schema, but
 * it is NOT a cloud endpoint — it is a signed file, verified fully offline.
 *
 ...
 * a later, per-surface step). The `packages/types/src/suite-contracts.ts`
 * mirror named in the design is intentionally deferred — the operative task
 * scopes this to the cloud-contracts location.
 */

import { verifySignedContainer, type LicenseClaims } from '@agiworkforce/licensing';
import { z } from 'zod';
