...
import { validateGatewaysCatalog } from './validate-gateways.mjs';
...
const RETIRED_MODELS_SCHEMA_VERSION = 2;
...
const REASONING_TOKEN_BILLING_CLASSES = new Set(['included_in_output', 'additional_to_output']);
...
const GOVERNANCE_FIELDS = [
  ...
  'reasoningTokenBillingSource',
  'reasoningTokenBillingVerifiedOn',
  'reasoningTokenBillingNote',
];
...
function normalizeProviderGovernance(governanceCatalog) {
  ...
  for (const [providerId, entry] of Object.entries(governanceCatalog.governance)) {
    ...
    if (entry.reasoningTokenBillingClass !== undefined) {
      assert.ok(
        REASONING_TOKEN_BILLING_CLASSES.has(entry.reasoningTokenBillingClass),
        `${label} reasoningTokenBillingClass ${String(entry.reasoningTokenBillingClass)} is not a known class`,
      );
      assert.ok(
        typeof entry.reasoningTokenBillingSource === 'string' &&
          (entry.reasoningTokenBillingSource.startsWith(GOVERNANCE_SOURCE_URL_PREFIX) ||
            entry.reasoningTokenBillingSource.startsWith(REPOSITORY_CONTRACT_SOURCE_PREFIX)),
        `${label} must cite an https source or the repository trust contract for reasoningTokenBillingClass`,
      );
      assert.ok(
        typeof entry.reasoningTokenBillingVerifiedOn === 'string' &&
          ISO_DATE_PATTERN.test(entry.reasoningTokenBillingVerifiedOn),
        `${label} must carry the calendar day reasoningTokenBillingClass was verified`,
      );
    }
...
function normalizeProviderComputePricing(computePricingCatalog) {
  ...
  for (const [providerId, entry] of Object.entries(computePricingCatalog.computePricing)) {
    ...
    assert.ok(
      entry.ramRatePerGibSecond === undefined ||
        (typeof entry.ramRatePerGibSecond === 'number' && entry.ramRatePerGibSecond > 0),
      `${label} ramRatePerGibSecond must be a positive number when declared`,
    );
    assert.ok(
      typeof entry.source === 'string' &&
        (entry.source.startsWith(GOVERNANCE_SOURCE_URL_PREFIX) ||
...
const GATEWAY_PROTOCOL_HARNESS_PROTOCOL = {
...
function gatewayGovernanceReviewedOn(governance) {
  const source = governance?.source;
  const verifiedOn = governance?.verifiedOn;
  if (typeof source !== 'string' || !source.startsWith(HTTPS_URL_PREFIX)) return undefined;
  if (typeof verifiedOn !== 'string' || !ISO_DAY_PATTERN.test(verifiedOn)) return undefined;
  return verifiedOn;
}
...
function normalizeGateways(gatewayCatalog, harnessCatalog) {
  const referenced = new Set(
    Object.values(harnessCatalog.harnesses)
      .map((harness) => harness.gatewayId)
      .filter((gatewayId) => gatewayId !== undefined),
  );
  const normalized = {};
  for (const gatewayId of [...referenced].sort()) {
    const gateway = gatewayCatalog.gateways[gatewayId];
    assert.ok(gateway, `Harness catalog references unknown gateway ${gatewayId}`);
    assert.ok(
      GATEWAY_PROTOCOL_HARNESS_PROTOCOL[gateway.protocol],
      `Gateway ${gatewayId} protocol ${gateway.protocol} maps to no harness wire protocol`,
    );
    for (const field of ['baseUrlEnv', 'apiKeyEnv']) {
      assert.ok(
        API_KEY_ENV_PATTERN.test(gateway[field] ?? ''),
    ...
    normalized[gatewayId] = defined({
      ...
      governanceReviewedOn: gatewayGovernanceReviewedOn(gateway.governance),
    });
  }
  return normalized;
}

function normalizeHarnesses(harnessCatalog, gatewayCatalog) {
  const normalized = {};
  for (const [harnessId, harness] of Object.entries(harnessCatalog.harnesses)) {
    ...
    } else if (harness.gatewayId !== undefined) {
      const gateway = gatewayCatalog.gateways[harness.gatewayId];
      assert.ok(gateway, `Harness ${harnessId} references unknown gateway ${harness.gatewayId}`);
      assert.equal(
        harness.baseUrl,
        undefined,
        `Harness ${harnessId} names a gateway and must take its base URL from the definition, not a literal`,
      );
      assert.equal(
        harness.apiKeyEnv,
        undefined,
        `Harness ${harnessId} names a gateway and must take its credential from the definition, not a literal`,
      );
      assert.equal(
        protocol,
        GATEWAY_PROTOCOL_HARNESS_PROTOCOL[gateway.protocol],
        `Harness ${harnessId} protocol ${protocol} does not speak gateway ${harness.gatewayId}'s ${gateway.protocol}`,
      );
    } else {
      assert.ok(
        typeof harness.baseUrl === 'string' && harness.baseUrl.length > 0,
        `Harness ${harnessId} declares protocol ${protocol} without a baseUrl or gatewayId to dispatch to`,
      );
      assert.equal(
    ...
    assert.equal(
      harness.gatewayId !== undefined && protocol === HARNESS_PROTOCOL.providerNative,
      false,
      `Harness ${harnessId} names a gateway without naming the wire protocol it speaks`,
    );
    assert.ok(
      hostPolicy === HARNESS_HOST_POLICY.allowlistOnly ||
        harness.baseUrl !== undefined ||
        harness.gatewayId !== undefined,
...
/**
 * Retirement is recorded, not inferred.
 *
 * A retired model leaves `models.curation.json` entirely, and once it is gone
 * nothing can reconstruct what it was: a conversation that used it keeps only
 * the id on the row, so it renders as a raw string with no name and no
 * capabilities. `retired` therefore carries the snapshot forward, and
 * `grandfathered` is the closed tail retired before that rule existed, whose
 * catalog entries no longer survive anywhere to be recovered.
 */
export function loadRetiredModels(file = RETIRED_MODELS_JSON) {
  const raw = readJson(file);
  assert.equal(
    raw.schemaVersion,
    RETIRED_MODELS_SCHEMA_VERSION,
    `retired-models.json must be schema ${RETIRED_MODELS_SCHEMA_VERSION}`,
  );
  const grandfathered = raw.grandfatheredWithoutMetadata ?? [];
  const records = raw.retiredModels ?? [];
  const seen = new Set();
  for (const record of records) {
    assert.ok(
      typeof record.id === 'string' && record.id.length > 0,
      'a retired record needs an id',
    );
    assert.ok(!seen.has(record.id), `${record.id} is listed twice in retiredModels`);
    seen.add(record.id);
    for (const field of RETIRED_RECORD_FIELDS) {
      assert.ok(
        record[field] !== undefined && record[field] !== null,
        `retired model ${record.id} must preserve ${field}; a model leaving the catalog brings its record with it`,
      );
    }
  }
  for (const id of grandfathered) {
    assert.ok(!seen.has(id), `${id} is both grandfathered and recorded; it belongs only in one`);
  }
  assert.deepEqual(
    [...raw.retiredModelIds].sort(),
    [...seen, ...grandfathered].sort(),
    'retiredModelIds must be exactly the recorded retirements plus the grandfathered tail',
  );
  return { records, grandfathered, ids: new Set(raw.retiredModelIds) };
}

function normalizeRetiredModels(retired) {
  const entries = [
    ...retired.records.map((record) => [record.id, { ...record, metadataPreserved: true }]),
    ...retired.grandfathered.map((id) => [id, { id, metadataPreserved: false }]),
  ];
...
function buildNormalizedRegistry(
  ...
  for (const [modelKey, model] of Object.entries(catalog.models)) {
    ...
    pricing[modelKey] = defined({
      ...
      sessionPerMinute: model.sessionPerMinuteCost,
  ...
  const familySnapshot = {
    ...
    limits,
    benchmarks,
    retiredModelKeys: retiredModels.ids,
  };
  ...
  return {
    ...
    gateways: normalizeGateways(gatewayCatalog, harnessCatalog),
    runtimeProfiles,
    capabilities,
    ...
    benchmarks,
    evidence,
    retiredModels: normalizeRetiredModels(retiredModels),
...
async function buildNormalizedArtifacts(catalog, familyCatalog, syncedSnapshot) {
  ...
  const gatewayCatalog = readJson(GATEWAYS_JSON);
  const gatewayErrors = validateGatewaysCatalog(gatewayCatalog);
  assert.deepEqual(gatewayErrors, [], `gateways.json is invalid:\n${gatewayErrors.join('\n')}`);
...
export function loadFamilySnapshot() {
  ...
  return {
    ...
    snapshot: {
      ...
      retiredModelKeys: loadRetiredModels().ids,
    },
  };
}
