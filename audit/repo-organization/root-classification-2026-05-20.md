...
| `audit/audit-log.md`                 | moved-audit    | Living audit fire log moved from root to `audit/`.                            |
...
| `app.json`                           | deleted        | Stale root Expo placeholder; canonical config is `apps/mobile/app.config.js`. |
...
3. Keep `audit/audit-log.md` under `audit/`; do not recreate root `AUDIT_LOG.md`.
4. Decide owner path for `ollama-manifest.json` and `skills-lock.json`.
