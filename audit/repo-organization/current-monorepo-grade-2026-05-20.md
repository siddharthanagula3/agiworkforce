...
The remaining work is no longer hidden repo entropy. Web, Mobile component-heavy domains, and the full Desktop component-domain tree now have explicit feature homes. The open organization work is narrower: Mobile's remaining layer-first services/hooks should continue moving only when ownership is obvious, and provisional CODEOWNERS should be replaced with real GitHub teams after the team exists.
...
| Product-code discoverability | A+    | Web feature-root split is fixed; Mobile component-heavy domains are under `src/features`; Desktop product UI domains are under `src/features`; `apps/desktop/src/components` and `apps/mobile/components` are now shared UI primitive roots only. |
...
4. **Web feature-root, Mobile component domains, Desktop component domains, and release drift are closed.**
   `apps/web/features` is now canonical, `apps/web/src/features` is forbidden by `pnpm check:structure-conventions`, and active Web commands use `@agiworkforce/web`. Mobile waitlist, projects, billing, schedules, component-heavy domains, voice services, and messaging service/state now have canonical domains under `apps/mobile/src/features`. Desktop product UI domains now have canonical homes under `apps/desktop/src/features`, and `apps/desktop/src/components` is reserved for shared UI primitives. The duplicate CLI release workflow is removed; `release-cli.yml`, `install.sh`, and Homebrew asset names align. Mobile's remaining feature-specific services/hooks still need one-domain-at-a-time moves when ownership is obvious.
