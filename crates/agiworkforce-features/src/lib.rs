...
pub enum Feature {
    ...
    ImageDetailOriginal,
    /// Compress request bodies (zstd) when sending streaming requests to agiworkforce-backend.
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openai.com/agiworkforce/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: JavaScript REPL is now available in /experimental. Enable it, then start a new chat or restart AgiWorkforce to use it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            name: "Apps",
            menu_description: "Use a connected ChatGPT App using \"$\". Install Apps via /apps command. Restart AgiWorkforce after enabling.",
            announcement: "NEW: Use ChatGPT Apps (Connectors) in AgiWorkforce via $ mentions. Enable in /experimental and restart AgiWorkforce!",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while AgiWorkforce is running a thread.",
