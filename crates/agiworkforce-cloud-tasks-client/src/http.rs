...
use agiworkforce_backend_client as backend;
use agiworkforce_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("AGIWORKFORCE_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
