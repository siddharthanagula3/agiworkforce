...
use agiworkforce_config::default_project_root_markers;
use agiworkforce_config::merge_toml_values;
use agiworkforce_config::project_root_markers_from_config;
...
use agiworkforce_utils_plugins::plugin_namespace_for_skill_path;
use dirs::home_dir;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$AGIWORKFORCE_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
...
async fn discover_skills_under_root(
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Agiworkforce itself.
