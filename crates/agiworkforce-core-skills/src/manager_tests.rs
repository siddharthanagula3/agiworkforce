...
fn write_user_skill(agiworkforce_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = agiworkforce_home.path().join("skills").join(dir);
...
fn user_config_layer(agiworkforce_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(agiworkforce_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(agiworkforce_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(agiworkforce_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(agiworkforce_home, user_config_toml),
            ConfigLayerEntry::new(
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let stale_system_skill_dir = agiworkforce_home.path().join("skills/.system/stale-skill");
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&agiworkforce_home, "");
    ...
    write_user_skill(&agiworkforce_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_manager, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_cwd_loads_repo_user_and_extra_roots_with_local_fs() {
    ...
    write_user_skill(&agiworkforce_home, "user", "user-skill", "from local user root");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&agiworkforce_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_agiworkforce_folder: repo_dot_codex.abs(),
                },
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    ...
    let config_layer_stack = config_stack(&agiworkforce_home, "[skills.bundled]\nenabled = false\n");
    let skills_manager = SkillsManager::new(
        agiworkforce_home.path().abs(),
        /*bundled_skills_enabled*/ false,
    );
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = agiworkforce_home.path().join("skills").join("demo");
    ...
    let parent_stack = config_stack(&agiworkforce_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&agiworkforce_home, &disabled_skill_config, &enabled_skill_config);
    let skills_manager = SkillsManager::new(
        agiworkforce_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );
