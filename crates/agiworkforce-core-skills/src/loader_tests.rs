...
use agiworkforce_utils_absolute_path::test_support::PathExt;
...
async fn make_config(agiworkforce_home: &TempDir) -> TestConfig {
    make_config_for_cwd(agiworkforce_home, agiworkforce_home.path().to_path_buf()).await
}
            ...
            dot_codex.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_agiworkforce_folder: dot_codex.abs(),
                    },
...
async fn make_config_for_cwd(agiworkforce_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = agiworkforce_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = agiworkforce_home.path().join("etc/codex/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let user_folder = home_folder.join("agiworkforce");
...
async fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    let layers = vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_agiworkforce_folder: project_dot_codex,
            },
            TomlValue::Table(toml::map::Map::new()),
            "marked untrusted",
        ),
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&agiworkforce_home, "demo", "dep-skill", "from json");
...
async fn loads_skill_interface_metadata_from_yaml() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Agiworkforce, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $AGIWORKFORCE_HOME/skills/cycle/loop -> $AGIWORKFORCE_HOME/skills/cycle
    let cycle_dir = agiworkforce_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&agiworkforce_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = agiworkforce_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = agiworkforce_home.path().join("skills");
    let outcome = load_skills_from_roots([SkillRoot {
...
async fn skips_hidden_and_invalid() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = agiworkforce_home.path().join("skills/.hidden");
    ...
    let invalid_dir = agiworkforce_home.path().join("skills/invalid");
...
async fn enforces_length_limits() {
    ...
    write_skill(&agiworkforce_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&agiworkforce_home).await;
    ...
    write_skill(&agiworkforce_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert_eq!(outcome.skills.len(), 1);
    assert_eq!(outcome.errors.len(), 1);
    assert!(
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_agiworkforce_dir() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_agiworkforce_dirs_under_project_root() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&agiworkforce_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_agiworkforce_dir_when_not_git_repo() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&agiworkforce_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_agiworkforce_dirs() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&agiworkforce_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&agiworkforce_home, file_path).await;
