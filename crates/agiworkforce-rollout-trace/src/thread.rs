...
use crate::protocol_event::agiworkforce_turn_trace_event;
...
pub const AGIWORKFORCE_ROLLOUT_TRACE_ROOT_ENV: &str = "AGIWORKFORCE_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `AGIWORKFORCE_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Agiworkforce
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(AGIWORKFORCE_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Agiworkforce turn lifecycle events from protocol lifecycle events.
    pub fn record_agiworkforce_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            agiworkforce_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Agiworkforce did while executing that boundary.
    pub fn record_tool_call_event(&self, agiworkforce_turn_id: impl Into<AgiworkforceTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(agiworkforce_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_agiworkforce_turn_id: impl Into<AgiworkforceTurnId>,
    ...
    pub fn record_agiworkforce_turn_started(&self, agiworkforce_turn_id: impl Into<AgiworkforceTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let agiworkforce_turn_id = agiworkforce_turn_id.into();
        context.append_with_context_best_effort(
            agiworkforce_turn_id.clone(),
            RawTraceEventPayload::AgiworkforceTurnStarted {
                agiworkforce_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    }
    ...
    pub fn start_code_cell_trace(
        &self,
        agiworkforce_turn_id: impl Into<AgiworkforceTurnId>,
        runtime_cell_id: impl Into<String>,
        model_visible_call_id: impl Into<String>,
        source_js: impl Into<String>,
    ) -> CodeCellTraceContext {
        let context = self.code_cell_trace_context(agiworkforce_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Agiworkforce turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        agiworkforce_turn_id: AgiworkforceTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            agiworkforce_turn_id: Some(agiworkforce_turn_id),
        };
