...
use crate::model::AgiworkforceTurn;
use crate::model::AgiworkforceTurnId;
...
impl TraceReducer {
    ...
    /// Starts a Agiworkforce turn inside an existing thread.
    pub(super) fn start_agiworkforce_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.agiworkforce_turns.contains_key(&agiworkforce_turn_id) {
            bail!("duplicate codex turn start for {agiworkforce_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.agiworkforce_turns.insert(
    ...
    /// Marks a Agiworkforce turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_agiworkforce_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.agiworkforce_turns.get(&agiworkforce_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "codex turn end for {agiworkforce_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.agiworkforce_turns.get_mut(&agiworkforce_turn_id) else {
            bail!("codex turn end referenced unknown turn {agiworkforce_turn_id}");
        };
