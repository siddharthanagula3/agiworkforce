...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        agiworkforce_turn_id: Option<String>,
        ...
        let thread_id = self.tool_thread_id(thread_id, agiworkforce_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, agiworkforce_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_agiworkforce_turn_id: agiworkforce_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        agiworkforce_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(agiworkforce_turn_id) = agiworkforce_turn_id else {
            bail!("tool call start did not include thread or Agiworkforce turn context");
        };
        self.rollout
            .agiworkforce_turns
            .get(agiworkforce_turn_id)
            .map(|turn| turn.thread_id.clone())
            .with_context(|| {
                format!("tool call start referenced unknown Agiworkforce turn {agiworkforce_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, agiworkforce_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(agiworkforce_turn_id) = agiworkforce_turn_id {
            let Some(turn) = self.rollout.agiworkforce_turns.get(agiworkforce_turn_id) else {
                bail!("tool call start referenced unknown Agiworkforce turn {agiworkforce_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Agiworkforce turn {agiworkforce_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
