...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let agiworkforce_turn_id = started.agiworkforce_turn_id.clone();
        ...
        let Some(turn) = self.rollout.agiworkforce_turns.get(&agiworkforce_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown codex turn {agiworkforce_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but codex turn {agiworkforce_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, Agiworkforce stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.agiworkforce_turn_id == agiworkforce_turn_id
