...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.agiworkforce_turns.get(&started.agiworkforce_turn_id) else {
            bail!(
                "compaction request {} referenced unknown codex turn {}",
                started.compaction_request_id,
                started.agiworkforce_turn_id
            );
        };
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                agiworkforce_turn_id: started.agiworkforce_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.agiworkforce_turns.get(&agiworkforce_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown codex turn {agiworkforce_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but codex turn {agiworkforce_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) agiworkforce_turn_id: String,
