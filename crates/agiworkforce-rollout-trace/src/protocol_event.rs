//! Mapping from Agiworkforce protocol events into raw rollout-trace events.
//!
...
//! `agiworkforce-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `agiworkforce-rollout-trace`.
//!
...
use agiworkforce_protocol::protocol::PatchApplyStatus;
...
pub(crate) struct AgiworkforceTurnTraceEvent {
    pub context_turn_id: AgiworkforceTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn agiworkforce_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<AgiworkforceTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let agiworkforce_turn_id = event.turn_id.clone();
            Some(AgiworkforceTurnTraceEvent {
                context_turn_id: agiworkforce_turn_id.clone(),
                payload: RawTraceEventPayload::AgiworkforceTurnStarted {
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(AgiworkforceTurnTraceEvent {
                context_turn_id: agiworkforce_turn_id.clone(),
                payload: RawTraceEventPayload::AgiworkforceTurnEnded {
                    agiworkforce_turn_id,
        ...
        EventMsg::TurnAborted(event) => {
            let agiworkforce_turn_id = event
                .turn_id
                .clone()
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a agiworkforce_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a agiworkforce_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a agiworkforce_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a agiworkforce_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a agiworkforce_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a agiworkforce_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a agiworkforce_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a agiworkforce_protocol::protocol::CollabCloseEndEvent),
}
