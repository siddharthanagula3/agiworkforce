...
use agiworkforce_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical Agiworkforce tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub agiworkforce_turn_id: AgiworkforceTurnId,
...
/// Raw invocation payload for the canonical Agiworkforce tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            agiworkforce_turn_id: invocation.agiworkforce_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == agiworkforce_code_mode::PUBLIC_TOOL_NAME
}
