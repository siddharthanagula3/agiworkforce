...
impl TraceWriter {
    ...
    pub fn append_with_context(
        ...
        let event = RawTraceEvent {
            ...
            agiworkforce_turn_id: context.agiworkforce_turn_id,
            payload,
        };
        inner.next_seq += 1;
...
mod tests {
    ...
    fn writer_records_payload_refs_and_replays_rollout_status() -> anyhow::Result<()> {
        ...
        writer.append(RawTraceEventPayload::AgiworkforceTurnEnded {
            agiworkforce_turn_id: "turn-1".to_string(),
        ...
        assert_eq!(rollout.agiworkforce_turns["turn-1"].thread_id, "thread-root");
        assert_eq!(
            rollout.agiworkforce_turns["turn-1"].execution.status,
