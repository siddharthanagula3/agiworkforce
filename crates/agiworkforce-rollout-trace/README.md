...
> **Privacy:** Rollout tracing is not telemetry. Agiworkforce does **not** upload or
> report these traces; it writes local bundles only when
> `AGIWORKFORCE_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Agiworkforce session. It records raw runtime evidence into a local bundle on
...
Hot-path Agiworkforce code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Agiworkforce got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["agiworkforce-core runtime"]
...
one from `AGIWORKFORCE_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Agiworkforce session fail just because diagnostic recording failed. Core emits raw
...
Agiworkforce rollout/session being observed. Keeping those separate lets us reason about
...
`agiworkforce_rollout_trace::replay_bundle` directly.
