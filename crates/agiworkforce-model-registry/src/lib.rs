...
pub struct AutoRoutingRequest<'a> {
    ...
    pub us_only: bool,
    /// Token estimates used to rank the routes of ONE canonical model by their
    /// own prices. Absent, ranking falls back to the same defaults the
    /// TypeScript resolver uses, which is what keeps the conformance fixture
...
/// How a route caches prompts, which decides what its cache prices mean.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Whether AGI Workforce may commercially serve traffic over this route.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// The prices charged by ONE route, not by the canonical model.
...
pub struct SelectedAutoRoute {
    ...
    pub route_id: String,
    pub harness_id: String,
    /// Ordered failover, one provider per entry. The selected model's own other
    /// routes come first, so an explicit selection can fail over without ever
    /// being answered with a different model; policy-derived substitutes follow
    /// only for Auto aliases.
...
/// Read the prices charged by ONE route.
///
/// Pricing is keyed by route rather than by canonical model because the same
/// model reached through a gateway is not billed at the first-party vendor's
/// list price.
...
/// Refusals that belong to ONE route rather than to the canonical model.
///
/// Kept apart from the model-level reasons so a blocked or feature-poor route
/// removes itself and leaves the model's other routes admissible.
...
/// through a different provider, harness and price sheet. Cheapest first, then
/// the model's default route, then route id — the last two so two equally
/// priced routes cannot reorder between runs, or between this resolver and the
/// TypeScript one. Health and cache affinity are live facts the TypeScript
/// stage layers on top; they cannot enter a pure function over static policy.
...
fn evaluate_eligibility<'a>(
    ...
    // The generated registry is deserialized into a HashMap, so iteration order
    // is not the file's. Sort before ranking or a tie between two routes of the
    // same model would resolve differently from one process to the next.
