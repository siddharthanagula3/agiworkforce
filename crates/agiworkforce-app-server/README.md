...
`codex app-server` is the interface Agiworkforce uses to power rich interfaces such as the [Agiworkforce VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$AGIWORKFORCE_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to `$AGIWORKFORCE_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `codex app-server generate-ts`, or a JSON Schema bundle via `codex app-server generate-json-schema`. Each output is specific to the version of Agiworkforce you used to run the command, so the generated artifacts are guaranteed to match that version.
...
The API exposes three top level primitives representing an interaction between a user and Agiworkforce:

- **Thread**: A conversation between a user and the Agiworkforce agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's Agiworkforce home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
you are developing a new Agiworkforce integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-Agiworkforce
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "agiworkforce_vscode",
      "title": "Agiworkforce VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `memory/reset` — experimental; clear the current `AGIWORKFORCE_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `review/start` — kick off Agiworkforce’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
Start a fresh thread when you need a new Agiworkforce conversation.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current Agiworkforce home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text or images) to a thread and trigger Agiworkforce generation. The `input` field is a list of discriminated unions:
...
Omit `prompt` to use Agiworkforce's default realtime backend prompt. Send `prompt: null` or
...
Use `review/start` to run Agiworkforce’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Agiworkforce will run).
...
Agiworkforce streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Agiworkforce will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, Agiworkforce generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
`codexErrorInfo` maps to the `AgiworkforceErrorInfo` enum. Common values:
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Agiworkforce whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`agiworkforce_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Agiworkforce skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Agiworkforce skill",
                "iconSmall": "icon.svg",
...
Agiworkforce supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Agiworkforce owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Agiworkforce persists tokens to disk and refreshes them automatically.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Agiworkforce can run without OpenAI credentials.
...
5. Verify the protocol crate:
   ...
   ```bash
   cargo test -p agiworkforce-app-server-protocol
   ```
