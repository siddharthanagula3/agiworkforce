...
use agiworkforce_app_server_protocol::RemoteControlConnectionStatus;
use agiworkforce_app_server_protocol::RemoteControlStatusChangedNotification;
use agiworkforce_app_server_protocol::ServerNotification;
use agiworkforce_config::types::AuthCredentialsStoreMode;
use agiworkforce_core::test_support::auth_manager_from_auth;
use agiworkforce_core::test_support::auth_manager_from_auth_with_home;
use agiworkforce_login::AuthDotJson;
use agiworkforce_login::AuthManager;
use agiworkforce_login::AgiworkforceAuth;
use agiworkforce_login::save_auth;
use agiworkforce_login::token_data::TokenData;
use agiworkforce_login::token_data::parse_chatgpt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(agiworkforce_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(agiworkforce_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(agiworkforce_home.path().to_path_buf(), "test-provider".to_string())
        .await
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
    ...
    let agiworkforce_home = TempDir::new().expect("temp dir should create");
    let (transport_event_tx, mut transport_event_rx) =
        mpsc::channel::<TransportEvent>(CHANNEL_CAPACITY);
    let shutdown_token = CancellationToken::new();
    let (remote_task, remote_handle) = start_remote_control(
        remote_control_url,
        Some(remote_control_state_runtime(&agiworkforce_home).await),
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    agiworkforce_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(agiworkforce_app_server_protocol::JSONRPCRequest {
        id: agiworkforce_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(agiworkforce_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(agiworkforce_app_server_protocol::JSONRPCRequest {
                    id: agiworkforce_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-agiworkforce-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-agiworkforce-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request.headers.get("x-agiworkforce-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(agiworkforce_app_server_protocol::JSONRPCRequest {
                id: agiworkforce_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: agiworkforce_app_server_protocol::RequestId::Integer(11),
                result: json!({
                    "userAgent": "agiworkforce-test-agent"
                }),
            },
        )))
        .await
...
async fn remote_control_http_mode_reuses_persisted_enrollment_before_reenrolling() {
    ...
    let remote_control_url = remote_control_url_for_listener(&listener);
    let agiworkforce_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&agiworkforce_home).await;
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        remote_control_url,
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&agiworkforce_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        /*initial_enabled*/ true,
...
async fn remote_control_http_mode_clears_stale_persisted_enrollment_after_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-agiworkforce-server-id"),
