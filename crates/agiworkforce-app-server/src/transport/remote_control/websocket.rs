...
const REMOTE_CONTROL_SUBSCRIBE_CURSOR_HEADER: &str = "x-agiworkforce-subscribe-cursor";
...
fn build_remote_control_websocket_request(
    ...
    set_remote_control_header(headers, "x-agiworkforce-server-id", &enrollment.server_id)?;
    ...
    set_remote_control_header(
        headers,
        "x-agiworkforce-protocol-version",
...
pub(crate) async fn load_remote_control_auth(
    ...
    Ok(RemoteControlConnectionAuth {
        auth_provider: agiworkforce_model_provider::auth_provider_from_auth(&auth),
