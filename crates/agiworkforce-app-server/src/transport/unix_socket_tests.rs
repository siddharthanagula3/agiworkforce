...
use agiworkforce_uds::UnixStream;
...
fn default_control_socket_path() -> AbsolutePathBuf {
    let agiworkforce_home = find_agiworkforce_home().expect("codex home");
    app_server_control_socket_path(&agiworkforce_home).expect("default control socket path")
}
