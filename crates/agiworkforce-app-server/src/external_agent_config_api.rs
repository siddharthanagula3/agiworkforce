...
use agiworkforce_app_server_protocol::CommandMigration;
...
use agiworkforce_app_server_protocol::HookMigration;
use agiworkforce_app_server_protocol::JSONRPCErrorError;
use agiworkforce_app_server_protocol::McpServerMigration;
use agiworkforce_app_server_protocol::MigrationDetails;
use agiworkforce_app_server_protocol::PluginsMigration;
use agiworkforce_app_server_protocol::SubagentMigration;
use agiworkforce_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use agiworkforce_external_agent_sessions::PendingSessionImport;
use agiworkforce_external_agent_sessions::prepare_validated_session_imports;
use agiworkforce_external_agent_sessions::record_imported_session;
...
impl ExternalAgentConfigApi {
    pub(crate) fn new(agiworkforce_home: PathBuf) -> Self {
        Self {
            migration_service: ExternalAgentConfigService::new(agiworkforce_home.clone()),
            agiworkforce_home,
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| agiworkforce_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
    ...
    pub(crate) fn prepare_validated_session_imports(
        ...
        prepare_validated_session_imports(&self.agiworkforce_home, sessions)
    }
    ...
    pub(crate) fn record_imported_session(
        ...
        if let Err(err) = record_imported_session(&self.agiworkforce_home, source_path, imported_thread_id)
        {
            tracing::warn!(
                error = %err,
