...
use agiworkforce_app_server_protocol::CommandExecOutputDeltaNotification;
use agiworkforce_app_server_protocol::CommandExecOutputStream;
use agiworkforce_app_server_protocol::CommandExecResizeParams;
use agiworkforce_app_server_protocol::CommandExecResizeResponse;
use agiworkforce_app_server_protocol::CommandExecResponse;
use agiworkforce_app_server_protocol::CommandExecTerminalSize;
use agiworkforce_app_server_protocol::CommandExecTerminateParams;
use agiworkforce_app_server_protocol::CommandExecTerminateResponse;
use agiworkforce_app_server_protocol::CommandExecWriteParams;
use agiworkforce_app_server_protocol::CommandExecWriteResponse;
use agiworkforce_app_server_protocol::JSONRPCErrorError;
use agiworkforce_app_server_protocol::ServerNotification;
use agiworkforce_core::config::StartedNetworkProxy;
use agiworkforce_core::exec::ExecExpiration;
use agiworkforce_core::exec::ExecExpirationOutcome;
use agiworkforce_core::exec::IO_DRAIN_TIMEOUT_MS;
use agiworkforce_core::sandboxing::ExecRequest;
use agiworkforce_protocol::exec_output::bytes_to_string_smart;
use agiworkforce_sandboxing::SandboxType;
use agiworkforce_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use agiworkforce_utils_pty::ProcessHandle;
use agiworkforce_utils_pty::SpawnedProcess;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match agiworkforce_core::sandboxing::execute_env(exec_request, /*stdout_stream*/ None)
                    .await
                {
                    Ok(output) => {
                        outgoing
        ...
        let spawned = if tty {
            agiworkforce_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            agiworkforce_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            agiworkforce_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            agiworkforce_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: agiworkforce_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: agiworkforce_app_server_protocol::RequestId::Integer(3),
        };
