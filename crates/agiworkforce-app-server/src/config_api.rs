...
use agiworkforce_app_server_protocol::ConfigReadResponse;
use agiworkforce_app_server_protocol::ConfigRequirements;
use agiworkforce_app_server_protocol::ConfigRequirementsReadResponse;
use agiworkforce_app_server_protocol::ConfigValueWriteParams;
use agiworkforce_app_server_protocol::ConfigWriteErrorCode;
use agiworkforce_app_server_protocol::ConfigWriteResponse;
use agiworkforce_app_server_protocol::ConfiguredHookHandler;
use agiworkforce_app_server_protocol::ConfiguredHookMatcherGroup;
use agiworkforce_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use agiworkforce_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use agiworkforce_app_server_protocol::JSONRPCErrorError;
use agiworkforce_app_server_protocol::ManagedHooksRequirements;
use agiworkforce_app_server_protocol::NetworkDomainPermission;
use agiworkforce_app_server_protocol::NetworkRequirements;
use agiworkforce_app_server_protocol::NetworkUnixSocketPermission;
use agiworkforce_app_server_protocol::SandboxMode;
use agiworkforce_config::ConfigRequirementsToml;
use agiworkforce_config::HookEventsToml;
use agiworkforce_config::HookHandlerConfig as CoreHookHandlerConfig;
use agiworkforce_config::ManagedHooksRequirementsToml;
use agiworkforce_config::MatcherGroup as CoreMatcherGroup;
use agiworkforce_config::ResidencyRequirement as CoreResidencyRequirement;
use agiworkforce_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use agiworkforce_core::ThreadManager;
use agiworkforce_core::config::Config;
use agiworkforce_core::plugins::PluginId;
use agiworkforce_core_plugins::loader::installed_plugin_telemetry_metadata;
use agiworkforce_core_plugins::toggles::collect_plugin_enabled_candidates;
...
impl ConfigApi {
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata =
                installed_plugin_telemetry_metadata(self.config_manager.agiworkforce_home(), &plugin_id)
                    .await;
            if enabled {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(agiworkforce_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(agiworkforce_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
) -> agiworkforce_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => agiworkforce_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: agiworkforce_config::NetworkRequirementsToml,
) -> NetworkRequirements {
...
mod tests {
    ...
    use agiworkforce_config::NetworkDomainPermissionToml as CoreNetworkDomainPermissionToml;
    use agiworkforce_config::NetworkDomainPermissionsToml as CoreNetworkDomainPermissionsToml;
    use agiworkforce_config::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    use agiworkforce_config::NetworkUnixSocketPermissionToml as CoreNetworkUnixSocketPermissionToml;
    use agiworkforce_config::NetworkUnixSocketPermissionsToml as CoreNetworkUnixSocketPermissionsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                agiworkforce_app_server_protocol::AskForApproval::Never,
                agiworkforce_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
            mapped.allowed_approvals_reviewers,
            Some(vec![
                agiworkforce_app_server_protocol::ApprovalsReviewer::User,
                agiworkforce_app_server_protocol::ApprovalsReviewer::AutoReview,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(agiworkforce_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn apply_runtime_feature_enablement_keeps_cli_overrides_above_config_and_runtime() {
        let agiworkforce_home = TempDir::new().expect("create temp dir");
        std::fs::write(
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        let agiworkforce_home = TempDir::new().expect("create temp dir");
        let user_config_path = agiworkforce_home.path().join("config.toml");
        ...
        let analytics_config = Arc::new(
            agiworkforce_core::config::ConfigBuilder::default()
                .build()
                .await
                .expect("load analytics config"),
        );
        let auth_manager = AuthManager::from_auth_for_testing(AgiworkforceAuth::from_api_key("test"));
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![agiworkforce_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: agiworkforce_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: agiworkforce_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: agiworkforce_utils_absolute_path::AbsolutePathBuf::try_from(
