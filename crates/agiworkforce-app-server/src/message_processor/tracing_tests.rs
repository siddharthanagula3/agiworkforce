...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("agiworkforce-app-server-message-processor-tests");
...
impl TracingHarness {
    ...
    async fn new_with_origin(origin: ConnectionOrigin) -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(agiworkforce_home.path(), &server.uri()).await?);
...
async fn build_test_config(agiworkforce_home: &Path, server_uri: &str) -> Result<Config> {
...
async fn build_test_processor(
    ...
    let auth_manager =
        AuthManager::shared_from_config(config.as_ref(), /*enable_agiworkforce_api_key_env*/ false).await;
    ...
    loop {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    notification,
                    agiworkforce_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
