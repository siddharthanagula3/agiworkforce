...
impl AgiworkforceMessageProcessor {
    ...
    async fn plugin_list_response(
        ...
        let (mut data, marketplace_load_errors) = match tokio::task::spawn_blocking(move || {
            ...
            Ok::<
                (
                    ...
                    Vec<agiworkforce_app_server_protocol::MarketplaceLoadErrorInfo>,
                ),
                MarketplaceError,
            >((
                outcome
                ...
                outcome
                    .errors
                    .into_iter()
                    .map(|err| agiworkforce_app_server_protocol::MarketplaceLoadErrorInfo {
        ...
        if config.features.enabled(Feature::RemotePlugin) {
            ...
            match agiworkforce_core_plugins::remote::fetch_remote_marketplaces(
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = agiworkforce_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(agiworkforce_core::plugins::AppConnectorId)
                    .collect::<Vec<_>>();
                let environment_manager = self.thread_manager.environment_manager();
                let app_summaries = plugin_app_helpers::load_plugin_app_summaries(
                    &config,
    ...
    async fn plugin_install_response(
        ...
        {
            return Err(invalid_request(
                "Agiworkforce plugins are disabled for this workspace",
            ));
        }
        ...
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(AgiworkforceAuth::is_chatgpt_auth),
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_detail =
            agiworkforce_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &plugin_name,
        ...
        let validated_bundle = agiworkforce_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &plugin_name,
            &remote_marketplace_name,
        ...
        let result = agiworkforce_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
        ...
        agiworkforce_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &plugin_name,
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        let (accessible_connectors, agiworkforce_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.agiworkforce_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !agiworkforce_apps_ready {
            warn!(
                plugin = plugin_id,
                "agiworkforce_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
        }
    ...
    async fn plugin_uninstall_response(
        ...
        if agiworkforce_core::plugins::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let uninstall_result = agiworkforce_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.agiworkforce_home.to_path_buf(),
            &plugin_id,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: agiworkforce_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
