...
use agiworkforce_core_plugins::marketplace::find_marketplace_manifest_path;
use agiworkforce_core_plugins::marketplace_add::MarketplaceAddRequest;
use agiworkforce_core_plugins::marketplace_add::add_marketplace;
use agiworkforce_core_plugins::marketplace_add::is_local_marketplace_source;
use agiworkforce_external_agent_migration::build_mcp_config_from_external;
use agiworkforce_external_agent_migration::count_missing_commands;
use agiworkforce_external_agent_migration::count_missing_subagents;
use agiworkforce_external_agent_migration::hook_migration_event_names;
use agiworkforce_external_agent_migration::import_commands;
use agiworkforce_external_agent_migration::import_hooks;
use agiworkforce_external_agent_migration::import_subagents;
use agiworkforce_external_agent_migration::missing_command_names;
use agiworkforce_external_agent_migration::missing_subagent_names;
use agiworkforce_external_agent_sessions::ExternalAgentSessionMigration;
use agiworkforce_external_agent_sessions::detect_recent_sessions;
...
impl ExternalAgentConfigService {
    pub(crate) fn new(agiworkforce_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(agiworkforce_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            agiworkforce_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.agiworkforce_home.join("config.toml"),
        ...
        let target_hooks = repo_root.map_or_else(
            || self.agiworkforce_home.join("hooks.json"),
        ...
        let target_subagents = repo_root.map_or_else(
            || self.agiworkforce_home.join("agents"),
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.agiworkforce_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.agiworkforce_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.agiworkforce_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.agiworkforce_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.agiworkforce_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.agiworkforce_home.clone(), request).await;
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Agiworkforce.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Agiworkforce");
    }
    rewritten
}
