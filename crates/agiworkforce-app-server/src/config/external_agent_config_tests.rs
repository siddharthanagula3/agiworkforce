...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let agiworkforce_home = root.path().join(".codex");
    (root, external_agent_home, agiworkforce_home)
}
...
fn service_for_paths(
    ...
    ExternalAgentConfigService::new_for_test(agiworkforce_home, external_agent_home)
}
...
async fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, external_agent_home, agiworkforce_home) = fixture_paths();
    let agents_skills = agiworkforce_home
        .parent()
    ...
    let items = service_for_paths(external_agent_home.clone(), agiworkforce_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                agiworkforce_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                agiworkforce_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, agiworkforce_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), agiworkforce_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::create_dir_all(&agiworkforce_home).expect("create codex home");
    fs::write(agiworkforce_home.join("config.toml"), "this is not valid = [toml")
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), agiworkforce_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        agiworkforce_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
    ...
    .expect("parse hooks");
    let _supported_hooks: agiworkforce_config::HooksFile =
...
Research with Agiworkforce carefully."""
"#,
    )
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            external_agent_home.join("settings.json"),
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"agiworkforce","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
        .expect("write settings");
    fs::write(
        external_agent_home
    ...
    service_for_paths(external_agent_home, agiworkforce_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
    ...
    assert_eq!(
        fs::read_to_string(agiworkforce_home.join("AGENTS.md")).expect("read agents"),
        "Agiworkforce guidance"
    );

    assert_eq!(
        fs::read_to_string(agiworkforce_home.join("config.toml")).expect("read config"),
        "sandbox_mode = \"workspace-write\"\n\n[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nCI = \"false\"\nFOO = \"bar\"\nMAX_RETRIES = \"3\"\nMY_TEAM = \"codex\"\n"
    );
    assert_eq!(
        fs::read_to_string(agents_skills.join("skill-a").join("SKILL.md"))
            .expect("read copied skill"),
        "Use Agiworkforce and Agiworkforce utilities."
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    assert!(!agiworkforce_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".agiworkforce-plugin")).expect("create plugin manifest dir");
    fs::create_dir_all(&agiworkforce_home).expect("create codex home");
    ...
    fs::write(
        plugin_root.join(".agiworkforce-plugin").join("plugin.json"),
        r#"{"name":"cloudflare","version":"0.1.0"}"#,
    )
    .expect("write plugin manifest");

    let outcome = service_for_paths(external_agent_home, agiworkforce_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
    ...
    let config = fs::read_to_string(agiworkforce_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, agiworkforce_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_agiworkforce_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, agiworkforce_home)
...
async fn detect_repo_filters_plugins_against_installed_marketplace() {
    ...
    let agiworkforce_home = root.path().join(".codex");
    let repo_root = root.path().join("repo");
    let marketplace_root = agiworkforce_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, agiworkforce_home)
        .import_plugins(
            /*cwd*/ None,
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(external_agent_home, agiworkforce_home)
