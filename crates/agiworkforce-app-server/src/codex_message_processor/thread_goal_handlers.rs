...
impl AgiworkforceMessageProcessor {
    pub(super) async fn thread_goal_set(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => {
                match find_thread_path_by_id_str(&self.config.agiworkforce_home, &thread_id.to_string())
                    .await
                {
                    Ok(Some(path)) => path,
                    Ok(None) => {
                    ...
                    if let Some(goal) = goal.as_ref().filter(|goal| {
                        ...
                        state_db
                            .replace_thread_goal(
                                thread_id,
                                objective,
                                status.unwrap_or(agiworkforce_state::ThreadGoalStatus::Active),
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            match find_thread_path_by_id_str(&self.config.agiworkforce_home, &thread_id.to_string()).await
            {
                Ok(Some(_)) => {}
                Ok(None) => {
...
fn thread_goal_status_to_state(status: ThreadGoalStatus) -> agiworkforce_state::ThreadGoalStatus {
    match status {
...
fn thread_goal_status_from_state(status: agiworkforce_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(super) fn api_thread_goal_from_state(goal: agiworkforce_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
