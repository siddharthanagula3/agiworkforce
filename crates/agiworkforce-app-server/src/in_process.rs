...
//!
//! # Relationship to `agiworkforce-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `agiworkforce-app-server-client`,
...
pub struct InProcessStartArgs {
    ...
    /// Whether auth loading should honor the `AGIWORKFORCE_API_KEY` environment variable.
...
/// through `agiworkforce-app-server-client`, which adds worker-task buffering,
...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_agiworkforce_api_key_env)
                .await;
        ...
        let config_manager = ConfigManager::new(
            args.config.agiworkforce_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    use agiworkforce_app_server_protocol::RemoteControlClientConnectionAudience;
    use agiworkforce_app_server_protocol::RemoteControlClientEnrollmentAudience;
    use agiworkforce_app_server_protocol::SessionSource as ApiSessionSource;
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "agiworkforce-in-process-test".to_string(),
                    title: None,
