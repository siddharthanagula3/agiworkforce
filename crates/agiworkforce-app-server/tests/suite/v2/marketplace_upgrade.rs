...
use agiworkforce_config::MarketplaceConfigUpdate;
use agiworkforce_config::record_user_marketplace;
...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    ...
    run_git(root, &["config", "user.name", "Agiworkforce Tests"])?;
...
fn disable_plugin_startup_tasks(agiworkforce_home: &Path) -> Result<()> {
    let config_path = agiworkforce_home.join("config.toml");
...
fn marketplace_install_root(agiworkforce_home: &Path) -> std::path::PathBuf {
    agiworkforce_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(agiworkforce_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&agiworkforce_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(agiworkforce_home.path())?;

    let mut mcp = McpProcess::new(agiworkforce_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(agiworkforce_home.path(), "debug")?;
    let tools_root = expected_installed_root(agiworkforce_home.path(), "tools")?;
...
async fn marketplace_upgrade_named_marketplace_only() -> Result<()> {
    ...
    assert!(
        !marketplace_install_root(agiworkforce_home.path())
            .join("debug")
            .exists()
    );
    Ok(())
