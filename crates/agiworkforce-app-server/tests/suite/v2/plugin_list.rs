...
use agiworkforce_core::config::set_project_trust_level;
...
fn write_plugins_enabled_config(agiworkforce_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
        agiworkforce_home.join("config.toml"),
        r#"[features]
plugins = true
"#,
    )
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(agiworkforce_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = agiworkforce_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.agiworkforce-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.agiworkforce-plugin/plugin.json"),
        r#"{"name":"valid-plugin"}"#,
    )?;
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_agiworkforce_plugins_disabled() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_agiworkforce_plugins_setting() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.agiworkforce-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        agiworkforce_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.agiworkforce-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    install_policy: PluginInstallPolicy::Available,
                    auth_policy: PluginAuthPolicy::OnInstall,
                    interface: Some(agiworkforce_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    std::fs::create_dir_all(agiworkforce_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(agiworkforce_home.path())?;
    std::fs::write(
        agiworkforce_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agiworkforce-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&agiworkforce_home, "agiworkforce-curated", "enabled-plugin")?;
    write_installed_plugin(&agiworkforce_home, "agiworkforce-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agiworkforce-curated",
...
[plugins."enabled-plugin@agiworkforce-curated"]
enabled = true

[plugins."disabled-plugin@agiworkforce-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "agiworkforce-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@agiworkforce-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@agiworkforce-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@agiworkforce-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    std::fs::create_dir_all(agiworkforce_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&agiworkforce_home, "agiworkforce-curated", "shared-plugin")?;
    std::fs::write(
        agiworkforce_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "agiworkforce-curated",
...
[plugins."shared-plugin@agiworkforce-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        ...
        r#"[plugins."shared-plugin@agiworkforce-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@agiworkforce-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@agiworkforce-curated");
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(agiworkforce_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        agiworkforce_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_openai_curated_marketplace(agiworkforce_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    let marker_path = agiworkforce_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(agiworkforce_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_list_remote_marketplace_replaces_local_marketplace_with_same_name() -> Result<()> {
    ...
    let local_plugin_root = agiworkforce_home
        .path()
        .join(".agents/plugins/plugins/local-linear/.agiworkforce-plugin");
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(agiworkforce_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(agiworkforce_home.path(), &["linear", "gmail"])?;
...
fn write_installed_plugin(
    ...
    let plugin_root = agiworkforce_home
        ...
        .join(plugin_name)
        .join("local/.agiworkforce-plugin");
...
fn write_plugin_sync_config(agiworkforce_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        agiworkforce_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = agiworkforce_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.agiworkforce-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(agiworkforce_home.join(".tmp"))?;
    std::fs::write(
