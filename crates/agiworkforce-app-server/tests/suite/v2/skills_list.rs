...
use agiworkforce_app_server_protocol::SkillsListExtraRootsForCwd;
...
use agiworkforce_exec_server::AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = agiworkforce_home.join("plugins/cache/chatgpt-global/linear/local");
    std::fs::create_dir_all(plugin_root.join(".agiworkforce-plugin"))?;
    std::fs::write(
        plugin_root.join(".agiworkforce-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(agiworkforce_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_agiworkforce_plugins_disabled() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&agiworkforce_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace Agiworkforce plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        agiworkforce_home.path(),
        &[(AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR, Some("none"))],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(agiworkforce_home.path().join(&relative_cwd))?;
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    write_skill(&agiworkforce_home, "demo")?;
    ...
    let skill_path = agiworkforce_home
        .path()
        .join("skills")
        .join("demo")
        .join("SKILL.md");
