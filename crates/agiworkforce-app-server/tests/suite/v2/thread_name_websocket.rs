...
async fn thread_name_updated_broadcasts_for_loaded_threads() -> Result<()> {
    ...
    let conversation_id = create_rollout(agiworkforce_home.path(), "2025-01-05T12-00-00")?;
    ...
    let result = async {
        ...
        assert_legacy_thread_name(agiworkforce_home.path(), &conversation_id, renamed).await?;
        assert_eq!(
            thread_name_update_rollout_count(agiworkforce_home.path(), &conversation_id).await?,
            1
        );
...
async fn thread_name_updated_broadcasts_for_not_loaded_threads() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml(agiworkforce_home.path(), &server.uri(), "never")?;
    let conversation_id = create_rollout(agiworkforce_home.path(), "2025-01-05T12-05-00")?;
...
fn create_rollout(agiworkforce_home: &std::path::Path, filename_ts: &str) -> Result<String> {
...
async fn assert_legacy_thread_name(
    ...
    assert_eq!(
        find_thread_name_by_id(agiworkforce_home, &thread_id)
            .await?
            .as_deref(),
        Some(expected_name)
    );
...
async fn thread_name_update_rollout_count(
    ...
    let rollout_path = find_thread_path_by_id_str(agiworkforce_home, conversation_id)
        .await?
