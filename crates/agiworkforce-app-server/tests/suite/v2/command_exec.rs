...
use agiworkforce_app_server_protocol::FileSystemSpecialPath;
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(agiworkforce_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$AGIWORKFORCE_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", agiworkforce_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let agiworkforce_home = TempDir::new()?;
    let command_dir = agiworkforce_home.path().join("command-cwd");
    ...
    assert!(
        !agiworkforce_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(agiworkforce_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "agiworkforce-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(agiworkforce_home.path()).await?;
