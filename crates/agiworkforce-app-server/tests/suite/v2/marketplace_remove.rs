...
fn write_installed_marketplace(agiworkforce_home: &std::path::Path, marketplace_name: &str) -> Result<()> {
    let root = marketplace_install_root(agiworkforce_home).join(marketplace_name);
...
async fn marketplace_remove_deletes_config_and_installed_root() -> Result<()> {
    ...
    record_user_marketplace(agiworkforce_home.path(), "debug", &configured_marketplace_update())?;
    write_installed_marketplace(agiworkforce_home.path(), "debug")?;
