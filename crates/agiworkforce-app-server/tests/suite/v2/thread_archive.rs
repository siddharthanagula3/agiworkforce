...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(agiworkforce_home.path(), &thread.id)
            .await?
            .is_none(),
    ...
    let discovered_path = find_thread_path_by_id_str(agiworkforce_home.path(), &thread.id)
        .await?
    ...
    // Verify file moved.
    let archived_directory = agiworkforce_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_archives_spawned_descendants() -> Result<()> {
    ...
    for thread_id in [parent_thread_id, child_thread_id, grandchild_thread_id] {
        assert!(
            find_thread_path_by_id_str(agiworkforce_home.path(), &thread_id.to_string())
                .await?
                .is_none(),
            "expected active rollout for {thread_id} to be archived"
        );
        assert!(
            find_archived_thread_path_by_id_str(agiworkforce_home.path(), &thread_id.to_string())
                .await?
                .is_some(),
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path = find_thread_path_by_id_str(agiworkforce_home.path(), &child_id)
        .await?
        .expect("child rollout path");
    let archived_child_path = agiworkforce_home
        .path()
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(agiworkforce_home.path(), &parent_id)
            .await?
            .is_none(),
        "parent should be archived even when a descendant is missing"
    );
    assert!(
        find_archived_thread_path_by_id_str(agiworkforce_home.path(), &parent_id)
            .await?
            .is_some(),
