...
async fn plugin_uninstall_removes_plugin_cache_and_config_entry() -> Result<()> {
    ...
    write_installed_plugin(&agiworkforce_home, "debug", "sample-plugin")?;
    std::fs::write(
...
async fn plugin_uninstall_tracks_analytics_event() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            "events": [{
                "event_type": "agiworkforce_plugin_uninstalled",
                "event_params": {
...
async fn plugin_uninstall_writes_remote_plugin_to_cloud_when_remote_plugin_enabled() -> Result<()> {
    ...
    let remote_plugin_cache_root = agiworkforce_home
        .path()
        .join("plugins/cache/chatgpt-global/linear");
    std::fs::create_dir_all(remote_plugin_cache_root.join("1.0.0/.agiworkforce-plugin"))?;
    std::fs::write(
        remote_plugin_cache_root.join("1.0.0/.agiworkforce-plugin/plugin.json"),
        r#"{"name":"linear","version":"1.0.0"}"#,
    )?;
    let legacy_remote_plugin_cache_root = agiworkforce_home
        .path()
        .join(format!("plugins/cache/chatgpt-global/{REMOTE_PLUGIN_ID}"));
    std::fs::create_dir_all(legacy_remote_plugin_cache_root.join("local/.agiworkforce-plugin"))?;
...
async fn plugin_uninstall_uses_detail_scope_for_cache_namespace() -> Result<()> {
    ...
    let workspace_cache_root = agiworkforce_home
        .path()
        .join("plugins/cache/chatgpt-workspace/linear");
    std::fs::create_dir_all(workspace_cache_root.join("1.0.0/.agiworkforce-plugin"))?;
    std::fs::write(
        workspace_cache_root.join("1.0.0/.agiworkforce-plugin/plugin.json"),
        r#"{"name":"linear","version":"1.0.0"}"#,
    )?;
    let global_cache_root = agiworkforce_home
        .path()
        .join("plugins/cache/chatgpt-global/linear");
    std::fs::create_dir_all(global_cache_root.join("1.0.0/.agiworkforce-plugin"))?;
