...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_agiworkforce_home = AbsolutePathBuf::try_from(agiworkforce_home.path().canonicalize()?)?;
    ...
    let message = timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            name: "agiworkforce_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        agiworkforce_home: response_agiworkforce_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("agiworkforce_vscode/"));
    assert_eq!(response_agiworkforce_home, expected_agiworkforce_home);
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("agiworkforce_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("agiworkforce_originator_via_env_var/"));
    assert_eq!(response_agiworkforce_home, expected_agiworkforce_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        agiworkforce_home.path(),
        &[("AGIWORKFORCE_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let agiworkforce_home = TempDir::new()?;
    let notify_file = agiworkforce_home.path().join("notify.json");
    let notify_capture = cargo_bin("agiworkforce-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(agiworkforce_home, server_uri, approval_policy, "")
}
