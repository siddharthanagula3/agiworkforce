...
use agiworkforce_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut agiworkforce_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = agiworkforce_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, agiworkforce_home: &Path) -> Result<()> {
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "agiworkforce_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("agiworkforce_thread_initialized event should be present"))
}
