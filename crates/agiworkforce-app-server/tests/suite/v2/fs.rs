...
use agiworkforce_app_server_protocol::FsChangedNotification;
use agiworkforce_app_server_protocol::FsCopyParams;
use agiworkforce_app_server_protocol::FsGetMetadataResponse;
use agiworkforce_app_server_protocol::FsReadDirectoryEntry;
use agiworkforce_app_server_protocol::FsReadFileResponse;
use agiworkforce_app_server_protocol::FsUnwatchParams;
use agiworkforce_app_server_protocol::FsWatchResponse;
...
async fn initialized_mcp(agiworkforce_home: &TempDir) -> Result<McpProcess> {
...
async fn fs_get_metadata_returns_only_used_fields() -> Result<()> {
    ...
    let file_path = agiworkforce_home.path().join("note.txt");
    std::fs::write(&file_path, "hello")?;

    let mut mcp = initialized_mcp(&agiworkforce_home).await?;
    let request_id = mcp
        .send_fs_get_metadata_request(agiworkforce_app_server_protocol::FsGetMetadataParams {
...
async fn fs_get_metadata_reports_symlink() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let file_path = agiworkforce_home.path().join("note.txt");
    let symlink_path = agiworkforce_home.path().join("note-link.txt");
    std::fs::write(&file_path, "hello")?;
...
async fn fs_methods_cover_current_fs_utils_surface() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let source_dir = agiworkforce_home.path().join("source");
    let nested_dir = source_dir.join("nested");
    let source_file = source_dir.join("root.txt");
    let copied_dir = agiworkforce_home.path().join("copied");
    let copy_file_path = agiworkforce_home.path().join("copy.txt");
    let nested_file = nested_dir.join("note.txt");

    let mut mcp = initialized_mcp(&agiworkforce_home).await?;

    let create_directory_request_id = mcp
        .send_fs_create_directory_request(agiworkforce_app_server_protocol::FsCreateDirectoryParams {
    ...
    let read_request_id = mcp
        .send_fs_read_file_request(agiworkforce_app_server_protocol::FsReadFileParams {
            path: absolute_path(nested_file.clone()),
        })
        .await?;
    ...
    let read_directory_request_id = mcp
        .send_fs_read_directory_request(agiworkforce_app_server_protocol::FsReadDirectoryParams {
    ...
    .await??;
    let mut entries =
        to_response::<agiworkforce_app_server_protocol::FsReadDirectoryResponse>(readdir_response)?
            .entries;
    ...
    let remove_request_id = mcp
        .send_fs_remove_request(agiworkforce_app_server_protocol::FsRemoveParams {
...
async fn fs_write_file_accepts_base64_bytes() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let file_path = agiworkforce_home.path().join("blob.bin");
...
async fn fs_methods_reject_relative_paths() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let absolute_file = agiworkforce_home.path().join("absolute.txt");
...
async fn fs_copy_rejects_directory_without_recursive() -> Result<()> {
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(agiworkforce_home.path().join("dest")),
            recursive: false,
        })
        .await?;
    let error = timeout(
...
async fn fs_copy_rejects_standalone_fifo_source() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let fifo_path = agiworkforce_home.path().join("named-pipe");
    let output = Command::new("mkfifo").arg(&fifo_path).output()?;
    if !output.status.success() {
        anyhow::bail!(
            "mkfifo failed: stdout={} stderr={}",
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(agiworkforce_home.path().join("copied")),
            recursive: false,
        })
        .await?;
    expect_error_message(
...
-> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let git_dir = agiworkforce_home.path().join("repo").join(".git");
    ...
    let watch_request_id = mcp
        .send_fs_watch_request(agiworkforce_app_server_protocol::FsWatchParams {
