...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let agiworkforce_home = TempDir::new()?;
    let token_file = agiworkforce_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(agiworkforce_home.path(), "ws://127.0.0.1:0", &auth_args).await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let agiworkforce_home = TempDir::new()?;
    let shared_secret_file = agiworkforce_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "agiworkforce-enroller".to_string(),
        "--ws-audience".to_string(),
        "agiworkforce-app-server".to_string(),
    ...
    let expired_token = signed_bearer_token(
        ...
        json!({
            ...
            "iss": "agiworkforce-enroller",
            "aud": "agiworkforce-app-server",
        }),
    )?;
...
async fn websocket_transport_allows_unauthenticated_non_loopback_startup_by_default() -> Result<()>
{
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(agiworkforce_home.path(), "ws://0.0.0.0:0", &[]).await?;
...
pub(super) async fn spawn_websocket_server(agiworkforce_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(agiworkforce_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    cmd.arg("--listen")
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .env("AGIWORKFORCE_HOME", agiworkforce_home)
        .env("RUST_LOG", "warn");
    let mut process = cmd
        .kill_on_drop(true)
        .spawn()
