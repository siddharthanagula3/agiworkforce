...
async fn thread_read_can_return_archived_threads_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(agiworkforce_home.path(), filename_ts, &conversation_id);
...
fn turn_user_texts(turns: &[agiworkforce_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
