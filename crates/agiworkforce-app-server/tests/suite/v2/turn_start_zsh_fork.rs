...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&agiworkforce_home, &workspace).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(agiworkforce_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(agiworkforce_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(agiworkforce_app_server_protocol::AskForApproval::UnlessTrusted),
...
async fn create_zsh_test_mcp_process(agiworkforce_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(agiworkforce_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = agiworkforce_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("agiworkforce-rs/app-server/tests/suite/zsh");
