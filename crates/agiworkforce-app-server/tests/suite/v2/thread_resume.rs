...
use agiworkforce_app_server_protocol::SessionSource;
...
use agiworkforce_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use agiworkforce_protocol::protocol::SessionSource as RolloutSessionSource;
...
const AGIWORKFORCE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Agiworkforce, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(agiworkforce_home.path(), &server.uri())?;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(agiworkforce_home.path(), &server.uri(), &server.uri())?;
...
async fn thread_goal_clear_deletes_goal_and_notifies() -> Result<()> {
    ...
    .await??;
    let get: agiworkforce_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_by_path_uses_remote_thread_store_error() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_with_remote_thread_store(agiworkforce_home.path(), &server.uri())?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(agiworkforce_home.path(), filename_ts, &conversation_id);
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let agiworkforce_home = TempDir::new()?;
    let config_toml = agiworkforce_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = agiworkforce_home.path().join("repo");
    ...
    let rollout_path = rollout_path(agiworkforce_home.path(), "2025-01-05T12-00-00", &thread_id);
    ...
    let session_meta = SessionMeta {
        ...
        cwd: repo_path.clone(),
        originator: "agiworkforce".to_string(),
    ...
    let state_db =
        StateRuntime::init(agiworkforce_home.path().to_path_buf(), "mock_provider".into()).await?;
    state_db
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let agiworkforce_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(agiworkforce_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(agiworkforce_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(agiworkforce_home.path()).await?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&agiworkforce_home, &server.uri())?;

    let mut primary = McpProcess::new(&agiworkforce_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(agiworkforce_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(agiworkforce_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(agiworkforce_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(agiworkforce_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(agiworkforce_home).await?;
    ...
    let mut second_mcp = McpProcess::new(agiworkforce_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(AGIWORKFORCE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(agiworkforce_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
fn create_config_toml_with_remote_thread_store(
    agiworkforce_home: &std::path::Path,
    server_uri: &str,
) -> std::io::Result<()> {
    let config_toml = agiworkforce_home.join("config.toml");
    std::fs::write(
...
command = "agiworkforce-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
fn setup_rollout_fixture(agiworkforce_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(agiworkforce_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(agiworkforce_home, filename_ts, &conversation_id);
    set_rollout_mtime(rollout_file_path.as_path(), expected_updated_at_rfc3339)?;
    let before_modified = std::fs::metadata(&rollout_file_path)?.modified()?;
    Ok(RolloutFixture {
        conversation_id,
