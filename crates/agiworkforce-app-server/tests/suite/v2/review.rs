...
use agiworkforce_app_server_protocol::ReviewTarget;
...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_with_approval_policy(agiworkforce_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": agiworkforce_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
fn create_config_toml(agiworkforce_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(agiworkforce_home, server_uri, "never")
}
