...
use agiworkforce_app_server_protocol::HookEventName;
use agiworkforce_app_server_protocol::HookHandlerType;
use agiworkforce_app_server_protocol::HookMetadata;
use agiworkforce_app_server_protocol::HookSource;
use agiworkforce_app_server_protocol::HooksListEntry;
...
fn write_user_hook_config(agiworkforce_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(agiworkforce_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = agiworkforce_home.join("plugins/cache/test/demo/local");
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(agiworkforce_home.path())?;
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(agiworkforce_home.path(), "{ not-json")?;
