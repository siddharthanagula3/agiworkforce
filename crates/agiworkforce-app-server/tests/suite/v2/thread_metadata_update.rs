...
use agiworkforce_app_server_protocol::GitInfo;
...
use agiworkforce_protocol::protocol::GitInfo as RolloutGitInfo;
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml(agiworkforce_home.path(), &server.uri())?;
    let _state_db = init_state_db(agiworkforce_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml(agiworkforce_home.path(), &server.uri())?;
    let state_db = init_state_db(agiworkforce_home.path()).await?;
    ...
    let rollout_path = rollout_path(agiworkforce_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(agiworkforce_home.path(), "2025-01-06T08-30-00", &thread_id);
...
async fn init_state_db(agiworkforce_home: &Path) -> Result<Arc<StateRuntime>> {
    let state_db = StateRuntime::init(agiworkforce_home.to_path_buf(), "mock_provider".into()).await?;
    state_db
