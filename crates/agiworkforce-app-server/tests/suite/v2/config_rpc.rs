...
use agiworkforce_app_server_protocol::ConfigEdit;
...
use agiworkforce_app_server_protocol::ToolsV2;
use agiworkforce_app_server_protocol::WriteStatus;
use agiworkforce_core::config::set_project_trust_level;
use agiworkforce_protocol::config_types::TrustLevel;
use agiworkforce_protocol::config_types::WebSearchContextSize;
use agiworkforce_protocol::config_types::WebSearchLocation;
...
fn write_config(agiworkforce_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let agiworkforce_home_path = agiworkforce_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(agiworkforce_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    write_config(&agiworkforce_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(agiworkforce_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_agiworkforce_folder: project_config
        }
    );
...
"#,
    ...
    let managed_path = agiworkforce_home.path().join("managed_config.toml");
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let agiworkforce_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", agiworkforce_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(agiworkforce_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let agiworkforce_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(agiworkforce_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[agiworkforce_app_server_protocol::ConfigLayer],
