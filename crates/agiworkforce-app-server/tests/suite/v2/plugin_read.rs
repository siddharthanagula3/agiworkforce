...
"#,
    ...
    write_installed_plugin(&agiworkforce_home, "openai-curated", "demo-plugin")?;
...
[plugins."demo-plugin@agiworkforce-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&agiworkforce_home, "agiworkforce-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "agiworkforce-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@agiworkforce-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&agiworkforce_home)?;
...
fn write_plugins_enabled_config(agiworkforce_home: &TempDir) -> Result<()> {
    std::fs::write(
        agiworkforce_home.path().join("config.toml"),
        r#"[features]
plugins = true
