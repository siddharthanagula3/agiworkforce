...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "AGIWORKFORCE_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                agiworkforce_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    let installed_path = agiworkforce_home
        .path()
    ...
    .await;
    configure_remote_plugin_test(agiworkforce_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".agiworkforce-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_rejects_invalid_remote_plugin_name() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    write_remote_plugin_catalog_config(agiworkforce_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = McpProcess::new(agiworkforce_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_agiworkforce_plugins_disabled() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Agiworkforce plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                agiworkforce_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(agiworkforce_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    write_analytics_config(agiworkforce_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        agiworkforce_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    write_connectors_config(agiworkforce_home.path(), &server_url)?;
    write_chatgpt_auth(
        agiworkforce_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_connectors_config(agiworkforce_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_analytics_config(agiworkforce_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        agiworkforce_home.join("config.toml"),
...
fn configure_remote_plugin_test(agiworkforce_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(agiworkforce_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        agiworkforce_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
