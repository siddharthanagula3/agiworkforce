...
async fn external_agent_config_import_sends_completion_notification_for_local_plugins() -> Result<()>
{
    ...
    let marketplace_root = agiworkforce_home.path().join("marketplace");
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(agiworkforce_home.path().join(".claude"))?;
    ...
    std::fs::write(
        agiworkforce_home.path().join(".claude").join("settings.json"),
        serde_json::to_string_pretty(&settings)?,
    )?;

    let home_dir = agiworkforce_home.path().display().to_string();
    let mut mcp =
        McpProcess::new_with_env(agiworkforce_home.path(), &[("HOME", Some(home_dir.as_str()))]).await?;
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let project_root = agiworkforce_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = agiworkforce_home.path().join(".claude/projects/repo");
...
async fn external_agent_config_import_rejects_undetected_session_paths() -> Result<()> {
    ...
    let undetected_session_path = agiworkforce_home.path().join("outside.jsonl");
    std::fs::create_dir_all(&project_root)?;
    std::fs::create_dir_all(&session_dir)?;
