...
const LOGIN_ISSUER_ENV_VAR: &str = "AGIWORKFORCE_APP_SERVER_LOGIN_ISSUER";
...
fn create_config_toml(agiworkforce_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(agiworkforce_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(agiworkforce_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(agiworkforce_home.path(), &[("OPENAI_API_KEY", None)]).await?;
    ...
    assert!(
        !agiworkforce_home.path().join("auth.json").exists(),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(agiworkforce_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<agiworkforce_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(agiworkforce_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![agiworkforce_app_server_protocol::UserInput::Text {
