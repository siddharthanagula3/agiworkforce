...
use agiworkforce_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use agiworkforce_app_server::INVALID_PARAMS_ERROR_CODE;
use agiworkforce_app_server_protocol::ByteRange;
use agiworkforce_app_server_protocol::ClientInfo;
use agiworkforce_app_server_protocol::CollabAgentStatus;
...
use agiworkforce_app_server_protocol::FileSystemAccessMode;
use agiworkforce_app_server_protocol::FileSystemPath;
use agiworkforce_app_server_protocol::FileSystemSandboxEntry;
...
use agiworkforce_app_server_protocol::PermissionProfile;
use agiworkforce_app_server_protocol::PermissionProfileFileSystemPermissions;
use agiworkforce_app_server_protocol::PermissionProfileNetworkPermissions;
...
use agiworkforce_app_server_protocol::TextElement;
...
use agiworkforce_app_server_protocol::TurnEnvironmentParams;
...
use agiworkforce_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "agiworkforce_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Agiworkforce VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(agiworkforce_home.path())?;
    let cache_path = agiworkforce_home.path().join("models_cache.json");
    ...
    let config_path = agiworkforce_home.path().join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("model = \"mock-model\"", &format!("model = \"{model}\"")),
    )?;
    write_test_skill(agiworkforce_home.path(), "alpha-skill")?;
    write_test_skill(agiworkforce_home.path(), "beta-skill")?;
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    mount_analytics_capture(&server, agiworkforce_home.path()).await?;

    let mut mcp = McpProcess::new_without_managed_config(agiworkforce_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let event = wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "agiworkforce_turn_event").await?;
...
async fn turn_start_rejects_invalid_permission_profile_before_starting_turn() -> Result<()> {
    ...
    std::fs::write(
        agiworkforce_home.path().join("managed_config.toml"),
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: agiworkforce_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Agiworkforce turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        agiworkforce_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-agiworkforce-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Agiworkforce turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = agiworkforce_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(agiworkforce_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(agiworkforce_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(agiworkforce_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let agiworkforce_home = tmp.path().join("agiworkforce_home");
    std::fs::create_dir(&agiworkforce_home)?;
    ...
    let mut mcp = McpProcess::new(&agiworkforce_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(agiworkforce_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_resolves_sticky_thread_environments_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&agiworkforce_home, &server.uri(), "never", &BTreeMap::default())?;

    let mut mcp = McpProcess::new_with_env(
        &agiworkforce_home,
        &[("AGIWORKFORCE_EXEC_SERVER_URL", Some("http://127.0.0.1:1"))],
    )
    .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![agiworkforce_app_server_protocol::FileUpdateChange {
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        agiworkforce_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(agiworkforce_home.path().join("config.toml"))?;
...
fn create_config_toml_with_sandbox(
    ...
    let feature_entries = features
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let config_toml = agiworkforce_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
...
fn write_test_skill(agiworkforce_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = agiworkforce_home.join("skills").join(name);
