...
use agiworkforce_core::ARCHIVED_SESSIONS_SUBDIR;
...
async fn init_mcp(agiworkforce_home: &Path) -> Result<McpProcess> {
...
async fn list_threads_with_sort(
    ...
    let request_id = mcp
        .send_thread_list_request(agiworkforce_app_server_protocol::ThreadListParams {
            cursor,
            limit,
            sort_key,
            sort_direction: None,
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(agiworkforce_home.path())?;

    let mut mcp = init_mcp(agiworkforce_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_runtime_config(agiworkforce_home.path(), &server.uri())?;
    let mut mcp = init_mcp(agiworkforce_home.path()).await?;
...
fn create_minimal_config(agiworkforce_home: &std::path::Path) -> std::io::Result<()> {
...
fn create_runtime_config(agiworkforce_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = agiworkforce_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let first_target_cwd = agiworkforce_home.path().join("first-target-cwd");
    let second_target_cwd = agiworkforce_home.path().join("second-target-cwd");
    fs::create_dir_all(&first_target_cwd)?;
    fs::create_dir_all(&second_target_cwd)?;
    set_rollout_cwd(
        rollout_path(agiworkforce_home.path(), "2025-01-02T10-00-00", &first_filtered_id).as_path(),
        &first_target_cwd,
    )?;
    set_rollout_cwd(
        rollout_path(
...
"#,
    ...
    let state_db =
        agiworkforce_state::StateRuntime::init(agiworkforce_home.path().to_path_buf(), "mock_provider".into())
            .await?;
    state_db
        .mark_backfill_complete(/*last_watermark*/ None)
        .await?;
    let rollout_config = agiworkforce_rollout::RolloutConfig {
        agiworkforce_home: agiworkforce_home.path().to_path_buf(),
        sqlite_home: agiworkforce_home.path().to_path_buf(),
        cwd: agiworkforce_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = agiworkforce_core::RolloutRecorder::list_threads(
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        agiworkforce_core::ThreadSortKey::CreatedAt,
        agiworkforce_core::SortDirection::Desc,
        &[],
...
"#,
    ...
    let stale_cwd = agiworkforce_home.path().join("stale-cwd");
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_backwards_cursor_can_seed_forward_delta_sync() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T10-00-00", &id_old).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T11-00-00", &id_watermark).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    ...
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(agiworkforce_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_dir = agiworkforce_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    fs::create_dir_all(&archived_dir)?;
    let archived_source = rollout_path(agiworkforce_home.path(), "2025-03-01T09-00-00", &archived_id);
