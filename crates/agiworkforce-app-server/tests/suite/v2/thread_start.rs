...
use agiworkforce_app_server_protocol::ThreadStatusChangedNotification;
...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    create_config_toml_without_approval_policy(agiworkforce_home.path(), &server.uri())?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_without_approval_policy(agiworkforce_home.path(), &server.uri())?;
    let global_agents_path = agiworkforce_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(agiworkforce_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(agiworkforce_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(agiworkforce_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(agiworkforce_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml_with_optional_approval_policy(
        agiworkforce_home, server_uri, /*approval_policy*/ None,
    )
}
