...
use agiworkforce_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    create_config_toml(agiworkforce_home.path())?;
    ...
    let thread_id = seed_stage1_output(&state_db, agiworkforce_home.path()).await?;
...
async fn seed_stage1_output(state_db: &Arc<StateRuntime>, agiworkforce_home: &Path) -> Result<ThreadId> {
    let now = Utc::now();
    let thread_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let worker_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        agiworkforce_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
    builder.updated_at = Some(now);
    builder.cwd = agiworkforce_home.to_path_buf();
...
fn create_config_toml(agiworkforce_home: &Path) -> std::io::Result<()> {
