...
use agiworkforce_app_server_protocol::RateLimitReachedType;
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(agiworkforce_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "agiworkforce_other",
                "metered_feature": "agiworkforce_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
...
fn write_chatgpt_base_url(agiworkforce_home: &Path, base_url: &str) -> std::io::Result<()> {
