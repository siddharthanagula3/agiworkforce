...
use agiworkforce_app_server_protocol::ExperimentalFeature;
...
use agiworkforce_app_server_protocol::ExperimentalFeatureStage;
...
-> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    let project_cwd = agiworkforce_home.path().join("project");
