...
//! the temporary `agiworkforce_home` must not contain rollout session files or sqlite
...
use agiworkforce_app_server::in_process;
use agiworkforce_app_server::in_process::InProcessServerEvent;
...
async fn thread_start_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under agiworkforce_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(agiworkforce_home.path(), &server.uri(), &store_id)?;
    ...
    let mut client = in_process::start(InProcessStartArgs {
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "agiworkforce-app-server-tests".to_string(),
                title: None,
    ...
    assert_no_local_persistence_artifacts(agiworkforce_home.path())?;

    Ok(())
}

fn assert_no_local_persistence_artifacts(agiworkforce_home: &Path) -> Result<()> {
    ...
    // the isolated test agiworkforce_home.
    assert!(
        !agiworkforce_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !agiworkforce_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !agiworkforce_state::state_db_path(agiworkforce_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(agiworkforce_home)?
    ...
    let mut entries = agiworkforce_home_entries(agiworkforce_home)?;
    // Bazel test runs may initialize shell snapshot storage under agiworkforce_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in agiworkforce_home"
    );

    Ok(())
}

fn agiworkforce_home_entries(agiworkforce_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(agiworkforce_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
