...
async fn turn_steer_requires_active_turn() -> Result<()> {
    ...
    mount_analytics_capture(&server, &agiworkforce_home).await?;

    let mut mcp = McpProcess::new_without_managed_config(&agiworkforce_home).await?;
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "agiworkforce_turn_steer_event").await?;
