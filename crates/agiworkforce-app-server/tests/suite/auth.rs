...
fn create_config_toml_forced_login(agiworkforce_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = agiworkforce_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_custom_provider(agiworkforce_home.path(), /*requires_openai_auth*/ false)?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let agiworkforce_home = TempDir::new()?;
    create_config_toml_forced_login(agiworkforce_home.path(), "chatgpt")?;
