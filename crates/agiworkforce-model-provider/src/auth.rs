...
use agiworkforce_agent_identity::AgentIdentityKey;
use agiworkforce_agent_identity::AgentTaskAuthorizationTarget;
use agiworkforce_agent_identity::authorization_header_for_agent_task;
...
struct AgentIdentityAuthProvider {
    auth: agiworkforce_login::auth::AgentIdentityAuth,
}
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> agiworkforce_protocol::error::Result<SharedAuthProvider> {
...
fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> agiworkforce_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Agiworkforce auth snapshot.
pub fn auth_provider_from_auth(auth: &AgiworkforceAuth) -> SharedAuthProvider {
    match auth {
        AgiworkforceAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        AgiworkforceAuth::ApiKey(_) | AgiworkforceAuth::Chatgpt(_) | AgiworkforceAuth::ChatgptAuthTokens(_) => {
