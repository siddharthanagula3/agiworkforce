...
use agiworkforce_api::AuthError;
use agiworkforce_api::AuthProvider;
use agiworkforce_api::SharedAuthProvider;
use agiworkforce_aws_auth::AwsAuthContext;
use agiworkforce_aws_auth::AwsAuthError;
use agiworkforce_aws_auth::AwsRequestToSign;
use agiworkforce_client::Request;
use agiworkforce_client::RequestBody;
use agiworkforce_client::RequestCompression;
...
async fn resolve_auth_method(aws: &ModelProviderAwsAuthInfo) -> Result<BedrockAuthMethod> {
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_agiworkforce_error)?;
...
fn aws_auth_error_to_agiworkforce_error(error: AwsAuthError) -> AgiworkforceErr {
    AgiworkforceErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // for SigV4 verification. Signing it makes the richer Agiworkforce agent request
