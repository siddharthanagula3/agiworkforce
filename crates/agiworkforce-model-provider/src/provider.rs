...
use agiworkforce_models_manager::manager::OpenAiModelsManager;
use agiworkforce_models_manager::manager::SharedModelsManager;
use agiworkforce_models_manager::manager::StaticModelsManager;
...
/// Optional provider-backed features that Agiworkforce may expose at runtime.
///
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Agiworkforce should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    async fn auth(&self) -> Option<AgiworkforceAuth>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Returns provider configuration adapted for the API client.
    async fn api_provider(&self) -> agiworkforce_protocol::error::Result<Provider> {
    ...
    async fn api_auth(&self) -> agiworkforce_protocol::error::Result<SharedAuthProvider> {
        let auth = self.auth().await;
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .map(|auth| match &auth {
                    AgiworkforceAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    AgiworkforceAuth::Chatgpt(_)
                    | AgiworkforceAuth::ChatgptAuthTokens(_)
                    | AgiworkforceAuth::AgentIdentity(_) => {
...
mod tests {
    ...
    use agiworkforce_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_agiworkforce_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("agiworkforce-model-provider-test-{}", std::process::id()))
    }
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("agiworkforce-bedrock".to_string()),
                region: None,
            })),
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_agiworkforce_home(), /*config_model_catalog*/ None);
    ...
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model =
            agiworkforce_models_manager::model_info::model_info_from_slug("custom-bedrock-model");
