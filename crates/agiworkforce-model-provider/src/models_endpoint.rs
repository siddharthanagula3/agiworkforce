...
use agiworkforce_api::ModelsClient;
...
impl OpenAiModelsEndpoint {
    ...
    fn auth_env(&self) -> AuthEnvTelemetry {
        let agiworkforce_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.agiworkforce_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, agiworkforce_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    async fn uses_agiworkforce_backend(&self) -> bool {
        self.auth()
            .await
            .as_ref()
            .is_some_and(AgiworkforceAuth::uses_agiworkforce_backend)
    }
    ...
    async fn list_models(
        ...
        .await
        .map_err(|_| AgiworkforceErr::Timeout)?
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "agiworkforce_otel.log_only",
            tracing::Level::INFO,
            ...
            auth.env_agiworkforce_api_key_present = self.auth_env.agiworkforce_api_key_env_present,
            auth.env_agiworkforce_api_key_enabled = self.auth_env.agiworkforce_api_key_env_enabled,
        ...
        tracing::event!(
            target: "agiworkforce_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "codex.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
