...
#[test]
fn create_seatbelt_args_with_read_only_git_and_agiworkforce_subpaths() {
    // Create a temporary workspace with two writable roots: one containing
    // top-level .git and .agiworkforce directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
        vulnerable_root_canonical,
        dot_git_canonical,
        dot_agiworkforce_canonical,
        empty_root,
    ...
    // write to .agiworkforce/config.toml in the vulnerable root.
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .agiworkforce), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_agiworkforce_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .agiworkforce/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_agiworkforce_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    // Verify that writing a file to the folder containing .git and .agiworkforce is allowed.
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.agiworkforce` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
...
struct PopulatedTmp {
    /// Path containing a .git and .agiworkforce subfolder.
    ...
    /// ran `git commit` themselves, or modified .agiworkforce/config.toml to
    ...
    dot_agiworkforce_canonical: PathBuf,

    /// Path without .git or .agiworkforce subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".agiworkforce")).expect("create .agiworkforce");
    fs::write(
        vulnerable_root.join(".agiworkforce").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .agiworkforce/config.toml");
    ...
    let dot_agiworkforce_canonical = vulnerable_root_canonical.join(".agiworkforce");
