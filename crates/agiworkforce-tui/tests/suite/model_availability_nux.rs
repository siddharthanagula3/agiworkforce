...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_agiworkforce_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let custom_catalog_path = agiworkforce_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(agiworkforce_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        agiworkforce_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let codex = if let Ok(path) = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce") {
        path
    } else {
    ...
    let exec_output = std::process::Command::new(&codex)
        ...
        .env("AGIWORKFORCE_HOME", agiworkforce_home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("AGIWORKFORCE_RS_SSE_FIXTURE", fixture_path)
        .output()
    ...
    env.insert(
        "AGIWORKFORCE_HOME".to_string(),
        agiworkforce_home.path().display().to_string(),
    );
    ...
    let spawned = agiworkforce_utils_pty::spawn_pty_process(
    ...
    let agiworkforce_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = agiworkforce_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let config_contents = std::fs::read_to_string(agiworkforce_home.path().join("config.toml"))?;
