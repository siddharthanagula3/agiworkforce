...
fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let codex = agiworkforce_binary(&repo_root)?;
    ...
    write_auth(agiworkforce_home.path())?;

    let session_name = format!("agiworkforce-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("AGIWORKFORCE_HOME={}", agiworkforce_home.path().display()))
            .arg("OPENAI_API_KEY=dummy")
            .arg(format!("AGIWORKFORCE_RS_SSE_FIXTURE={}", fixture.display()))
            .arg(codex)
            .arg("-c")
    ...
    let agiworkforce_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!agiworkforce_pane.is_empty(), "tmux did not report a pane id");
    ...
    check(
        Command::new("tmux")
            .arg("send-keys")
            .arg("-t")
            .arg(&agiworkforce_pane)
            .arg("-l")
            .arg(draft),
    )?;
    let baseline_capture =
        wait_for_capture_contains(&agiworkforce_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&agiworkforce_pane)?;
    let final_row =
...
fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("agiworkforce-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&agiworkforce_pane)?;
    let restored_row =
...
fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("agiworkforce-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn agiworkforce_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce") {
        return Ok(path);
    }

    let fallback = repo_root.join("agiworkforce-rs/target/debug/codex");
    anyhow::ensure!(
        fallback.is_file(),
        "codex binary is unavailable; run `cargo build -p agiworkforce-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(agiworkforce_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(agiworkforce_home: &Path) -> Result<()> {
    std::fs::write(
