...
"#,
    ...
    std::fs::write(agiworkforce_home.join("config.toml"), config_contents)?;

    let AgiworkforceCliOutput { exit_code, output } = run_agiworkforce_cli(agiworkforce_home, cwd).await?;
    assert_ne!(0, exit_code, "Agiworkforce CLI should exit nonzero.");
    assert!(
...
struct AgiworkforceCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_agiworkforce_cli(
    agiworkforce_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<AgiworkforceCliOutput> {
    let agiworkforce_cli = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce")?;
    let mut env = HashMap::new();
    env.insert(
        "AGIWORKFORCE_HOME".to_string(),
        agiworkforce_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = agiworkforce_utils_pty::spawn_pty_process(
        agiworkforce_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
