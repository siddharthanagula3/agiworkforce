...
//! under `{AGIWORKFORCE_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(agiworkforce_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = agiworkforce_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(agiworkforce_home);
    let agiworkforce_home_owned = agiworkforce_home.map(Path::to_path_buf);
    ...
    let preview_home = agiworkforce_home_owned.clone();
    ...
    SelectionViewParams {
        ...
        subtitle: Some(theme_picker_subtitle(
            agiworkforce_home_owned.as_deref(),
            terminal_width,
        )),
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*agiworkforce_home*/ None, /*terminal_width*/ None,
        );
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle =
            theme_picker_subtitle(/*agiworkforce_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
