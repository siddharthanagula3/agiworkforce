...
use agiworkforce_login::load_auth_dot_json;
...
pub(crate) fn load_local_chatgpt_auth(
    ...
    let auth = load_auth_dot_json(agiworkforce_home, auth_credentials_store_mode)
...
mod tests {
    ...
    fn write_chatgpt_auth(agiworkforce_home: &Path, plan_type: &str) {
        ...
        let auth = AuthDotJson {
            ...
            tokens: Some(TokenData {
                id_token: agiworkforce_login::token_data::parse_chatgpt_jwt_claims(&id_token)
        ...
        save_auth(agiworkforce_home, &auth, AuthCredentialsStoreMode::File)
    ...
    fn loads_local_chatgpt_auth_from_managed_auth() {
        ...
        write_chatgpt_auth(agiworkforce_home.path(), "business");
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let agiworkforce_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(agiworkforce_home.path(), "self_serve_business_usage_based");
