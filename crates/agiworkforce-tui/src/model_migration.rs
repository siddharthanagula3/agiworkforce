...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Agiworkforce just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like Agiworkforce to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
...
mod tests {
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5.1-agiworkforce-mini",
                "gpt-5.1-agiworkforce-max",
                /*model_link*/ None,
                Some(
                ...
                "gpt-5.1-agiworkforce-max".to_string(),
                Some("Agiworkforce-optimized flagship for deep and fast reasoning.".to_string()),
                /*can_opt_out*/ true,
            ),
        );
    ...
    fn prompt_snapshot_gpt5_codex() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-codex",
                "gpt-5.1-agiworkforce-max",
                Some("https://www.codex.com/models/gpt-5.1-agiworkforce-max".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-agiworkforce-max".to_string(),
                Some("Agiworkforce-optimized flagship for deep and fast reasoning.".to_string()),
    ...
    #[test]
    fn prompt_snapshot_gpt5_agiworkforce_mini() {
        let backend = VT100Backend::new(/*width*/ 60, /*height*/ 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        let screen = ModelMigrationScreen::new(
            FrameRequester::test_dummy(),
            migration_copy_for_models(
                "gpt-5-agiworkforce-mini",
                "gpt-5.1-agiworkforce-mini",
                Some("https://www.codex.com/models/gpt-5.1-agiworkforce-mini".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-agiworkforce-mini".to_string(),
