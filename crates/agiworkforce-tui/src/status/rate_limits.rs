...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `codex` or `agiworkforce_other`).
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let combine_non_agiworkforce_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_agiworkforce_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_agiworkforce_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_agiworkforce_single_limit_renders_combined_row() {
        let now = Local::now();
    ...
    #[test]
    fn non_agiworkforce_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "agiworkforce-other".to_string(),
            captured_at: now,
