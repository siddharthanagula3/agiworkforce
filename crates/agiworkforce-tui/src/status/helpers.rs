...
mod tests {
    ...
    async fn test_config(agiworkforce_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let agiworkforce_home = TempDir::new().expect("temp codex home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = agiworkforce_home.path().join(DEFAULT_AGENTS_MD_FILENAME);
        let config = test_config(&agiworkforce_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let agiworkforce_home = TempDir::new().expect("temp codex home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = agiworkforce_home.path().join(LOCAL_AGENTS_MD_FILENAME);
        let config = test_config(&agiworkforce_home, &cwd).await;
