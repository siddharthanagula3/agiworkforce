...
//! | `AGIWORKFORCE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static AGIWORKFORCE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// fork reloads complete). The first call persists `name` and `agiworkforce_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), agiworkforce_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let agiworkforce_home_set_ok = AGIWORKFORCE_HOME.set(agiworkforce_home.clone()).is_ok();
    ...
    if !override_set_ok || !agiworkforce_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, agiworkforce_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = agiworkforce_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$AGIWORKFORCE_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = agiworkforce_home {
...
fn custom_theme_path(name: &str, agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{agiworkforce_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, agiworkforce_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, agiworkforce_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, agiworkforce_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {AGIWORKFORCE_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = agiworkforce_home
...
fn build_default_theme() -> Theme {
    ...
    let agiworkforce_home = AGIWORKFORCE_HOME
        .get()
        .and_then(|agiworkforce_home| agiworkforce_home.as_deref());
    resolve_theme_with_override(name, agiworkforce_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = AGIWORKFORCE_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, agiworkforce_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{AGIWORKFORCE_HOME}/themes/`.
...
/// found in `{agiworkforce_home}/themes/`.
pub(crate) fn list_available_themes(agiworkforce_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*agiworkforce_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*agiworkforce_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*agiworkforce_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*agiworkforce_home*/ None).is_none());
    }
