//! The main Agiworkforce TUI chat surface.
//!
...
use agiworkforce_app_server_protocol::CollabAgentToolCallStatus;
use agiworkforce_app_server_protocol::CommandExecutionRequestApprovalParams;
use agiworkforce_app_server_protocol::ConfigLayerSource;
use agiworkforce_app_server_protocol::ErrorNotification;
use agiworkforce_app_server_protocol::FileChangeRequestApprovalParams;
use agiworkforce_app_server_protocol::GuardianApprovalReviewAction;
use agiworkforce_app_server_protocol::ItemCompletedNotification;
use agiworkforce_app_server_protocol::ItemStartedNotification;
use agiworkforce_app_server_protocol::McpServerStatusDetail;
use agiworkforce_app_server_protocol::ModelVerification as AppServerModelVerification;
use agiworkforce_app_server_protocol::ServerNotification;
use agiworkforce_app_server_protocol::ServerRequest;
use agiworkforce_app_server_protocol::ThreadGoal as AppThreadGoal;
use agiworkforce_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use agiworkforce_app_server_protocol::TurnCompletedNotification;
use agiworkforce_app_server_protocol::TurnPlanStepStatus;
...
use agiworkforce_core_skills::model::SkillMetadata;
...
use agiworkforce_otel::RuntimeMetricsSummary;
use agiworkforce_otel::SessionTelemetry;
use agiworkforce_plugin::PluginCapabilitySummary;
use agiworkforce_protocol::ThreadId;
use agiworkforce_protocol::account::PlanType;
use agiworkforce_protocol::approvals::ElicitationRequestEvent;
...
use agiworkforce_protocol::config_types::Settings;
#[cfg(target_os = "windows")]
use agiworkforce_protocol::config_types::WindowsSandboxLevel;
use agiworkforce_protocol::items::AgentMessageContent;
use agiworkforce_protocol::items::AgentMessageItem;
use agiworkforce_protocol::items::UserMessageItem;
...
use agiworkforce_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use agiworkforce_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
#[cfg(test)]
use agiworkforce_protocol::protocol::AgentMessageDeltaEvent;
#[cfg(test)]
...
#[cfg(test)]
use agiworkforce_protocol::protocol::ErrorEvent;
#[cfg(test)]
...
use agiworkforce_protocol::protocol::ExecCommandOutputDeltaEvent;
use agiworkforce_protocol::protocol::ExecCommandSource;
#[cfg(test)]
use agiworkforce_protocol::protocol::ExitedReviewModeEvent;
use agiworkforce_protocol::protocol::GuardianAssessmentAction;
use agiworkforce_protocol::protocol::GuardianAssessmentDecisionSource;
use agiworkforce_protocol::protocol::GuardianAssessmentEvent;
use agiworkforce_protocol::protocol::GuardianAssessmentStatus;
use agiworkforce_protocol::protocol::ImageGenerationBeginEvent;
use agiworkforce_protocol::protocol::ImageGenerationEndEvent;
use agiworkforce_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::McpListToolsResponseEvent;
use agiworkforce_protocol::protocol::McpStartupStatus;
use agiworkforce_protocol::protocol::McpToolCallBeginEvent;
use agiworkforce_protocol::protocol::McpToolCallEndEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::ModelVerification as CoreModelVerification;
use agiworkforce_protocol::protocol::Op;
use agiworkforce_protocol::protocol::PatchApplyBeginEvent;
use agiworkforce_protocol::protocol::RateLimitReachedType;
...
use agiworkforce_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
#[cfg(test)]
use agiworkforce_protocol::protocol::StreamErrorEvent;
use agiworkforce_protocol::protocol::TerminalInteractionEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::ThreadGoalStatus as ProtocolThreadGoalStatus;
use agiworkforce_protocol::protocol::TokenUsage;
use agiworkforce_protocol::protocol::TokenUsageInfo;
use agiworkforce_protocol::protocol::TurnAbortReason;
#[cfg(test)]
use agiworkforce_protocol::protocol::TurnCompleteEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::TurnDiffEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::UndoCompletedEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::UndoStartedEvent;
use agiworkforce_protocol::protocol::UserMessageEvent;
use agiworkforce_protocol::protocol::ViewImageToolCallEvent;
#[cfg(test)]
use agiworkforce_protocol::protocol::WarningEvent;
use agiworkforce_protocol::protocol::WebSearchBeginEvent;
use agiworkforce_protocol::protocol::WebSearchEndEvent;
use agiworkforce_protocol::request_permissions::RequestPermissionsEvent;
use agiworkforce_protocol::request_user_input::RequestUserInputEvent;
use agiworkforce_protocol::request_user_input::RequestUserInputQuestionOption;
...
use agiworkforce_terminal_detection::Multiplexer;
use agiworkforce_terminal_detection::TerminalInfo;
use agiworkforce_terminal_detection::TerminalName;
use agiworkforce_terminal_detection::terminal_info;
use agiworkforce_utils_absolute_path::AbsolutePathBuf;
use agiworkforce_utils_sleep_inhibitor::SleepInhibitor;
...
use agiworkforce_protocol::plan_tool::StepStatus;
use agiworkforce_protocol::plan_tool::UpdatePlanArgs;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: agiworkforce_feedback::AgiworkforceFeedback,
...
#[cfg(test)]
fn core_rate_limit_error_kind(info: &CoreAgiworkforceErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        CoreAgiworkforceErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        CoreAgiworkforceErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        CoreAgiworkforceErrorInfo::ResponseTooManyFailedAttempts {
...
#[cfg(test)]
fn is_core_cyber_policy_error(info: &CoreAgiworkforceErrorInfo) -> bool {
    matches!(info, CoreAgiworkforceErrorInfo::CyberPolicy)
}

fn is_app_server_cyber_policy_error(info: &AppServerAgiworkforceErrorInfo) -> bool {
    matches!(info, AppServerAgiworkforceErrorInfo::CyberPolicy)
}
...
/// and by sending requests back to agiworkforce-core.
///
...
pub(crate) struct ChatWidget {
    ...
    agiworkforce_rate_limit_reached_type: Option<RateLimitReachedType>,
    ...
    /// Tracks whether agiworkforce-core currently considers an agent turn to be in progress.
    ///
    ...
    session_network_proxy: Option<agiworkforce_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
) -> agiworkforce_protocol::protocol::SessionConfiguredEvent {
    agiworkforce_protocol::protocol::SessionConfiguredEvent {
...
fn hook_output_entry_from_notification(
    entry: agiworkforce_app_server_protocol::HookOutputEntry,
) -> agiworkforce_protocol::protocol::HookOutputEntry {
    agiworkforce_protocol::protocol::HookOutputEntry {
...
fn hook_run_summary_from_notification(
    run: agiworkforce_app_server_protocol::HookRunSummary,
) -> agiworkforce_protocol::protocol::HookRunSummary {
    agiworkforce_protocol::protocol::HookRunSummary {
        id: run.id,
...
fn hook_started_event_from_notification(
    notification: agiworkforce_app_server_protocol::HookStartedNotification,
) -> agiworkforce_protocol::protocol::HookStartedEvent {
    agiworkforce_protocol::protocol::HookStartedEvent {
...
fn hook_completed_event_from_notification(
    notification: agiworkforce_app_server_protocol::HookCompletedNotification,
) -> agiworkforce_protocol::protocol::HookCompletedEvent {
    agiworkforce_protocol::protocol::HookCompletedEvent {
        turn_id: notification.turn_id,
        run: hook_run_summary_from_notification(notification.run),
    }
...
fn app_server_request_id_to_mcp_request_id(
    request_id: &agiworkforce_app_server_protocol::RequestId,
) -> agiworkforce_protocol::mcp::RequestId {
    match request_id {
        agiworkforce_app_server_protocol::RequestId::String(value) => {
            agiworkforce_protocol::mcp::RequestId::String(value.clone())
        }
        agiworkforce_app_server_protocol::RequestId::Integer(value) => {
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        proposed_execpolicy_amendment: params
            ...
            .map(agiworkforce_app_server_protocol::ExecPolicyAmendment::into_core),
        ...
        available_decisions: params.available_decisions.map(|decisions| {
            decisions
                ...
                .map(|decision| match decision {
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::Accept => {
                        agiworkforce_protocol::protocol::ReviewDecision::Approved
                    }
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession => {
                        agiworkforce_protocol::protocol::ReviewDecision::ApprovedForSession
                    }
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::AcceptWithExecpolicyAmendment {
                        execpolicy_amendment,
                    } => agiworkforce_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: execpolicy_amendment.into_core(),
                    },
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                        network_policy_amendment,
                    } => agiworkforce_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                        network_policy_amendment: network_policy_amendment.into_core(),
                    },
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::Decline => {
                        agiworkforce_protocol::protocol::ReviewDecision::Denied
                    }
                    agiworkforce_app_server_protocol::CommandExecutionApprovalDecision::Cancel => {
        ...
        parsed_cmd: params
            ...
            .unwrap_or_default()
            .into_iter()
            .map(agiworkforce_app_server_protocol::CommandAction::into_core)
            .collect(),
    }
}
...
fn request_permissions_from_params(
    params: agiworkforce_app_server_protocol::PermissionsRequestApprovalParams,
...
fn request_user_input_from_params(params: ToolRequestUserInputParams) -> RequestUserInputEvent {
    RequestUserInputEvent {
        ...
        questions: params
            ...
            .into_iter()
            .map(
                |question| agiworkforce_protocol::request_user_input::RequestUserInputQuestion {
                    id: question.id,
...
fn web_search_action_to_core(
    action: agiworkforce_app_server_protocol::WebSearchAction,
) -> agiworkforce_protocol::models::WebSearchAction {
    match action {
        agiworkforce_app_server_protocol::WebSearchAction::Search { query, queries } => {
            agiworkforce_protocol::models::WebSearchAction::Search { query, queries }
        }
        agiworkforce_app_server_protocol::WebSearchAction::OpenPage { url } => {
            agiworkforce_protocol::models::WebSearchAction::OpenPage { url }
        }
        agiworkforce_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            agiworkforce_protocol::models::WebSearchAction::FindInPage { url, pattern }
        }
        agiworkforce_app_server_protocol::WebSearchAction::Other => {
            agiworkforce_protocol::models::WebSearchAction::Other
        }
    }
}
...
impl ChatWidget {
    ...
    fn handle_app_server_steer_rejected_error(
        &mut self,
        agiworkforce_error_info: &AppServerAgiworkforceErrorInfo,
    ) -> bool {
        matches!(
            agiworkforce_error_info,
            AppServerAgiworkforceErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_agiworkforce_limit = limit_id.eq_ignore_ascii_case("agiworkforce");
            if is_agiworkforce_limit
                && let Some(rate_limit_reached_type) = snapshot.rate_limit_reached_type
            {
                self.agiworkforce_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_agiworkforce_limit {
            ...
            let high_usage = is_agiworkforce_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            self.agiworkforce_rate_limit_reached_type = None;
        }
        self.refresh_status_line();
    }
    ...
    fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Agiworkforce is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.agiworkforce_rate_limit_reached_type.map(|kind| {
        ...
        self.agiworkforce_rate_limit_reached_type = rate_limit_reached_type;

        match rate_limit_reached_type {
            Some(RateLimitReachedType::WorkspaceOwnerCreditsDepleted) => {
                self.on_error(
                    "You're out of credits. Your workspace is out of credits. Add credits to continue using Agiworkforce."
                        .to_string(),
                );
            }
    ...
    fn handle_non_retry_error(
        &mut self,
        message: String,
        agiworkforce_error_info: Option<AppServerAgiworkforceErrorInfo>,
    ) {
        if agiworkforce_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if agiworkforce_error_info
            .as_ref()
            .is_some_and(is_app_server_cyber_policy_error)
        {
            self.on_cyber_policy_error();
        } else if let Some(info) = agiworkforce_error_info
            .as_ref()
    ...
    fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        if ev.status == GuardianAssessmentStatus::TimedOut {
            let cell = if let Some(command) = guardian_command(&ev.action) {
                history_cell::new_approval_decision_cell(
                    command,
                    agiworkforce_protocol::protocol::ReviewDecision::TimedOut,
                    history_cell::ApprovalDecisionActor::Guardian,
                )
            } else {
                match &ev.action {
    ...
    fn on_patch_apply_end(&mut self, event: agiworkforce_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
            ...
            CollabAgentTool::SendInput => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::interaction_end(
                        agiworkforce_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                ...
                if let Some(receiver_thread_id) = first_receiver {
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            agiworkforce_protocol::protocol::CollabResumeBeginEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
                        ...
                        self.on_collab_event(multi_agents::resume_end(
                            agiworkforce_protocol::protocol::CollabResumeEndEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
            ...
            CollabAgentTool::Wait => {
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    self.on_collab_event(multi_agents::waiting_begin(
                        agiworkforce_protocol::protocol::CollabWaitingBeginEvent {
                            sender_thread_id,
                    ...
                    self.on_collab_event(multi_agents::waiting_end(
                        agiworkforce_protocol::protocol::CollabWaitingEndEvent {
                            sender_thread_id,
                            call_id: id,
                            agent_statuses,
                            statuses,
            ...
            CollabAgentTool::CloseAgent => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::close_end(
                        agiworkforce_protocol::protocol::CollabCloseEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                            receiver_agent_nickname: first_receiver_metadata
    ...
    pub(crate) fn handle_history_entry_response(
        &mut self,
        event: agiworkforce_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let agiworkforce_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: agiworkforce_protocol::protocol::HookStartedEvent) {
        self.flush_answer_stream_with_separator();
    ...
    fn on_hook_completed(&mut self, event: agiworkforce_protocol::protocol::HookCompletedEvent) {
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: agiworkforce_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new_with_app_event(common: ChatWidgetInit) -> Self {
        Self::new_with_op_target(common, AgiworkforceOpTarget::AppEvent)
    }
    ...
    fn new_with_op_target(common: ChatWidgetInit, agiworkforce_op_target: AgiworkforceOpTarget) -> Self {
        ...
        let mut widget = Self {
            ...
            agiworkforce_rate_limit_reached_type: None,
    ...
    fn submit_user_message_with_history_and_shell_escape_policy(
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = agiworkforce_connectors::metadata::connector_mention_slug(&app);
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = UserMessageItem {
                    ...
                    content: content
                        .into_iter()
                        .map(agiworkforce_app_server_protocol::UserInput::into_core)
                        .collect(),
                };
            ...
            ThreadItem::AgentMessage {
                ...
                self.on_agent_message_item_completed(AgentMessageItem {
                    ...
                    memory_citation: memory_citation.map(|citation| {
                        agiworkforce_protocol::memory_citation::MemoryCitation {
                            entries: citation
                                .entries
                                .into_iter()
                                .map(
                    ...
                    self.on_exec_command_end(ExecCommandEndEvent {
                        ...
                        status: match status {
                            agiworkforce_app_server_protocol::CommandExecutionStatus::Completed => {
                                agiworkforce_protocol::protocol::ExecCommandStatus::Completed
                            }
                            agiworkforce_app_server_protocol::CommandExecutionStatus::Failed => {
                                agiworkforce_protocol::protocol::ExecCommandStatus::Failed
                            }
                            agiworkforce_app_server_protocol::CommandExecutionStatus::Declined => {
                                agiworkforce_protocol::protocol::ExecCommandStatus::Declined
                            }
                            agiworkforce_app_server_protocol::CommandExecutionStatus::InProgress => {
                                agiworkforce_protocol::protocol::ExecCommandStatus::Failed
                            }
            ...
            ThreadItem::FileChange {
                ...
                if !matches!(
                    status,
                    agiworkforce_app_server_protocol::PatchApplyStatus::InProgress
                ) {
                    self.on_patch_apply_end(agiworkforce_protocol::protocol::PatchApplyEndEvent {
                        call_id: id,
                        turn_id: turn_id.clone(),
                        stdout: String::new(),
                        stderr: String::new(),
                        success: !matches!(
                            status,
                            agiworkforce_app_server_protocol::PatchApplyStatus::Failed
                        ),
                        changes: file_update_changes_to_core(changes),
                        status: match status {
                            agiworkforce_app_server_protocol::PatchApplyStatus::Completed => {
                                agiworkforce_protocol::protocol::PatchApplyStatus::Completed
                            }
                            agiworkforce_app_server_protocol::PatchApplyStatus::Failed => {
                                agiworkforce_protocol::protocol::PatchApplyStatus::Failed
                            }
                            agiworkforce_app_server_protocol::PatchApplyStatus::Declined => {
                                agiworkforce_protocol::protocol::PatchApplyStatus::Declined
                            }
                            agiworkforce_app_server_protocol::PatchApplyStatus::InProgress => {
                                agiworkforce_protocol::protocol::PatchApplyStatus::Failed
                            }
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(agiworkforce_protocol::protocol::McpToolCallEndEvent {
                    call_id: id,
                    invocation: agiworkforce_protocol::protocol::McpInvocation {
                        server,
                        tool,
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => {
                            let result = *result;
                            Ok(agiworkforce_protocol::mcp::CallToolResult {
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(WebSearchEndEvent {
                    ...
                    action: action
                        ...
                        .unwrap_or(agiworkforce_protocol::models::WebSearchAction::Other),
                });
            }
        ...
        match notification {
            ...
            ServerNotification::ThreadNameUpdated(notification) => {
                match ThreadId::from_string(&notification.thread_id) {
                    Ok(thread_id) => self.on_thread_name_updated(
                        agiworkforce_protocol::protocol::ThreadNameUpdatedEvent {
                            thread_id,
            ...
            ServerNotification::CommandExecutionOutputDelta(notification) => {
                self.on_exec_command_output_delta(ExecCommandOutputDeltaEvent {
                    call_id: notification.item_id,
                    stream: agiworkforce_protocol::protocol::ExecOutputStream::Stdout,
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.agiworkforce_error_info,
                    );
                }
            }
            ...
            ServerNotification::ThreadRealtimeStarted(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_started(
                        agiworkforce_protocol::protocol::RealtimeConversationStartedEvent {
            ...
            ServerNotification::ThreadRealtimeItemAdded(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agiworkforce_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agiworkforce_protocol::protocol::RealtimeEvent::ConversationItemAdded(
                                notification.item,
                            ),
                        },
                    );
            ...
            ServerNotification::ThreadRealtimeOutputAudioDelta(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agiworkforce_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agiworkforce_protocol::protocol::RealtimeEvent::AudioOut(
            ...
            ServerNotification::ThreadRealtimeError(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        agiworkforce_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: agiworkforce_protocol::protocol::RealtimeEvent::Error(
            ...
            ServerNotification::ThreadRealtimeClosed(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_closed(
                        agiworkforce_protocol::protocol::RealtimeConversationClosedEvent {
    ...
    fn on_mcp_server_elicitation_request(
        &mut self,
        request_id: agiworkforce_protocol::mcp::RequestId,
        params: agiworkforce_app_server_protocol::McpServerElicitationRequestParams,
    ) {
        let request = agiworkforce_protocol::approvals::ElicitationRequestEvent {
            turn_id: params.turn_id,
            server_name: params.server_name,
            id: request_id,
            request: match params.request {
                agiworkforce_app_server_protocol::McpServerElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                } => agiworkforce_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema: serde_json::to_value(requested_schema)
                        .unwrap_or(serde_json::Value::Null),
                },
                agiworkforce_app_server_protocol::McpServerElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
                } => agiworkforce_protocol::approvals::ElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.agiworkforce_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
    ...
    fn on_guardian_review_notification(
        ...
        id: String,
        turn_id: String,
        review: agiworkforce_app_server_protocol::GuardianApprovalReview,
        decision_source: Option<agiworkforce_app_server_protocol::AutoReviewDecisionSource>,
        ...
        self.on_guardian_assessment(GuardianAssessmentEvent {
            ...
            status: match review.status {
                agiworkforce_app_server_protocol::GuardianApprovalReviewStatus::InProgress => {
                    GuardianAssessmentStatus::InProgress
                }
                agiworkforce_app_server_protocol::GuardianApprovalReviewStatus::Approved => {
                    GuardianAssessmentStatus::Approved
                }
                agiworkforce_app_server_protocol::GuardianApprovalReviewStatus::Denied => {
                    GuardianAssessmentStatus::Denied
                }
                agiworkforce_app_server_protocol::GuardianApprovalReviewStatus::TimedOut => {
                    GuardianAssessmentStatus::TimedOut
                }
                agiworkforce_app_server_protocol::GuardianApprovalReviewStatus::Aborted => {
                    GuardianAssessmentStatus::Aborted
                }
            },
            risk_level: review.risk_level.map(|risk_level| match risk_level {
                agiworkforce_app_server_protocol::GuardianRiskLevel::Low => {
                    agiworkforce_protocol::protocol::GuardianRiskLevel::Low
                }
                agiworkforce_app_server_protocol::GuardianRiskLevel::Medium => {
                    agiworkforce_protocol::protocol::GuardianRiskLevel::Medium
                }
                agiworkforce_app_server_protocol::GuardianRiskLevel::High => {
                    agiworkforce_protocol::protocol::GuardianRiskLevel::High
                }
                agiworkforce_app_server_protocol::GuardianRiskLevel::Critical => {
                    agiworkforce_protocol::protocol::GuardianRiskLevel::Critical
                }
            }),
            user_authorization: review.user_authorization.map(|user_authorization| {
                match user_authorization {
                    agiworkforce_app_server_protocol::GuardianUserAuthorization::Unknown => {
                        agiworkforce_protocol::protocol::GuardianUserAuthorization::Unknown
                    }
                    agiworkforce_app_server_protocol::GuardianUserAuthorization::Low => {
                        agiworkforce_protocol::protocol::GuardianUserAuthorization::Low
                    }
                    agiworkforce_app_server_protocol::GuardianUserAuthorization::Medium => {
                        agiworkforce_protocol::protocol::GuardianUserAuthorization::Medium
                    }
                    agiworkforce_app_server_protocol::GuardianUserAuthorization::High => {
                        agiworkforce_protocol::protocol::GuardianUserAuthorization::High
                    }
                }
            }),
            rationale: review.rationale,
            decision_source: decision_source.map(|source| match source {
                agiworkforce_app_server_protocol::AutoReviewDecisionSource::Agent => {
    ...
    #[cfg(test)]
    pub(crate) fn handle_agiworkforce_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    #[cfg(test)]
    pub(crate) fn handle_agiworkforce_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_agiworkforce_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                    self.on_cyber_policy_error();
                } else if let Some(kind) = agiworkforce_error_info
                    .as_ref()
            ...
            EventMsg::ItemCompleted(event) => {
                ...
                if let agiworkforce_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let agiworkforce_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn open_theme_picker(&mut self) {
        let agiworkforce_home = crate::legacy_core::config::find_agiworkforce_home().ok();
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::AgiworkforceOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("Choose a communication style for Agiworkforce.".dim()));
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for Agiworkforce.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-codex")
            || preset.model.starts_with("gpt-5.1-agiworkforce-max")
                        ...
                        == WindowsSandboxLevel::Disabled
                    {
                        ...
                        if crate::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && crate::legacy_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.agiworkforce_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match agiworkforce_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.agiworkforce_home.as_path(),
            cwd.as_path(),
            &env_map,
            &policy,
            Some(self.config.agiworkforce_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When Agiworkforce runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the Agiworkforce agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/codex/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(&mut self, preset: ApprovalPreset) {
        ...
        lines.push(line![""]);
        lines.push(line![
            "You can still use Agiworkforce in a non-admin sandbox. It carries greater risk if prompt injected."
        ]);
        lines.push(line![
        ...
        let items = vec![
            ...
            SelectionItem {
                name: "Use Agiworkforce with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
    ...
    fn set_service_tier_selection(&mut self, service_tier: Option<ServiceTier>) {
        ...
        self.app_event_tx
            .send(AppEvent::AgiworkforceOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                /*approval_policy*/ None,
                /*approvals_reviewer*/ None,
                /*permission_profile*/ None,
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            let connector_label = agiworkforce_connectors::metadata::connector_display_label(connector);
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload Agiworkforce."
            };
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.agiworkforce_op_target {
            AgiworkforceOpTarget::Direct(agiworkforce_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = agiworkforce_op_tx.send(op.into_core()) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            AgiworkforceOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::AgiworkforceOp(op));
            }
        }
        true
    }
