    ...
    fn advance_search_from(
        ...
        while offset < total_entries {
                ...
                && let Some(log_id) = self.history_log_id
            {
                ...
                app_event_tx.send(AppEvent::AgiworkforceOp(AppCommand::from(
...
mod tests {
    ...
    fn navigation_with_async_fetch() {
        ...
        let AppEvent::AgiworkforceOp(op) = event2 else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
    ...
    fn search_matches_local_history_and_stops_at_boundaries() {
        ...
        history.record_local_submission(HistoryEntry::new("cargo test -p agiworkforce-tui".to_string()));
    ...
    fn search_fetches_persistent_history_until_match() {
        ...
        let AppEvent::AgiworkforceOp(op) = rx.try_recv().expect("expected latest lookup") else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
        ...
        let AppEvent::AgiworkforceOp(op) = rx.try_recv().expect("expected next lookup") else {
            panic!("unexpected event variant");
        };
        assert_eq!(
            Op::GetHistoryEntryRequest {
