//! Transcript/history cells for the Agiworkforce TUI.
//!
...
#[cfg(test)]
use agiworkforce_mcp::qualified_mcp_tool_name_prefix;
...
use agiworkforce_utils_cli::format_env_display;
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: agiworkforce_protocol::protocol::ReviewDecision,
        ...
        NetworkPolicyAmendment {
            ...
            NetworkPolicyRuleAction::Allow => (
                ...
                vec![
                    actor.subject().into(),
                    "persisted".bold(),
                    " Agiworkforce network access to ".into(),
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Agiworkforce".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Agiworkforce is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
        ...
        && matches!(
            ...
            PermissionProfile::Disabled
                | PermissionProfile::Managed {
                    ...
                    network: agiworkforce_protocol::protocol::NetworkSandboxPolicy::Enabled,
                }
        )
}
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI Agiworkforce (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("OpenAI Agiworkforce").bold(),
            Span::from(" ").dim(),
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<agiworkforce_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<agiworkforce_protocol::mcp::CallToolResult, String>,
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, agiworkforce_protocol::mcp::Tool>,
...
mod tests {
    ...
    use agiworkforce_otel::RuntimeMetricTotals;
    use agiworkforce_otel::RuntimeMetricsSummary;
    ...
    fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
        ...
        let cmd = "set -o pipefail\ncargo test -p agiworkforce-tui --quiet".to_string();
        let call_id = "c1".to_string();
        let mut cell = ExecCell::new(
            ExecCall {
                call_id: call_id.clone(),
