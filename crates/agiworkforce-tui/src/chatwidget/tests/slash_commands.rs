...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::AgiworkforceOp(AppCommand::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::AgiworkforceOp(AppCommand::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::AgiworkforceOp(AppCommand::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Agiworkforce op to be sent, got {other:?}"),
    }
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::AgiworkforceOp(AppCommand::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(agiworkforce_config::types::KeybindingsSpec::One(
        agiworkforce_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            agiworkforce_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: agiworkforce_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: agiworkforce_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::AgiworkforceOp(AppCommand::OverrideTurnContext {
