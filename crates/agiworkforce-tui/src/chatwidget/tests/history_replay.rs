...
use agiworkforce_protocol::protocol::FileSystemSpecialPath;
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = agiworkforce_protocol::protocol::SessionConfiguredEvent {
        ...
        permission_profile: agiworkforce_protocol::models::PermissionProfile::read_only(),
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.agiworkforce_home =
        agiworkforce_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local AGIWORKFORCE_HOME"
    );
