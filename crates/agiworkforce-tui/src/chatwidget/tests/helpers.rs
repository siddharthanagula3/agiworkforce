...
pub(super) async fn test_config() -> Config {
    ...
    let agiworkforce_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_agiworkforce_home(agiworkforce_home.clone(), Vec::new())
            .await
            .expect("config");
    config.agiworkforce_home = agiworkforce_home.abs();
    config.sqlite_home = agiworkforce_home.clone();
    config.log_dir = agiworkforce_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        placeholder_text: "Ask Agiworkforce to do anything".to_string(),
    ...
    let mut widget = ChatWidget {
        ...
        normal_placeholder_text: "Ask Agiworkforce to do anything".to_string(),
...
fn test_model_info(slug: &str, priority: i32, supports_fast_mode: bool) -> ModelInfo {
    let additional_speed_tiers = if supports_fast_mode {
        vec![agiworkforce_protocol::openai_models::SPEED_TIER_FAST]
    } else {
        Vec::new()
    };
...
fn hook_event_label(event_name: agiworkforce_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        ...
        agiworkforce_protocol::protocol::HookEventName::PermissionRequest => "PermissionRequest",
        agiworkforce_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
