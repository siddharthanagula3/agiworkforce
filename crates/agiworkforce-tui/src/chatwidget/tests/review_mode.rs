...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::AgiworkforceOp(AppCommand::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::AgiworkforceOp(AppCommand::Review { review_request }) => {
            assert_eq!(
                review_request,
                ReviewRequest {
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        agiworkforce_app_server_protocol::ServerNotification::TurnStarted(
            agiworkforce_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: agiworkforce_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: agiworkforce_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        agiworkforce_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            agiworkforce_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: agiworkforce_app_server_protocol::ThreadGoal {
                    ...
                    status: agiworkforce_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        agiworkforce_app_server_protocol::ServerNotification::TurnCompleted(
            agiworkforce_app_server_protocol::TurnCompletedNotification {
                ...
                turn: agiworkforce_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: agiworkforce_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
