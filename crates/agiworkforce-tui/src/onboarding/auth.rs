...
fn onboarding_request_id() -> agiworkforce_app_server_protocol::RequestId {
    agiworkforce_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<agiworkforce_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with ChatGPT to use Agiworkforce as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Decide how much autonomy you want to grant Agiworkforce".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://developers.openai.com/codex/security\u{7}Agiworkforce docs\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  Agiworkforce can make mistakes".into(),
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Agiworkforce will use usage-based billing with your API key.".into(),
        ];
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .agiworkforce_home(agiworkforce_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                agiworkforce_app_server_client::EnvironmentManager::default_for_tests(),
            ),
