use std::path::PathBuf;

use crate::app_command::AppCommand;
use agiworkforce_protocol::ThreadId;
use agiworkforce_protocol::approvals::ElicitationAction;
use agiworkforce_protocol::mcp::RequestId as McpRequestId;
use agiworkforce_protocol::protocol::ConversationAudioParams;
use agiworkforce_protocol::protocol::ReviewDecision;
use agiworkforce_protocol::protocol::ReviewRequest;
use agiworkforce_protocol::request_permissions::RequestPermissionsResponse;
use agiworkforce_protocol::request_user_input::RequestUserInputResponse;
use tokio::sync::mpsc::UnboundedSender;

use crate::app_event::AppEvent;
use crate::session_log;

#[derive(Clone, Debug)]
pub(crate) struct AppEventSender {
    pub app_event_tx: UnboundedSender<AppEvent>,
}

impl AppEventSender {
    pub(crate) fn new(app_event_tx: UnboundedSender<AppEvent>) -> Self {
        Self { app_event_tx }
    }

    /// Send an event to the app event channel. If it fails, we swallow the
    /// error and log it.
    pub(crate) fn send(&self, event: AppEvent) {
        // Record inbound events for high-fidelity session replay.
        // Avoid double-logging Ops; those are logged at the point of submission.
        if !matches!(event, AppEvent::AgiworkforceOp(_)) {
            session_log::log_inbound_app_event(&event);
        }
        if let Err(e) = self.app_event_tx.send(event) {
            tracing::error!("failed to send event: {e}");
        }
    }

    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::interrupt()));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, review_request: ReviewRequest) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::review(review_request)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }

    #[cfg_attr(target_os = "linux", allow(dead_code))]
    pub(crate) fn realtime_conversation_audio(&self, params: ConversationAudioParams) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::realtime_conversation_audio(
            params,
        )));
    }

    pub(crate) fn user_input_answer(&self, id: String, response: RequestUserInputResponse) {
        self.send(AppEvent::AgiworkforceOp(AppCommand::user_input_answer(
            id, response,
        )));
    }

    pub(crate) fn exec_approval(&self, thread_id: ThreadId, id: String, decision: ReviewDecision) {
        self.send(AppEvent::SubmitThreadOp {
            thread_id,
            op: AppCommand::exec_approval(id, /*turn_id*/ None, decision),
        });
    }

    pub(crate) fn request_permissions_response(
        &self,
        thread_id: ThreadId,
        id: String,
        response: RequestPermissionsResponse,
    ) {
        self.send(AppEvent::SubmitThreadOp {
            thread_id,
            op: AppCommand::request_permissions_response(id, response),
        });
    }

    pub(crate) fn patch_approval(&self, thread_id: ThreadId, id: String, decision: ReviewDecision) {
        self.send(AppEvent::SubmitThreadOp {
            thread_id,
            op: AppCommand::patch_approval(id, decision),
        });
    }

    pub(crate) fn resolve_elicitation(
        &self,
        thread_id: ThreadId,
        server_name: String,
        request_id: McpRequestId,
        decision: ElicitationAction,
        content: Option<serde_json::Value>,
        meta: Option<serde_json::Value>,
    ) {
        self.send(AppEvent::SubmitThreadOp {
            thread_id,
            op: AppCommand::resolve_elicitation(server_name, request_id, decision, content, meta),
        });
    }
}
