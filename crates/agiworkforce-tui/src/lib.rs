...
// The standalone `agiworkforce-tui` binary prints a short help message before the
...
use crate::legacy_core::config::find_agiworkforce_home;
...
use agiworkforce_app_server_client::RemoteAppServerClient;
use agiworkforce_app_server_client::RemoteAppServerConnectArgs;
use agiworkforce_app_server_protocol::Account as AppServerAccount;
use agiworkforce_app_server_protocol::AuthMode as AppServerAuthMode;
use agiworkforce_app_server_protocol::ConfigWarningNotification;
use agiworkforce_app_server_protocol::Thread as AppServerThread;
...
use agiworkforce_rollout::read_session_meta_line;
...
use agiworkforce_utils_path as path_utils;
...
pub(crate) use agiworkforce_app_server_client::legacy_core;
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        agiworkforce_feedback::AgiworkforceFeedback::new(),
        /*log_db*/ None,
        environment_manager,
    )
    .await?;
    ...
    Fut: Future<Output = std::io::Result<InProcessAppServerClient>>,
{
    ...
    let client = start_client(InProcessClientStartArgs {
        ...
        environment_manager,
        config_warnings,
        session_source: agiworkforce_protocol::protocol::SessionSource::Cli,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = agiworkforce_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let agiworkforce_home = match find_agiworkforce_home() {
        Ok(agiworkforce_home) => agiworkforce_home.to_path_buf(),
        Err(err) => {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&agiworkforce_home).await?;
    ...
    // Agiworkforce CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("agiworkforce-tui.log"))?;
    ...
    let feedback = agiworkforce_feedback::AgiworkforceFeedback::new();
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: agiworkforce_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !remote_mode {
            cloud_requirements = cloud_requirements_loader_for_storage(
                initial_config.agiworkforce_home.to_path_buf(),
                /*enable_agiworkforce_api_key_env*/ false,
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_agiworkforce_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
        ...
        match tui_alternate_screen {
            ...
            AltScreenMode::Auto => {
                ...
                !matches!(
                    ...
                    Some(agiworkforce_terminal_detection::Multiplexer::Zellij {})
                )
            }
        }
    }
...
mod tests {
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        ...
        let state_runtime = agiworkforce_state::StateRuntime::init(
        ...
        let mut app_server =
            AppServerSession::new(agiworkforce_app_server_client::AppServerClient::InProcess(
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        ...
        TurnContextItem {
            ...
            summary: config
                ...
                .unwrap_or(agiworkforce_protocol::config_types::ReasoningSummary::Auto),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let agiworkforce_home = temp_dir.path().to_path_buf();
