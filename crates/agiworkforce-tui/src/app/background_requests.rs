...
impl App {
    ...
    pub(super) fn refresh_plugin_mentions(&mut self) {
        ...
        tokio::spawn(async move {
            let plugins = PluginsManager::new(config.agiworkforce_home.to_path_buf())
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<agiworkforce_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: agiworkforce_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
