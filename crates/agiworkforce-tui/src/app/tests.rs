...
use agiworkforce_app_server_protocol::AdditionalFileSystemPermissions;
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::AgiworkforceOp(AppCommand::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.agiworkforce_home = agiworkforce_home.path().to_path_buf().abs();
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    ...
    let agiworkforce_home = tempdir()?;
    app.config.agiworkforce_home = agiworkforce_home.path().to_path_buf().abs();
    app.config.sqlite_home = agiworkforce_home.path().to_path_buf();

    let memory_root = agiworkforce_home.path().join("memories");
...
-> Result<()> {
    ...
    let agiworkforce_home = tempdir()?;
    app.config.agiworkforce_home = agiworkforce_home.path().to_path_buf().abs();
    let config_toml_path = agiworkforce_home.path().join("config.toml").abs();
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: agiworkforce_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: agiworkforce_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        agiworkforce_app_server_client::AppServerEvent::ServerNotification(
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    agiworkforce_app_server_protocol::ItemStartedNotification {
                        thread_id: "thread-1".to_string(),
                        turn_id: "turn-1".to_string(),
                        item: ThreadItem::CollabAgentToolCall {
                            id: "wait-1".to_string(),
                            tool: agiworkforce_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                agiworkforce_app_server_protocol::CollabAgentToolCallStatus::InProgress,
