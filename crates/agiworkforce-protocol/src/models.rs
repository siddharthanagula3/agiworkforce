...
use agiworkforce_utils_image::error::ImageProcessingError;
...
/// Legacy file system permission overlay with explicit read/write path lists.
/// Used by the sandboxing layer for additive permission grants.
#[derive(Debug, Clone, Default, Eq, Hash, PartialEq, Serialize, Deserialize, JsonSchema, TS)]
pub struct SandboxFileSystemPermissions {
    pub read: Option<Vec<AbsolutePathBuf>>,
    pub write: Option<Vec<AbsolutePathBuf>>,
}

impl SandboxFileSystemPermissions {
    pub fn is_empty(&self) -> bool {
...
pub struct SimplePermissionProfile {
    pub network: Option<NetworkPermissions>,
    pub file_system: Option<SandboxFileSystemPermissions>,
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Agiworkforce could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Agiworkforce cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
// (Moved event mapping logic into agiworkforce-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__agiworkforce_apps__gmail_get_recent_emails",
            "namespace": "mcp__agiworkforce_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__agiworkforce_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__agiworkforce_apps__gmail".to_string()),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__agiworkforce_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Agiworkforce cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
