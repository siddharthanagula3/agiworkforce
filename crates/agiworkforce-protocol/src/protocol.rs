//! Defines the protocol for a AgiWorkforce session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for AgiWorkforce:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::agiworkforce_thread::AgiWorkforceThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by agiworkforce-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request AgiWorkforce to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request AgiWorkforce to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down agiworkforce instance.
    Shutdown,
...
/// privileges of the agent (e.g. `.agiworkforce`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Make .agents/skills and .agiworkforce/config.toml and related files read-only
    // to the agent, by default.
    for subdir in &[".agents", ".agiworkforce"] {
        #[allow(clippy::expect_used)]
        let top_level_agiworkforce = writable_root.join(subdir).expect("valid relative path");
        if top_level_agiworkforce.as_path().is_dir() {
            subpaths.push(top_level_agiworkforce);
        }
    }
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.agiworkforce_error_info
            .as_ref()
            .is_none_or(AgiWorkforceErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::AgiWorkforce),
...
impl Product {
    pub fn to_app_platform(self) -> &'static str {
        match self {
            Self::Chatgpt => "chat",
            Self::AgiWorkforce => "agiworkforce",
            Self::Atlas => "atlas",
        }
    }
    ...
    pub fn from_session_source_name(value: &str) -> Option<Self> {
        ...
        match normalized.as_str() {
            ...
            "agiworkforce" => Some(Self::AgiWorkforce),
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_agiworkforce() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("agiworkforce".to_string()).restriction_product(),
            Some(Product::AgiWorkforce)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::AgiWorkforce])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::AgiWorkforce]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".agiworkforce")).expect("create .agiworkforce");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let cwd_absolute =
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_agiworkforce.as_path())
        );
    }
