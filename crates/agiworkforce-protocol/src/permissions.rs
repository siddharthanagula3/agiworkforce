...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer AgiWorkforce degrades to warn-and-ignore
    /// instead of failing to load. AgiWorkforce 0.112.0 rejected unknown values here,
...
impl FileSystemSandboxPolicy {
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.agiworkforce -> <root>/decoy`, bwrap must still see
            // `<root>/.agiworkforce`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.agiworkforce` so downstream sandboxes can still detect and mask
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "AGIWORKFORCE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn effective_runtime_roots_canonicalize_symlinked_paths() {
        ...
        let agiworkforce_dir = real_root.join(".agiworkforce");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&agiworkforce_dir).expect("create .agiworkforce");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_agiworkforce)
        );
    }
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-agiworkforce");
        let dot_agiworkforce = root.join(".agiworkforce");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_agiworkforce).expect("create .agiworkforce symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_agiworkforce = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".agiworkforce"),
        )
        .expect("absolute .agiworkforce symlink");
    ...
    fn tmpdir_special_path_canonicalizes_symlinked_tmpdir() {
        ...
        let agiworkforce_dir = real_tmpdir.join(".agiworkforce");
