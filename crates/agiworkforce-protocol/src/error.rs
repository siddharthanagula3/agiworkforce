...
pub type Result<T> = std::result::Result<T, AgiworkforceErr>;
...
pub enum AgiworkforceErr {
    ...
    #[error(
        "Agiworkforce ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `AgiworkforceError::Spawn` variant.
    ...
    #[error("agiworkforce-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for AgiworkforceErr {
...
impl AgiworkforceErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            AgiworkforceErr::TurnAborted
            | AgiworkforceErr::Interrupted
            | AgiworkforceErr::EnvVar(_)
            | AgiworkforceErr::Fatal(_)
            | AgiworkforceErr::UsageNotIncluded
            | AgiworkforceErr::QuotaExceeded
            | AgiworkforceErr::InvalidImageRequest()
            | AgiworkforceErr::InvalidRequest(_)
            | AgiworkforceErr::RefreshTokenFailed(_)
            | AgiworkforceErr::UnsupportedOperation(_)
            | AgiworkforceErr::Sandbox(_)
            | AgiworkforceErr::LandlockSandboxExecutableNotProvided
            | AgiworkforceErr::RetryLimit(_)
            | AgiworkforceErr::ContextWindowExceeded
            | AgiworkforceErr::ThreadNotFound(_)
            | AgiworkforceErr::AgentLimitReached { .. }
            | AgiworkforceErr::Spawn
            | AgiworkforceErr::SessionConfiguredNotFirstEvent
            | AgiworkforceErr::UsageLimitReached(_)
            | AgiworkforceErr::ServerOverloaded
            | AgiworkforceErr::CyberPolicy { .. } => false,
            AgiworkforceErr::Stream(..)
            | AgiworkforceErr::Timeout
            | AgiworkforceErr::UnexpectedStatus(_)
    ...
    /// Minimal shim so that existing `e.downcast_ref::<AgiworkforceErr>()` checks continue to compile
    ...
    pub fn to_agiworkforce_protocol_error(&self) -> AgiworkforceErrorInfo {
        match self {
            AgiworkforceErr::ContextWindowExceeded => AgiworkforceErrorInfo::ContextWindowExceeded,
            AgiworkforceErr::UsageLimitReached(_)
            | AgiworkforceErr::QuotaExceeded
            ...
            AgiworkforceErr::SessionConfiguredNotFirstEvent
            | AgiworkforceErr::InternalServerError
            | AgiworkforceErr::InternalAgentDied => AgiworkforceErrorInfo::InternalServerError,
            AgiworkforceErr::UnsupportedOperation(_)
            | AgiworkforceErr::ThreadNotFound(_)
            | AgiworkforceErr::AgentLimitReached { .. } => AgiworkforceErrorInfo::BadRequest,
            AgiworkforceErr::Sandbox(_) => AgiworkforceErrorInfo::SandboxError,
            _ => AgiworkforceErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            agiworkforce_error_info: Some(self.to_agiworkforce_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            AgiworkforceErr::RetryLimit(err) => Some(err.status),
            AgiworkforceErr::UnexpectedStatus(err) => Some(err.status),
            AgiworkforceErr::ConnectionFailed(err) => err.source.status(),
            AgiworkforceErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("agiworkforce")
        {
            return write!(
                f,
...
pub fn get_error_message_ui(e: &AgiworkforceErr) -> String {
    let message = match e {
        AgiworkforceErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        AgiworkforceErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
