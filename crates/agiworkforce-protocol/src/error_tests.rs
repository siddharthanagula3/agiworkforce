...
fn server_overloaded_maps_to_protocol() {
    let err = AgiworkforceErr::ServerOverloaded;
    assert_eq!(
        err.to_agiworkforce_protocol_error(),
        AgiworkforceErrorInfo::ServerOverloaded
    );
}
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = AgiworkforceErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.agiworkforce_error_info,
        Some(AgiworkforceErrorInfo::ResponseStreamConnectionFailed {
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_agiworkforce_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        ...
        let expected = format!(
            "You've hit your usage limit for agiworkforce_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Agiworkforce, start a free trial of <PLAN> today".to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Agiworkforce, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
