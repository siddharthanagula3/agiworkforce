...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("agiworkforce_otel.log_only"));
        assert!(is_log_export_target("agiworkforce_otel.network_proxy"));
        assert!(!is_log_export_target("agiworkforce_otel.trace_safe"));
        assert!(!is_log_export_target("agiworkforce_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("agiworkforce_otel.trace_safe"));
        assert!(is_trace_safe_target("agiworkforce_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("agiworkforce_otel.log_only"));
        assert!(!is_trace_safe_target("agiworkforce_otel.network_proxy"));
    }
