...
fn auth_env_metadata() -> AuthEnvTelemetryMetadata {
    AuthEnvTelemetryMetadata {
        ...
        agiworkforce_api_key_env_present: false,
        agiworkforce_api_key_env_enabled: true,
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    assert!(
        logs.iter()
            .all(|log| { log.record.target().map(Cow::as_ref) == Some("agiworkforce_otel.log_only") })
    );
...
fn otel_export_routing_policy_routes_api_request_auth_observability() {
    ...
    assert_eq!(
        request_log_attrs
            .get("auth.env_agiworkforce_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
