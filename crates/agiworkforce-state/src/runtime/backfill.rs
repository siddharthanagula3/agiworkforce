...
mod tests {
    ...
    async fn init_removes_legacy_state_db_files() {
        ...
        tokio::fs::create_dir_all(&agiworkforce_home)
            .await
            .expect("create agiworkforce_home");
        ...
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = agiworkforce_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = agiworkforce_home.join(format!(
        ...
        let unrelated_path = agiworkforce_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = agiworkforce_home.join("123");
        tokio::fs::write(&numeric_path, b"keep")
            .await
            .expect("write numeric");

        let _runtime = StateRuntime::init(agiworkforce_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = agiworkforce_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(agiworkforce_home.join(current_name))
                .await
