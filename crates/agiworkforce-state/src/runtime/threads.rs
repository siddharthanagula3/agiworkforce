...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let agiworkforce_home = unique_temp_dir();
        let runtime = StateRuntime::init(agiworkforce_home.clone(), "test-provider".to_string())
            .await
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = agiworkforce_home.join("first");
        let second_cwd = agiworkforce_home.join("second");
        let other_cwd = agiworkforce_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&agiworkforce_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(agiworkforce_git_utils::GitSha::new("rollout-sha")),
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&agiworkforce_home, first_id, agiworkforce_home.clone());
        first.updated_at = updated_at;
        let mut second = test_thread_metadata(&agiworkforce_home, second_id, agiworkforce_home.clone());
        ...
        let mut older = test_thread_metadata(&agiworkforce_home, older_id, agiworkforce_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            agiworkforce_protocol::protocol::TokenCountEvent {
                info: Some(agiworkforce_protocol::protocol::TokenUsageInfo {
                    total_token_usage: agiworkforce_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: agiworkforce_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let agiworkforce_home = unique_temp_dir();
        let runtime = StateRuntime::init(agiworkforce_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
