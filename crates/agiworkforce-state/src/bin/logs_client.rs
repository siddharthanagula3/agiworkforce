...
#[derive(Debug, Parser)]
#[command(name = "agiworkforce-state-logs")]
#[command(about = "Tail Agiworkforce logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to AGIWORKFORCE_HOME. Defaults to $AGIWORKFORCE_HOME or ~/.codex.
    #[arg(long, env = "AGIWORKFORCE_HOME")]
    agiworkforce_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --agiworkforce-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let agiworkforce_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(agiworkforce_home, "logs-client".to_string()).await?;
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["agiworkforce-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["agiworkforce-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["agiworkforce-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["agiworkforce-state-logs", "--level", "WARN"])
