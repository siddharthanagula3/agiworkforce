...
//!
//! # async fn example(state_db: std::sync::Arc<agiworkforce_state::StateRuntime>) {
...
mod tests {
    ...
    fn temp_agiworkforce_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("agiworkforce-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let agiworkforce_home = temp_agiworkforce_home();
