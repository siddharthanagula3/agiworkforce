...
impl StateRuntime {
    /// Initialize the state runtime using the provided Agiworkforce home and default provider.
    ///
    ...
    pub async fn init(agiworkforce_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        ...
        .await;
        let state_path = state_db_path(agiworkforce_home.as_path());
        let logs_path = logs_db_path(agiworkforce_home.as_path());
    ...
    /// Return the configured Agiworkforce home directory for this runtime.
    pub fn agiworkforce_home(&self) -> &Path {
...
pub fn state_db_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(state_db_filename())
}
...
pub fn logs_db_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(agiworkforce_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read agiworkforce_home for {db_label} db cleanup {}: {err}",
...
mod tests {
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&agiworkforce_home)
            .await
