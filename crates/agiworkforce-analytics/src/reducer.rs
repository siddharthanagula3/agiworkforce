...
use crate::events::AgiworkforceTurnEventParams;
use crate::events::AgiworkforceTurnEventRequest;
use crate::events::AgiworkforceTurnSteerEventParams;
use crate::events::AgiworkforceTurnSteerEventRequest;
...
use crate::events::agiworkforce_compaction_event_params;
...
use agiworkforce_app_server_protocol::ClientResponse;
...
use agiworkforce_git_utils::collect_git_info;
...
struct CompletedTurnState {
    ...
    turn_error: Option<AgiworkforceErrorInfo>,
    completed_at: u64,
...
impl AnalyticsReducer {
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "agiworkforce_guardian_review",
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = agiworkforce_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(AgiworkforceAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = agiworkforce_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(AgiworkforceAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(AgiworkforceHookRunEventRequest {
            event_type: "agiworkforce_hook_run",
            event_params: agiworkforce_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(AgiworkforcePluginUsedEventRequest {
            event_type: "agiworkforce_plugin_used",
            event_params: agiworkforce_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = AgiworkforcePluginEventRequest {
            event_type: plugin_state_event_type(state),
            event_params: agiworkforce_plugin_metadata(plugin),
        };
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.agiworkforce_error_info),
    ...
    fn ingest_compaction(&mut self, input: AgiworkforceCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some(connection_id) = self.thread_connections.get(&input.thread_id) else {
            tracing::warn!(
                thread_id = %input.thread_id,
                turn_id = %input.turn_id,
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            AgiworkforceCompactionEventRequest {
                event_type: "agiworkforce_compaction_event",
                event_params: agiworkforce_compaction_event_params(
                    input,
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(AgiworkforceTurnSteerEventRequest {
            event_type: "agiworkforce_turn_steer_event",
            event_params: AgiworkforceTurnSteerEventParams {
                thread_id: pending_request.thread_id,
    ...
    fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::TurnEvent(Box::new(
            AgiworkforceTurnEventRequest {
                event_type: "agiworkforce_turn_event",
                event_params: agiworkforce_turn_event_params(
                    app_server_client,
                    runtime,
                    turn_id.to_string(),
                    turn_state,
...
fn agiworkforce_turn_event_params(
    app_server_client: AgiworkforceAppServerClientMetadata,
    runtime: AgiworkforceRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> AgiworkforceTurnEventParams {
...
fn analytics_turn_status(status: agiworkforce_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        agiworkforce_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        agiworkforce_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        agiworkforce_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        agiworkforce_app_server_protocol::TurnStatus::InProgress => None,
    }
}
