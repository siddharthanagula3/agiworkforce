...
use crate::events::AgiworkforceAppMentionedEventRequest;
use crate::events::AgiworkforceAppServerClientMetadata;
use crate::events::AgiworkforceAppUsedEventRequest;
use crate::events::AgiworkforceCompactionEventRequest;
use crate::events::AgiworkforceHookRunEventRequest;
use crate::events::AgiworkforcePluginEventRequest;
use crate::events::AgiworkforcePluginUsedEventRequest;
use crate::events::AgiworkforceRuntimeMetadata;
use crate::events::AgiworkforceTurnEventRequest;
...
use crate::events::agiworkforce_app_metadata;
use crate::events::agiworkforce_hook_run_metadata;
use crate::events::agiworkforce_plugin_metadata;
use crate::events::agiworkforce_plugin_used_metadata;
...
use crate::facts::AgiworkforceCompactionEvent;
...
use agiworkforce_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use agiworkforce_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use agiworkforce_app_server_protocol::InitializeCapabilities;
use agiworkforce_app_server_protocol::InitializeParams;
use agiworkforce_app_server_protocol::JSONRPCErrorError;
use agiworkforce_app_server_protocol::NonSteerableTurnKind;
use agiworkforce_app_server_protocol::PermissionProfile as AppServerPermissionProfile;
use agiworkforce_app_server_protocol::RequestId;
use agiworkforce_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use agiworkforce_app_server_protocol::ServerNotification;
use agiworkforce_app_server_protocol::SessionSource as AppServerSessionSource;
...
use agiworkforce_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
use agiworkforce_login::default_client::DEFAULT_ORIGINATOR;
use agiworkforce_login::default_client::originator;
use agiworkforce_plugin::AppConnectorId;
use agiworkforce_plugin::PluginCapabilitySummary;
use agiworkforce_plugin::PluginId;
use agiworkforce_plugin::PluginTelemetryMetadata;
...
fn sample_app_server_client_metadata() -> AgiworkforceAppServerClientMetadata {
    AgiworkforceAppServerClientMetadata {
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        client_name: Some("agiworkforce-tui".to_string()),
...
fn sample_runtime_metadata() -> AgiworkforceRuntimeMetadata {
    AgiworkforceRuntimeMetadata {
        agiworkforce_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(agiworkforce_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    agiworkforce_error_info: Option<agiworkforce_app_server_protocol::AgiworkforceErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: agiworkforce_error_info.map(|agiworkforce_error_info| AppServerTurnError {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "agiworkforce-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "agiworkforce-tui".to_string(),
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(AgiworkforceAppMentionedEventRequest {
        event_type: "agiworkforce_app_mentioned",
        event_params: agiworkforce_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(AgiworkforceAppUsedEventRequest {
        event_type: "agiworkforce_app_used",
        event_params: agiworkforce_app_metadata(
            &tracking,
            AppInvocation {
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(AgiworkforceCompactionEventRequest {
        event_type: "agiworkforce_compaction_event",
        event_params: crate::events::agiworkforce_compaction_event_params(
            AgiworkforceCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "agiworkforce-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "agiworkforce_rs_version": "0.1.0",
                    "runtime_os": "macos",
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "agiworkforce_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            app_server_client: AgiworkforceAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("agiworkforce-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
            ...
            runtime: AgiworkforceRuntimeMetadata {
                agiworkforce_rs_version: "0.1.0".to_string(),
                runtime_os: "macos".to_string(),
                runtime_os_version: "15.3.1".to_string(),
                runtime_arch: "aarch64".to_string(),
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                runtime: AgiworkforceRuntimeMetadata {
                    agiworkforce_rs_version: "0.99.0".to_string(),
    ...
    assert_eq!(payload[0]["event_type"], "agiworkforce_thread_initialized");
    assert_eq!(
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["agiworkforce_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        agiworkforce_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agiworkforce_compaction_event");
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agiworkforce_guardian_review");
...
fn subagent_thread_started_thread_spawn_serializes_parent_thread_id() {
    let parent_thread_id =
        agiworkforce_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(AgiworkforcePluginUsedEventRequest {
        event_type: "agiworkforce_plugin_used",
        event_params: agiworkforce_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(AgiworkforcePluginEventRequest {
        event_type: "agiworkforce_plugin_installed",
        event_params: agiworkforce_plugin_metadata(sample_plugin_metadata()),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(AgiworkforceHookRunEventRequest {
        event_type: "agiworkforce_hook_run",
        event_params: agiworkforce_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(agiworkforce_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(agiworkforce_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let unknown = serde_json::to_value(agiworkforce_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(agiworkforce_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: agiworkforce_protocol::protocol::SkillScope::User,
                    skill_path,
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agiworkforce_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "agiworkforce_app_mentioned");
    assert_eq!(payload[1]["event_type"], "agiworkforce_app_used");
    assert_eq!(payload[2]["event_type"], "agiworkforce_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "agiworkforce_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(AgiworkforceTurnEventRequest {
        event_type: "agiworkforce_turn_event",
        event_params: crate::events::AgiworkforceTurnEventParams {
            thread_id: "thread-2".to_string(),
            turn_id: "turn-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "agiworkforce_cli_rs",
                    "client_name": "agiworkforce-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("agiworkforce_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["app_server_client"]["product_client_id"],
        json!("agiworkforce-tui")
    );
    assert_eq!(
        payload["event_params"]["runtime"]["agiworkforce_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("agiworkforce_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(payload["event_params"]["turn_id"], json!("turn-2"));
    assert_eq!(
        payload["event_params"]["app_server_client"],
        json!({
            "product_client_id": "agiworkforce-tui",
            "client_name": "agiworkforce-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(agiworkforce_app_server_protocol::AgiworkforceErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
