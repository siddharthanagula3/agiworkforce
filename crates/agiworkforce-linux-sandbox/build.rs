...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=AGIWORKFORCE_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=AGIWORKFORCE_SKIP_VENDORED_BWRAP");
    ...
    if target_os != "linux" || env::var_os("AGIWORKFORCE_SKIP_VENDORED_BWRAP").is_some() {
        return;
    }
...
///
/// Priority:
/// 1. `AGIWORKFORCE_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `agiworkforce-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("AGIWORKFORCE_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "AGIWORKFORCE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set AGIWORKFORCE_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under agiworkforce-rs/vendor.",
        vendor_dir.display()
    ))
}
