...
use agiworkforce_core::error::AgiWorkforceErr;
use agiworkforce_core::error::Result;
use agiworkforce_core::error::SandboxErr;
...
pub(crate) fn apply_sandbox_policy_to_current_thread(
    ...
    if apply_landlock_fs && !sandbox_policy.has_full_disk_write_access() {
        if !sandbox_policy.has_full_disk_read_access() {
            return Err(AgiWorkforceErr::UnsupportedOperation(
...
///
/// # Errors
/// Returns [`AgiWorkforceErr::Sandbox`] variants when the ruleset fails to apply.
///
...
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    if status.ruleset == landlock::RulesetStatus::NotEnforced {
        return Err(AgiWorkforceErr::Sandbox(SandboxErr::LandlockRestrict));
    }
