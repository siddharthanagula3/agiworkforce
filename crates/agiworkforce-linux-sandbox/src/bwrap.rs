...
//! - sensitive subpaths such as `.git` and `.agiworkforce` remain read-only even when
...
use agiworkforce_core::error::Result;
...
/// inside a writable root (e.g., `.agiworkforce -> ./decoy`). In that case we mount
