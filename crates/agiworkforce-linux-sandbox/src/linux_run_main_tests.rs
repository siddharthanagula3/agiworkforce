...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/agiworkforce-arg0-session/agiworkforce-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
            ...
            "/proc".to_string(),
            "--argv0".to_string(),
            "agiworkforce-linux-sandbox".to_string(),
            "--".to_string(),
            "/bin/true".to_string(),
        ]
    );
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/agiworkforce-arg0-session/agiworkforce-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--agiworkforce-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
