# agiworkforce-linux-sandbox

This crate is responsible for producing:

- a `agiworkforce-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Agiworkforce CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `agiworkforce-exec` CLI can check if its arg0 is `agiworkforce-linux-sandbox` and, if so, execute as if it were `agiworkforce-linux-sandbox`
  - this should also be true of the `codex` multitool CLI

On Linux, Agiworkforce prefers the first `bwrap` found on `PATH`
...
binary.
Agiworkforce also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the vendored helper. Agiworkforce surfaces the same startup warning
...
it cannot create the required user namespaces, so Agiworkforce rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the vendored bubblewrap
  path.
- If `bwrap` is missing, Agiworkforce also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Agiworkforce surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Agiworkforce rejects sandboxed
