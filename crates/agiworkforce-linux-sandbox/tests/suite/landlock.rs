...
use agiworkforce_core::exec::process_exec_tool_call;
...
fn agiworkforce_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_agiworkforce-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> agiworkforce_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<agiworkforce_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let agiworkforce_linux_sandbox_exe = Some(agiworkforce_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &agiworkforce_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(AgiworkforceErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(AgiworkforceErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<agiworkforce_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> agiworkforce_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(AgiworkforceErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let agiworkforce_linux_sandbox_exe: Option<PathBuf> = Some(agiworkforce_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_agiworkforce_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let agiworkforce_target = dot_codex.join("config.toml");
    ...
    let agiworkforce_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", agiworkforce_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
    ...
    assert_ne!(agiworkforce_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_agiworkforce_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
...
"#,
    ...
    let mkdir_agiworkforce_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".codex"],
            &subdir,
            std::slice::from_ref(&subdir),
    ...
    assert_ne!(mkdir_agiworkforce_output.exit_code, 0);
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = agiworkforce_linux_sandbox_exe()
        .parent()
