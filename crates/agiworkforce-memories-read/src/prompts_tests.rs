...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let agiworkforce_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = agiworkforce_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&agiworkforce_home)
        .await
        .unwrap();
