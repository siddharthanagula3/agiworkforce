...
const DEFAULT_COMMIT_MESSAGE: &str = "agiworkforce snapshot";
...
pub fn create_ghost_commit_with_report(
    ...
    let large_untracked_dirs = existing_untracked
        ...
        .collect::<Vec<_>>();
    let index_tempdir = Builder::new().prefix("agiworkforce-git-index-").tempdir()?;
    ...
    //   GIT_INDEX_FILE=/tmp/index git commit-tree <tree> -p <parent> -m "agiworkforce snapshot"
...
fn restore_to_commit_inner(
    ...
    // While this might leave some AgiWorkforce-staged changes in the index (if AgiWorkforce ran `git add`),
...
fn default_commit_identity() -> Vec<(OsString, OsString)> {
    vec![
        (
            ...
            OsString::from("AgiWorkforce Snapshot"),
        ),
        (
            OsString::from("GIT_AUTHOR_EMAIL"),
            OsString::from("snapshot@agiworkforce.local"),
        ),
        (
...
mod tests {
    ...
    fn restore_from_subdirectory_restores_files_relatively() -> Result<(), GitToolingError> {
        ...
        assert!(!workspace.join("agiworkforce-rs").exists());
    ...
    fn restore_from_subdirectory_preserves_parent_vscode() -> Result<(), GitToolingError> {
        ...
        let workspace = repo.join("agiworkforce-rs");
