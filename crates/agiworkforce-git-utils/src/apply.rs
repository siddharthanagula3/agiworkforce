...
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    ...
    if let Ok(cfg) = std::env::var("AGIWORKFORCE_APPLY_GIT_CFG") {
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        let _ = run(root, &["git", "config", "user.email", "agiworkforce@example.com"]);
        let _ = run(root, &["git", "config", "user.name", "AGI Workforce"]);
        dir
    }
