...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Agiworkforce git baseline\n\nCo-authored-by: Agiworkforce <noreply@openai.com>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = agiworkforce_signature();
...
fn agiworkforce_signature() -> gix::actor::Signature {
    gix::actor::Signature {
