...
//!
//! If the keyring is not available or fails, we fall back to AGIWORKFORCE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempAgiworkforceHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create AGIWORKFORCE_HOME temp dir");
            unsafe {
                std::env::set_var("AGIWORKFORCE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
