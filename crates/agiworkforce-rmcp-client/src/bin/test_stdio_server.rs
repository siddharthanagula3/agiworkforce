...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Agiworkforce TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Agiworkforce TUI):
    /// - Build this binary: `cargo build -p agiworkforce-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `codex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Agiworkforce TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            ...
            name: "agiworkforce-memo".to_string(),
            title: Some("Agiworkforce Memo".to_string()),
            description: Some(
...
/// Scenarios for `image_scenario`, intended to exercise Agiworkforce TUI handling of MCP image outputs.
///
