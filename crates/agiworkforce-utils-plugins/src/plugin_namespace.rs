...
const DISCOVERABLE_PLUGIN_MANIFEST_PATHS: &[&str] =
    &[".agiworkforce-plugin/plugin.json", ".claude-plugin/plugin.json"];
...
/// plugin manifest (same `name` rules as full manifest loading in agiworkforce-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".agiworkforce-plugin")).expect("mkdir manifest");
        fs::write(
