...
pub(crate) struct ConfiguredHandler {
    pub event_name: agiworkforce_protocol::protocol::HookEventName,
    ...
    pub(crate) fn new(
        ...
        if cfg!(windows) {
            return Self {
                handlers: Vec::new(),
                warnings: vec![
                    "Disabled `agiworkforce_hooks` for this session because `hooks.json` lifecycle hooks are not supported on Windows yet."
                        .to_string(),
                ],
                shell,
            };
