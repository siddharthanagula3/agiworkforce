//! Registry of model providers supported by Agiworkforce.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Agiworkforce works out-of-the-box.
//!   2. User-defined entries inside `~/.codex/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use agiworkforce_api::Provider as ApiProvider;
use agiworkforce_api::RetryConfig as ApiRetryConfig;
use agiworkforce_api::is_azure_responses_provider;
...
use agiworkforce_protocol::error::EnvVarError;
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> AgiworkforceResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> AgiworkforceResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> AgiworkforceResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        AgiworkforceErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Agiworkforce CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These AGIWORKFORCE_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_agiworkforce_oss_base_url = format!(
        "http://localhost:{agiworkforce_oss_port}/v1",
        agiworkforce_oss_port = std::env::var("AGIWORKFORCE_OSS_PORT")
            .ok()
    ...
    let agiworkforce_oss_base_url = std::env::var("AGIWORKFORCE_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_agiworkforce_oss_base_url);
    create_oss_provider_with_base_url(&agiworkforce_oss_base_url, wire_api)
}
