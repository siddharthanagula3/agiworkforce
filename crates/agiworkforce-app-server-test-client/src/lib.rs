...
use agiworkforce_app_server_protocol::DynamicToolSpec;
...
use agiworkforce_app_server_protocol::InitializeResponse;
use agiworkforce_app_server_protocol::JSONRPCMessage;
use agiworkforce_app_server_protocol::JSONRPCNotification;
use agiworkforce_app_server_protocol::JSONRPCRequest;
...
use agiworkforce_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "agiworkforce-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $AGIWORKFORCE_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Agiworkforce app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Agiworkforce", version, about = "Bootstrap Agiworkforce app-server", long_about = None)]
struct Cli {
    /// Path to the `codex` CLI binary. When set, requests use stdio by
    /// spawning `codex app-server` as a child process.
    #[arg(long, env = "AGIWORKFORCE_BIN", global = true)]
    agiworkforce_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--agiworkforce-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "AGIWORKFORCE_APP_SERVER_URL", global = true)]
    url: Option<String>,
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/agiworkforce-app-server-test-client/`
    Serve {
    ...
    /// Send a user message through the Agiworkforce app-server.
    SendMessage {
        /// User message to send to Agiworkforce.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Agiworkforce app-server.
    GetAccountRateLimits,
    /// List the available models from the Agiworkforce app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Agiworkforce app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "AGIWORKFORCE_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let agiworkforce_bin = agiworkforce_bin.unwrap_or_else(|| PathBuf::from("agiworkforce"));
            serve(&agiworkforce_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(agiworkforce_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(agiworkforce_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(agiworkforce_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(agiworkforce_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if agiworkforce_bin.is_some() && url.is_some() {
        bail!("--agiworkforce-bin and --url are mutually exclusive");
    }
    if let Some(agiworkforce_bin) = agiworkforce_bin {
        return Ok(Endpoint::SpawnAgiworkforce(agiworkforce_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if agiworkforce_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --agiworkforce-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(agiworkforce_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(agiworkforce_bin);
        if let Some(agiworkforce_bin_parent) = agiworkforce_bin.parent() {
            let mut path = OsString::from(agiworkforce_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", agiworkforce_bin.display()))?;
...
fn serve(agiworkforce_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/agiworkforce-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,agiworkforce_=trace {}",
        shell_quote(&agiworkforce_bin.display().to_string())
    );
    for override_kv in config_overrides {
...
pub async fn send_message_v2(
    agiworkforce_bin: &Path,
    config_overrides: &[String],
    user_message: String,
    dynamic_tools: &Option<Vec<DynamicToolSpec>>,
) -> Result<()> {
    let endpoint = Endpoint::SpawnAgiworkforce(agiworkforce_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut AgiworkforceClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = AgiworkforceClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = AgiworkforceClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (agiworkforce_bin, url) {
        (Some(_), Some(_)) => bail!("--agiworkforce-bin and --url are mutually exclusive"),
        (Some(agiworkforce_bin), None) => {
            let server = BackgroundAppServer::spawn(&agiworkforce_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve agiworkforce-app-server-test-client binary path")?;
...
impl AgiworkforceClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnAgiworkforce(agiworkforce_bin) => Self::spawn_stdio(agiworkforce_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(agiworkforce_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let agiworkforce_bin_display = agiworkforce_bin.display();
        let mut cmd = Command::new(agiworkforce_bin);
        if let Some(agiworkforce_bin_parent) = agiworkforce_bin.parent() {
            let mut path = OsString::from(agiworkforce_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut agiworkforce_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{agiworkforce_bin_display}` app-server"))?;

        let stdin = agiworkforce_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = agiworkforce_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `agiworkforce-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "agiworkforce-toy-app-server".to_string(),
                    title: Some("Agiworkforce Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agiworkforce_app_server_protocol::LoginAccountParams::Chatgpt {
                agiworkforce_streamlined_login: false,
            },
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agiworkforce_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = agiworkforce_core::otel_init::build_provider(
            &config,
...
impl Drop for AgiworkforceClient {
    fn drop(&mut self) {
