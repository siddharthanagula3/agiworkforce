...
const RESOURCES_DIRNAME: &str = "agiworkforce-resources";
...
pub enum InstallContext {
    ...
    /// A Agiworkforce binary launched through the npm-managed `codex.js` shim.
    Npm,
    /// A Agiworkforce binary launched through the bun-managed `codex.js` shim.
    Bun,
    /// A Agiworkforce binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Agiworkforce binaries, custom
    /// internal launchers, and tests that execute Agiworkforce from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let agiworkforce_home = agiworkforce_utils_home_dir::find_agiworkforce_home().ok();
        Self::from_exe_with_agiworkforce_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_agiworkforce_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        ...
        if let Some(exe_path) = current_exe
            && let Some(standalone_context) = standalone_install_context(exe_path, agiworkforce_home)
        {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("AGIWORKFORCE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("AGIWORKFORCE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_context(
    ...
    let canonical_agiworkforce_home = std::fs::canonicalize(agiworkforce_home?).ok()?;
    let release_dir = canonical_exe.parent()?.to_path_buf();
    let releases_root = canonical_agiworkforce_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let agiworkforce_home = tempfile::tempdir()?;
        let release_dir = agiworkforce_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "codex.exe" } else { "agiworkforce" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_agiworkforce_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*agiworkforce_home*/ Some(agiworkforce_home.path()),
        );
        assert_eq!(
            context,
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_agiworkforce_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/codex")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*agiworkforce_home*/ None,
        );
        assert_eq!(npm_context, InstallContext::Npm);

        let bun_context = InstallContext::from_exe_with_agiworkforce_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/codex")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
