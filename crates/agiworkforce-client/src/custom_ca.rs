...
//!
//! - read CA material from `AGIWORKFORCE_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
pub const AGIWORKFORCE_CA_CERT_ENV: &str = "AGIWORKFORCE_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set AGIWORKFORCE_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
fn build_reqwest_client_with_env(
    ...
    info!(
        agiworkforce_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(AGIWORKFORCE_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: AGIWORKFORCE_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_agiworkforce_env() {
        let env = map_env(&[
            (AGIWORKFORCE_CA_CERT_ENV, "/tmp/codex.pem"),
