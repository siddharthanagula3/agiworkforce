...
//! detailed explanation of what "hermetic" means here lives in `agiworkforce_client::custom_ca`; these
...
const AGIWORKFORCE_CA_CERT_ENV: &str = "AGIWORKFORCE_CA_CERTIFICATE";
...
fn run_probe(envs: &[(&str, &Path)]) -> std::process::Output {
    ...
    cmd.env_remove(AGIWORKFORCE_CA_CERT_ENV);
...
#[test]
fn uses_agiworkforce_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(AGIWORKFORCE_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_agiworkforce_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (AGIWORKFORCE_CA_CERT_ENV, cert_path.as_path()),
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("AGIWORKFORCE_CA_CERTIFICATE"));
