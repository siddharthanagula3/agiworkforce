...
use agiworkforce_rollout::RolloutRecorder;
use agiworkforce_rollout::find_archived_thread_path_by_id_str;
use agiworkforce_rollout::find_thread_name_by_id;
use agiworkforce_rollout::find_thread_path_by_id_str;
use agiworkforce_rollout::read_session_meta_line;
use agiworkforce_rollout::read_thread_item_from_rollout;
...
fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.agiworkforce_home.join(rollout_path)
    } else {
        rollout_path
    };
...
async fn resolve_rollout_path(
    ...
    if include_archived {
        match find_thread_path_by_id_str(store.config.agiworkforce_home.as_path(), &thread_id.to_string())
            .await
            ...
            None => find_archived_thread_path_by_id_str(
                store.config.agiworkforce_home.as_path(),
                &thread_id.to_string(),
            )
            .await
            .map_err(|err| ThreadStoreError::InvalidRequest {
        ...
        find_thread_path_by_id_str(store.config.agiworkforce_home.as_path(), &thread_id.to_string())
            .await
            .map_err(|err| ThreadStoreError::InvalidRequest {
                message: format!("failed to locate thread id {thread_id}: {err}"),
            })
...
async fn read_thread_from_rollout_path(
    ...
    let archived = path.starts_with(
        store
            .config
            .agiworkforce_home
            .join(agiworkforce_rollout::ARCHIVED_SESSIONS_SUBDIR),
    );
    let mut thread =
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.agiworkforce_home.as_path(), &thread.thread_id).await
    {
...
async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<agiworkforce_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.agiworkforce_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten(),
    };
...
mod tests {
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        agiworkforce_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
