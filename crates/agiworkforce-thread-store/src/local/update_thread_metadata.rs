...
use agiworkforce_rollout::ARCHIVED_SESSIONS_SUBDIR;
use agiworkforce_rollout::append_rollout_item_to_path;
use agiworkforce_rollout::append_thread_name;
...
pub(super) async fn update_thread_metadata(
    ...
    agiworkforce_rollout::state_db::reconcile_rollout(
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.agiworkforce_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
async fn resolve_rollout_path(
    ...
    find_archived_thread_path_by_id_str(store.config.agiworkforce_home.as_path(), &thread_id.to_string())
        .await
        .map_err(|err| ThreadStoreError::InvalidRequest {
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &std::path::Path) -> bool {
    path.starts_with(store.config.agiworkforce_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = agiworkforce_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
