...
- Do not add build-time protobuf generation to `agiworkforce-thread-store` unless the Bazel/Cargo story is intentionally changed.
...
```sh
./agiworkforce-rs/thread-store/scripts/generate-proto.sh
```
