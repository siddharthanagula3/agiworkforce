...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(agiworkforce_home)?.contains_current_source(source_path)
}
...
pub fn record_imported_session(
    ...
    let mut ledger = load_import_ledger(agiworkforce_home)?;
    ...
    save_import_ledger(agiworkforce_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(agiworkforce_home);
...
fn import_ledger_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(SESSION_IMPORT_LEDGER_FILE)
}
