...
pub fn prepare_pending_session_imports(
    ...
    for session in requested_sessions {
        let has_been_imported = match has_current_session_been_imported(agiworkforce_home, &session.path) {
...
pub fn prepare_validated_session_imports(
    ...
    requested_sessions
        .into_iter()
        .filter_map(|session| pending_session_import(agiworkforce_home, session))
        .collect()
}
...
mod tests {
    ...
    fn rejects_session_that_was_not_detected() {
        ...
        let agiworkforce_home = root.path().join("agiworkforce-home");
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        record_imported_session(&agiworkforce_home, &source_path, ThreadId::new()).expect("record import");
