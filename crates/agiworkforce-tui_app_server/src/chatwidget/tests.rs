...
use agiworkforce_app_server_protocol::CollabAgentTool as AppServerCollabAgentTool;
use agiworkforce_app_server_protocol::CollabAgentToolCallStatus as AppServerCollabAgentToolCallStatus;
use agiworkforce_app_server_protocol::ErrorNotification;
use agiworkforce_app_server_protocol::FileUpdateChange;
use agiworkforce_app_server_protocol::GuardianApprovalReview;
use agiworkforce_app_server_protocol::GuardianApprovalReviewStatus;
use agiworkforce_app_server_protocol::GuardianRiskLevel as AppServerGuardianRiskLevel;
use agiworkforce_app_server_protocol::ItemCompletedNotification;
use agiworkforce_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use agiworkforce_app_server_protocol::ItemGuardianApprovalReviewStartedNotification;
use agiworkforce_app_server_protocol::ItemStartedNotification;
use agiworkforce_app_server_protocol::MarketplaceInterface;
use agiworkforce_app_server_protocol::PatchApplyStatus as AppServerPatchApplyStatus;
...
use agiworkforce_app_server_protocol::ThreadItem as AppServerThreadItem;
use agiworkforce_app_server_protocol::Turn as AppServerTurn;
use agiworkforce_app_server_protocol::TurnCompletedNotification;
use agiworkforce_app_server_protocol::TurnError as AppServerTurnError;
use agiworkforce_app_server_protocol::TurnStartedNotification;
use agiworkforce_app_server_protocol::TurnStatus as AppServerTurnStatus;
use agiworkforce_app_server_protocol::UserInput as AppServerUserInput;
...
async fn resumed_initial_messages_render_history() {
    ...
    chat.handle_agiworkforce_event(Event {
        id: "initial".into(),
...
async fn thread_snapshot_replay_does_not_duplicate_agent_message_history() {
    ...
    chat.handle_agiworkforce_event_replay(Event {
        id: "turn-1".into(),
...
fn test_model_catalog(config: &Config) -> Arc<ModelCatalog> {
    ...
    Arc::new(ModelCatalog::new(
        agiworkforce_core::test_support::all_model_presets().clone(),
...
async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget {
        app_event_tx,
        agiworkforce_op_target: super::AgiWorkforceOpTarget::Direct(op_tx),
        bottom_pane: bottom,
        active_cell: None,
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        limit_id: Some("agiworkforce_other".to_string()),
        limit_name: Some("agiworkforce_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 90.0,
            window_minutes: Some(60),
            resets_at: Some(200),
    ...
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("agiworkforce_other")
        .expect("agiworkforce_other snapshot should exist");
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_agiworkforce_limit() {
...
async fn live_app_server_stream_recovery_restores_previous_status_header() {
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                message: "Reconnecting... 1/5".to_string(),
                agiworkforce_error_info: Some(AgiWorkforceErrorInfo::Other.into()),
                additional_details: None,
            },
            will_retry: true,
            thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                agiworkforce_error_info: Some(AgiWorkforceErrorInfo::ServerOverloaded.into()),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
async fn live_app_server_invalid_thread_name_update_is_ignored() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            agiworkforce_app_server_protocol::ThreadNameUpdatedNotification {
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    chat.handle_agiworkforce_event(Event {
        ...
        msg: EventMsg::Error(ErrorEvent {
            message: "server overloaded".to_string(),
            agiworkforce_error_info: Some(AgiWorkforceErrorInfo::ServerOverloaded),
        }),
    });
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto agiworkforce_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
...
async fn stream_error_updates_status_indicator() {
    ...
    chat.handle_agiworkforce_event(Event {
        ...
        msg: EventMsg::StreamError(StreamErrorEvent {
            ...
            agiworkforce_error_info: Some(AgiWorkforceErrorInfo::Other),
