...
use agiworkforce_protocol::approvals::ExecApprovalRequestEvent;
...
use agiworkforce_protocol::protocol::FinalOutput;
use agiworkforce_protocol::protocol::GetHistoryEntryResponseEvent;
...
fn command_execution_decision_to_review_decision(
    decision: agiworkforce_app_server_protocol::CommandExecutionApprovalDecision,
) -> agiworkforce_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&agiworkforce_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&agiworkforce_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[agiworkforce_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&agiworkforce_protocol::models::PermissionProfile>,
) -> Vec<agiworkforce_protocol::protocol::ReviewDecision> {
...
fn session_summary(
    ...
    let resume_command = agiworkforce_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| agiworkforce_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| agiworkforce_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            agiworkforce_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            agiworkforce_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| {
                        ...
                        scope: match skill.scope {
                            agiworkforce_app_server_protocol::SkillScope::User => {
                                agiworkforce_protocol::protocol::SkillScope::User
                            }
                            agiworkforce_app_server_protocol::SkillScope::Repo => {
                                agiworkforce_protocol::protocol::SkillScope::Repo
                            }
                            agiworkforce_app_server_protocol::SkillScope::System => {
                                agiworkforce_protocol::protocol::SkillScope::System
                            }
                            agiworkforce_app_server_protocol::SkillScope::Admin => {
                                agiworkforce_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| agiworkforce_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        disabled_folders.push((
            dot_agiworkforce_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
fn emit_system_bwrap_warning(app_event_tx: &AppEventSender) {
    let Some(message) = agiworkforce_core::config::system_bwrap_warning() else {
        return;
    };
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.agiworkforce_error_info,
        Some(AppServerAgiWorkforceErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        &self,
        tui: &mut tui::Tui,
        cfg: agiworkforce_core::config::Config,
    ...
    fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, AGIWORKFORCE_CLI_VERSION)
    }
    ...
    /// `$AGIWORKFORCE_HOME/history.jsonl`, bypassing the app-server RPC layer.
        ...
        match op.view() {
            ...
            AppCommandView::ListSkills { cwds, force_reload } => {
                let response = app_server
                    .skills_list(agiworkforce_app_server_protocol::SkillsListParams {
                        cwds: cwds.to_vec(),
                        force_reload,
    ...
    pub async fn run(
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            agiworkforce_core::default_client::originator().value,
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.agiworkforce_home.clone();
        ...
        match event {
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = agiworkforce_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    agiworkforce_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                session_telemetry.counter(
                                    agiworkforce_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    /*inc*/ 1,
                                    &tags,
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match agiworkforce_core::windows_sandbox_read_grants::grant_read_root_non_elevated(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder::new(&self.config.agiworkforce_home)
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder::new(&self.config.agiworkforce_home)
                    .with_profile(profile)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.agiworkforce_home.clone();
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                if let Err(err) = ConfigEditsBuilder::new(&self.config.agiworkforce_home)
                    .with_profile(profile)
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let edit = agiworkforce_core::config::edit::status_line_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.agiworkforce_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                let edit = agiworkforce_core::config::edit::syntax_theme_edit(&name);
                let apply_result = ConfigEditsBuilder::new(&self.config.agiworkforce_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn spawn_world_writable_scan(
        ...
        tokio::task::spawn_blocking(move || {
            let result = agiworkforce_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
type McpInventoryMaps = (
    HashMap<String, agiworkforce_protocol::mcp::Tool>,
    HashMap<String, Vec<agiworkforce_protocol::mcp::Resource>>,
    HashMap<String, Vec<agiworkforce_protocol::mcp::ResourceTemplate>>,
...
mod tests {
    ...
    use agiworkforce_app_server_protocol::AdditionalPermissionProfile;
    ...
    use agiworkforce_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
    use agiworkforce_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
    use agiworkforce_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
    use agiworkforce_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
    use agiworkforce_app_server_protocol::RequestId as AppServerRequestId;
    use agiworkforce_app_server_protocol::ServerNotification;
    use agiworkforce_app_server_protocol::ServerRequest;
    ...
    use agiworkforce_app_server_protocol::TokenUsageBreakdown;
    use agiworkforce_app_server_protocol::Turn;
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                auth_status: agiworkforce_app_server_protocol::McpAuthStatus::Unsupported,
            },
            McpServerStatus {
    ...
    -> Result<()> {
        ...
        let model = agiworkforce_core::test_support::get_model_offline(config.model.as_deref());
        ...
        while let Ok(event) = app_event_rx.try_recv() {
            match event {
                ...
                AppEvent::AgiWorkforceOp(Op::UserTurn { items, .. }) => {
                    submitted_items = Some(items);
                }
                _ => {}
            }
    ...
    async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
        ...
        let config = std::fs::read_to_string(agiworkforce_home.path().join("config.toml"))?;
...
"#;
        ...
        app.enqueue_thread_notification(
            ...
            ServerNotification::ThreadStarted(ThreadStartedNotification {
                thread: Thread {
                    ...
                    source: agiworkforce_app_server_protocol::SessionSource::Unknown,
                    agent_nickname: Some("Robie".to_string()),
                    agent_role: Some("explorer".to_string()),
                    git_info: None,
    ...
    async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()>
    {
        ...
        assert_eq!(
            app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
                agiworkforce_app_server_protocol::SkillsChangedNotification {},
            )),
            None
        );
        Ok(())
    ...
    async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
        ...
        app.chat_widget
            .set_service_tier(Some(agiworkforce_protocol::config_types::ServiceTier::Fast));
    ...
    fn thread_event_store_rebase_preserves_resolved_request_state() {
        ...
        store.push_notification(ServerNotification::ServerRequestResolved(
            agiworkforce_app_server_protocol::ServerRequestResolvedNotification {
                request_id: AppServerRequestId::Integer(1),
                thread_id: thread_id.to_string(),
            },
        ));
    ...
    fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
        let turn_error = AppServerTurnError {
            ...
            agiworkforce_error_info: Some(AppServerAgiWorkforceErrorInfo::ActiveTurnNotSteerable {
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder::new(&app.config.agiworkforce_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    -> Result<()> {
        ...
        let agiworkforce_home = tempdir()?;
        app.config.agiworkforce_home = agiworkforce_home.path().to_path_buf();
        std::fs::write(agiworkforce_home.path().join("config.toml"), "[broken")?;
    ...
    async fn refresh_in_memory_config_from_disk_uses_active_chat_widget_cwd() -> Result<()> {
        ...
        app.chat_widget.handle_agiworkforce_event(Event {
            id: String::new(),
    ...
    async fn fresh_session_config_uses_current_service_tier() {
        ...
        assert_eq!(
            config.service_tier,
            Some(agiworkforce_protocol::config_types::ServiceTier::Fast)
        );
    }
