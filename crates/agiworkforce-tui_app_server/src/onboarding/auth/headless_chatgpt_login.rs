...
use agiworkforce_core::auth::CLIENT_ID;
...
pub(super) fn start_headless_chatgpt_login(widget: &mut AuthModeWidget) {
    let mut opts = ServerOptions::new(
        widget.agiworkforce_home.clone(),
        CLIENT_ID.to_string(),
    ...
    let agiworkforce_home = widget.agiworkforce_home.clone();
...
async fn fallback_to_browser_login(
    request_handle: agiworkforce_app_server_client::AppServerRequestHandle,
    sign_in_state: Arc<RwLock<SignInState>>,
    request_frame: FrameRequester,
    error: Arc<RwLock<Option<String>>>,
    cancel: Arc<Notify>,
