...
use agiworkforce_core::state_db::get_state_db;
...
pub async fn run_main(
    ...
    let log_dir = agiworkforce_core::config::log_dir(&config)?;
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        agiworkforce_core::otel_init::build_provider(
            &config,
    ...
    let log_db_layer = agiworkforce_core::state_db::get_state_db(&config)
        .await
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !remote_mode {
            cloud_requirements = cloud_requirements_loader_for_storage(
                initial_config.agiworkforce_home.clone(),
                /*enable_agiworkforce_api_key_env*/ false,
...
mod tests {
    ...
    use agiworkforce_core::config::ConfigBuilder;
    use agiworkforce_core::config::ConfigOverrides;
    use agiworkforce_core::config::ProjectConfig;
