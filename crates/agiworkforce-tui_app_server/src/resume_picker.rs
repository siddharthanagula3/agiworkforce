...
use agiworkforce_core::ThreadItem;
use agiworkforce_core::ThreadSortKey;
use agiworkforce_core::ThreadsPage;
...
async fn run_session_picker_with_loader(
    ...
    let agiworkforce_home = config.agiworkforce_home.as_path();
...
impl PickerState {
    ...
    async fn update_thread_names(&mut self) {
        ...
        let names = find_thread_names_by_ids(&self.agiworkforce_home, &missing_ids)
            .await
            .unwrap_or_default();
