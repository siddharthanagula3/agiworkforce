...
use agiworkforce_core::config::Constrained;
use agiworkforce_core::config::ConstraintResult;
use agiworkforce_core::config::types::ApprovalsReviewer;
use agiworkforce_core::config::types::Notifications;
use agiworkforce_core::config::types::WindowsSandboxModeToml;
use agiworkforce_core::config_loader::ConfigLayerStackOrdering;
use agiworkforce_core::find_thread_name_by_id;
use agiworkforce_core::plugins::PluginsManager;
use agiworkforce_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
...
#[cfg(test)]
use agiworkforce_protocol::protocol::McpStartupCompleteEvent;
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        skill_metadata: params.skill_metadata.map(|metadata| {
            agiworkforce_protocol::approvals::ExecApprovalRequestSkillMetadata {
...
fn app_server_patch_changes_to_core(
    changes: Vec<agiworkforce_app_server_protocol::FileUpdateChange>,
) -> HashMap<PathBuf, agiworkforce_protocol::protocol::FileChange> {
    changes
        .into_iter()
        .map(|change| {
            let path = PathBuf::from(change.path);
            let file_change = match change.kind {
                agiworkforce_app_server_protocol::PatchChangeKind::Add => {
                    agiworkforce_protocol::protocol::FileChange::Add {
                        content: change.diff,
                    }
                }
                agiworkforce_app_server_protocol::PatchChangeKind::Delete => {
                    agiworkforce_protocol::protocol::FileChange::Delete {
                        content: change.diff,
                    }
                }
                agiworkforce_app_server_protocol::PatchChangeKind::Update { move_path } => {
                    agiworkforce_protocol::protocol::FileChange::Update {
...
impl ChatWidget {
    ...
    fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
        ...
        let agiworkforce_home = self.config.agiworkforce_home.clone();
        tokio::spawn(async move {
            ...
            match find_thread_name_by_id(&agiworkforce_home, &forked_from_id).await {
    ...
    fn show_feedback_note(
        ...
        category: crate::app_event::FeedbackCategory,
        include_logs: bool,
        snapshot: agiworkforce_feedback::FeedbackSnapshot,
    ) {
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_agiworkforce_limit = limit_id.eq_ignore_ascii_case("codex");
    ...
    fn on_hook_completed(&mut self, event: agiworkforce_protocol::protocol::HookCompletedEvent) {
        ...
        for entry in event.run.entries {
            let prefix = match entry.kind {
                agiworkforce_protocol::protocol::HookOutputEntryKind::Warning => "warning: ",
                agiworkforce_protocol::protocol::HookOutputEntryKind::Stop => "stop: ",
                agiworkforce_protocol::protocol::HookOutputEntryKind::Feedback => "feedback: ",
                agiworkforce_protocol::protocol::HookOutputEntryKind::Context => "hook context: ",
                agiworkforce_protocol::protocol::HookOutputEntryKind::Error => "error: ",
            };
    ...
    pub(crate) fn new_with_op_sender(
        ...
        agiworkforce_op_tx: UnboundedSender<Op>,
    ) -> Self {
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Logout => {
                if let Err(e) = agiworkforce_core::auth::logout(
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::AgiWorkforceOp(Op::Undo));
            // }
            SlashCommand::Diff => {
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = agiworkforce_core::connectors::connector_mention_slug(&app);
    ...
    fn handle_thread_item(
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = agiworkforce_protocol::items::UserMessageItem {
                    id,
                    content: content
                        .into_iter()
                ...
                let agiworkforce_protocol::protocol::EventMsg::UserMessage(event) =
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(agiworkforce_protocol::protocol::McpToolCallEndEvent {
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => Ok(agiworkforce_protocol::mcp::CallToolResult {
        ...
        match notification {
            ...
            ServerNotification::HookStarted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_started(agiworkforce_protocol::protocol::HookStartedEvent {
            ...
            ServerNotification::HookCompleted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_completed(agiworkforce_protocol::protocol::HookCompletedEvent {
                        turn_id: notification.turn_id,
                        run,
                    });
                }
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| agiworkforce_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn status_line_value_for_item(&self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::AgiWorkforceVersion => Some(AGIWORKFORCE_CLI_VERSION.to_string()),
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::AgiWorkforceOp(
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if agiworkforce_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && agiworkforce_core::windows_sandbox::sandbox_setup_is_complete(
    ...
    fn set_service_tier_selection(&mut self, service_tier: Option<ServiceTier>) {
        ...
        self.app_event_tx.send(AppEvent::AgiWorkforceOp(
            AppCommand::override_turn_context(
                /*cwd*/ None,
                /*approval_policy*/ None,
                /*approvals_reviewer*/ None,
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = agiworkforce_core::util::resume_command(Some(name), thread_id)
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.agiworkforce_op_target {
            ...
            AgiWorkforceOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::AgiWorkforceOp(op.into()));
            }
        }
        true
    }
...
fn hook_event_label(event_name: agiworkforce_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
