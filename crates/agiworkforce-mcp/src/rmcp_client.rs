...
use crate::agiworkforce_apps::CachedAgiworkforceAppsToolsLoad;
use crate::agiworkforce_apps::AgiworkforceAppsToolsCacheContext;
use crate::agiworkforce_apps::filter_disallowed_agiworkforce_apps_tools;
use crate::agiworkforce_apps::load_cached_agiworkforce_apps_tools;
use crate::agiworkforce_apps::load_startup_cached_agiworkforce_apps_tools_snapshot;
use crate::agiworkforce_apps::normalize_agiworkforce_apps_callable_name;
use crate::agiworkforce_apps::normalize_agiworkforce_apps_callable_namespace;
use crate::agiworkforce_apps::normalize_agiworkforce_apps_tool_title;
use crate::agiworkforce_apps::write_cached_agiworkforce_apps_tools_if_needed;
...
use agiworkforce_config::McpServerTransportConfig;
use agiworkforce_config::types::OAuthCredentialsStoreMode;
use agiworkforce_exec_server::HttpClient;
use agiworkforce_exec_server::ReqwestHttpClient;
use agiworkforce_protocol::protocol::Event;
use agiworkforce_rmcp_client::ExecutorStdioServerLauncher;
use agiworkforce_rmcp_client::LocalStdioServerLauncher;
use agiworkforce_rmcp_client::RmcpClient;
use agiworkforce_rmcp_client::StdioServerLauncher;
...
/// MCP server capability indicating that Agiworkforce should include [`SandboxState`]
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) agiworkforce_apps_tools_cache_context: Option<AgiworkforceAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.agiworkforce_apps_tools_cache_context.as_ref()
            && let CachedAgiworkforceAppsToolsLoad::Hit(tools) =
                load_cached_agiworkforce_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.agiworkforce_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        agiworkforce_apps_tools_cache_context: Option<AgiworkforceAppsToolsCacheContext>,
        ...
        let startup_snapshot = load_startup_cached_agiworkforce_apps_tools_snapshot(
            &server_name,
            agiworkforce_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        agiworkforce_apps_tools_cache_context,
                    },
                )
                .await
            }
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_agiworkforce_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_agiworkforce_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_agiworkforce_apps_tool_title(server_name, connector_name.as_deref(), title);
    ...
    if server_name == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_agiworkforce_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "agiworkforce-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Agiworkforce".into()),
            description: None,
            icons: None,
            website_url: None,
        },
    ...
    write_cached_agiworkforce_apps_tools_if_needed(
        &server_name,
        agiworkforce_apps_tools_cache_context.as_ref(),
        &tools,
    );
...
mod tests {
    ...
    #[test]
    fn agiworkforce_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
