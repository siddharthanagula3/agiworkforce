...
use crate::agiworkforce_apps::agiworkforce_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeEnvironment;

pub const AGIWORKFORCE_APPS_MCP_SERVER_NAME: &str = "agiworkforce_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const AGIWORKFORCE_CONNECTORS_TOKEN_ENV_VAR: &str = "AGIWORKFORCE_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `agiworkforce_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `agiworkforce-mcp` crate needs to construct server transports, enforce MCP
...
/// [`with_agiworkforce_apps_mcp`] and snapshot collection helpers so config objects do
...
pub struct McpConfig {
    ...
    /// Agiworkforce home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `agiworkforce-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn with_agiworkforce_apps_mcp(
    mut servers: HashMap<String, McpServerConfig>,
    auth: Option<&AgiworkforceAuth>,
    config: &McpConfig,
) -> HashMap<String, McpServerConfig> {
    if config.apps_enabled && auth.is_some_and(AgiworkforceAuth::uses_agiworkforce_backend) {
        servers.insert(
            AGIWORKFORCE_APPS_MCP_SERVER_NAME.to_string(),
            agiworkforce_apps_mcp_server_config(config),
        );
    } else {
        servers.remove(AGIWORKFORCE_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub fn effective_mcp_servers(
    ...
    with_agiworkforce_apps_mcp(servers, auth, config)
}
...
pub async fn read_mcp_resource(
    ...
    let (manager, cancel_token) = McpConnectionManager::new(
        ...
        agiworkforce_apps_tools_cache_key(auth),
...
pub(crate) fn agiworkforce_apps_mcp_url(config: &McpConfig) -> String {
    agiworkforce_apps_mcp_url_for_base_url(
...
fn agiworkforce_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(AGIWORKFORCE_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(AGIWORKFORCE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(AGIWORKFORCE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_agiworkforce_apps_base_url(base_url: &str) -> String {
...
fn agiworkforce_apps_mcp_url_for_base_url(base_url: &str, apps_mcp_path_override: Option<&str>) -> String {
    let base_url = normalize_agiworkforce_apps_base_url(base_url);
...
fn agiworkforce_apps_mcp_server_config(config: &McpConfig) -> McpServerConfig {
    let url = agiworkforce_apps_mcp_url(config);

    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: agiworkforce_apps_mcp_bearer_token_env_var(),
            http_headers: None,
