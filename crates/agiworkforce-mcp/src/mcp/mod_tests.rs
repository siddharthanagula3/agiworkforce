...
fn test_mcp_config(agiworkforce_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn agiworkforce_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
...
#[test]
fn agiworkforce_apps_mcp_url_uses_legacy_agiworkforce_apps_path() {
    let config = test_mcp_config(PathBuf::from("/tmp"));

    assert_eq!(
        agiworkforce_apps_mcp_url(&config),
        "https://chatgpt.com/backend-api/wham/apps"
    );
}

#[test]
fn agiworkforce_apps_server_config_uses_legacy_agiworkforce_apps_path() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    let auth = AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing();

    let mut servers = with_agiworkforce_apps_mcp(HashMap::new(), /*auth*/ None, &config);
    assert!(!servers.contains_key(AGIWORKFORCE_APPS_MCP_SERVER_NAME));

    config.apps_enabled = true;

    servers = with_agiworkforce_apps_mcp(servers, Some(&auth), &config);
    let server = servers
...
#[test]
fn agiworkforce_apps_server_config_uses_configured_apps_mcp_path_override() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    config.apps_mcp_path_override = Some("/custom/mcp".to_string());
    config.apps_enabled = true;
    let auth = AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing();

    let servers = with_agiworkforce_apps_mcp(HashMap::new(), Some(&auth), &config);
    let server = servers
        .get(AGIWORKFORCE_APPS_MCP_SERVER_NAME)
        .expect("codex apps should be present when apps is enabled");
    let url = match &server.transport {
...
#[tokio::test]
async fn effective_mcp_servers_preserve_user_servers_and_add_agiworkforce_apps() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let mut config = test_mcp_config(agiworkforce_home.path().to_path_buf());
    config.apps_enabled = true;
    let auth = AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing();
    ...
    let agiworkforce_apps = effective
        .get(AGIWORKFORCE_APPS_MCP_SERVER_NAME)
    ...
    match &agiworkforce_apps.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => {
            assert_eq!(url, "https://chatgpt.com/backend-api/wham/apps");
        }
        other => panic!("expected streamable http transport, got {other:?}"),
