use super::*;
use crate::agiworkforce_apps::AGIWORKFORCE_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use crate::agiworkforce_apps::AgiworkforceAppsToolsCacheContext;
use crate::agiworkforce_apps::load_startup_cached_agiworkforce_apps_tools_snapshot;
use crate::agiworkforce_apps::read_cached_agiworkforce_apps_tools;
use crate::agiworkforce_apps::write_cached_agiworkforce_apps_tools;
...
fn create_agiworkforce_apps_tools_cache_context(
    agiworkforce_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> AgiworkforceAppsToolsCacheContext {
    AgiworkforceAppsToolsCacheContext {
        agiworkforce_home,
        user_key: AgiworkforceAppsToolsCacheKey {
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "upload").tool;
...
#[test]
fn agiworkforce_apps_tools_cache_is_overwritten_by_last_write() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_agiworkforce_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_agiworkforce_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_agiworkforce_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 =
        read_cached_agiworkforce_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn agiworkforce_apps_tools_cache_is_scoped_per_user() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_agiworkforce_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_agiworkforce_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_agiworkforce_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_agiworkforce_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn agiworkforce_apps_tools_cache_filters_disallowed_connectors() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached = read_cached_agiworkforce_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn agiworkforce_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": AGIWORKFORCE_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(AGIWORKFORCE_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_agiworkforce_apps_tools(&cache_context).is_none());
}

#[test]
fn agiworkforce_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_agiworkforce_apps_tools_loads_from_disk_cache() {
    let agiworkforce_home = tempdir().expect("tempdir");
    let cache_context = create_agiworkforce_apps_tools_cache_context(
        agiworkforce_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_agiworkforce_apps_tools(&cache_context, &cached_tools);

    let startup_snapshot = load_startup_cached_agiworkforce_apps_tools_snapshot(
        AGIWORKFORCE_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    );
    ...
    assert_eq!(startup_tools[0].server_name, AGIWORKFORCE_APPS_MCP_SERVER_NAME);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    manager.clients.insert(
        AGIWORKFORCE_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
    ...
    let tool = tools
        .get("mcp__agiworkforce_apps__calendar_create_event")
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, AGIWORKFORCE_APPS_MCP_SERVER_NAME);
...
fn elicitation_capability_enabled_for_custom_servers() {
    for server_name in [AGIWORKFORCE_APPS_MCP_SERVER_NAME, "custom_mcp"] {
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = AGIWORKFORCE_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
