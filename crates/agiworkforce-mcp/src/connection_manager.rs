//! Aggregates MCP server connections for Agiworkforce.
//!
...
use crate::agiworkforce_apps::AgiworkforceAppsToolsCacheKey;
...
use agiworkforce_protocol::protocol::McpStartupFailure;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        agiworkforce_apps_tools_cache_key: AgiworkforceAppsToolsCacheKey,
        ...
        let agiworkforce_apps_auth_provider = auth
            .filter(|auth| auth.uses_agiworkforce_backend())
            .map(agiworkforce_model_provider::auth_provider_from_auth);
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let agiworkforce_apps_tools_cache_context = if server_name == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
                Some(AgiworkforceAppsToolsCacheContext {
                    agiworkforce_home: agiworkforce_home.clone(),
                    user_key: agiworkforce_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == AGIWORKFORCE_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    agiworkforce_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    pub async fn hard_refresh_agiworkforce_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(AGIWORKFORCE_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{AGIWORKFORCE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{AGIWORKFORCE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_agiworkforce_apps_tools_if_needed(
            AGIWORKFORCE_APPS_MCP_SERVER_NAME,
            managed_client.agiworkforce_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
