...
use crate::state_db;
use agiworkforce_file_search as file_search;
...
impl From<agiworkforce_state::Anchor> for Cursor {
    fn from(anchor: agiworkforce_state::Anchor) -> Self {
...
pub async fn get_threads(
    ...
    let root = agiworkforce_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.agiworkforce/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let state_db_ctx = state_db::open_if_present(agiworkforce_home, "").await;
    ...
    let mut root = agiworkforce_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(agiworkforce_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(agiworkforce_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
