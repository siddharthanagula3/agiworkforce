//! Persist Agiworkforce session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use agiworkforce_protocol::protocol::GitInfo as ProtocolGitInfo;
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Agiworkforce home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let agiworkforce_home = config.agiworkforce_home();
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(agiworkforce_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = agiworkforce_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(agiworkforce_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(agiworkforce_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    let mut dir = config.agiworkforce_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(agiworkforce_state::rollout_item_affects_thread_metadata)
    {
...
impl From<agiworkforce_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: agiworkforce_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
...
fn thread_item_from_state_metadata(item: agiworkforce_state::ThreadMetadata) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &agiworkforce_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
