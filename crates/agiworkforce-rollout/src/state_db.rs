...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match agiworkforce_state::StateRuntime::init(
    ...
    if backfill_state.status != agiworkforce_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = agiworkforce_state::state_db_path(config.sqlite_home());
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == agiworkforce_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
pub async fn list_thread_ids_db(
    context: Option<&agiworkforce_state::StateRuntime>,
    agiworkforce_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.agiworkforce_home() != agiworkforce_home {
        warn!(
            "state db agiworkforce_home mismatch: expected {}, got {}",
            ctx.agiworkforce_home().display(),
            agiworkforce_home.display()
        );
    }
