...
fn test_config(agiworkforce_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: agiworkforce_home.clone(),
        cwd: agiworkforce_home.clone(),
        agiworkforce_home,
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let agiworkforce_home = dir.path().to_path_buf();
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let agiworkforce_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = agiworkforce_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = agiworkforce_home.join("sessions");
