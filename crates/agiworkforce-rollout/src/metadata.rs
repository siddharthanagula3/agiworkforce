...
use agiworkforce_state::BackfillState;
use agiworkforce_state::BackfillStats;
use agiworkforce_state::BackfillStatus;
use agiworkforce_state::DB_ERROR_METRIC;
use agiworkforce_state::DB_METRIC_BACKFILL;
use agiworkforce_state::DB_METRIC_BACKFILL_DURATION_MS;
...
pub(crate) async fn backfill_sessions(
    ...
    let backfill_state = match runtime.get_backfill_state().await {
        ...
        Err(err) => {
            warn!(
                ...
                config.agiworkforce_home().display()
            );
    ...
    let sessions_root = config.agiworkforce_home().join(SESSIONS_SUBDIR);
    let archived_root = config.agiworkforce_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.agiworkforce_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(agiworkforce_home: &Path, path: &Path) -> String {
    path.strip_prefix(agiworkforce_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
