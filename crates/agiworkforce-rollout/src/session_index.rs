...
pub async fn append_thread_name(
    ...
    append_session_index_entry(agiworkforce_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(agiworkforce_home);
...
pub async fn find_thread_path_by_name_str(
    ...
    let Some(thread_id) = find_thread_id_by_name(agiworkforce_home, name).await? else {
        return Ok(None);
    };
    super::list::find_thread_path_by_id_str(agiworkforce_home, &thread_id.to_string()).await
}

fn session_index_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(SESSION_INDEX_FILE)
}
