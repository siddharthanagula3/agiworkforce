...
use agiworkforce_protocol::protocol::RealtimeInputAudioSpeechStarted;
use agiworkforce_protocol::protocol::RealtimeResponseCancelled;
use serde_json::Map as JsonMap;
use serde_json::Value;
use tracing::debug;

const AGIWORKFORCE_TOOL_NAME: &str = "codex";
    ...
    let item = parsed
        ...
        .find(|item| {
            item.get("type").and_then(Value::as_str) == Some("function_call")
                && item.get("name").and_then(Value::as_str) == Some(AGIWORKFORCE_TOOL_NAME)
        })?
        .as_object()?;
...
fn parse_handoff_requested_event(item: &JsonMap<String, Value>) -> Option<RealtimeEvent> {
    ...
    if item_type != Some("function_call") || item_name != Some(AGIWORKFORCE_TOOL_NAME) {
        return None;
    }
