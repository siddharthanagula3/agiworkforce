...
use agiworkforce_client::maybe_build_rustls_client_config_with_custom_ca;
...
mod tests {
    ...
    #[tokio::test]
    async fn realtime_v2_session_update_includes_agiworkforce_tool_and_handoff_output_item() {
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
        let addr = listener.local_addr().expect("local addr");
