...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            agiworkforce_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == LINUX_SANDBOX_ARG0 {
        ...
        agiworkforce_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        agiworkforce_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == AGIWORKFORCE_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                match agiworkforce_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
    ...
    match prepend_path_entry_for_agiworkforce_aliases() {
...
/// [`agiworkforce_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
...
///     [`agiworkforce_core::config::Config`].
///
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "AGIWORKFORCE_";

/// Load env vars from ~/.codex/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `AGIWORKFORCE_`.
fn load_dotenv() {
    if let Ok(agiworkforce_home) = find_agiworkforce_home()
        && let Ok(iter) = dotenvy::from_path_iter(agiworkforce_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `AGIWORKFORCE_` keys.
...
pub fn prepend_path_entry_for_agiworkforce_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if agiworkforce_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (agiworkforce_home: {agiworkforce_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a AGIWORKFORCE_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = agiworkforce_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{}" {AGIWORKFORCE_CORE_APPLY_PATCH_ARG1} %*
"#,
                    exe.display()
                ),
            )?;
