...
pub fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => Some(agiworkforce_lmstudio::DEFAULT_OSS_MODEL),
        OLLAMA_OSS_PROVIDER_ID => Some(agiworkforce_ollama::DEFAULT_OSS_MODEL),
        _ => None,
    }
}
...
pub async fn ensure_oss_provider_ready(
    ...
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => {
            agiworkforce_lmstudio::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        OLLAMA_OSS_PROVIDER_ID => {
            agiworkforce_ollama::ensure_responses_supported(&config.model_provider).await?;
            agiworkforce_ollama::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        _ => {
...
mod tests {
    ...
    fn test_get_default_model_for_provider_lmstudio() {
        ...
        assert_eq!(result, Some(agiworkforce_lmstudio::DEFAULT_OSS_MODEL));
    }

    #[test]
    fn test_get_default_model_for_provider_ollama() {
        let result = get_default_model_for_oss_provider(OLLAMA_OSS_PROVIDER_ID);
        assert_eq!(result, Some(agiworkforce_ollama::DEFAULT_OSS_MODEL));
    }
