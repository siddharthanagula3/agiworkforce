...
#[tokio::test]
async fn save_image_generation_result_saves_base64_to_png_in_agiworkforce_home() {
    let agiworkforce_home = tempfile::tempdir().expect("create codex home");
    let agiworkforce_home = agiworkforce_home.path().abs();
    let expected_path = image_generation_artifact_path(&agiworkforce_home, "session-1", "ig_save_base64");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path =
        save_image_generation_result(&agiworkforce_home, "session-1", "ig_save_base64", "Zm9v")
            .await
...
async fn save_image_generation_result_rejects_data_url_payload() {
    ...
    let err = save_image_generation_result(&agiworkforce_home, "session-1", "ig_456", result)
        .await
...
async fn save_image_generation_result_overwrites_existing_file() {
    let agiworkforce_home = tempfile::tempdir().expect("create codex home");
    let agiworkforce_home = agiworkforce_home.path().abs();
    let existing_path = image_generation_artifact_path(&agiworkforce_home, "session-1", "ig_overwrite");
    ...
    let saved_path = save_image_generation_result(&agiworkforce_home, "session-1", "ig_overwrite", "Zm9v")
        .await
        .expect("image should be saved");
...
async fn save_image_generation_result_sanitizes_call_id_for_agiworkforce_home_output_path() {
    let agiworkforce_home = tempfile::tempdir().expect("create codex home");
    let agiworkforce_home = agiworkforce_home.path().abs();
    let expected_path = image_generation_artifact_path(&agiworkforce_home, "session-1", "../ig/..");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path = save_image_generation_result(&agiworkforce_home, "session-1", "../ig/..", "Zm9v")
        .await
        .expect("image should be saved");
...
async fn save_image_generation_result_rejects_non_standard_base64() {
    let agiworkforce_home = tempfile::tempdir().expect("create codex home");
    let agiworkforce_home = agiworkforce_home.path().abs();
    let err = save_image_generation_result(&agiworkforce_home, "session-1", "ig_urlsafe", "_-8")
        .await
