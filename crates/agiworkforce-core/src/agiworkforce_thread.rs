...
impl ThreadConfigSnapshot {
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        agiworkforce_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct AgiworkforceThreadTurnContextOverrides {
...
pub struct AgiworkforceThread {
    pub(crate) codex: Agiworkforce,
...
/// (formerly called a conversation) in Agiworkforce.
impl AgiworkforceThread {
    pub(crate) fn new(
        codex: Agiworkforce,
        rollout_path: Option<PathBuf>,
    ...
    pub async fn apply_external_goal_set(&self, status: agiworkforce_state::ThreadGoalStatus) {
        if let Err(err) = self
            .codex
            .session
    ...
    pub async fn validate_turn_context_overrides(
        &self,
        overrides: AgiworkforceThreadTurnContextOverrides,
    ) -> ConstraintResult<()> {
        let AgiworkforceThreadTurnContextOverrides {
            cwd,
            approval_policy,
            approvals_reviewer,
            sandbox_policy,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> AgiworkforceResult<()> {
    ...
    #[cfg(test)]
    pub(crate) async fn append_message(&self, message: ResponseItem) -> AgiworkforceResult<String> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> AgiworkforceResult<()> {
        if items.is_empty() {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> AgiworkforceResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard.checked_add(1).ok_or_else(|| {
            AgiworkforceErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> AgiworkforceResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(AgiworkforceErr::InvalidRequest(
...
fn pending_message_input_item(message: &ResponseItem) -> AgiworkforceResult<ResponseInputItem> {
    match message {
        ...
        _ => Err(AgiworkforceErr::InvalidRequest(
