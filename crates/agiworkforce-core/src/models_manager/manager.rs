...
impl ModelsManager {
    ...
    ///
    /// Uses `agiworkforce_home` to store cached model metadata and initializes with bundled catalog
    ...
    pub fn new_with_provider(
        ...
        let cache_path = agiworkforce_home.join(MODEL_CACHE_FILE);
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        ...
        let auth_env = collect_auth_env_telemetry(
            &self.provider,
            self.auth_manager.agiworkforce_api_key_env_enabled(),
        );
