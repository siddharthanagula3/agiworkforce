...
use agiworkforce_mcp::ToolInfo as McpToolInfo;
use agiworkforce_mcp::filter_non_agiworkforce_apps_mcp_tools_only;
...
pub(crate) fn build_mcp_tool_exposure(
    ...
    let mut deferred_tools = filter_non_agiworkforce_apps_mcp_tools_only(all_mcp_tools);
    if let Some(connectors) = connectors {
        deferred_tools.extend(filter_agiworkforce_apps_mcp_tools(
            all_mcp_tools,
            connectors,
            config,
        ));
    ...
    let direct_tools =
        filter_agiworkforce_apps_mcp_tools(all_mcp_tools, explicitly_enabled_connectors, config);
...
fn filter_agiworkforce_apps_mcp_tools(
    ...
    mcp_tools
        ...
        .filter(|(_, tool)| {
            ...
            allowed.contains(connector_id) && connectors::agiworkforce_app_tool_is_enabled(config, tool)
        })
