...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("VALID_NAME", "ok")
        .env("PWD", "/tmp/stale")
        .env("NEXTEST_BIN_EXE_agiworkforce-write-config-schema", "/path/to/bin")
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_agiworkforce-write-config-schema"));
...
async fn write_rollout_stub(agiworkforce_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let agiworkforce_home = dir.path().abs();
    ...
    write_rollout_stub(&agiworkforce_home, live_session).await?;
    fs::write(&live_snapshot, "live").await?;
    fs::write(&orphan_snapshot, "orphan").await?;
    fs::write(&invalid_snapshot, "invalid").await?;

    cleanup_stale_snapshots(&agiworkforce_home, ThreadId::new()).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&agiworkforce_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&agiworkforce_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&agiworkforce_home, active_session).await?;
