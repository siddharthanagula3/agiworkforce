        ...
        .record_into_history(
            &[
                ...
                ResponseItem::FunctionCallOutput {
                    call_id: "call-1".to_string(),
                    output: agiworkforce_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/agiworkforce-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[agiworkforce_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            agiworkforce_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    let prompt = build_guardian_prompt_items(
        ...
        GuardianApprovalRequest::Shell {
            ...
            cwd: test_path_buf("/repo/agiworkforce-rs/core").abs(),
    ...
    assert!(text.contains("The Agiworkforce agent has requested the following action:\n"));
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Agiworkforce agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agiworkforce_core__guardian__tests__network_access_guardian_prompt_layout",
...
-> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(agiworkforce_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agiworkforce_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            agiworkforce_protocol::user_input::UserInput::Text { text, .. } => text,
            agiworkforce_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
    ...
    assert!(
        prompt_text.contains(&format!(
            ">>> TRANSCRIPT END\nReviewed Agiworkforce session id: {}\n",
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(agiworkforce_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "agiworkforce_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(agiworkforce_protocol::openai_models::ReasoningEffort::Low),
    )
