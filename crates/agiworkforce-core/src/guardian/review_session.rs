...
use agiworkforce_analytics::GuardianReviewSessionKind;
...
use crate::agiworkforce_delegate::run_agiworkforce_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            agiworkforce_linux_sandbox_exe: spawn_config.agiworkforce_linux_sandbox_exe.clone(),
...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, codex: Agiworkforce) {
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, codex: Agiworkforce) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            codex.session.get_config().await.as_ref(),
        );
        self.state
...
async fn spawn_guardian_review_session(
    ...
    let codex = Box::pin(run_agiworkforce_thread_interactive(
        spawn_config,
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<agiworkforce_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
async fn interrupt_and_drain_turn(codex: &Agiworkforce) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::AgiworkforceHooks));
    }
