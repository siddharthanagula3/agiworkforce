...
pub fn sandbox_setup_is_complete(agiworkforce_home: &Path) -> bool {
    agiworkforce_windows_sandbox::sandbox_setup_is_complete(agiworkforce_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_agiworkforce_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = agiworkforce_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = agiworkforce_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if agiworkforce_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            agiworkforce_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
...
pub fn run_legacy_setup_preflight(
    ...
    agiworkforce_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        agiworkforce_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    agiworkforce_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let agiworkforce_home = request.agiworkforce_home;
    let active_profile = request.active_profile;
    let setup_agiworkforce_home = agiworkforce_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_agiworkforce_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_agiworkforce_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(agiworkforce_home.as_path())
