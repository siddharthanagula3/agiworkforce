...
use agiworkforce_protocol::models::MacOsContactsPermission;
use agiworkforce_protocol::models::MacOsPreferencesPermission;
...
async fn make_config(agiworkforce_home: &TempDir) -> Config {
    make_config_for_cwd(agiworkforce_home, agiworkforce_home.path().to_path_buf()).await
}

async fn make_config_for_cwd(agiworkforce_home: &TempDir, cwd: PathBuf) -> Config {
    let trust_root = cwd
        .ancestors()
...
async fn loads_skill_permissions_from_yaml() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "permissions-skill", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_permissions_do_not_create_profile() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "permissions-empty", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_macos_permissions_from_yaml() {
    let agiworkforce_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&agiworkforce_home, "demo", "permissions-macos", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
