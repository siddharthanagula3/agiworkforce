...
use agiworkforce_config::ConfigEditsBuilder;
use agiworkforce_config::McpServerConfig;
use agiworkforce_config::McpServerTransportConfig;
use agiworkforce_config::load_global_mcp_servers;
use agiworkforce_login::default_client::is_first_party_originator;
...
use agiworkforce_rmcp_client::perform_oauth_login;
...
use agiworkforce_mcp::McpOAuthLoginSupport;
use agiworkforce_mcp::mcp_permission_prompt_is_auto_approved;
use agiworkforce_mcp::oauth_login_support;
use agiworkforce_mcp::resolve_oauth_scopes;
use agiworkforce_mcp::should_retry_without_scopes;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(agiworkforce_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&agiworkforce_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&agiworkforce_home)
