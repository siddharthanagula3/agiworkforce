...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = agiworkforce_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let agiworkforce_home = tmp.path();
    let link_a = agiworkforce_home.join("a.toml");
    let link_b = agiworkforce_home.join("b.toml");
    let config_path = agiworkforce_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(agiworkforce_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#,
    ...
    apply_blocking(agiworkforce_home, None, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !agiworkforce_home.join(CONFIG_TOML_FILE).exists(),
