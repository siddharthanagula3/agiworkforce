...
use agiworkforce_config::ConfigError;
...
use agiworkforce_config::ConfigRequirementsWithSources;
use agiworkforce_config::FilesystemDenyReadPattern;
...
use agiworkforce_config::config_error_from_toml;
...
use agiworkforce_config::loader::load_requirements_toml;
...
async fn ignore_user_config_keeps_empty_user_layer() -> std::io::Result<()> {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when AGIWORKFORCE_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .agiworkforce_home(tmp.path().to_path_buf())
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when AGIWORKFORCE_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![agiworkforce_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(agiworkforce_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(agiworkforce_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let agiworkforce_home = tmp.path().join("home");
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(agiworkforce_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: agiworkforce_config::HookEventsToml {
                pre_tool_use: vec![agiworkforce_config::MatcherGroup {
                    matcher: Some("^Bash$".to_string()),
                    hooks: vec![agiworkforce_config::HookHandlerConfig::Command {
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            agiworkforce_config::SandboxModeRequirement::ReadOnly,
            agiworkforce_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                agiworkforce_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_agiworkforce_folder } => Some(dot_agiworkforce_folder),
            _ => None,
        })
        .collect();
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_agiworkforce_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let agiworkforce_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agiworkforce_home).await?;
    tokio::fs::write(agiworkforce_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_agiworkforce_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
#[tokio::test]
async fn agiworkforce_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let agiworkforce_home = home_dir.join(".codex");
    tokio::fs::create_dir_all(&agiworkforce_home).await?;
    tokio::fs::write(agiworkforce_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn agiworkforce_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: ConfigLayerSource::Project {
                dot_agiworkforce_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let agiworkforce_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&agiworkforce_home_untrusted).await?;
    make_config_for_test(
        &agiworkforce_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = agiworkforce_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let agiworkforce_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&agiworkforce_home_unknown).await?;
    tokio::fs::write(
        agiworkforce_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let agiworkforce_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&agiworkforce_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let agiworkforce_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&agiworkforce_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &agiworkforce_home,
...
mod requirements_exec_policy_tests {
    ...
    use agiworkforce_config::ConfigRequirementsWithSources;
    use agiworkforce_config::RequirementSource;
    use agiworkforce_config::RequirementsExecPolicyDecisionToml;
    use agiworkforce_config::RequirementsExecPolicyParseError;
    use agiworkforce_config::RequirementsExecPolicyPatternTokenToml;
    use agiworkforce_config::RequirementsExecPolicyPrefixRuleToml;
    use agiworkforce_config::RequirementsExecPolicyToml;
    ...
    fn config_stack_for_dot_agiworkforce_folder_with_requirements(
        dot_agiworkforce_folder: &Path,
