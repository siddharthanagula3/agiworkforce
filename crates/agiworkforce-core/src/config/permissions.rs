...
/// for Agiworkforce to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_agiworkforce_runtime(
...
// Adding a new `:special_path` must not make older Agiworkforce versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Agiworkforce. Filesystem access will remain restricted. Upgrade Agiworkforce if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Agiworkforce and will be ignored. Upgrade Agiworkforce if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Agiworkforce and will be ignored. Upgrade Agiworkforce if this path is required."
            ),
        },
    );
}
