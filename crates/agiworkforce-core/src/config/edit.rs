...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for Agiworkforce-managed fast defaults.
...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&agiworkforce_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(agiworkforce_home: &Path) -> Self {
        Self {
