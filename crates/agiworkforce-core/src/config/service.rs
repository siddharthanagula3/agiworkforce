...
impl ConfigService {
    ...
    pub fn new_with_defaults(agiworkforce_home: PathBuf) -> Self {
        Self {
            agiworkforce_home,
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<agiworkforce_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    /// include any in-repo .agiworkforce/ folders because there is no cwd/project root
