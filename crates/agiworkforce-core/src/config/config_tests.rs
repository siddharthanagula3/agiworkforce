...
"#,
    ...
    let err = load_global_mcp_servers(agiworkforce_home.path())
        .await
...
"#
    ...
    let home_agents_dir = agiworkforce_home.path().join("agents");
...
"#;
    ...
    let agiworkforce_home_temp_dir = TempDir::new().unwrap();
    ...
    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        agiworkforce_home: agiworkforce_home_temp_dir,
        cfg,
        model_provider_map,
        openai_provider,
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(agiworkforce_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(agiworkforce_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
