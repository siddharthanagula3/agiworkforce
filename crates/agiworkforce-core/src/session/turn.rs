...
use agiworkforce_analytics::AppInvocation;
use agiworkforce_analytics::CompactionPhase;
use agiworkforce_analytics::CompactionReason;
use agiworkforce_analytics::InvocationType;
use agiworkforce_analytics::TurnResolvedConfigFact;
use agiworkforce_analytics::build_track_events_context;
...
use agiworkforce_protocol::protocol::AgentMessageContentDeltaEvent;
use agiworkforce_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use agiworkforce_protocol::protocol::ReasoningContentDeltaEvent;
use agiworkforce_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use agiworkforce_tools::ToolName;
...
pub(crate) async fn run_turn(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = agiworkforce_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    loop {
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    for run in sess.hooks().preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(agiworkforce_protocol::protocol::HookStartedEvent {
            ...
            Err(AgiworkforceErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(AgiworkforceErr::InvalidImageRequest()) => {
                {
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = agiworkforce_connectors::metadata::connector_mention_slug(connector);
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = agiworkforce_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> AgiworkforceResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(AgiworkforceErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(AgiworkforceErr::ContextWindowExceeded);
            }
            Err(AgiworkforceErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(AgiworkforceErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                AgiworkforceErr::Stream(_, requested_delay) => {
...
pub(crate) async fn built_tools(
    ...
    skills_outcome: Option<&SkillLoadOutcome>,
    cancellation_token: &CancellationToken,
) -> AgiworkforceResult<Arc<ToolRouter>> {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, AgiworkforceResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> AgiworkforceResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, AgiworkforceResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: AgiworkforceResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(agiworkforce_async_utils::CancelErr::Cancelled) => break Err(AgiworkforceErr::TurnAborted),
        };
        ...
        let event = match event {
            ...
            None => {
                break Err(AgiworkforceErr::Stream(
    ...
    if cancellation_token.is_cancelled() {
        return Err(AgiworkforceErr::TurnAborted);
    }
