...
use core_test_support::agiworkforce_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                agiworkforce_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_additional_permissions_requests_past_policy_validation() {
    ...
    let (mut session, mut turn_context_raw) = make_session_and_context().await;
    turn_context_raw.agiworkforce_linux_sandbox_exe = agiworkforce_linux_sandbox_exe_or_skip!();
    turn_context_raw
        .approval_policy
        .set(AskForApproval::OnRequest)
        .expect("test setup should allow updating approval policy");
    ...
    turn_context_raw.permission_profile = agiworkforce_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agiworkforce_tools::ToolName::plain("shell"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let agiworkforce_home = tempdir().expect("create codex home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_agiworkforce_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(agiworkforce_thread_store::LocalThreadStore::new(
        agiworkforce_rollout::RolloutConfig::from_view(&config),
    ));

    let AgiworkforceSpawnOk { codex, .. } = Agiworkforce::spawn(AgiworkforceSpawnArgs {
        config,
        auth_manager,
        models_manager,
