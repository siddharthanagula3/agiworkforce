...
use agiworkforce_analytics::SubAgentThreadStartedInput;
...
use agiworkforce_exec_server::FileSystemSandboxContext;
use agiworkforce_features::FEATURES;
use agiworkforce_features::Feature;
use agiworkforce_features::unstable_features_warning_event;
use agiworkforce_hooks::Hooks;
use agiworkforce_hooks::HooksConfig;
use agiworkforce_login::AuthManager;
use agiworkforce_login::AgiworkforceAuth;
use agiworkforce_login::auth_env_telemetry::collect_auth_env_telemetry;
use agiworkforce_login::default_client::originator;
use agiworkforce_mcp::McpConnectionManager;
use agiworkforce_mcp::McpRuntimeEnvironment;
use agiworkforce_mcp::ToolInfo;
use agiworkforce_mcp::agiworkforce_apps_tools_cache_key;
...
use agiworkforce_network_proxy::NetworkProxyAuditMetadata;
use agiworkforce_network_proxy::normalize_host;
use agiworkforce_otel::current_span_trace_id;
...
use agiworkforce_protocol::approvals::ExecPolicyAmendment;
...
use agiworkforce_protocol::dynamic_tools::DynamicToolResponse;
...
use agiworkforce_protocol::models::format_allow_prefixes;
...
use agiworkforce_protocol::protocol::TurnContextNetworkItem;
...
use agiworkforce_protocol::request_permissions::RequestPermissionsResponse;
...
use agiworkforce_rollout::state_db;
use agiworkforce_rollout_trace::AgentResultTracePayload;
use agiworkforce_rollout_trace::ThreadStartedTraceMetadata;
use agiworkforce_rollout_trace::ThreadTraceContext;
...
use agiworkforce_thread_store::CreateThreadParams;
use agiworkforce_thread_store::LiveThread;
use agiworkforce_thread_store::LiveThreadInitGuard;
use agiworkforce_thread_store::LocalThreadStore;
use agiworkforce_thread_store::ResumeThreadParams;
use agiworkforce_thread_store::ThreadEventPersistenceMode;
...
use crate::agiworkforce_thread::ThreadConfigSnapshot;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                agiworkforce_error_info: Some(AgiworkforceErrorInfo::BadRequest),
            },
...
use agiworkforce_mcp::compute_auth_statuses;
use agiworkforce_mcp::with_agiworkforce_apps_mcp;
use agiworkforce_otel::SessionTelemetry;
use agiworkforce_otel::THREAD_STARTED_METRIC;
...
use agiworkforce_protocol::protocol::ModelVerification;
use agiworkforce_protocol::protocol::ModelVerificationEvent;
...
use agiworkforce_protocol::protocol::RequestUserInputEvent;
...
use agiworkforce_protocol::protocol::SkillDependencies as ProtocolSkillDependencies;
use agiworkforce_protocol::protocol::SkillErrorInfo;
use agiworkforce_protocol::protocol::SkillInterface as ProtocolSkillInterface;
use agiworkforce_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
use agiworkforce_protocol::protocol::SkillToolDependency as ProtocolSkillToolDependency;
...
use agiworkforce_tools::ToolsConfig;
use agiworkforce_tools::ToolsConfigParams;
use agiworkforce_utils_absolute_path::AbsolutePathBuf;
use agiworkforce_utils_readiness::ReadinessFlag;
#[cfg(test)]
use agiworkforce_utils_stream_parser::ProposedPlanSegment;

/// The high-level interface to the Agiworkforce system.
/// It operates as a queue pair where you send submissions and receive events.
pub struct Agiworkforce {
...
/// Wrapper returned by [`Agiworkforce::spawn`] containing the spawned [`Agiworkforce`] and
/// the unique session id.
pub struct AgiworkforceSpawnOk {
    pub codex: Agiworkforce,
    pub thread_id: ThreadId,
}

pub(crate) struct AgiworkforceSpawnArgs {
...
impl Agiworkforce {
    /// Spawn a new [`Agiworkforce`] and initialize the session.
    pub(crate) async fn spawn(args: AgiworkforceSpawnArgs) -> AgiworkforceResult<AgiworkforceSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if agiworkforce_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(AgiworkforceSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: AgiworkforceSpawnArgs) -> AgiworkforceResult<AgiworkforceSpawnOk> {
        let AgiworkforceSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
            environment_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| AgiworkforceErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            agiworkforce_models_manager::manager::RefreshStrategy::Offline
        } else {
            agiworkforce_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "agiworkforce_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this Agiworkforce session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.agiworkforce_home)
        })?;
        ...
        Ok(AgiworkforceSpawnOk { codex, thread_id })
    }

    /// Submit the `op` wrapped in a `Submission` with a unique ID.
    pub async fn submit(&self, op: Op) -> AgiworkforceResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so Agiworkforce is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> AgiworkforceResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| AgiworkforceErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: agiworkforce_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
        handlers::persist_thread_memory_mode_update(&self.session, mode).await
    }

    pub async fn shutdown_and_wait(&self) -> AgiworkforceResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(AgiworkforceErr::InternalAgentDied) => {}
    ...
    pub async fn next_event(&self) -> AgiworkforceResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
async fn thread_title_from_state_db(
    ...
    find_thread_name_by_id(agiworkforce_home, &conversation_id)
        .await
        .ok()
        .flatten()
}
...
impl Session {
    ...
    /// Builds the `x-agiworkforce-beta-features` header value for this session.
    ///
    ...
    pub(crate) async fn agiworkforce_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.agiworkforce_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Agiworkforce performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (previous_cwd, permission_profile_changed, next_cwd, agiworkforce_home, session_source) = {
            let mut state = self.state.lock().await;
            ...
            let agiworkforce_home = updated.agiworkforce_home.clone();
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_agiworkforce_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &agiworkforce_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| agiworkforce_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&agiworkforce_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                agiworkforce_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    fn normalize_request_permissions_response(
        ...
        let requested_simple: SimplePermissionProfile =
            AdditionalPermissionProfile::from(requested_permissions).into();
        let granted_simple: SimplePermissionProfile =
            AdditionalPermissionProfile::from(response.permissions).into();
        let intersected = intersect_permission_profiles(requested_simple, granted_simple);
        let intersected_permissions: AdditionalPermissionProfile = intersected.into();
        RequestPermissionsResponse {
            permissions: intersected_permissions.into(),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        agiworkforce_error: AgiworkforceErr,
    ) {
        let additional_details = agiworkforce_error.to_string();
        let agiworkforce_error_info = AgiworkforceErrorInfo::ResponseStreamDisconnected {
            http_status_code: agiworkforce_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            agiworkforce_error_info: Some(agiworkforce_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<agiworkforce_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
