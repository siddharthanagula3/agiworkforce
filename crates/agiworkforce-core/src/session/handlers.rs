...
use agiworkforce_mcp::collect_mcp_snapshot_from_manager;
...
use agiworkforce_protocol::protocol::ThreadMemoryMode;
    ...
    tokio::spawn(async move {
        ...
        let event = Event {
            ...
            msg: EventMsg::GetHistoryEntryResponse(
                agiworkforce_protocol::protocol::GetHistoryEntryResponseEvent {
                    offset,
                    log_id,
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Agiworkforce will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
    ...
    let agiworkforce_home = sess.agiworkforce_home().await;
    if let Err(err) =
        crate::rollout::append_thread_name(&agiworkforce_home, sess.conversation_id, &name).await
    {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Agiworkforce instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(agiworkforce_rollout_trace::RolloutStatus::Completed);
    true
}
