...
use agiworkforce_config::NetworkDomainPermissionToml;
use agiworkforce_config::NetworkDomainPermissionsToml;
...
use agiworkforce_models_manager::model_info;
...
use agiworkforce_protocol::request_permissions::PermissionGrantScope;
use agiworkforce_protocol::request_permissions::RequestPermissionProfile;
use tracing::Span;
...
use agiworkforce_execpolicy::NetworkRuleProtocol;
...
use agiworkforce_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use agiworkforce_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use agiworkforce_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use agiworkforce_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use agiworkforce_protocol::protocol::GranularApprovalConfig;
use agiworkforce_protocol::protocol::InitialHistory;
use agiworkforce_protocol::protocol::InterAgentCommunication;
...
use agiworkforce_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
...
use agiworkforce_protocol::protocol::ThreadGoalStatus;
...
use agiworkforce_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "agiworkforce-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        agiworkforce_protocol::protocol::SessionSource::Exec,
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        agiworkforce_home.join(agiworkforce_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<agiworkforce_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&agiworkforce_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            source: agiworkforce_hooks::SessionStartSource::Startup,
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn agiworkforce_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { agiworkforce_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> agiworkforce_sandboxing::SandboxablePreference {
            agiworkforce_sandboxing::SandboxablePreference::Auto
        }
    }
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let agiworkforce_home = session.agiworkforce_home().await;
    std::fs::create_dir_all(&agiworkforce_home).expect("create codex home");
    let config_toml_path = agiworkforce_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = agiworkforce_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let mut builder = test_codex().with_config(move |config| {
        config.permissions.permission_profile = agiworkforce_config::Constrained::allow_any(
            PermissionProfile::from_legacy_sandbox_policy(&sandbox_policy),
        );
        config
    ...
    let expected_permission_profile =
        agiworkforce_protocol::models::PermissionProfile::from_legacy_sandbox_policy(
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            agiworkforce_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        agiworkforce_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
            fork_config.clone(),
            std::sync::Arc::new(agiworkforce_thread_store::LocalThreadStore::new(
                agiworkforce_rollout::RolloutConfig::from_view(&fork_config),
            )),
            rollout_path,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "agiworkforce_core__agiworkforce_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            agiworkforce_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            agiworkforce_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            agiworkforce_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.agiworkforce_error_info,
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(agiworkforce_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(agiworkforce_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
...
async fn build_test_config(agiworkforce_home: &Path) -> Config {
    ConfigBuilder::without_managed_config_for_tests()
        .agiworkforce_home(agiworkforce_home.to_path_buf())
        .build()
        .await
...
fn turn_environments_for_tests(
    environment: &Arc<agiworkforce_exec_server::Environment>,
    cwd: &agiworkforce_utils_absolute_path::AbsolutePathBuf,
) -> Vec<TurnEnvironment> {
    vec![TurnEnvironment {
        environment_id: agiworkforce_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_preserves_profile_file_system_policy_on_cwd_only_update() {
    ...
    session_configuration.permission_profile = agiworkforce_config::Constrained::allow_any(
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = agiworkforce_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        agiworkforce_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let agiworkforce_home = parent_config.agiworkforce_home.clone();
    let skill_dir = agiworkforce_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&agiworkforce_exec_server::LOCAL_FS));
    ...
    let role_path = agiworkforce_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let agiworkforce_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(agiworkforce_home.path()).await;
    config
        .features
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.agiworkforce_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        Arc::new(agiworkforce_thread_store::LocalThreadStore::new(
            agiworkforce_rollout::RolloutConfig::from_view(config.as_ref()),
        )),
        agiworkforce_rollout_trace::ThreadTraceContext::disabled(),
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        agiworkforce_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: agiworkforce_rollout_trace::ThreadTraceContext::disabled(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(agiworkforce_thread_store::LocalThreadStore::new(
            agiworkforce_rollout::RolloutConfig::from_view(config.as_ref()),
        )),
        ...
        environment_manager: Arc::new(agiworkforce_exec_server::EnvironmentManager::default_for_tests()),
    };
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    agiworkforce_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("agiworkforce-core-tests");
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserTurn {
            ...
            approvals_reviewer: Some(agiworkforce_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        agiworkforce_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, AgiworkforceErr::InvalidRequest(_)));
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(agiworkforce_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn agiworkforce_thread_store::ThreadStore> = store.clone();
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        agiworkforce_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
pub(crate) async fn make_session_and_context_with_dynamic_tools_and_rx(
    ...
    make_session_and_context_with_auth_and_config_and_rx(
        AgiworkforceAuth::from_api_key("Test API Key"),
        dynamic_tools,
        |_config| {},
    )
    .await
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = agiworkforce_state::ThreadMetadataBuilder::new(
...
async fn interrupt_accounts_active_goal_before_pausing() -> anyhow::Result<()> {
    ...
    assert_eq!(
        agiworkforce_protocol::protocol::ThreadGoalStatus::Paused,
        goal.status
    );
...
async fn goal_test_state_db(sess: &Session) -> anyhow::Result<crate::StateDbHandle> {
    ...
    agiworkforce_state::StateRuntime::init(config.sqlite_home.clone(), config.model_provider_id.clone())
        .await
}
...
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    ...
    assert_eq!(agiworkforce_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn external_goal_mutation_accounts_active_turn_before_status_change() -> anyhow::Result<()> {
    ...
    state_db
        .update_thread_goal(
            ...
            agiworkforce_state::ThreadGoalUpdate {
                status: Some(agiworkforce_state::ThreadGoalStatus::Complete),
                token_budget: None,
    ...
    sess.goal_runtime_apply(GoalRuntimeEvent::ExternalSet {
        status: agiworkforce_state::ThreadGoalStatus::Complete,
    })
    .await?;
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(agiworkforce_state::ThreadGoalStatus::Complete, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_active_goal_set_marks_current_turn_for_accounting() -> anyhow::Result<()> {
    ...
    state_db
        .replace_thread_goal(
            ...
            agiworkforce_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    sess.goal_runtime_apply(GoalRuntimeEvent::ExternalSet {
        status: agiworkforce_state::ThreadGoalStatus::Active,
    })
    .await?;
...
async fn completed_goal_accounts_current_turn_tokens_before_tool_response() -> anyhow::Result<()> {
    ...
    let state_db = agiworkforce_state::StateRuntime::init(
    ...
    assert_eq!(
        agiworkforce_state::ThreadGoalStatus::Complete,
...
async fn create_goal_tool_rejects_existing_goal() {
    ...
    handler
        .handle(ToolInvocation {
            ...
            tool_name: agiworkforce_tools::ToolName::plain("create_goal"),
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let response = handler
        .handle(ToolInvocation {
            ...
            tool_name: agiworkforce_tools::ToolName::plain("update_goal"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "status": "paused",
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: agiworkforce_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agiworkforce_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let agiworkforce_home = temp.path().join("home");
    ...
    write_project_trust_config(&agiworkforce_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .agiworkforce_home(agiworkforce_home)
        .fallback_cwd(Some(nested))
        .build()
        .await?;

    let preview = preview_session_start_hooks(&config).await?;
    let expected_source_path = agiworkforce_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let agiworkforce_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&agiworkforce_home)?;
        write_project_trust_config(&agiworkforce_home, &trust_entries).await?;
