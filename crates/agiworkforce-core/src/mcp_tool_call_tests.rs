...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        agiworkforce_apps_meta: None,
        openai_file_input_params: None,
    }
}

fn write_sample_plugin_mcp(agiworkforce_home: &std::path::Path) {
    let plugin_root = agiworkforce_home.join("plugins/cache/test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".agiworkforce-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.agiworkforce_home)
...
"#,
    ...
    std::fs::write(
        turn_context.config.agiworkforce_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
...
#[test]
fn openai_file_params_are_only_honored_for_agiworkforce_apps() {
    ...
    assert_eq!(
        openai_file_input_params_for_server(AGIWORKFORCE_APPS_MCP_SERVER_NAME, meta),
...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    let request = build_mcp_tool_approval_elicitation_request(
        ...
        McpToolApprovalElicitationRequest {
            server: AGIWORKFORCE_APPS_MCP_SERVER_NAME,
...
#[test]
fn agiworkforce_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGIWORKFORCE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_agiworkforce_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGIWORKFORCE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn agiworkforce_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: AGIWORKFORCE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn agiworkforce_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: AGIWORKFORCE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_AGIWORKFORCE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
async fn mcp_tool_call_request_meta_includes_turn_started_at_unix_ms() {
    ...
    let turn_metadata = meta
        .get(crate::X_AGIWORKFORCE_TURN_METADATA_HEADER)
...
#[tokio::test]
async fn agiworkforce_apps_tool_call_request_meta_includes_turn_metadata_and_agiworkforce_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        agiworkforce_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_agiworkforce_apps metadata should be an object"),
        ),
        openai_file_input_params: None,
    };
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_AGIWORKFORCE_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_AGIWORKFORCE_APPS_META_KEY: {
...
#[tokio::test]
async fn agiworkforce_apps_tool_call_request_meta_includes_call_id_without_existing_agiworkforce_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: agiworkforce_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_agiworkforce_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            AGIWORKFORCE_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_agiworkforce_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");

    persist_agiworkforce_app_tool_approval(&tmp.path().abs(), "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(agiworkforce_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_agiworkforce_dir = project_dir.path().join(".codex");
    std::fs::create_dir_all(&project_agiworkforce_dir).expect("create project .codex dir");
    std::fs::write(
        project_agiworkforce_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&agiworkforce_home)
        .set_project_trust_level(
            project_dir.path(),
            agiworkforce_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_agiworkforce_dir.join(CONFIG_TOML_FILE))
