...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".agiworkforce-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.agiworkforce_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.agiworkforce_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.agiworkforce_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(agiworkforce_home: PathBuf, name: &str, description: &str) {
    let skill_dir = agiworkforce_home.join(format!("skills/{name}"));
