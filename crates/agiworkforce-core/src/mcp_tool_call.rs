...
use agiworkforce_hooks::PermissionRequestDecision;
use agiworkforce_mcp::AGIWORKFORCE_APPS_MCP_SERVER_NAME;
use agiworkforce_mcp::SandboxState;
use agiworkforce_mcp::declared_openai_file_input_param_names;
use agiworkforce_mcp::mcp_permission_prompt_is_auto_approved;
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
    ...
    if server == AGIWORKFORCE_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    maybe_track_agiworkforce_app_used(sess, turn_context, &server, &tool_name).await;
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                agiworkforce_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_agiworkforce_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    agiworkforce_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    openai_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_AGIWORKFORCE_APPS_META_KEY: &str = "_agiworkforce_apps";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, agiworkforce_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if let Some(turn_metadata) = turn_context.turn_metadata_state.current_meta_value() {
        request_meta.insert(
            crate::X_AGIWORKFORCE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        let mut agiworkforce_apps_meta = metadata
            .and_then(|metadata| metadata.agiworkforce_apps_meta.clone())
            .unwrap_or_default();
        agiworkforce_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_AGIWORKFORCE_APPS_META_KEY.to_string(),
            serde_json::Value::Object(agiworkforce_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__agiworkforce_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "agiworkforce_approval_kind";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == AGIWORKFORCE_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        agiworkforce_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_AGIWORKFORCE_APPS_META_KEY))
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_agiworkforce_app_tool_approval(&turn_context.config.agiworkforce_home, &connector_id, &tool_name)
            .await
    } else {
...
async fn persist_agiworkforce_app_tool_approval(
...
async fn persist_non_app_mcp_tool_approval(
    ...
    if let Some(plugin_config_name) = plugin_config_name {
        return ConfigEditsBuilder::new(&config.agiworkforce_home)
            .with_edits([ConfigEdit::SetPath {
                segments: vec![
                    "plugins".to_string(),
                    plugin_config_name,
...
async fn custom_mcp_tool_approval_config_folder(
    ...
    let servers = load_global_mcp_servers(&config.agiworkforce_home).await?;
    Ok(servers
        .contains_key(server)
        .then(|| config.agiworkforce_home.clone()))
}
