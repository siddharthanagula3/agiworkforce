...
use agiworkforce_thread_store::ListThreadsParams;
use agiworkforce_thread_store::LocalThreadStore;
use agiworkforce_thread_store::ThreadSortKey;
...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = agiworkforce_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(agiworkforce_home, model_provider_id.as_str()).await? {
        create_marker(&marker_path).await?;
...
async fn has_recorded_sessions(agiworkforce_home: &Path, default_provider: &str) -> io::Result<bool> {
    let store = LocalThreadStore::new(agiworkforce_rollout::RolloutConfig {
        agiworkforce_home: agiworkforce_home.to_path_buf(),
        sqlite_home: agiworkforce_home.to_path_buf(),
        cwd: agiworkforce_home.to_path_buf(),
...
async fn has_threads(store: &LocalThreadStore, archived: bool) -> io::Result<bool> {
    store
        .list_threads(ListThreadsParams {
            ...
            sort_direction: agiworkforce_thread_store::SortDirection::Desc,
