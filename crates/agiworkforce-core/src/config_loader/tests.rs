...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    let agiworkforce_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agiworkforce_home).await?;
    make_config_for_test(&agiworkforce_home, &project_root, TrustLevel::Trusted, None).await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        ...
        if let Some(trust_level) = trust_level {
            make_config_for_test(&agiworkforce_home, &project_root, trust_level, None).await?;
