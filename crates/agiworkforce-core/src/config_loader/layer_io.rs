...
use agiworkforce_config::io_error_from_config_error;
...
#[cfg(unix)]
const CODEX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/agiworkforce/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/agiworkforce/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(agiworkforce_home)),
    )?;
...
pub(super) fn managed_config_default_path(agiworkforce_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = agiworkforce_home;
