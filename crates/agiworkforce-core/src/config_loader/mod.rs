...
pub use agiworkforce_config::AppRequirementToml;
pub use agiworkforce_config::AppsRequirementsToml;
...
pub use agiworkforce_config::ConfigError;
pub use agiworkforce_config::ConfigLayerEntry;
pub use agiworkforce_config::ConfigLayerStack;
pub use agiworkforce_config::ConfigLayerStackOrdering;
pub use agiworkforce_config::ConfigLoadError;
pub use agiworkforce_config::ConfigRequirements;
pub use agiworkforce_config::ConfigRequirementsToml;
pub use agiworkforce_config::ConstrainedWithSource;
pub use agiworkforce_config::FeatureRequirementsToml;
pub use agiworkforce_config::LoaderOverrides;
pub use agiworkforce_config::McpServerIdentity;
pub use agiworkforce_config::McpServerRequirement;
pub use agiworkforce_config::NetworkConstraints;
pub use agiworkforce_config::NetworkRequirementsToml;
pub use agiworkforce_config::RequirementSource;
pub use agiworkforce_config::ResidencyRequirement;
pub use agiworkforce_config::SandboxModeRequirement;
pub use agiworkforce_config::Sourced;
pub use agiworkforce_config::TextPosition;
pub use agiworkforce_config::TextRange;
pub use agiworkforce_config::WebSearchModeRequirement;
pub(crate) use agiworkforce_config::build_cli_overrides_layer;
pub(crate) use agiworkforce_config::config_error_from_toml;
pub use agiworkforce_config::format_config_error;
pub use agiworkforce_config::format_config_error_with_source;
pub(crate) use agiworkforce_config::io_error_from_config_error;
pub use agiworkforce_config::merge_toml_values;
#[cfg(test)]
pub(crate) use agiworkforce_config::version_for_toml;

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/agiworkforce/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/agiworkforce/config.toml";
...
pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    agiworkforce_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}
...
/// - system    `/etc/agiworkforce/requirements.toml` (Unix) or
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/agiworkforce/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Codex\config.toml` (Windows)
/// - user      `${AGIWORKFORCE_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.agiworkforce/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.agiworkforce/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    if let Some(config) = managed_config_from_mdm {
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, agiworkforce_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/agiworkforce/requirements.toml"))
}
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_agiworkforce_folder: dot_agiworkforce_folder.clone(),
    };
...
/// from a file like `/etc/agiworkforce/managed_config.toml` that has the same
