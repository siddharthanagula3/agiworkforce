...
use agiworkforce_execpolicy::AmendError;
use agiworkforce_execpolicy::Decision;
use agiworkforce_execpolicy::Error as ExecPolicyRuleError;
...
use agiworkforce_execpolicy::blocking_append_allow_prefix_rule;
use agiworkforce_execpolicy::blocking_append_network_rule;
...
use agiworkforce_shell_command::is_dangerous_command::command_might_be_dangerous;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(agiworkforce_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: agiworkforce_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(agiworkforce_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &agiworkforce_execpolicy::Error) -> String {
