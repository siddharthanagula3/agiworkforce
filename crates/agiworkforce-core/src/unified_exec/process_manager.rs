...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Agiworkforce sandbox network proxy.";
...
) -> agiworkforce_exec_server::ExecEnvPolicy {
    agiworkforce_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<agiworkforce_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> agiworkforce_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Agiworkforce(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == agiworkforce_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let agiworkforce_home = crate::config::find_agiworkforce_home().map_err(|err| {
                UnifiedExecError::create_process(format!(
            ...
            let spawned = match request.windows_sandbox_level {
                agiworkforce_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    agiworkforce_windows_sandbox::spawn_windows_sandbox_session_elevated(
                        policy_json.as_str(),
                ...
                agiworkforce_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | agiworkforce_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    agiworkforce_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        policy_json.as_str(),
                        request.windows_sandbox_policy_cwd.as_path(),
                        agiworkforce_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            agiworkforce_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                agiworkforce_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            agiworkforce_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Agiworkforce(AgiworkforceErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
