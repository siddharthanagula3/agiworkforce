...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export AGIWORKFORCE_INTERACTIVE_SHELL_VAR=codex\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $AGIWORKFORCE_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
        out_2.truncated_output().contains("agiworkforce"),
...
async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
    ...
    .await?;
    assert!(
        out_3.truncated_output().contains("agiworkforce"),
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export AGIWORKFORCE_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $AGIWORKFORCE_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
    ...
    assert!(result.truncated_output().contains("agiworkforce"));
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = agiworkforce_exec_server::Environment::default_for_tests();
