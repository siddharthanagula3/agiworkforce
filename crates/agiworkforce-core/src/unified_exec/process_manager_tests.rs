...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("AGIWORKFORCE_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("AGIWORKFORCE_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            "AGIWORKFORCE_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_server_params_use_env_policy_overlay_contract() {
    let cwd: agiworkforce_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let sandbox_policy = agiworkforce_protocol::protocol::SandboxPolicy::DangerFullAccess;
    let file_system_sandbox_policy =
        agiworkforce_protocol::permissions::FileSystemSandboxPolicy::from(&sandbox_policy);
    let network_sandbox_policy = agiworkforce_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile =
        agiworkforce_protocol::models::PermissionProfile::from_runtime_permissions_with_enforcement(
            agiworkforce_protocol::models::SandboxEnforcement::from_legacy_sandbox_policy(&sandbox_policy),
    ...
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: agiworkforce_exec_server::ExecEnvPolicy {
                inherit: agiworkforce_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: agiworkforce_sandboxing::SandboxType::None,
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let agiworkforce_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
