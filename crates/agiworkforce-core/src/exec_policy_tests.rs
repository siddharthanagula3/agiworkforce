...
use agiworkforce_config::RequirementsExecPolicy;
...
async fn test_config() -> (TempDir, Config) {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .agiworkforce_home(home.path().to_path_buf())
        .build()
        .await
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_agiworkforce_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        agiworkforce_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(agiworkforce_config::Sourced::new(
            agiworkforce_config::RequirementsExecPolicy::new(requirements_exec_policy),
            agiworkforce_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(agiworkforce_config::CONFIG_TOML_FILE))
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_agiworkforce_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_agiworkforce_folder: project_dot_agiworkforce_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let agiworkforce_home = temp.path().join("home_ignore_user_config");
    let rules_dir = agiworkforce_home.join(RULES_DIR_NAME);
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let agiworkforce_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(agiworkforce_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(agiworkforce_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(agiworkforce_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let agiworkforce_home = temp.path().join("home_execpolicy_nested");
    ...
    fs::create_dir_all(&agiworkforce_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    for (name, trust_entries, expected_decision) in cases {
        let agiworkforce_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&agiworkforce_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let agiworkforce_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&agiworkforce_home)?;
        write_project_trust_config(&agiworkforce_home, &trust_entries).await?;
