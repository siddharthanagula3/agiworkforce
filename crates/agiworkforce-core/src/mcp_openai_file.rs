//! Bridges Apps SDK-style `openai/fileParams` metadata into Agiworkforce's MCP flow.
//!
//! Strategy:
...
//!
//! Model-visible schema masking is owned by `agiworkforce-mcp` alongside MCP tool
//! inventory, so this module only handles the execution-time argument rewrite.

use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use agiworkforce_api::upload_local_file;
...
async fn build_uploaded_local_argument_value(
    ...
    let Some(auth) = auth else {
        return Err(
            "ChatGPT auth is required to upload local files for Agiworkforce Apps tools".to_string(),
        );
    };
    ...
    let upload_auth = agiworkforce_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    async fn openai_file_argument_rewrite_requires_declared_file_params() {
        ...
        let arguments = Some(serde_json::json!({
            "file": "/tmp/agiworkforce-smoke-file.txt"
        }));
