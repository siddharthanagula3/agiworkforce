...
fn make_mcp_tool(
    ...
    let tool_namespace = if server_name == AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        connector_name
            .map(sanitize_name)
...
async fn directly_exposes_explicit_apps_without_deferred_overlap() {
    ...
    assert_eq!(
        tool_names,
        vec!["mcp__agiworkforce_apps__calendar_create_event".to_string()]
    );
    assert_eq!(
    ...
    assert!(!deferred_tools.contains_key("mcp__agiworkforce_apps__calendar_create_event"));
