...
use agiworkforce_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-Agiworkforce thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_agiworkforce_thread_interactive(
    config: Config,
...
) -> Result<Agiworkforce, AgiworkforceErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let AgiworkforceSpawnOk { codex, .. } = Box::pin(Agiworkforce::spawn(AgiworkforceSpawnArgs {
        config,
        auth_manager,
        models_manager,
        ...
        parent_rollout_thread_trace: agiworkforce_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let agiworkforce_for_events = Arc::clone(&codex);
    ...
    let agiworkforce_for_ops = Arc::clone(&codex);
    tokio::spawn(async move {
        forward_ops(agiworkforce_for_ops, rx_ops, cancel_token_ops).await;
    });
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_agiworkforce_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_agiworkforce_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn shutdown_delegate(codex: &Agiworkforce) {
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                agiworkforce_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                agiworkforce_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
...
where
    F: core::future::Future<Output = agiworkforce_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, agiworkforce_protocol::protocol::ReviewDecision::Abort)
                .await;
