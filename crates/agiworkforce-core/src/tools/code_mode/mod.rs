...
use agiworkforce_code_mode::CodeModeNestedToolCall;
...
use agiworkforce_tools::collect_code_mode_tool_definitions;
...
impl CodeModeService {
    ...
    ) -> Result<agiworkforce_code_mode::WaitOutcome, String> {
...
async fn call_nested_tool(
    ...
    let (tool_call_name, payload) =
        if let Some(tool_info) = exec.session.resolve_mcp_tool_info(&tool_name).await {
            let raw_arguments = match serialize_function_tool_arguments(&tool_name, Some(input)) {
            ...
            match build_nested_tool_payload(tool_runtime.find_spec(&tool_name), &tool_name, Some(input)) {
