...
use agiworkforce_code_mode::CodeModeTurnHost;
use agiworkforce_code_mode::RuntimeResponse;
...
use agiworkforce_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use agiworkforce_utils_output_truncation::truncate_function_output_items_with_policy;

pub(crate) use execute_handler::CodeModeExecuteHandler;
use response_adapter::into_function_call_output_content_items;
pub(crate) use wait_handler::CodeModeWaitHandler;

pub(crate) const PUBLIC_TOOL_NAME: &str = agiworkforce_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = agiworkforce_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = agiworkforce_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
pub(crate) struct CodeModeService {
    inner: agiworkforce_code_mode::CodeModeService,
}

impl CodeModeService {
    pub(crate) fn new(_js_repl_node_path: Option<PathBuf>) -> Self {
        Self {
            inner: agiworkforce_code_mode::CodeModeService::new(),
        }
    }
    ...
    pub(crate) async fn execute(
        &self,
        request: agiworkforce_code_mode::ExecuteRequest,
    ...
    pub(crate) async fn wait(
        &self,
        request: agiworkforce_code_mode::WaitRequest,
    ) -> Result<RuntimeResponse, String> {
    ...
    ) -> Option<agiworkforce_code_mode::CodeModeTurnWorker> {
...
pub(super) async fn build_enabled_tools(
    exec: &ExecContext,
) -> Vec<agiworkforce_code_mode::ToolDefinition> {
...
fn enabled_tool_from_spec(spec: ToolSpec) -> Option<agiworkforce_code_mode::ToolDefinition> {
    let tool_name = spec.name().to_string();
    if !agiworkforce_code_mode::is_code_mode_nested_tool(&tool_name) {
        return None;
    }

    match spec {
        ToolSpec::Function(tool) => Some(agiworkforce_code_mode::ToolDefinition {
            name: tool_name,
            description: tool.description,
            kind: agiworkforce_code_mode::CodeModeToolKind::Function,
            input_schema: serde_json::to_value(&tool.parameters).ok(),
            output_schema: tool.output_schema,
        }),
        ToolSpec::Freeform(tool) => Some(agiworkforce_code_mode::ToolDefinition {
            name: tool_name,
            description: tool.description,
            kind: agiworkforce_code_mode::CodeModeToolKind::Freeform,
            input_schema: None,
            output_schema: None,
        }),
        ToolSpec::LocalShell {}
...
fn tool_kind_for_spec(spec: &ToolSpec) -> agiworkforce_code_mode::CodeModeToolKind {
    if matches!(spec, ToolSpec::Freeform(_)) {
        agiworkforce_code_mode::CodeModeToolKind::Freeform
    } else {
        agiworkforce_code_mode::CodeModeToolKind::Function
    }
}
...
fn tool_kind_for_name(
    ...
    tool_name: &str,
) -> Result<agiworkforce_code_mode::CodeModeToolKind, String> {
    spec.as_ref()
