        ...
        match payload {
                ...
                if tool_name.namespace.is_none() && tool_name.name.as_str() == WAIT_TOOL_NAME =>
            {
                ...
                let wait_response = exec
                    ...
                    .services
                    .code_mode_service
                    .wait(agiworkforce_code_mode::WaitRequest {
                ...
                if let agiworkforce_code_mode::WaitOutcome::LiveCell(response) = &wait_response
                    && !matches!(response, agiworkforce_code_mode::RuntimeResponse::Yielded { .. })
                {
                    ...
                    let runtime_cell_id = match response {
                        agiworkforce_code_mode::RuntimeResponse::Yielded { cell_id, .. }
                        | agiworkforce_code_mode::RuntimeResponse::Terminated { cell_id, .. }
                        | agiworkforce_code_mode::RuntimeResponse::Result { cell_id, .. } => cell_id,
                    };
                    exec.session
                        .services
