...
use agiworkforce_rollout_trace::ToolCallRequester;
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let registry = ToolRegistry::with_handler_for_test(
        agiworkforce_tools::ToolName::plain("test_tool"),
        Arc::new(TestHandler),
    );
    ...
    let replayed = agiworkforce_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
...
async fn missing_code_mode_wait_traces_only_the_wait_tool_call() -> anyhow::Result<()> {
    ...
    let registry = ToolRegistry::with_handler_for_test(
        agiworkforce_tools::ToolName::plain(WAIT_TOOL_NAME),
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        agiworkforce_tools::ToolName::plain(tool_name),
        source,
...
fn test_invocation_with_payload(
    ...
    turn: Arc<TurnContext>,
    call_id: &str,
    tool_name: agiworkforce_tools::ToolName,
    source: ToolCallSource,
    payload: ToolPayload,
) -> ToolInvocation {
    ToolInvocation {
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace =
        agiworkforce_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_agiworkforce_turn_started(turn.sub_id.as_str());
