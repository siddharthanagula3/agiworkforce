...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(
        ...
        ResponseItem::FunctionCall {
            ...
            namespace: Some("mcp__agiworkforce_apps__calendar".to_string()),
            arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__agiworkforce_apps__calendar", tool_name)
    );
...
async fn model_visible_specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert!(
        router
            .find_spec(&ToolName::namespaced("agiworkforce_app", hidden_tool))
            .is_some()
    );
    assert_eq!(
        namespace_function_names(&router.specs(), "agiworkforce_app"),
        vec![hidden_tool.to_string(), visible_tool.to_string()]
    );
    assert_eq!(
        namespace_function_names(&router.model_visible_specs(), "agiworkforce_app"),
