...
use agiworkforce_network_proxy::NetworkProxyConstraints;
use agiworkforce_network_proxy::NetworkProxyState;
...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = agiworkforce_network_proxy::build_config_state(
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_agiworkforce_marker() {
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_agiworkforce_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport AGIWORKFORCE_THREAD_ID='parent-thread'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$AGIWORKFORCE_THREAD_ID\"".to_string(),
    ];
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
        &session_shell,
        &dir.path().abs(),
        &HashMap::new(),
        &HashMap::from([("AGIWORKFORCE_THREAD_ID".to_string(), "nested-thread".to_string())]),
    );
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("AGIWORKFORCE_THREAD_ID", "nested-thread")
        .output()
        .expect("run rewritten command");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_agiworkforce_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_agiworkforce_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport AGIWORKFORCE_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${AGIWORKFORCE_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$AGIWORKFORCE_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "AGIWORKFORCE_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("AGIWORKFORCE_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
