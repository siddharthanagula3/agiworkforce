...
use agiworkforce_execpolicy::MatchOptions;
...
use agiworkforce_protocol::protocol::NetworkPolicyRuleAction;
...
use agiworkforce_shell_command::bash::parse_shell_lc_single_command_prefix;
use agiworkforce_shell_escalation::EscalateServer;
use agiworkforce_shell_escalation::EscalationDecision;
use agiworkforce_shell_escalation::EscalationExecution;
use agiworkforce_shell_escalation::EscalationPermissions;
use agiworkforce_shell_escalation::EscalationPolicy;
use agiworkforce_shell_escalation::EscalationSession;
use agiworkforce_shell_escalation::ExecParams;
use agiworkforce_shell_escalation::ExecResult;
use agiworkforce_shell_escalation::PreparedExec;
use agiworkforce_shell_escalation::ResolvedPermissionProfile;
use agiworkforce_shell_escalation::ShellCommandExecutor;
use agiworkforce_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Agiworkforce(agiworkforce_protocol::error::AgiworkforceErr::Io(std::io::Error::new(std::io::ErrorKind::Other, err.to_string()))))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        agiworkforce_linux_sandbox_exe: ctx.turn.agiworkforce_linux_sandbox_exe.clone(),
...
struct CoreShellCommandExecutor {
    ...
    network: Option<agiworkforce_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["AGIWORKFORCE_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            agiworkforce_linux_sandbox_exe: self.agiworkforce_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Agiworkforce(AgiworkforceErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Agiworkforce(AgiworkforceErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
