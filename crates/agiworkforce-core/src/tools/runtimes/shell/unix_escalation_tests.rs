...
use agiworkforce_execpolicy::PolicyParser;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "AGIWORKFORCE_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let expected_hook_command =
        agiworkforce_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(agiworkforce_execpolicy::Policy::empty())),
        ...
        stopwatch: agiworkforce_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        agiworkforce_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        agiworkforce_shell_escalation::EscalationDecision::Escalate(
            agiworkforce_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
