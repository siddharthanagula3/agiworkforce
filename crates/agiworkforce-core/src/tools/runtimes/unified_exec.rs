...
use agiworkforce_shell_command::powershell::prefix_powershell_script_with_utf8;
...
impl<'a> ToolRuntime<UnifiedExecRequest, UnifiedExecProcess> for UnifiedExecRuntime<'a> {
    ...
    async fn run(
                    ...
                    return self
                        ...
                        .map_err(|err| match err {
                            UnifiedExecError::SandboxDenied { output, .. } => {
                                ToolError::Agiworkforce(AgiworkforceErr::Sandbox(SandboxErr::Denied {
        ...
        let mut exec_env = attempt
            .env_for(command, options, managed_network)
            .map_err(|err| ToolError::Agiworkforce(err.into()))?;
        exec_env.exec_server_env_config = req.exec_server_env_config.clone();
        let Some(environment) = ctx.turn.environment.as_ref() else {
            return Err(ToolError::Rejected(
                "exec_command is unavailable in this session".to_string(),
