...
use agiworkforce_sandboxing::policy_transforms::effective_permission_profile;
...
impl ApplyPatchRuntime {
    ...
    fn build_guardian_review_request(
        ...
        GuardianApprovalRequest::ApplyPatch {
            ...
            cwd: AbsolutePathBuf::try_from(req.action.cwd.clone())
                .or_else(|_| AbsolutePathBuf::from_absolute_path(std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("/"))))
                .expect("cannot determine cwd for apply_patch guardian request"),
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let result = agiworkforce_apply_patch::apply_patch(
            &req.action.patch,
            &mut stdout,
            &mut stderr,
        );
