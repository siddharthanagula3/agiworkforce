...
*/
use crate::exec_env::AGIWORKFORCE_THREAD_ID_ENV_VAR;
use crate::path_utils;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::tools::sandboxing::ToolError;
#[cfg(target_os = "macos")]
use agiworkforce_network_proxy::AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER;
...
pub(crate) fn build_sandbox_command(
    ...
    Ok(SandboxCommand {
        ...
        cwd: cwd.to_path_buf(),
        env: env.clone(),
        additional_permissions: additional_permissions.map(|ap| ap.into()),
    })
}
...
pub(crate) fn exec_env_for_sandbox_permissions(
        ...
        && env.contains_key(PROXY_ACTIVE_ENV_KEY)
    {
        ...
        // Only macOS injects a Agiworkforce-owned SSH wrapper for the managed SOCKS proxy.
        #[cfg(target_os = "macos")]
        if env
            .get(PROXY_GIT_SSH_COMMAND_ENV_KEY)
            .is_some_and(|command| command.starts_with(AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER))
        {
...
/// runtime-only vars like `AGIWORKFORCE_THREAD_ID` without pretending they came from
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(AGIWORKFORCE_THREAD_ID_ENV_VAR) {
        override_env.insert(AGIWORKFORCE_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    build_override_exports_for_keys("__AGIWORKFORCE_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__AGIWORKFORCE_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__AGIWORKFORCE_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__AGIWORKFORCE_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_agiworkforce_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_agiworkforce_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", AGIWORKFORCE_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__AGIWORKFORCE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
