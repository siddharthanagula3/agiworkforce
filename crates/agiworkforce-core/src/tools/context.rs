...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the Agiworkforce tool call id
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    agiworkforce_protocol::models::ContentItem::InputText { text }
                    | agiworkforce_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    agiworkforce_protocol::models::ContentItem::InputImage { image_url, detail } => {
