...
use agiworkforce_network_proxy::NetworkDecision;
use agiworkforce_network_proxy::NetworkPolicyDecider;
use agiworkforce_network_proxy::NetworkPolicyRequest;
use agiworkforce_network_proxy::NetworkProtocol;
                ...
                PermissionRequestDecision::Deny { message } => {
                    if let Some(owner_call) = owner_call.as_ref() {
                        self.record_call_outcome(
                            ...
                            NetworkApprovalOutcome::DeniedByPolicy(message.unwrap_or_else(|| "Permission denied".to_string())),
                        )
                        .await;
                    }
                    pending.set_decision(PendingApprovalDecision::Deny).await;
