...
impl JobProgressEmitter {
    ...
    async fn maybe_emit(
        ...
        turn: &TurnContext,
        job_id: &str,
        progress: &agiworkforce_state::AgentJobProgress,
        force: bool,
    ) -> anyhow::Result<()> {
...
mod spawn_agents_on_csv {
    ...
    pub async fn handle(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            items.push(agiworkforce_state::AgentJobItemCreateParams {
                item_id,
        ...
        let _job = db
            .create_agent_job(
                &agiworkforce_state::AgentJobCreateParams {
                    id: job_id.clone(),
                    name: job_name,
        ...
        let failed_item_errors = if progress.failed_items > 0 {
            let items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(agiworkforce_state::AgentJobItemStatus::Failed),
                    Some(5),
                )
                .await
                .unwrap_or_default();
...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<agiworkforce_state::StateRuntime>, FunctionCallError> {
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<agiworkforce_state::StateRuntime>,
    job_id: String,
    ...
    loop {
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(agiworkforce_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
...
async fn export_job_csv_snapshot(
    db: Arc<agiworkforce_state::StateRuntime>,
    job: &agiworkforce_state::AgentJob,
) -> anyhow::Result<()> {
    let items = db
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(agiworkforce_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, agiworkforce_state::AgentJobItemStatus::Running) {
...
fn build_worker_prompt(
    job: &agiworkforce_state::AgentJob,
    item: &agiworkforce_state::AgentJobItem,
...
fn job_runtime_timeout(job: &agiworkforce_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &agiworkforce_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &agiworkforce_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
fn render_job_csv(
    headers: &[String],
    items: &[agiworkforce_state::AgentJobItem],
