...
fn default_runtime_manager_uses_openai_codex_release_base() {
    let agiworkforce_home = TempDir::new().expect("create temp codex home");
    let manager = default_runtime_manager(agiworkforce_home.path().to_path_buf());
...
fn load_cached_runtime_reads_pinned_cache_path() {
    let agiworkforce_home = TempDir::new().expect("create temp codex home");
    let platform =
        agiworkforce_artifacts::ArtifactRuntimePlatform::detect_current().expect("detect platform");
    ...
    let runtime = agiworkforce_artifacts::load_cached_runtime(
        &agiworkforce_home
            .path()
            .join(agiworkforce_artifacts::DEFAULT_CACHE_ROOT_RELATIVE),
        versions::ARTIFACT_RUNTIME,
    )
