...
use agiworkforce_apply_patch::ApplyPatchArgs;
use agiworkforce_apply_patch::ApplyPatchFileChange;
use agiworkforce_apply_patch::Hunk;
use agiworkforce_apply_patch::parse_patch_streaming;
...
use agiworkforce_protocol::protocol::PatchApplyUpdatedEvent;
use agiworkforce_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use agiworkforce_sandboxing::policy_transforms::merge_additional_permission_profiles as merge_permission_profiles;
use agiworkforce_sandboxing::policy_transforms::normalize_additional_permissions;
use agiworkforce_tools::ApplyPatchToolArgs;
...
fn format_update_chunks_for_progress(chunks: &[agiworkforce_apply_patch::UpdateFileChunk]) -> String {
...
fn file_paths_for_action(action: &ApplyPatchAction) -> Vec<AbsolutePathBuf> {
    ...
    let Ok(cwd) = AbsolutePathBuf::try_from(action.cwd.clone()) else {
        return keys;
    };
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &agiworkforce_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
    ...
    normalize_additional_permissions(permissions.into()).ok().map(|p| p.into())
}
...
async fn effective_patch_permissions(
    ...
    agiworkforce_protocol::permissions::FileSystemSandboxPolicy,
) {
    ...
    let granted_permissions_simple: Option<agiworkforce_protocol::models::SimplePermissionProfile> =
        granted_permissions.map(|p| p.into());
    let file_system_sandbox_policy = effective_file_system_sandbox_policy(
        &base_file_system_sandbox_policy,
        granted_permissions_simple.as_ref(),
    );
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let _ = fs;
        let _ = sandbox;
        match agiworkforce_apply_patch::maybe_parse_apply_patch_verified(
            &command,
            cwd.as_path(),
        )
        {
            agiworkforce_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            agiworkforce_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            agiworkforce_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            agiworkforce_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let _ = fs;
    let _ = sandbox;
    match agiworkforce_apply_patch::maybe_parse_apply_patch_verified(command, cwd.as_path())
    {
        agiworkforce_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            session
        ...
        agiworkforce_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
