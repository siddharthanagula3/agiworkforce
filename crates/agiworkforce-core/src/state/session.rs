...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_source: Option<agiworkforce_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    pub(crate) fn set_pending_session_start_source(
        &mut self,
        value: Option<agiworkforce_hooks::SessionStartSource>,
    ) {
...
// missing values as the default `"agiworkforce"` bucket.
fn merge_rate_limit_fields(
    previous: Option<&RateLimitSnapshot>,
    mut snapshot: RateLimitSnapshot,
) -> RateLimitSnapshot {
    if snapshot.limit_id.is_none() {
        snapshot.limit_id = Some("agiworkforce".to_string());
    }
