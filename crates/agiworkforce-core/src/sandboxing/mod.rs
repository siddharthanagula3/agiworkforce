...
command transformation live in the agiworkforce-sandboxing crate; this module keeps
...
use crate::spawn::AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use agiworkforce_protocol::models::SandboxPermissions;
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: agiworkforce_exec_server::ExecEnvPolicy,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        let cwd = AbsolutePathBuf::try_from(cwd.clone())
            .unwrap_or_else(|_| windows_sandbox_policy_cwd.clone());
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
...
) -> agiworkforce_protocol::error::Result<ExecToolCallOutput> {
