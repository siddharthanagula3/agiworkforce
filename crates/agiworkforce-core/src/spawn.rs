...
///
/// 1. The process was spawned by Agiworkforce as part of a shell tool call.
...
pub const AGIWORKFORCE_SANDBOX_ENV_VAR: &str = "AGIWORKFORCE_SANDBOX";
...
/// `AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }

    // If this Agiworkforce process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                agiworkforce_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                agiworkforce_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
