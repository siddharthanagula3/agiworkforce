...
use agiworkforce_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    async fn new() -> Self {
        ...
        let manager = ThreadManager::with_models_provider_and_home_for_tests(
            ...
            std::sync::Arc::new(agiworkforce_exec_server::EnvironmentManager::default_for_tests()),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<AgiworkforceThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<AgiworkforceThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<AgiworkforceThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, AgiworkforceErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let AgiworkforceErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected AgiworkforceErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let AgiworkforceErr::AgentLimitReached { max_threads } = err else {
        panic!("expected AgiworkforceErr::AgentLimitReached");
    };
...
async fn resume_agent_from_rollout_reads_archived_rollout_path() {
    ...
    let store = LocalThreadStore::new(agiworkforce_rollout::RolloutConfig::from_view(&harness.config));
    store
