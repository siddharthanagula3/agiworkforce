...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.agiworkforce_home);
...
fn artifact_memories_for_phase2(
    selection: &agiworkforce_state::Phase2InputSelection,
) -> Vec<Stage1Output> {
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            agiworkforce_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[agiworkforce_state::Stage1Output],
        reason: &'static str,
    ) {
        session.services.session_telemetry.counter(
            metrics::MEMORY_PHASE_TWO_JOBS,
...
mod agent {
    ...
    pub(super) fn get_config(config: Arc<Config>) -> Option<Config> {
        ...
        match AbsolutePathBuf::from_absolute_path(agent_config.agiworkforce_home.clone()) {
            Ok(agiworkforce_home) => writable_roots.push(agiworkforce_home),
            Err(err) => warn!(
                "memory phase-2 consolidation could not add agiworkforce_home writable root {}: {err}",
                agent_config.agiworkforce_home.display()
            ),
        }
        // The consolidation agent only needs local agiworkforce_home write access and no network.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<agiworkforce_state::Stage1Output>,
        thread_id: ThreadId,
        phase_two_e2e_timer: Option<agiworkforce_otel::Timer>,
    ) {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[agiworkforce_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
