...
mod phase2 {
    ...
    struct DispatchHarness {
        ...
        state_db: Arc<agiworkforce_state::StateRuntime>,
    }

    impl DispatchHarness {
        async fn new() -> Self {
            let agiworkforce_home = tempfile::tempdir().expect("create temp codex home");
            let mut config = test_config();
            config.agiworkforce_home = agiworkforce_home.path().to_path_buf();
        ...
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.agiworkforce_home.as_path()),
                    "consolidation subagent should have agiworkforce_home as writable root"
                );
            }
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.agiworkforce_home);
