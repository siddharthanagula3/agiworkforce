...
pub(crate) fn curated_plugins_repo_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub(crate) fn read_curated_plugins_sha(agiworkforce_home: &Path) -> Option<String> {
    read_sha_file(agiworkforce_home.join(CURATED_PLUGINS_SHA_FILE).as_path())
}

pub(crate) fn sync_openai_plugins_repo(agiworkforce_home: &Path) -> Result<String, String> {
    sync_openai_plugins_repo_with_transport_overrides(agiworkforce_home, "git", GITHUB_API_BASE_URL)
}
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    match sync_openai_plugins_repo_via_git(agiworkforce_home, git_binary) {
        ...
        Err(err) => {
            ...
            sync_openai_plugins_repo_via_http(agiworkforce_home, api_base_url)
        }
    }
}
...
pub(super) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(agiworkforce_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(agiworkforce_home.as_path()).await {
            warn!(
                agiworkforce_home = %agiworkforce_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(agiworkforce_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}

fn startup_remote_plugin_sync_prerequisites_ready(agiworkforce_home: &Path) -> bool {
    agiworkforce_home
        .join(".tmp/plugins/.agents/plugins/marketplace.json")
        .is_file()
        && agiworkforce_home.join(".tmp/plugins.sha").is_file()
}

async fn wait_for_startup_remote_plugin_sync_prerequisites(agiworkforce_home: &Path) -> bool {
    let deadline = tokio::time::Instant::now() + STARTUP_REMOTE_PLUGIN_SYNC_PREREQUISITE_TIMEOUT;
    loop {
        if startup_remote_plugin_sync_prerequisites_ready(agiworkforce_home) {
            return true;
        }
...
async fn write_startup_remote_plugin_sync_marker(agiworkforce_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(agiworkforce_home);
