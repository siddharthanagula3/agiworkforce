...
use agiworkforce_tools::DiscoverablePluginInfo;
...
async fn list_tool_suggest_discoverable_plugins_returns_uninstalled_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(agiworkforce_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack"]);
    write_plugins_feature_config(agiworkforce_home.path());

    let config = load_plugins_config(agiworkforce_home.path()).await;
...
async fn list_tool_suggest_discoverable_plugins_deduplicates_allowlisted_configured_plugin() {
    ...
    let marketplace_root = agiworkforce_home
        .path()
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(agiworkforce_home.path());
    write_file(
        &curated_root.join("plugins/slack/.agiworkforce-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(agiworkforce_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    write_curated_plugin_sha(agiworkforce_home.path());
    write_plugins_feature_config(agiworkforce_home.path());

    PluginsManager::new(agiworkforce_home.path().to_path_buf())
    ...
    let refreshed_config = load_plugins_config(agiworkforce_home.path()).await;
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["build-ios-apps", "life-science-research"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.agiworkforce-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("build-ios-apps/.agiworkforce-plugin/plugin.json")
            .count(),
        1
    );
    assert_eq!(
        normalized_logs
            .matches("life-science-research/.agiworkforce-plugin/plugin.json")
            .count(),
        1
    );
}
