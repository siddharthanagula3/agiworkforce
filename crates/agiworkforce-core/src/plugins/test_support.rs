...
pub(crate) fn write_curated_plugin_sha(agiworkforce_home: &Path) {
    write_curated_plugin_sha_with(agiworkforce_home, TEST_CURATED_PLUGIN_SHA);
}
...
pub(crate) async fn load_plugins_config(agiworkforce_home: &Path) -> crate::config::Config {
    ConfigBuilder::default()
        .agiworkforce_home(agiworkforce_home.to_path_buf())
        .fallback_cwd(Some(agiworkforce_home.to_path_buf()))
        .build()
        .await
