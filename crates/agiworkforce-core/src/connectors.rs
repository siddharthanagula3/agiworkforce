...
// Re-export for downstream crates that historically imported these from
// `agiworkforce_core::connectors::*`.
pub use agiworkforce_connectors::filter::filter_disallowed_connectors;
pub use agiworkforce_connectors::merge::merge_connectors;
pub use agiworkforce_connectors::metadata::connector_display_label;
pub use agiworkforce_connectors::metadata::connector_install_url;

/// Stub: union plugin-supplied connector ids onto an existing connector list.
///
/// The full implementation lives upstream as part of the chatgpt module; this
/// no-op variant unblocks builds that import `merge_plugin_apps` and will be
/// replaced with the real merge once the chatgpt crate is fully wired.
pub fn merge_plugin_apps(
    connectors: Vec<agiworkforce_app_server_protocol::AppInfo>,
    _plugin_apps: Vec<crate::plugins::AppConnectorId>,
) -> Vec<agiworkforce_app_server_protocol::AppInfo> {
    connectors
}
...
use agiworkforce_config::types::AppsConfigToml;
...
use agiworkforce_login::default_client::create_client;
...
use agiworkforce_mcp::ToolPluginProvenance;
...
pub struct AccessibleConnectorsStatus {
    ...
    pub agiworkforce_apps_ready: bool,
}
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let discoverable_connectors =
        agiworkforce_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref())
    {
        return Some(Vec::new());
    }
    let cache_key = accessible_connectors_cache_key(config, auth.as_ref());
    read_cached_accessible_connectors(&cache_key).map(|connectors| {
        agiworkforce_connectors::filter::filter_disallowed_connectors(
            connectors,
            originator().value.as_str(),
        )
    })
...
pub(crate) fn refresh_accessible_connectors_cache_from_mcp_tools(
    ...
    let accessible_connectors = agiworkforce_connectors::filter::filter_disallowed_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    {
        let cached_connectors = agiworkforce_connectors::filter::filter_disallowed_connectors(
            cached_connectors,
            originator().value.as_str(),
        );
    ...
    let mcp_servers = with_agiworkforce_apps_mcp(HashMap::new(), auth.as_ref(), &mcp_config);
    ...
    let (mut mcp_connection_manager, cancel_token) = McpConnectionManager::new(
        ...
        agiworkforce_apps_tools_cache_key(auth.as_ref()),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_agiworkforce_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{AGIWORKFORCE_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let agiworkforce_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(AGIWORKFORCE_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(AGIWORKFORCE_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(AGIWORKFORCE_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if agiworkforce_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(AgiworkforceAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(AgiworkforceAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + agiworkforce_connectors::CONNECTORS_CACHE_TTL,
        connectors: connectors.to_vec(),
    });
}

async fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.agiworkforce_home.to_path_buf())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    let Some(auth) = auth.filter(|auth| auth.uses_agiworkforce_backend()) else {
        return Ok(Vec::new());
    };
    ...
    agiworkforce_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
...
pub(crate) fn accessible_connectors_from_mcp_tools(
    ...
    let tools = mcp_tools.values().filter_map(|tool| {
        if tool.server_name != AGIWORKFORCE_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(agiworkforce_connectors::accessible::AccessibleConnectorTool {
    ...
    agiworkforce_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn agiworkforce_app_tool_is_enabled(config: &Config, tool_info: &ToolInfo) -> bool {
    if tool_info.server_name != AGIWORKFORCE_APPS_MCP_SERVER_NAME {
        return true;
    }
