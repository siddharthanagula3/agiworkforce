...
pub(crate) enum GoalRuntimeEvent<'a> {
    ...
    ExternalSet {
        status: agiworkforce_state::ThreadGoalStatus,
    },
    ExternalClear,
    ThreadResumed,
}
...
impl Session {
    ...
    pub(crate) fn goal_runtime_apply<'a>(
        ...
        match event {
            ...
            GoalRuntimeEvent::ToolCompleted {
                ...
                if tool_name != agiworkforce_tools::UPDATE_GOAL_TOOL_NAME {
    ...
    pub(crate) async fn set_thread_goal(
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            agiworkforce_state::ThreadGoalAccountingMode::ActiveOnly,
        )
        .await?;
        ...
        let goal = if let Some(objective) = objective.as_deref() {
            ...
            let same_nonterminal_goal = existing_goal.as_ref().is_some_and(|goal| {
                goal.objective == objective
                    && goal.status != agiworkforce_state::ThreadGoalStatus::Complete
            });
            if same_nonterminal_goal {
                    ...
                    .update_thread_goal(
                        ...
                        agiworkforce_state::ThreadGoalUpdate {
                            status: status
                                ...
                                .or(Some(agiworkforce_state::ThreadGoalStatus::Active)),
                            token_budget,
                ...
                state_db
                    .replace_thread_goal(
                        ...
                        status
                            .map(state_goal_status_from_protocol)
                            .unwrap_or(agiworkforce_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == agiworkforce_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != agiworkforce_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != agiworkforce_state::ThreadGoalStatus::Active {
    ...
    async fn apply_external_thread_goal_status(
        ...
        match status {
                ...
                match self.state_db_for_thread_goals().await {
                    Ok(Some(state_db)) => {
                        match state_db.get_thread_goal(self.conversation_id).await {
                            Ok(Some(goal))
                                if goal.status == agiworkforce_state::ThreadGoalStatus::Active =>
                            {
                                let turn_id = self
                                    .active_turn_context()
                                    .await
            ...
            agiworkforce_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            agiworkforce_state::ThreadGoalStatus::Paused | agiworkforce_state::ThreadGoalStatus::Complete => {
                self.clear_stopped_thread_goal_runtime_state().await;
            }
        }
    }
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            agiworkforce_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    agiworkforce_state::ThreadGoalStatus::Active => false,
                    agiworkforce_state::ThreadGoalStatus::BudgetLimited => {
                        matches!(budget_limit_steering, BudgetLimitSteering::Suppressed)
                    }
                    agiworkforce_state::ThreadGoalStatus::Paused
                    | agiworkforce_state::ThreadGoalStatus::Complete => true,
                };
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
            ...
            agiworkforce_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == agiworkforce_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != agiworkforce_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        ...
        mode: agiworkforce_state::ThreadGoalAccountingMode,
            ...
            .await?
        {
            ...
            agiworkforce_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
    ...
    async fn pause_active_thread_goal_for_interrupt(&self) -> anyhow::Result<()> {
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            agiworkforce_state::ThreadGoalAccountingMode::ActiveStatusOnly,
        )
        .await?;
    ...
    async fn activate_paused_thread_goal_after_resume(&self) -> anyhow::Result<bool> {
        ...
        if goal.status != agiworkforce_state::ThreadGoalStatus::Paused {
            let goal_id = goal.goal_id.clone();
            let is_active = goal.status == agiworkforce_state::ThreadGoalStatus::Active;
            if is_active {
                self.goal_runtime
                    .accounting
                    .lock()
        ...
        let Some(goal) = state_db
            .update_thread_goal(
                self.conversation_id,
                agiworkforce_state::ThreadGoalUpdate {
                    status: Some(agiworkforce_state::ThreadGoalStatus::Active),
                    token_budget: None,
            ...
            Ok(Some(state_db)) => match state_db.get_thread_goal(self.conversation_id).await {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == agiworkforce_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
...
pub(crate) fn protocol_goal_from_state(goal: agiworkforce_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
pub(crate) fn protocol_goal_status_from_state(
    ...
    match status {
        agiworkforce_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        agiworkforce_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        agiworkforce_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        agiworkforce_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

pub(crate) fn state_goal_status_from_protocol(
    status: ThreadGoalStatus,
) -> agiworkforce_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => agiworkforce_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => agiworkforce_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::BudgetLimited => agiworkforce_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => agiworkforce_state::ThreadGoalStatus::Complete,
    }
}
