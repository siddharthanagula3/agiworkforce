...
fn disabled_user_turn(test: &TestAgiworkforce, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestAgiworkforce, rel_path: &str) -> anyhow::Result<PathBuf> {
...
async fn view_image_tool_returns_unsupported_message_for_text_only_model() -> anyhow::Result<()> {
    ...
    let test = builder.build_remote_aware(&server).await?;
    let TestAgiworkforce { codex, .. } = &test;
