...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAgiworkforce) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Agiworkforce helper binary
...
impl Expectation {
    fn verify(&self, test: &TestAgiworkforce, result: &CommandResult) -> Result<()> {
        match self {
...
async fn wait_for_completion_without_approval(test: &TestAgiworkforce) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestAgiworkforce) {
...
async fn wait_for_spawned_thread(test: &TestAgiworkforce) -> Result<Arc<AgiworkforceThread>> {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/agiworkforce-fallback-rule-test.txt && echo hello > /tmp/agiworkforce-fallback-rule-test.txt";
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://agiworkforce-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
