...
use anyhow::Result;
use agiworkforce_core::compact::SUMMARY_PREFIX;
...
use core_test_support::test_codex::TestAgiworkforceBuilder;
...
fn remote_realtime_test_agiworkforce_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAgiworkforceBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_codex()
        .with_auth(AgiworkforceAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(codex: &agiworkforce_core::AgiworkforceThread) -> Result<()> {
    codex
...
async fn close_realtime_conversation(codex: &agiworkforce_core::AgiworkforceThread) -> Result<()> {
...
async fn wait_for_turn_complete(codex: &agiworkforce_core::AgiworkforceThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAgiworkforceHarness::with_builder(
        test_codex().with_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_codex().with_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let new_thread = test
        ...
        .start_thread_with_tools(
            test.config.clone(),
            agiworkforce_core::thread_store_from_config(&test.config),
            dynamic_tools,
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "agiworkforce_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "agiworkforce_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAgiworkforceHarness::with_builder(
        test_codex()
            .with_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    let override_harness = TestAgiworkforceHarness::with_builder(
        test_codex()
            .with_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_agiworkforce_builder(&realtime_server).with_config(|config| {
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_agiworkforce_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_agiworkforce_builder(&realtime_server);
    let test = builder.build(&server).await?;
