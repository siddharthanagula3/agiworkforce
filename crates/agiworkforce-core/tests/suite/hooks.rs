...
use agiworkforce_protocol::items::parse_hook_prompt_fragment;
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_config(|config| {
            config
                .features
                .enable(Feature::AgiworkforceHooks)
    ...
    let hook_inputs = read_stop_hook_inputs(test.agiworkforce_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.agiworkforce_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(
        hook_inputs[0]
            .get("transcript_path")
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.agiworkforce_home_path())?;
...
"#,
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://agiworkforce-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
    let args = serde_json::json!({ "command": command });
    ...
    assert_single_permission_request_hook_input(
        test.agiworkforce_home_path(),
        command,
        Some("network-access http://agiworkforce-network-test.invalid:80"),
    )?;
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.agiworkforce_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn plugin_pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    fs::create_dir_all(plugin_root.join(".agiworkforce-plugin"))
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.agiworkforce_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn pre_tool_use_does_not_fire_for_plan_tool() -> Result<()> {
    ...
    let hook_log_path = test.agiworkforce_home_path().join("pre_tool_use_hook_log.jsonl");
    assert!(
...
async fn post_tool_use_records_additional_context_for_shell_command() -> Result<()> {
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.agiworkforce_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.agiworkforce_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.agiworkforce_home_path())?;
...
async fn post_tool_use_does_not_fire_for_plan_tool() -> Result<()> {
    ...
    let hook_log_path = test.agiworkforce_home_path().join("post_tool_use_hook_log.jsonl");
    assert!(
        !hook_log_path.exists(),
