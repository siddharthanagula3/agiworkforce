...
pub async fn apply_patch_harness() -> Result<TestAgiworkforceHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAgiworkforceBuilder) -> TestAgiworkforceBuilder,
) -> Result<TestAgiworkforceHarness> {
    ...
    Box::pin(TestAgiworkforceHarness::with_remote_aware_builder(builder)).await
}

async fn submit_without_wait(harness: &TestAgiworkforceHarness, prompt: &str) -> Result<()> {
...
pub async fn mount_apply_patch(
    harness: &TestAgiworkforceHarness,
    call_id: &str,
    patch: &str,
    assistant_msg: &str,
...
async fn apply_patch_cli_uses_agiworkforce_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let agiworkforce_linux_sandbox_exe = harness
        .test()
        .config
        .agiworkforce_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include agiworkforce-linux-sandbox helper");
    assert_eq!(
        agiworkforce_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&agiworkforce_protocol::protocol::FileChange::Add {
            content: "hello\n".to_string(),
        })
    );
    assert_eq!(
