...
async fn agiworkforce_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    agiworkforce_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agiworkforce_returns_json_result_for_gpt5_codex() -> anyhow::Result<()> {
    agiworkforce_returns_json_result("gpt-5.4".to_string()).await
}

async fn agiworkforce_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("agiworkforce_output_schema".into()))
    ...
    let TestAgiworkforce { codex, cwd, .. } = test_codex().build(&server).await?;
