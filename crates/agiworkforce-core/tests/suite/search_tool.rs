...
use agiworkforce_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
const CALENDAR_CREATE_TOOL: &str = "mcp__agiworkforce_apps__calendar_create_event";
const CALENDAR_LIST_TOOL: &str = "mcp__agiworkforce_apps__calendar_list_events";
const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__agiworkforce_apps__calendar";
...
fn configured_builder(apps_base_url: String) -> TestAgiworkforceBuilder {
    test_codex()
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    assert_eq!(
        ...
        McpInvocation {
            server: "agiworkforce_apps".to_string(),
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_agiworkforce_apps"),
        Some(&json!({
            "call_id": "calendar-call-1",
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-agiworkforce-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-agiworkforce-turn-metadata/turn_id")
            .and_then(Value::as_str)
            .is_some_and(|turn_id| !turn_id.is_empty()),
        "apps tools/call should include turn metadata turn_id: {apps_tool_call:?}"
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-agiworkforce-turn-metadata/turn_started_at_unix_ms")
    ...
    let first_request_turn_metadata: Value = serde_json::from_str(
        &requests[0]
            .header("x-agiworkforce-turn-metadata")
...
async fn tool_search_returns_deferred_dynamic_tool_and_routes_follow_up_call() -> Result<()> {
    ...
    let new_thread = base_test
        ...
        .start_thread_with_tools(
            ...
            agiworkforce_core::thread_store_from_config(&base_test.config),
            vec![dynamic_tool],
    ...
    assert_eq!(request.namespace.as_deref(), Some("agiworkforce_app"));
    ...
    assert_eq!(
        ...
        vec![json!({
            "type": "namespace",
            "name": "agiworkforce_app",
            "description": "Tools in the agiworkforce_app namespace.",
            "tools": [{
                "type": "function",
                "name": tool_name,
