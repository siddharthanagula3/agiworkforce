...
use agiworkforce_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use agiworkforce_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use agiworkforce_core::ModelClient;
use agiworkforce_core::ModelClientSession;
use agiworkforce_core::Prompt;
use agiworkforce_core::ResponseEvent;
use agiworkforce_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(agiworkforce_login::default_client::get_agiworkforce_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-agiworkforce-installation-id"].as_str(),
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-agiworkforce-primary-used-percent": "100.0",
            "x-agiworkforce-secondary-used-percent": "87.5",
            "x-agiworkforce-primary-over-secondary-limit-percent": "95.0",
            "x-agiworkforce-primary-window-minutes": "15",
            "x-agiworkforce-secondary-window-minutes": "60"
        }
    });
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-agiworkforce-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-agiworkforce-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-agiworkforce-turn-metadata"]
            .as_str()
