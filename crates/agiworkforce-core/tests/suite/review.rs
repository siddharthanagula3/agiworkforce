...
use agiworkforce_core::REVIEW_PROMPT;
use agiworkforce_core::config::Config;
use agiworkforce_core::review_format::render_review_output_text;
use agiworkforce_protocol::models::ContentItem;
use agiworkforce_protocol::models::ResponseItem;
use agiworkforce_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use agiworkforce_protocol::protocol::ReviewCodeLocation;
use agiworkforce_protocol::protocol::ReviewFinding;
use agiworkforce_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Agiworkforce sandbox network restrictions.
    skip_if_no_network!();
    ...
    let codex = new_conversation_for_server(&server, agiworkforce_home.clone(), |_| {}).await;
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codex = new_conversation_for_server(&server, agiworkforce_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = agiworkforce_home.path().join("resume.jsonl");
    {
        ...
        let user = agiworkforce_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = agiworkforce_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codex =
        resume_conversation_for_server(&server, agiworkforce_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let codex = new_conversation_for_server(&server, agiworkforce_home.clone(), move |config| {
...
async fn new_conversation_for_server<F>(
    server: &MockServer,
    agiworkforce_home: Arc<TempDir>,
    mutator: F,
) -> Arc<AgiworkforceThread>
where
    F: FnOnce(&mut Config) + Send + 'static,
{
    let base_url = format!("{}/v1", server.uri());
    let mut builder = test_codex()
        .with_home(agiworkforce_home)
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, agiworkforce_home, resume_path)
        .await
