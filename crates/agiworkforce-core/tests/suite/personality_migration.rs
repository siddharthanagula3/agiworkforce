...
use agiworkforce_core::SESSIONS_SUBDIR;
use agiworkforce_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use agiworkforce_core::personality_migration::PersonalityMigrationStatus;
use agiworkforce_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(agiworkforce_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(agiworkforce_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(agiworkforce_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = agiworkforce_home
...
async fn write_archived_session_with_user_event(agiworkforce_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = agiworkforce_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(agiworkforce_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = agiworkforce_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
