...
use agiworkforce_core::ModelClient;
use agiworkforce_core::NewThread;
use agiworkforce_core::Prompt;
use agiworkforce_core::ResponseEvent;
...
use agiworkforce_protocol::models::LocalShellAction;
use agiworkforce_protocol::models::LocalShellExecAction;
use agiworkforce_protocol::models::LocalShellStatus;
use agiworkforce_protocol::models::MessagePhase;
use agiworkforce_protocol::models::ReasoningItemContent;
use agiworkforce_protocol::models::ReasoningItemReasoningSummary;
use agiworkforce_protocol::models::ResponseItem;
use agiworkforce_protocol::models::WebSearchAction;
...
use core_test_support::test_codex::TestAgiworkforce;
...
/// Writes an `auth.json` into the provided `agiworkforce_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        agiworkforce_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match agiworkforce_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path()) {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = agiworkforce_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = agiworkforce_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![agiworkforce_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = agiworkforce_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Agiworkforce to resume from our file
    let agiworkforce_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(agiworkforce_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, agiworkforce_home, session_path.clone())
        .await
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_codex().with_auth(AgiworkforceAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.agiworkforce_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-agiworkforce-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let mut config = load_default_config_for_test(&agiworkforce_home).await;
    ...
    let model_info =
        agiworkforce_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(AgiworkforceAuth::from_api_key(
            "unused-api-key",
        ))),
        conversation_id,
    ...
    let mut stream = client_session
        .stream(
            ...
            &agiworkforce_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_codex()
        .with_auth(AgiworkforceAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_codex()
        .with_auth(create_dummy_agiworkforce_auth())
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match AgiworkforceAuth::from_auth_storage(
    ...
    .await
    {
        Ok(Some(auth)) => agiworkforce_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No AgiworkforceAuth found in agiworkforce_home"),
        Err(e) => panic!("Failed to load AgiworkforceAuth: {e}"),
    };
...
async fn skills_append_to_developer_message() {
    ...
    let agiworkforce_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = agiworkforce_home.path().join("skills/demo");
    ...
    let agiworkforce_home_path = agiworkforce_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(agiworkforce_home.clone())
        .with_auth(AgiworkforceAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = agiworkforce_home_path.abs();
        });
    let codex = builder
        .build(&server)
        .await
    ...
    let _agiworkforce_home_guard = agiworkforce_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let agiworkforce_home_parent = TempDir::new().unwrap();
    let long_home_parent = agiworkforce_home_parent
        .path()
        .join("agiworkforce-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let agiworkforce_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = agiworkforce_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            config.cwd = agiworkforce_home_path.abs();
            let user_config_path = agiworkforce_home_path.join("config.toml").abs();
    ...
    let _agiworkforce_home_guard = agiworkforce_home;
    let _agiworkforce_home_parent_guard = agiworkforce_home_parent;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAgiworkforce { codex, .. } = test_codex()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAgiworkforce { codex, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let conversation_id = ThreadId::new();
    let auth_manager =
        agiworkforce_core::test_support::auth_manager_from_auth(AgiworkforceAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        conversation_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-agiworkforce-primary-used-percent", "12.5")
        .insert_header("x-agiworkforce-secondary-used-percent", "40.0")
        .insert_header("x-agiworkforce-primary-window-minutes", "10")
        .insert_header("x-agiworkforce-secondary-window-minutes", "60")
        .insert_header("x-agiworkforce-primary-reset-at", "1704069000")
        .insert_header("x-agiworkforce-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_codex()
        .with_auth(AgiworkforceAuth::from_api_key("test"))
        .with_config(move |config| {
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-agiworkforce-primary-used-percent", "100.0")
        .insert_header("x-agiworkforce-secondary-used-percent", "87.5")
        .insert_header("x-agiworkforce-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-agiworkforce-primary-window-minutes", "15")
        .insert_header("x-agiworkforce-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_codex();
    let agiworkforce_fixture = builder.build(&server).await?;
    let codex = agiworkforce_fixture.codex.clone();
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let expected_context_window_message = AgiworkforceErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_agiworkforce_auth() -> AgiworkforceAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Agiworkforce sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
