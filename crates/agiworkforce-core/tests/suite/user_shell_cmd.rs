...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:AGIWORKFORCE_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${AGIWORKFORCE_SANDBOX:-not-set}\"""#.to_string();
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_codex::test_codex().with_config(|config| {
        ...
        config.permissions.permission_profile =
            agiworkforce_config::Constrained::allow_any(PermissionProfile::from_runtime_permissions(
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:AGIWORKFORCE_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${AGIWORKFORCE_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
