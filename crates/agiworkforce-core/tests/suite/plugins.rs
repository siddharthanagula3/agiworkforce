...
) -> Result<Arc<agiworkforce_core::AgiworkforceThread>> {
...
async fn wait_for_sample_mcp_ready(codex: &agiworkforce_core::AgiworkforceThread) -> Result<()> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let agiworkforce_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agiworkforce_home.as_ref());
    write_plugin_app_plugin(agiworkforce_home.as_ref());
    let codex = build_apps_enabled_plugin_test_codex(
        &server,
        Arc::clone(&agiworkforce_home),
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![agiworkforce_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(agiworkforce_home.as_ref());
    write_plugin_mcp_plugin(agiworkforce_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(agiworkforce_home.as_ref());

    let codex =
        build_apps_enabled_plugin_test_codex(&server, agiworkforce_home, apps_server.chatgpt_base_url)
            .await?;
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![agiworkforce_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__agiworkforce_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__agiworkforce_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let agiworkforce_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agiworkforce_home.as_ref());
    let codex = build_analytics_plugin_test_codex(&server, agiworkforce_home).await?;
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "agiworkforce_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(agiworkforce_login::default_client::originator().value)
    );
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    write_plugin_mcp_plugin(agiworkforce_home.as_ref(), &rmcp_test_server_bin);
    let codex = build_plugin_test_codex(&server, agiworkforce_home).await?;
    wait_for_sample_mcp_ready(&codex).await?;
