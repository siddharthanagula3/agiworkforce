...
) -> Result<(TestAgiworkforce, ResponseMock)> {
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__agiworkforce_apps__calendar_timezone_option_99"
);
if (!tool) {
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "agiworkforce_app_hidden_dynamic_tool");
const out = await tools.agiworkforce_app_hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("agiworkforce_app_hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("A hidden dynamic tool.")
                    ...
                    && description.contains("agiworkforce_app_hidden_dynamic_tool(args:")
            })
    );
