...
fn enable_trusted_project(config: &mut agiworkforce_core::config::Config) {
...
async fn submit_skill_turn(test: &TestAgiworkforce, skill_path: PathBuf, prompt: &str) -> Result<()> {
...
async fn live_skills_reload_refreshes_skill_cache_after_skill_change() -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(test.agiworkforce_home_path().join("skills/demo/SKILL.md"))?;
    ...
    write_skill(test.agiworkforce_home_path(), "demo", "demo skill", skill_v2);
