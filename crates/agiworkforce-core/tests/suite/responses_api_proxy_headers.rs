...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-agiworkforce-window-id"))?;
    let child_window_id = child
        .header("x-agiworkforce-window-id")
        .ok_or_else(|| anyhow!("child request missing x-agiworkforce-window-id"))?;
    ...
    assert_eq!(
        child.header("x-agiworkforce-parent-thread-id").as_deref(),
...
async fn submit_turn_with_timeout(test: &TestAgiworkforce, prompt: &str) -> Result<()> {
