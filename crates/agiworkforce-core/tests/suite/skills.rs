...
fn write_home_skill(agiworkforce_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = agiworkforce_home.join("skills").join(dir);
...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    core_test_support::wait_for_event(test.codex.as_ref(), |event| {
        matches!(event, agiworkforce_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn list_skills_includes_repo_and_home_skills_remote_aware() -> Result<()> {
    ...
    let response =
        core_test_support::wait_for_event_match(test.codex.as_ref(), |event| match event {
            agiworkforce_protocol::protocol::EventMsg::ListSkillsResponse(response) => {
                Some(response.clone())
            }
            _ => None,
        })
    ...
    assert_eq!(repo_skill.scope, agiworkforce_protocol::protocol::SkillScope::Repo);
    ...
    assert_eq!(home_skill.scope, agiworkforce_protocol::protocol::SkillScope::User);
...
async fn list_skills_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let thread_manager = ThreadManager::new(
        &config,
        agiworkforce_core::test_support::auth_manager_from_auth(AgiworkforceAuth::from_api_key("dummy")),
        SessionSource::Exec,
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.agiworkforce_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, agiworkforce_protocol::protocol::SkillScope::System);
