...
use agiworkforce_core::config::AgentRoleConfig;
...
async fn wait_for_spawned_thread_id(test: &TestAgiworkforce) -> Result<String> {
...
) -> Result<(TestAgiworkforce, String)> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_codex::TestAgiworkforceBuilder,
    ) -> core_test_support::test_codex::TestAgiworkforceBuilder,
) -> Result<(TestAgiworkforce, String)> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.agiworkforce_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
