...
const REMOTE_MODEL_SLUG: &str = "agiworkforce-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Agiworkforce".to_string(),
    ...
    let manager = agiworkforce_core::test_support::models_manager_with_provider(
        agiworkforce_home.path().to_path_buf(),
        agiworkforce_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-agiworkforce-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-agiworkforce-test");
...
async fn remote_models_long_model_slug_is_sent_with_high_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-agiworkforce-test";
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "agiworkforce-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "agiworkforce-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "agiworkforce-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    agiworkforce_core::test_support::all_model_presets()
        .iter()
