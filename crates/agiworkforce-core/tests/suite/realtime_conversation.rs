...
use agiworkforce_login::OPENAI_API_KEY_ENV_VAR;
...
use agiworkforce_protocol::protocol::RealtimeNoopRequested;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "AGIWORKFORCE_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in agiworkforce_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let mut metadata_builder = agiworkforce_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.agiworkforce_error_info, Some(AgiworkforceErrorInfo::BadRequest));
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(AgiworkforceAuth::from_api_key("dummy")),
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("agiworkforce-rs/core"))?;
    ...
    assert!(startup_context.contains("agiworkforce-rs/"));
