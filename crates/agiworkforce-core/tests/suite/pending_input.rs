...
async fn build_codex(server: &StreamingSseServer) -> Arc<AgiworkforceThread> {
    test_codex()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming Agiworkforce test session: {err}"))
        .codex
}

async fn submit_user_input(codex: &AgiworkforceThread, text: &str) {
    codex
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestAgiworkforce, text: &str) {
...
async fn steer_user_input(codex: &AgiworkforceThread, text: &str) {
    codex
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(codex: &AgiworkforceThread, text: &str) {
    codex
...
async fn wait_for_reasoning_item_started(codex: &AgiworkforceThread) {
    wait_for_event(codex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(codex: &AgiworkforceThread, text: &str) {
...
async fn wait_for_turn_complete(codex: &AgiworkforceThread) {
