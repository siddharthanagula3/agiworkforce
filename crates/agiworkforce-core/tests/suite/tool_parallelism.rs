...
async fn run_turn(test: &TestAgiworkforce, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestAgiworkforce, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_agiworkforce_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestAgiworkforce> {
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_agiworkforce_with_test_tool(&server).await?;
