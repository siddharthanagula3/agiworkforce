...
use agiworkforce_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let NewThread {
        thread: agiworkforce_fork1,
        ..
    } = thread_manager
        .fork_thread(
            ForkSnapshot::TruncateBeforeNthUserMessage(1),
            config_for_fork.clone(),
            agiworkforce_core::thread_store_from_config(&config_for_fork),
            base_path.clone(),
    ...
    let fork1_path = agiworkforce_fork1.rollout_path().expect("rollout path");
    ...
    let fork2_path = agiworkforce_fork2.rollout_path().expect("rollout path");
