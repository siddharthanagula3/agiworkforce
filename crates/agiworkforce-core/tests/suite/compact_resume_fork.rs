...
//! request payload that Agiworkforce would send to the model and assert that the
...
use agiworkforce_core::compact::SUMMARIZATION_PROMPT;
...
fn network_disabled() -> bool {
    std::env::var(AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<AgiworkforceThread>) {
...
async fn user_turn(conversation: &Arc<AgiworkforceThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<AgiworkforceThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<AgiworkforceThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = agiworkforce_core::test_support::auth_manager_from_auth(
        agiworkforce_login::AgiworkforceAuth::from_api_key("dummy"),
    );
