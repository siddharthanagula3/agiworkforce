...
use chrono::Utc;
use agiworkforce_core::EventPersistenceMode;
use agiworkforce_core::RolloutRecorder;
use agiworkforce_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(agiworkforce_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = agiworkforce_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(agiworkforce_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(agiworkforce_home, "sessions", id)
}

async fn upsert_thread_metadata(agiworkforce_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(agiworkforce_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
...
#[tokio::test]
async fn find_handles_gitignore_covering_agiworkforce_home_directory() {
    let repo = TempDir::new().unwrap();
    let agiworkforce_home = repo.path().join(".codex");
    std::fs::create_dir_all(&agiworkforce_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".codex/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&agiworkforce_home, id);

    let found = find_thread_path_by_id_str(&agiworkforce_home, &id.to_string())
        .await
        .unwrap();
