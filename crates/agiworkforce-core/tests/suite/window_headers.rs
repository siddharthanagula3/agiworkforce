...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let forked = resumed
        ...
        .fork_thread(
            ...
            agiworkforce_core::thread_store_from_config(&resumed.config),
            rollout_path,
...
async fn submit_user_turn(codex: &Arc<AgiworkforceThread>, text: &str) -> Result<()> {
    codex
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(codex: &Arc<AgiworkforceThread>) -> Result<()> {
...
async fn shutdown_thread(codex: &Arc<AgiworkforceThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-agiworkforce-window-id")
        .expect("missing x-agiworkforce-window-id header");
