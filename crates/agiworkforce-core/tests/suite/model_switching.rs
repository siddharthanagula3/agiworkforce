...
fn read_only_user_turn(test: &TestAgiworkforce, items: Vec<UserInput>, model: String) -> Op {
...
fn image_generation_artifact_path(agiworkforce_home: &Path, session_id: &str, call_id: &str) -> PathBuf {
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-agiworkforce-personality";
...
async fn model_change_from_image_to_text_strips_prior_image_content() -> Result<()> {
    ...
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &agiworkforce_protocol::models::image_open_tag_text()),
        "second request should preserve the image open tag text"
    );
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &agiworkforce_protocol::models::image_close_tag_text()),
