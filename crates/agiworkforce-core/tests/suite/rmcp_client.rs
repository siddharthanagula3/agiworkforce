...
use agiworkforce_exec_server::HttpRequestParams;
use agiworkforce_login::AgiworkforceAuth;
use agiworkforce_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use agiworkforce_utils_cargo_bin::cargo_bin;
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/agiworkforce-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn wait_for_mcp_tool(fixture: &TestAgiworkforce, tool_name: &str) -> anyhow::Result<()> {
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: agiworkforce_protocol::openai_models::ReasoningEffort::Medium,
    ...
    let fixture = test_codex()
        .with_auth(AgiworkforceAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
                server_name,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Agiworkforce should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Agiworkforce can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Agiworkforce will call the MCP echo tool
    ...
    // Phase 3: configure Agiworkforce with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Agiworkforce begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in AGIWORKFORCE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire AGIWORKFORCE_HOME through the code.
#[test]
#[serial(agiworkforce_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Agiworkforce will call the OAuth-backed
    ...
    // Phase 3: seed an isolated AGIWORKFORCE_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _agiworkforce_home_guard = EnvVarGuard::set("AGIWORKFORCE_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Agiworkforce with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Agiworkforce begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Agiworkforce fixture.
