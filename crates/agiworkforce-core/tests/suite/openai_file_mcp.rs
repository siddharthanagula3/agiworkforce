...
const DOCUMENT_EXTRACT_NAMESPACE: &str = "mcp__agiworkforce_apps__calendar";
const DOCUMENT_EXTRACT_TOOL: &str = "_extract_text";
const DOCUMENT_EXTRACT_HOOK_MATCHER: &str = "mcp__agiworkforce_apps__calendar_extract_text";
...
async fn agiworkforce_apps_file_params_upload_local_paths_before_mcp_tool_call() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            ...
            if let Err(err) = config.features.enable(Feature::AgiworkforceHooks) {
                panic!("test config should allow feature update: {err}");
            }
        });
