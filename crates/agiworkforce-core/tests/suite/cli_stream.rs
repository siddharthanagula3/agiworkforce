...
use agiworkforce_utils_cargo_bin::find_resource;
...
fn repo_root() -> std::path::PathBuf {
    ...
    agiworkforce_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("AGIWORKFORCE_HOME", home.path())
    ...
    //     10,
    //     None,
    //     agiworkforce_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Agiworkforce to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// 2. Configures codex to read from this fixture via AGIWORKFORCE_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("AGIWORKFORCE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("AGIWORKFORCE_RS_SSE_FIXTURE", fixture);
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("AGIWORKFORCE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("AGIWORKFORCE_RS_SSE_FIXTURE", &fixture);

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume agiworkforce-cli run failed");
