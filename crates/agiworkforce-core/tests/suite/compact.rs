...
async fn assert_compaction_uses_turn_lifecycle_id(codex: &std::sync::Arc<agiworkforce_core::AgiworkforceThread>) {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Agiworkforce.
    ...
    // Shut down Agiworkforce to flush rollout entries before inspecting the file.
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        agiworkforce_protocol::models::ResponseItem::Compaction {
