...
fn skill_script_command(test: &TestAgiworkforce, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestAgiworkforce) -> Option<ExecApprovalRequestEvent> {
...
async fn wait_for_turn_complete(test: &TestAgiworkforce) {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/agiworkforce-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
