...
const SNAPSHOT_MARKER_VAR: &str = "AGIWORKFORCE_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(agiworkforce_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = agiworkforce_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let agiworkforce_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&agiworkforce_home).await?;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.agiworkforce_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestAgiworkforceHarness::with_builder(builder).await?;
    let agiworkforce_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(agiworkforce_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let agiworkforce_home = home.path().to_path_buf();
