...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let agiworkforce_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-agiworkforce-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-agiworkforce-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-agiworkforce-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(request.header("x-agiworkforce-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        ...
        .expect("x-agiworkforce-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-agiworkforce-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-agiworkforce-turn-metadata"
    );
