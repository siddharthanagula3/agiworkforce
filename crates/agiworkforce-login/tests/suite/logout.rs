...
use agiworkforce_login::REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR;
use agiworkforce_login::logout_with_revoke;
use agiworkforce_login::save_auth;
use agiworkforce_login::token_data::IdTokenInfo;
...
async fn logout_with_revoke_revokes_refresh_token_then_removes_auth() -> Result<()> {
    ...
    let removed = logout_with_revoke(agiworkforce_home.path(), AuthCredentialsStoreMode::File).await?;
