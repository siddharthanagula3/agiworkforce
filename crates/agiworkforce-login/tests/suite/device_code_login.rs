...
use agiworkforce_login::auth::load_auth_dot_json;
use agiworkforce_login::run_device_code_login;
use serde_json::json;
use std::sync::Arc;
...
fn server_opts(
    agiworkforce_home: &tempfile::TempDir,
    issuer: String,
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&agiworkforce_home, issuer, AuthCredentialsStoreMode::File);

    run_device_code_login(opts)
        .await
        .expect("device code login integration should succeed");

    let auth = load_auth_dot_json(agiworkforce_home.path(), AuthCredentialsStoreMode::File)
...
async fn device_code_login_rejects_workspace_mismatch() -> anyhow::Result<()> {
    ...
    let issuer = mock_server.uri();
    let mut opts = server_opts(&agiworkforce_home, issuer, AuthCredentialsStoreMode::File);
