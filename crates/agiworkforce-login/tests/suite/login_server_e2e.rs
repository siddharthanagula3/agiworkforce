...
use agiworkforce_login::ServerOptions;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = agiworkforce_home.clone();

    let opts = ServerOptions {
        agiworkforce_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: agiworkforce_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = agiworkforce_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_agiworkforce_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let agiworkforce_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_agiworkforce_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Agiworkforce"),
        "error body should clearly explain the Agiworkforce access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_agiworkforce_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Agiworkforce to retry"),
    ...
    assert!(
        !body.contains("get access to Agiworkforce"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let mut opts = ServerOptions::new(
        ...
        agiworkforce_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_agiworkforce_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        agiworkforce_home: first_agiworkforce_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: agiworkforce_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_agiworkforce_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        agiworkforce_home: second_agiworkforce_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: agiworkforce_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
