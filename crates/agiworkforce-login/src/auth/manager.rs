...
impl AgiWorkforceAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(agiworkforce_home.to_path_buf(), storage_mode);
...
pub fn read_agiworkforce_api_key_from_env() -> Option<String> {
    env::var(AGIWORKFORCE_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(agiworkforce_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(agiworkforce_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(agiworkforce_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(agiworkforce_home, auth_credentials_store_mode)?;
