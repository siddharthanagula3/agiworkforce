...
pub(super) fn delete_file_if_exists(agiworkforce_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(agiworkforce_home: PathBuf) -> Self {
        Self { agiworkforce_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `AGIWORKFORCE_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.agiworkforce_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.agiworkforce_home)
    }
}
...
fn compute_store_key(agiworkforce_home: &Path) -> std::io::Result<String> {
    let canonical = agiworkforce_home
        .canonicalize()
        .unwrap_or_else(|_| agiworkforce_home.to_path_buf());
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.agiworkforce_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.agiworkforce_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.agiworkforce_home)?;
...
// A global in-memory store for mapping agiworkforce_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(agiworkforce_home: PathBuf) -> Self {
        Self { agiworkforce_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(agiworkforce_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(agiworkforce_home)),
