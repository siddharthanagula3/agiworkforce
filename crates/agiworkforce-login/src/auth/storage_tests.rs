...
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    ...
    let storage = FileAuthStorage::new(agiworkforce_home.path().to_path_buf());
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(agiworkforce_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let agiworkforce_home = PathBuf::from("~/.codex");

    let key = compute_store_key(agiworkforce_home.as_path())?;
...
fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    let auth_file = get_auth_file(agiworkforce_home.path());
    std::fs::write(&auth_file, "stale")?;
