...
#[test]
fn test_get_agiworkforce_user_agent() {
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("agiworkforce-tui"), true);
    assert_eq!(is_first_party_originator("agiworkforce_vscode"), true);
    assert_eq!(is_first_party_originator("Agiworkforce Something Else"), true);
    assert_eq!(is_first_party_originator("agiworkforce_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("agiworkforce_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("agiworkforce_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("agiworkforce_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Agiworkforce UA for that originator
    let expected_ua = get_agiworkforce_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "agiworkforce_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "agiworkforce_cli_rs/0.0.0 (bad_suffix)"
    );
}
