...
use agiworkforce_protocol::auth::KnownPlan as InternalKnownPlan;
...
#[tokio::test]
#[serial(agiworkforce_auth_env)]
async fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let _agent_guard = EnvVarGuard::remove(AGIWORKFORCE_AGENT_IDENTITY_ENV_VAR);
    let auth = AgiworkforceAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = AgiworkforceAuth::from_auth_dot_json(
        agiworkforce_home.path(),
...
fn write_auth_file(params: AuthFileParams, agiworkforce_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(agiworkforce_home);
...
async fn load_auth_reads_agent_identity_from_env() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let _agent_guard = EnvVarGuard::set(AGIWORKFORCE_AGENT_IDENTITY_ENV_VAR, &agent_identity);

    let chatgpt_base_url = format!("{}/backend-api", server.uri());
    let _authapi_guard =
        EnvVarGuard::set("AGIWORKFORCE_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = super::load_auth(
        agiworkforce_home.path(),
        /*enable_agiworkforce_api_key_env*/ false,
        AuthCredentialsStoreMode::File,
    ...
    let AgiworkforceAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(agiworkforce_home.path()).exists(),
...
#[tokio::test]
#[serial(agiworkforce_auth_env)]
async fn load_auth_keeps_agiworkforce_api_key_env_precedence() {
    let agiworkforce_home = tempdir().unwrap();
    let record = agent_identity_record("account-123");
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _agent_guard = EnvVarGuard::set(AGIWORKFORCE_AGENT_IDENTITY_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(AGIWORKFORCE_API_KEY_ENV_VAR, "sk-env");
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        agiworkforce_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/agiworkforce-backend/agent-identity",
...
async fn assert_agent_identity_plan_alias(
    ...
    let _authapi_guard =
        EnvVarGuard::set("AGIWORKFORCE_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = AgiworkforceAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
