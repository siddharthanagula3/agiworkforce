...
use agiworkforce_core::content_items_to_text;
use agiworkforce_core::resolve_installation_id;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<AgiworkforceThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<agiworkforce_otel::Timer> {
...
impl MemoryStartupContext {
    ...
    pub(crate) async fn stage_one_request_context(
        ...
        let turn_metadata_header =
            agiworkforce_core::build_turn_metadata_header(&config.cwd, /*sandbox*/ None).await;
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let agiworkforce_protocol::models::ResponseItem::Message { content, .. } = item
