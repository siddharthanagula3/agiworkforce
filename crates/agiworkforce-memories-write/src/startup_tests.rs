...
use agiworkforce_git_utils::diff_since_latest_init;
use agiworkforce_git_utils::reset_git_repository;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<agiworkforce_state::StateRuntime>> {
    let db =
        agiworkforce_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestAgiworkforce) {
...
async fn seed_stage1_output(
    db: &agiworkforce_state::StateRuntime,
    ...
    let mut metadata_builder = agiworkforce_state::ThreadMetadataBuilder::new(
        thread_id,
        agiworkforce_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = agiworkforce_home.join(format!("workspace-{rollout_slug}"));
...
) -> anyhow::Result<agiworkforce_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn shutdown_test_codex(test: &TestAgiworkforce) -> anyhow::Result<()> {
