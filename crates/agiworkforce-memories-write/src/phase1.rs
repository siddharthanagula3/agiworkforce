...
use agiworkforce_rollout::INTERACTIVE_SESSION_SOURCES;
use agiworkforce_rollout::should_persist_response_item_for_memories;
use agiworkforce_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<agiworkforce_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            agiworkforce_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<agiworkforce_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: agiworkforce_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> agiworkforce_protocol::error::Result<String> {
        let filtered = items
            .iter()
            .filter_map(|item| {
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            AgiworkforceErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
        ...
        let serialized =
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: agiworkforce_protocol::models::FunctionCallOutputPayload {
                        body: agiworkforce_protocol::models::FunctionCallOutputBody::Text(
