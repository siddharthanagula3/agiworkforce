...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            ...
            agiworkforce_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            agiworkforce_state::Phase2JobClaimOutcome::SkippedCooldown => {
...
mod agent {
    ...
    pub(super) fn get_config(config: &Config) -> Option<Config> {
        ...
        let consolidation_sandbox_policy = SandboxPolicy::WorkspaceWrite {
            writable_roots,
            read_only_access: agiworkforce_protocol::protocol::ReadOnlyAccess::default(),
    ...
    pub(super) fn handle(
        ...
        memory_root: agiworkforce_utils_absolute_path::AbsolutePathBuf,
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &agiworkforce_core::AgiworkforceThread,
    ) -> AgentStatus {
