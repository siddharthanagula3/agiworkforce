...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_agiworkforce_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_agiworkforce_tool_call_param(),
                create_tool_for_agiworkforce_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_tool_call_agiworkforce_session_reply(
        &self,
        ...
        let thread_id = match agiworkforce_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_agiworkforce_uuid.lock().await;
        ...
        if let Err(e) = agiworkforce_arc
            .submit_with_id(Submission {
                ...
                op: agiworkforce_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
        ...
        self.running_requests_id_to_agiworkforce_uuid
            .lock()
            .await
            .remove(&request_id);
    }
