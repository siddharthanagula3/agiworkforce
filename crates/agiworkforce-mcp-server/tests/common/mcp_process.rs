...
impl McpProcess {
    pub async fn new(agiworkforce_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(agiworkforce_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce-mcp-server")
            .context("should find binary for agiworkforce-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = agiworkforce_login::default_client::originator().value;
    ...
    pub async fn send_agiworkforce_tool_call(
        &mut self,
        params: AgiworkforceToolCallParam,
    ) -> anyhow::Result<i64> {
        let agiworkforce_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "agiworkforce".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(agiworkforce_tool_call_params)?),
        )
        .await
    }
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `agiworkforce-mcp-server` child process.
        //
