...
use agiworkforce_core::spawn::AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use agiworkforce_mcp_server::AgiworkforceToolCallParam;
use agiworkforce_mcp_server::ExecApprovalElicitRequestParams;
use agiworkforce_mcp_server::ExecApprovalResponse;
use agiworkforce_mcp_server::PatchApprovalElicitRequestParams;
use agiworkforce_mcp_server::PatchApprovalResponse;
use agiworkforce_protocol::protocol::FileChange;
use agiworkforce_protocol::protocol::ReviewDecision;
use agiworkforce_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(AGIWORKFORCE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Agiworkforce sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "agiworkforce" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let agiworkforce_request_id = mcp_process
        .send_agiworkforce_tool_call(AgiworkforceToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            agiworkforce_request_id.to_string(),
            params.agiworkforce_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    let agiworkforce_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(agiworkforce_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(agiworkforce_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    agiworkforce_mcp_tool_call_id: String,
    agiworkforce_event_id: String,
    thread_id: agiworkforce_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Agiworkforce to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let agiworkforce_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        agiworkforce_elicitation: "exec-approval".to_string(),
        agiworkforce_mcp_tool_call_id,
        agiworkforce_event_id,
        agiworkforce_command: command,
        agiworkforce_cwd: workdir.to_path_buf(),
        agiworkforce_call_id: "call1234".to_string(),
        agiworkforce_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "agiworkforce" tool request that will trigger the apply_patch command
    let agiworkforce_request_id = mcp_process
        .send_agiworkforce_tool_call(AgiworkforceToolCallParam {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_agiworkforce_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = agiworkforce_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn agiworkforce_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    let mut mcp_process = McpProcess::new(agiworkforce_home.path()).await?;
    ...
    .await??;
    assert_eq!(agiworkforce_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(agiworkforce_response.id, RequestId::Number(agiworkforce_request_id));
    assert_eq!(
        agiworkforce_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": agiworkforce_response
                    .result
...
async fn create_mcp_process(responses: Vec<String>) -> anyhow::Result<McpHandle> {
    ...
    Ok(McpHandle {
        process: mcp_process,
        server,
        dir: agiworkforce_home,
    })
}

/// Create a Agiworkforce config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(agiworkforce_home: &Path, server_uri: &str) -> std::io::Result<()> {
