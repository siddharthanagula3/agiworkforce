...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use Agiworkforce backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        let uses_agiworkforce_backend = self
            .auth_manager()
            .is_some_and(AuthManager::current_auth_uses_agiworkforce_backend);
        presets = ModelPreset::filter_by_auth(presets, uses_agiworkforce_backend);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_agiworkforce_backend().await || self.endpoint_client.has_command_auth()
    }
