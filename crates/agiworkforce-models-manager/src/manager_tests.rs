...
use agiworkforce_login::AuthCredentialsStoreMode;
use agiworkforce_login::AuthManager;
use agiworkforce_login::AgiworkforceAuth;
use agiworkforce_login::ExternalAuth;
use agiworkforce_login::ExternalAuthRefreshContext;
use agiworkforce_login::ExternalAuthTokens;
use agiworkforce_login::TokenData;
...
struct TestModelsEndpoint {
    ...
    uses_agiworkforce_backend: bool,
...
impl TestModelsEndpoint {
    fn new(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            ...
            uses_agiworkforce_backend: true,
    ...
    fn without_refresh(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            has_command_auth: false,
            uses_agiworkforce_backend: false,
            responses: Mutex::new(responses.into()),
            fetch_count: AtomicUsize::new(0),
        })
    }
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    async fn uses_agiworkforce_backend(&self) -> bool {
        self.uses_agiworkforce_backend
    }
...
fn openai_manager_for_tests(
    agiworkforce_home: std::path::PathBuf,
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(agiworkforce_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(agiworkforce_home: &Path) -> AgiworkforceAuth {
    let auth_dot_json = agiworkforce_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: agiworkforce_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(agiworkforce_home).expect("codex home should be created");
    std::fs::write(
    ...
    AgiworkforceAuth::from_auth_storage(
        agiworkforce_home,
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager = openai_manager_for_tests(agiworkforce_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = openai_manager_for_tests(agiworkforce_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(agiworkforce_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        agiworkforce_home.path().to_path_buf(),
        endpoint.clone(),
