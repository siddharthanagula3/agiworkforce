...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = agiworkforce_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("agiworkforce_cloud_tasks_tui");
    let ua = agiworkforce_login::default_client::get_agiworkforce_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("agiworkforce-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await
        && let Some(auth) = am.auth().await
        && auth.uses_agiworkforce_backend()
    {
        headers.extend(agiworkforce_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
