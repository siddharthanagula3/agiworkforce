...
pub struct AttemptView {
    ...
    pub status: agiworkforce_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: agiworkforce_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<agiworkforce_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<agiworkforce_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<agiworkforce_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    ...
    impl agiworkforce_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            &self,
            env: Option<&str>,
            limit: Option<i64>,
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: agiworkforce_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: agiworkforce_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
            ...
            Ok(agiworkforce_cloud_tasks_client::TaskListPage {
                tasks: limited,
        ...
        async fn get_task_summary(
            &self,
            id: TaskId,
        ) -> agiworkforce_cloud_tasks_client::Result<TaskSummary> {
        ...
        async fn get_task_messages(
            &self,
            _id: TaskId,
        ) -> agiworkforce_cloud_tasks_client::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn get_task_text(
            ...
            Ok(agiworkforce_cloud_tasks_client::TaskText {
                ...
                attempt_status: agiworkforce_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
