...
impl LocalSecretsBackend {
    pub fn new(agiworkforce_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            agiworkforce_home,
            keyring_store,
        }
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.agiworkforce_home.join("secrets")
    }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.agiworkforce_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(agiworkforce_home.path().to_path_buf(), keyring);
