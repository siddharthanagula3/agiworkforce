...
impl SecretsManager {
    pub fn new(agiworkforce_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(agiworkforce_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub(crate) fn compute_keyring_account(agiworkforce_home: &Path) -> String {
