...
use agiworkforce_app_server_protocol::SkillInterface;
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(agiworkforce_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(agiworkforce_home.clone())
...
fn ensure_chatgpt_auth(auth: Option<&AgiworkforceAuth>) -> Result<&AgiworkforceAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    Ok(request
        ...
        .headers(agiworkforce_model_provider::auth_provider_from_auth(auth).to_auth_headers()))
}
