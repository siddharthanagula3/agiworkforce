...
use agiworkforce_config::RemoveMarketplaceConfigOutcome;
use agiworkforce_config::remove_user_marketplace_config;
use agiworkforce_plugin::validate_plugin_segment;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || remove_marketplace_sync(agiworkforce_home.as_path(), request))
        .await
        .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(agiworkforce_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(agiworkforce_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        .unwrap();
        let installed_root = marketplace_install_root(agiworkforce_home.path()).join("debug");
        ...
        let config =
            fs::read_to_string(agiworkforce_home.path().join(agiworkforce_config::CONFIG_TOML_FILE)).unwrap();
    ...
    fn remove_marketplace_sync_keeps_installed_root_when_config_removal_fails() {
        let agiworkforce_home = TempDir::new().unwrap();
        fs::write(
            agiworkforce_home.path().join(agiworkforce_config::CONFIG_TOML_FILE),
            "[marketplaces.debug\n",
        )
        .unwrap();
        let installed_root = marketplace_install_root(agiworkforce_home.path()).join("debug");
