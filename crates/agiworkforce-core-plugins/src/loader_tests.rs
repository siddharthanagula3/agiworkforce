...
fn plugin_root() -> (tempfile::TempDir, AbsolutePathBuf) {
    ...
    fs::create_dir_all(plugin_root.join(".agiworkforce-plugin")).expect("create manifest dir");
...
fn write_manifest(plugin_root: &AbsolutePathBuf, manifest: &str) {
    fs::write(plugin_root.join(".agiworkforce-plugin/plugin.json"), manifest).expect("write manifest");
}
