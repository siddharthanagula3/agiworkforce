...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::Agiworkforce).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_agiworkforce_backend()) {
        request =
...
fn ensure_agiworkforce_backend_auth(
    auth: Option<&AgiworkforceAuth>,
