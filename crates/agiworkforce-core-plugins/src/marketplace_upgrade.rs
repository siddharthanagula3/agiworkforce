...
use agiworkforce_config::types::MarketplaceConfig;
use agiworkforce_config::types::MarketplaceSourceType;
...
pub fn upgrade_configured_git_marketplaces(
    ...
    for marketplace in marketplaces {
        match upgrade_configured_git_marketplace(agiworkforce_home, &install_root, &marketplace) {
...
fn marketplace_install_root(agiworkforce_home: &Path) -> PathBuf {
...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(agiworkforce_home, marketplace)?;
        record_user_marketplace(agiworkforce_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(agiworkforce_home, &expected.name)?;
    match current {
