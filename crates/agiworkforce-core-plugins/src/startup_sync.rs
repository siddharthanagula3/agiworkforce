...
use agiworkforce_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use agiworkforce_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
// Keep this comfortably above a normal sync attempt so we do not race another Agiworkforce process.
...
pub fn curated_plugins_repo_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(agiworkforce_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(agiworkforce_home).as_path())
}

fn curated_plugins_sha_path(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_openai_plugins_repo(agiworkforce_home: &Path) -> Result<String, String> {
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    match sync_openai_plugins_repo_via_git(agiworkforce_home, git_binary) {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(agiworkforce_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(agiworkforce_home) {
...
fn sync_openai_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(agiworkforce_home);
    let sha_path = curated_plugins_sha_path(agiworkforce_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(agiworkforce_home: &Path) -> bool {
    curated_plugins_repo_path(agiworkforce_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && agiworkforce_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
