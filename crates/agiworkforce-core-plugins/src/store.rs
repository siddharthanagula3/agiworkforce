...
impl PluginStore {
    pub fn new(agiworkforce_home: PathBuf) -> Self {
        Self::try_new(agiworkforce_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(agiworkforce_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(agiworkforce_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(agiworkforce_home.join(PLUGINS_DATA_DIR))
