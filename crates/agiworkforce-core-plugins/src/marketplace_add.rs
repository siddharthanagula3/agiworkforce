...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || add_marketplace_sync(agiworkforce_home.as_path(), request))
        .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(agiworkforce_home, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let install_root = marketplace_install_root(agiworkforce_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(agiworkforce_home, &install_root, &install_metadata)?
    {
        let marketplace_name = validate_marketplace_source_root(&existing_root)?;
        record_added_marketplace_entry(agiworkforce_home, &marketplace_name, &install_metadata)?;
    ...
    if let MarketplaceSource::Local { path } = &source {
        ...
        if find_marketplace_root_by_name(agiworkforce_home, &install_root, &marketplace_name)?.is_some() {
            return Err(MarketplaceAddError::InvalidRequest(format!(
...
mod tests {
    ...
    fn add_marketplace_sync_installs_marketplace_and_updates_config() -> Result<()> {
        ...
        let config = fs::read_to_string(agiworkforce_home.path().join(agiworkforce_config::CONFIG_TOML_FILE))?;
    ...
    fn add_marketplace_sync_rejects_sparse_checkout_for_local_directory_source() -> Result<()> {
        ...
        assert!(
            !agiworkforce_home
                .path()
                .join(agiworkforce_config::CONFIG_TOML_FILE)
                .exists()
        );
        Ok(())
    }
    ...
    fn add_marketplace_sync_treats_existing_local_directory_source_as_already_added() -> Result<()>
    {
        ...
        let first_result = add_marketplace_sync_with_cloner(agiworkforce_home.path(), request.clone(), {
            |_url, _ref_name, _sparse_paths, _destination| {
                panic!("git cloner should not be called for local marketplace sources")
            }
        })?;
        let second_result = add_marketplace_sync_with_cloner(agiworkforce_home.path(), request, {
            |_url, _ref_name, _sparse_paths, _destination| {
                panic!("git cloner should not be called for local marketplace sources")
            }
        })?;
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.agiworkforce-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.agiworkforce-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;
