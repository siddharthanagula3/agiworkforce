...
use agiworkforce_config::HooksFile;
use agiworkforce_config::types::McpServerConfig;
use agiworkforce_config::types::PluginConfig;
use agiworkforce_config::types::PluginMcpServerConfig;
use agiworkforce_core_skills::SkillMetadata;
use agiworkforce_core_skills::config_rules::SkillConfigRules;
use agiworkforce_core_skills::config_rules::resolve_disabled_skill_paths;
use agiworkforce_core_skills::config_rules::skill_config_rules_from_stack;
use agiworkforce_core_skills::loader::SkillRoot;
use agiworkforce_core_skills::loader::load_skills_from_roots;
use agiworkforce_exec_server::LOCAL_FS;
use agiworkforce_plugin::AppConnectorId;
use agiworkforce_plugin::LoadedPlugin;
use agiworkforce_plugin::PluginCapabilitySummary;
use agiworkforce_plugin::PluginHookSource;
use agiworkforce_plugin::PluginId;
use agiworkforce_plugin::PluginIdError;
use agiworkforce_plugin::PluginLoadOutcome;
...
use agiworkforce_utils_plugins::find_plugin_manifest_path;
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(agiworkforce_home.to_path_buf()).map_err(|err| err.to_string())?;
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_agiworkforce_home(
            agiworkforce_home,
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(agiworkforce_home, &source).map_err(|err| {
...
fn configured_plugins_from_agiworkforce_home(
    agiworkforce_home: &Path,
...
pub fn configured_curated_plugin_ids_from_agiworkforce_home(agiworkforce_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_agiworkforce_home(
        agiworkforce_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".agiworkforce-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(agiworkforce_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
        ...
        && oauth.contains_key("callbackPort")
    {
        warn!(
            plugin = %plugin_root.display(),
            "plugin MCP server OAuth callbackPort is ignored; Agiworkforce uses global MCP OAuth callback settings"
        );
    }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = agiworkforce_home.join("plugins/.marketplace-plugin-source-staging");
