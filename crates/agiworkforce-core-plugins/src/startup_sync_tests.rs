...
fn write_curated_plugin_sha(agiworkforce_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(agiworkforce_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(agiworkforce_home.join(".tmp")) else {
        return false;
    };
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_openai_plugins_repo_via_http(agiworkforce_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.agiworkforce-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_agiworkforce_home_tmp_dir() {
...
if [ "$1" = "clone" ]; then
  dest="$5"
  mkdir -p "$dest/.git" "$dest/.agents/plugins" "$dest/plugins/gmail/.agiworkforce-plugin"
...
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$dest/plugins/gmail/.agiworkforce-plugin/plugin.json"
  exit 0
fi
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.agiworkforce-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.agiworkforce-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    let commit_status = Command::new("git")
        ...
        .arg(&work_repo)
        .arg("-c")
        .arg("user.name=Agiworkforce Test")
        .arg("-c")
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.agiworkforce-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.agiworkforce-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.agiworkforce-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");
