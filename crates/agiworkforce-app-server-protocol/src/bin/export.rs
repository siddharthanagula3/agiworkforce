...
#[derive(Parser, Debug)]
#[command(
    about = "Generate TypeScript bindings and JSON Schemas for the Agiworkforce app-server protocol"
)]
struct Args {
...
fn main() -> Result<()> {
    let args = Args::parse();
    agiworkforce_app_server_protocol::generate_ts_with_options(
        &args.out_dir,
        args.prettier.as_deref(),
        agiworkforce_app_server_protocol::GenerateTsOptions {
