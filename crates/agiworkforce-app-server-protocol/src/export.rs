...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("agiworkforce_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("agiworkforce_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let flat_v2_bundle =
            read_json_value(&output_dir.join("agiworkforce_app_server_protocol.v2.schemas.json"))?;
