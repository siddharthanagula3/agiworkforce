...
use agiworkforce_protocol::protocol::ContextCompactedEvent;
...
impl ThreadHistoryBuilder {
    ...
    fn handle_response_item(&mut self, item: &agiworkforce_protocol::models::ResponseItem) {
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            agiworkforce_protocol::items::TurnItem::Plan(plan) => {
            ...
            agiworkforce_protocol::items::TurnItem::UserMessage(_)
            | agiworkforce_protocol::items::TurnItem::HookPrompt(_)
            | agiworkforce_protocol::items::TurnItem::AgentMessage(_)
            | agiworkforce_protocol::items::TurnItem::Reasoning(_)
            | agiworkforce_protocol::items::TurnItem::WebSearch(_)
            | agiworkforce_protocol::items::TurnItem::ImageGeneration(_)
            | agiworkforce_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &agiworkforce_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &agiworkforce_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &agiworkforce_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            agiworkforce_error_info: payload.agiworkforce_error_info.clone().map(Into::into),
...
pub fn convert_patch_changes(
    changes: &HashMap<std::path::PathBuf, agiworkforce_protocol::protocol::FileChange>,
...
fn convert_dynamic_tool_content_items(
    items: &[agiworkforce_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            agiworkforce_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            agiworkforce_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
fn map_patch_change_kind(change: &agiworkforce_protocol::protocol::FileChange) -> PatchChangeKind {
    match change {
...
fn format_file_change_diff(change: &agiworkforce_protocol::protocol::FileChange) -> String {
    match change {
        agiworkforce_protocol::protocol::FileChange::Add { content } => content.clone(),
        agiworkforce_protocol::protocol::FileChange::Delete { content } => content.clone(),
...
mod tests {
    ...
    use agiworkforce_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use agiworkforce_protocol::items::TurnItem as CoreTurnItem;
    use agiworkforce_protocol::items::UserMessageItem as CoreUserMessageItem;
    use agiworkforce_protocol::items::build_hook_prompt_message;
    use agiworkforce_protocol::models::MessagePhase as CoreMessagePhase;
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                agiworkforce_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(agiworkforce_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                agiworkforce_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                agiworkforce_error_info: Some(AgiWorkforceErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                agiworkforce_error_info: Some(
                    crate::protocol::v2::AgiWorkforceErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(agiworkforce_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
                content: vec![agiworkforce_protocol::models::ContentItem::InputText {
