use anyhow::Context;
use anyhow::Result;
use agiworkforce_app_server_protocol::generate_json_with_experimental;
use agiworkforce_app_server_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use agiworkforce_app_server_protocol::read_schema_fixture_subtree;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let json_bundle =
        agiworkforce_utils_cargo_bin::find_resource!("schema/json/agiworkforce_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/agiworkforce_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
