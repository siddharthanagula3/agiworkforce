...
use crate::test_agiworkforce::TestAgiworkforce;
use crate::test_agiworkforce::test_codex;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `agiworkforce-execve-wrapper` binary");
        return Ok(None);
    };
