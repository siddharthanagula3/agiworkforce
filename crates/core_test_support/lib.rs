...
use anyhow::ensure;
use agiworkforce_arg0::Arg0PathEntryGuard;
use agiworkforce_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
use agiworkforce_config::NetworkRequirementsToml;
...
pub use agiworkforce_utils_absolute_path::test_support::PathBufExt;
pub use agiworkforce_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    agiworkforce_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    agiworkforce_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(agiworkforce_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = agiworkforce_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("agiworkforce-rs"));
...
pub async fn load_default_config_for_test(agiworkforce_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        agiworkforce_linux_sandbox_exe: Some(
            find_agiworkforce_linux_sandbox_exe().expect("should find binary for agiworkforce-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_agiworkforce_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce-linux-sandbox")
}
...
pub async fn wait_for_event<F>(
    codex: &AgiworkforceThread,
    predicate: F,
) -> agiworkforce_protocol::protocol::EventMsg
where
    F: FnMut(&agiworkforce_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(codex, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(codex: &AgiworkforceThread, matcher: F) -> T
where
    F: Fn(&agiworkforce_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    agiworkforce_core::spawn::AGIWORKFORCE_SANDBOX_ENV_VAR
}
...
const REMOTE_ENV_ENV_VAR: &str = "AGIWORKFORCE_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    agiworkforce_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    agiworkforce_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    agiworkforce_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! agiworkforce_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_agiworkforce_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("agiworkforce-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
