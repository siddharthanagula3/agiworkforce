...
const AGIWORKFORCE_APP_PRODUCT_NAMES: [&str; 6] = [
    "AGI Workforce",
    "Codex (Dev)",
    "Codex (Agent)",
    "Codex (Nightly)",
...
pub(crate) fn resolve_machine_js_runtime() -> Option<JsRuntime> {
    resolve_js_runtime_from_candidates(
        ...
        agiworkforce_app_runtime_candidates(),
    )
}
...
pub(crate) fn resolve_js_runtime_from_candidates(
    ...
    agiworkforce_app_candidates: Vec<PathBuf>,
) -> Option<JsRuntime> {
    node_runtime.or(electron_runtime).or_else(|| {
        agiworkforce_app_candidates
            .into_iter()
...
pub(crate) fn agiworkforce_app_runtime_candidates() -> Vec<PathBuf> {
    match std::env::consts::OS {
        "macos" => {
            ...
            roots
                .into_iter()
                .flat_map(|root| {
                    AGIWORKFORCE_APP_PRODUCT_NAMES
                        .into_iter()
