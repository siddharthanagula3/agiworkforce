...
use agiworkforce_package_manager::ArchiveFormat;
...
#[test]
fn default_release_locator_uses_openai_agiworkforce_github_releases() {
...
fn load_cached_runtime_reads_installed_runtime() {
    let agiworkforce_home = TempDir::new().unwrap_or_else(|error| panic!("{error}"));
    let runtime_version = "2.5.6";
    let platform =
        ArtifactRuntimePlatform::detect_current().unwrap_or_else(|error| panic!("{error}"));
    let install_dir = agiworkforce_home
        .path()
    ...
    let runtime = load_cached_runtime(
        &agiworkforce_home.path().join(DEFAULT_CACHE_ROOT_RELATIVE),
        runtime_version,
    )
    .unwrap_or_else(|error| panic!("{error}"));
...
fn load_cached_runtime_uses_custom_cache_root() {
    let agiworkforce_home = TempDir::new().unwrap_or_else(|error| panic!("{error}"));
    let runtime_version = "2.5.6";
    let custom_cache_root = agiworkforce_home.path().join("runtime-cache");
    let platform =
        ArtifactRuntimePlatform::detect_current().unwrap_or_else(|error| panic!("{error}"));
