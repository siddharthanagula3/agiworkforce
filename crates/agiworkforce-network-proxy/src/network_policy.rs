...
const AUDIT_TARGET: &str = "agiworkforce_otel.network_proxy";
...
mod tests {
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        assert!(event.target.starts_with("agiworkforce_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        let metadata = NetworkProxyAuditMetadata {
            ...
            originator: Some("agiworkforce_cli_rs".to_string()),
        ...
        assert_eq!(event.field("originator"), Some("agiworkforce_cli_rs"));
