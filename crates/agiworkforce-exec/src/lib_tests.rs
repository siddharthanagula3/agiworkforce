...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("agiworkforce-exec-tests");
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(agiworkforce_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
fn session_configured_from_thread_response_uses_review_policy_from_response() {
    let response = ThreadStartResponse {
        ...
        approvals_reviewer: agiworkforce_app_server_protocol::ApprovalsReviewer::AutoReview,
        ...
        permission_profile: Some(
            agiworkforce_protocol::models::PermissionProfile::from_legacy_sandbox_policy(
                &agiworkforce_protocol::protocol::SandboxPolicy::new_workspace_write_policy(),
            )
            .into(),
        ),
