...
/// at some point, we must ensure that the smaller `agiworkforce-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce-exec")?)
        .arg(AGIWORKFORCE_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
