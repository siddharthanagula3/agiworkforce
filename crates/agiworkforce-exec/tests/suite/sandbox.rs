#![cfg(unix)]
use agiworkforce_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use agiworkforce_core::exec::build_exec_request;
    ...
    let agiworkforce_linux_sandbox_exe = None;
...
async fn spawn_command_under_sandbox(
    ...
    use agiworkforce_core::spawn_command_under_linux_sandbox;
    use agiworkforce_protocol::models::PermissionProfile;

    let agiworkforce_linux_sandbox_exe = core_test_support::find_agiworkforce_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_agiworkforce_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let dot_agiworkforce_metadata = tokio::fs::symlink_metadata(&dot_codex).await;
    if let Ok(metadata) = dot_agiworkforce_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_codex.display()
        );
    } else if let Err(err) = &dot_agiworkforce_metadata {
        assert_eq!(
            err.kind(),
