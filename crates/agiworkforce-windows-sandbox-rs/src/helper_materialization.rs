...
pub(crate) fn helper_bin_dir(agiworkforce_home: &Path) -> PathBuf {
    sandbox_bin_dir(agiworkforce_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, agiworkforce_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(
    ...
    let destination = helper_bin_dir(agiworkforce_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(agiworkforce_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), agiworkforce_home.display());
    ...
    let destination = helper_bin_dir(agiworkforce_home).join(kind.file_name());
    log_note(
        &format!(
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let agiworkforce_home = Path::new(r"C:\Users\example\.agiworkforce");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.agiworkforce\.sandbox-bin"),
            helper_bin_dir(agiworkforce_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_destination = helper_bin_dir(&agiworkforce_home).join("codex-command-runner.exe");
