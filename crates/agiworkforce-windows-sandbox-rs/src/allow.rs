        ...
        let add_writable_root =
            |root: PathBuf,
             ...
             add_deny: &mut dyn FnMut(PathBuf)| {
                ...
                for protected_subdir in [".git", ".agiworkforce", ".agents"] {
...
mod tests {
    ...
    #[test]
    fn denies_agiworkforce_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let agiworkforce_dir = command_cwd.join(".agiworkforce");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&agiworkforce_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&agiworkforce_dir).unwrap(),
