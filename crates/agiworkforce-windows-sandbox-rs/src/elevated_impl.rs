mod windows_impl {
    ...
    fn ensure_agiworkforce_home_exists(p: &Path) -> Result<()> {
    ...
    /// Resolves the command runner path, preferring AGIWORKFORCE_HOME/.sandbox/bin.
    fn find_runner_exe(agiworkforce_home: &Path, log_dir: Option<&Path>) -> PathBuf {
        resolve_helper_for_launch(HelperExecutable::CommandRunner, agiworkforce_home, log_dir)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = agiworkforce_home.join(".sandbox");
        ensure_agiworkforce_home_exists(&sandbox_base)?;
        ...
        let caps = load_or_create_cap_sids(agiworkforce_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => (
                ...
                vec![
                    ...
                    crate::cap::workspace_cap_sid_for_cwd(agiworkforce_home, cwd)?,
                ],
            ),
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_exe = find_runner_exe(agiworkforce_home, logs_base_dir);
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box::new(SpawnRequest {
                        ...
                        agiworkforce_home: sandbox_base.clone(),
                        real_agiworkforce_home: agiworkforce_home.to_path_buf(),
                        cap_sids,
                        timeout_ms,
                        tty: false,
                        stdin_open: false,
