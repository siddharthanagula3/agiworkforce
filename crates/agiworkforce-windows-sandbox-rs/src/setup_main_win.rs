...
use agiworkforce_windows_sandbox::ensure_allow_write_aces;
use agiworkforce_windows_sandbox::extract_setup_failure;
use agiworkforce_windows_sandbox::hide_newly_created_users;
use agiworkforce_windows_sandbox::is_command_cwd_root;
use agiworkforce_windows_sandbox::load_or_create_cap_sids;
use agiworkforce_windows_sandbox::log_note;
use agiworkforce_windows_sandbox::path_mask_allows;
use agiworkforce_windows_sandbox::protect_workspace_agents_dir;
use agiworkforce_windows_sandbox::protect_workspace_agiworkforce_dir;
use agiworkforce_windows_sandbox::sandbox_bin_dir;
use agiworkforce_windows_sandbox::sandbox_dir;
use agiworkforce_windows_sandbox::sandbox_secrets_dir;
use agiworkforce_windows_sandbox::string_from_sid_bytes;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(agiworkforce_home) = std::env::var("AGIWORKFORCE_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&agiworkforce_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.agiworkforce_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.agiworkforce_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.agiworkforce_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.agiworkforce_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.agiworkforce_home).display()
            ),
        ))
    })?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.agiworkforce_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.agiworkforce_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.agiworkforce_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.agiworkforce_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.agiworkforce_home).join("sandbox_users.json");
        if legacy_users.exists() {
            let _ = std::fs::remove_file(&legacy_users);
        }
    }

    // Protect the current workspace's `.agiworkforce` and `.agents` directories from tampering
    ...
    match unsafe { protect_workspace_agiworkforce_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            let cwd_codex = payload.command_cwd.join(".agiworkforce");
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect workspace .agiworkforce {}",
                    cwd_codex.display()
                ),
            )?;
        }
