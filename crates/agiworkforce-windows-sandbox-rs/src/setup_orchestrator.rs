...
pub const OFFLINE_USERNAME: &str = "AgiWorkforceSandboxOffline";
pub const ONLINE_USERNAME: &str = "AgiWorkforceSandboxOnline";
...
pub fn sandbox_dir(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(".sandbox")
}

pub fn sandbox_bin_dir(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(agiworkforce_home: &Path) -> PathBuf {
    sandbox_dir(agiworkforce_home).join("setup_marker.json")
}

pub fn sandbox_users_path(agiworkforce_home: &Path) -> PathBuf {
    sandbox_secrets_dir(agiworkforce_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, agiworkforce_home);
...
fn run_setup_refresh_inner(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| agiworkforce_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(agiworkforce_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(agiworkforce_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(agiworkforce_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(agiworkforce_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, agiworkforce_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, agiworkforce_home)
    }
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(agiworkforce_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(agiworkforce_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(agiworkforce_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(agiworkforce_home);
    ...
    run_setup_exe(&payload, needs_elevation, agiworkforce_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, agiworkforce_home);
    let mut read_roots = if let Some(roots) = read_roots_override {
        canonical_existing(&roots)
    } else {
        gather_read_roots(command_cwd, policy, agiworkforce_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, agiworkforce_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to AGIWORKFORCE_HOME or anything under AGIWORKFORCE_HOME/.sandbox,
    // AGIWORKFORCE_HOME/.sandbox-bin, or AGIWORKFORCE_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let agiworkforce_home_key = canonical_path_key(agiworkforce_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(agiworkforce_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(agiworkforce_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(agiworkforce_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != agiworkforce_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let agiworkforce_home = tmp.path().join("codex-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &agiworkforce_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&agiworkforce_home)).expect("canonical helper dir");
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &agiworkforce_home);
