...
use agiworkforce_windows_sandbox::PipeSpawnHandles;
use agiworkforce_windows_sandbox::SandboxPolicy;
use agiworkforce_windows_sandbox::StderrMode;
use agiworkforce_windows_sandbox::StdinMode;
use agiworkforce_windows_sandbox::allow_null_device;
use agiworkforce_windows_sandbox::convert_string_sid_to_sid;
use agiworkforce_windows_sandbox::create_readonly_token_with_caps_from;
use agiworkforce_windows_sandbox::create_workspace_write_token_with_caps_from;
use agiworkforce_windows_sandbox::get_current_token_for_restriction;
use agiworkforce_windows_sandbox::hide_current_user_profile_dir;
use agiworkforce_windows_sandbox::ipc_framed::ErrorPayload;
use agiworkforce_windows_sandbox::ipc_framed::ExitPayload;
use agiworkforce_windows_sandbox::ipc_framed::FramedMessage;
use agiworkforce_windows_sandbox::ipc_framed::Message;
use agiworkforce_windows_sandbox::ipc_framed::OutputPayload;
use agiworkforce_windows_sandbox::ipc_framed::OutputStream;
use agiworkforce_windows_sandbox::ipc_framed::decode_bytes;
use agiworkforce_windows_sandbox::ipc_framed::encode_bytes;
use agiworkforce_windows_sandbox::ipc_framed::read_frame;
use agiworkforce_windows_sandbox::ipc_framed::write_frame;
use agiworkforce_windows_sandbox::log_note;
use agiworkforce_windows_sandbox::parse_policy;
use agiworkforce_windows_sandbox::read_handle_loop;
use agiworkforce_windows_sandbox::spawn_process_with_pipes;
...
fn read_spawn_request(
    reader: &mut File,
) -> Result<agiworkforce_windows_sandbox::ipc_framed::SpawnRequest> {
...
fn spawn_ipc_process(
    req: &agiworkforce_windows_sandbox::ipc_framed::SpawnRequest,
) -> Result<IpcSpawnedProcess> {
    let log_dir = req.agiworkforce_home.clone();
    hide_current_user_profile_dir(req.agiworkforce_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_agiworkforce_home={}",
            req.cwd.display(),
            req.command,
            req.real_agiworkforce_home.display()
        ),
        Some(&req.agiworkforce_home),
    );
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = agiworkforce_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
...
pub fn main() -> Result<()> {
    ...
    let msg = FramedMessage {
        ...
        message: Message::SpawnReady {
            payload: agiworkforce_windows_sandbox::ipc_framed::SpawnReady {
