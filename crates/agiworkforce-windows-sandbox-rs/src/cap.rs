...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.agiworkforce`)
    /// without permanently affecting other workspaces.
    #[serde(default)]
    pub workspace_by_cwd: HashMap<String, String>,
}

pub fn cap_sid_file(agiworkforce_home: &Path) -> PathBuf {
    agiworkforce_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(agiworkforce_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(agiworkforce_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(agiworkforce_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(agiworkforce_home);
    let mut caps = load_or_create_cap_sids(agiworkforce_home)?;
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let agiworkforce_home = temp.path().join("codex-home");
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&agiworkforce_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&agiworkforce_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&agiworkforce_home).expect("load caps");
