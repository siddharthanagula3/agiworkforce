...
use crate::serialize::{
    ...
    convert_message_to_openai, gemini_function_declarations_json, ollama_chat_request_body,
    ollama_nativize_message_values, openai_function_tools_json, set_openai_max_tokens,
};
...
fn llm_byte_stream(
    resp: reqwest::Response,
) -> impl Stream<Item = Result<Bytes, LlmError>> + Unpin {
                    ...
                    if let Some(choices) = event.get("choices").and_then(|c| c.as_array())
                        && let Some(choice) = choices.first() {
                                ...
                                .and_then(|t| t.as_array())
                            {
                                for tc in tc_array {
                                    let index =
                                        tc.get("index").and_then(serde_json::Value::as_u64).unwrap_or(0)
                                            as usize;
