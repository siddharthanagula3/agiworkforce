...
use agiworkforce_app_server_protocol::ConfigLayer;
...
impl ConfigLayerEntry {
    ...
    // Get the `.agiworkforce/` folder associated with this config layer, if any.
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_agiworkforce_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_agiworkforce_folder: current_project_dot_agiworkforce_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_agiworkforce_folder {
                ...
                if previous == current_project_dot_agiworkforce_folder
                    || !current_project_dot_agiworkforce_folder
                        .as_path()
                        .ancestors()
