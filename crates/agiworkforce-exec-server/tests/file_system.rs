...
use anyhow::Context;
use anyhow::Result;
use agiworkforce_exec_server::CopyOptions;
...
use agiworkforce_exec_server::LocalFileSystem;
use agiworkforce_exec_server::ReadDirectoryEntry;
...
use common::exec_server::TestAgiworkforceHelperPaths;
use common::exec_server::exec_server;
#[cfg(target_os = "linux")]
use common::exec_server::exec_server_with_env;
use common::exec_server::test_agiworkforce_helper_paths;

struct FileSystemContext {
    file_system: Arc<dyn ExecutorFileSystem>,
    _helper_paths: Option<TestAgiworkforceHelperPaths>,
...
async fn create_file_system_context(use_remote: bool) -> Result<FileSystemContext> {
    if use_remote {
        ...
        let helper_paths = test_agiworkforce_helper_paths()?;
        let runtime_paths = ExecServerRuntimePaths::new(
            helper_paths.agiworkforce_exe.clone(),
            helper_paths.agiworkforce_linux_sandbox_exe.clone(),
        )?;
        Ok(FileSystemContext {
...
async fn file_system_sandboxed_write_allows_explicit_alias_roots(use_remote: bool) -> Result<()> {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("agiworkforce-fs-sandbox-alias-")
