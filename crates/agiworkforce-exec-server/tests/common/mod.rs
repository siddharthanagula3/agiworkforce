...
use agiworkforce_exec_server::AGIWORKFORCE_FS_HELPER_ARG1;
use agiworkforce_exec_server::ExecServerRuntimePaths;
use agiworkforce_sandboxing::landlock::AGIWORKFORCE_LINUX_SANDBOX_ARG0;
use agiworkforce_test_binary_support::TestBinaryDispatchGuard;
use agiworkforce_test_binary_support::TestBinaryDispatchMode;
use agiworkforce_test_binary_support::configure_test_binary_dispatch;
use ctor::ctor;

pub(crate) mod exec_server;

pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--agiworkforce-test-delayed-output-after-exit-parent";

const DELAYED_OUTPUT_AFTER_EXIT_CHILD_ARG: &str = "--agiworkforce-test-delayed-output-after-exit-child";

#[ctor]
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("agiworkforce-exec-server-tests", |exe_name, argv1| {
        if argv1 == Some(AGIWORKFORCE_FS_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if exe_name == AGIWORKFORCE_LINUX_SANDBOX_ARG0 {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let agiworkforce_linux_sandbox_exe = if cfg!(target_os = "linux") {
        TEST_BINARY_DISPATCH_GUARD
            .as_ref()
            .and_then(|guard| guard.paths().agiworkforce_linux_sandbox_exe.clone())
            .or_else(|| Some(current_exe.clone()))
    } else {
        None
    };
    Ok((current_exe, agiworkforce_linux_sandbox_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let exit_code = match runtime.block_on(agiworkforce_exec_server::run_main(&listen_url, runtime_paths))
    {
        Ok(()) => 0,
        Err(error) => {
