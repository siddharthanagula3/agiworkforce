...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestAgiworkforceHelperPaths,
    child: Child,
...
pub(crate) struct TestAgiworkforceHelperPaths {
    pub(crate) agiworkforce_exe: PathBuf,
    pub(crate) agiworkforce_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_agiworkforce_helper_paths() -> anyhow::Result<TestAgiworkforceHelperPaths> {
    let (helper_binary, agiworkforce_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestAgiworkforceHelperPaths {
        agiworkforce_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.agiworkforce_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("AGIWORKFORCE_HOME", agiworkforce_home.path());
    child.envs(env);
