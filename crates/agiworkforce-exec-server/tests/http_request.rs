...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
        ...
        .send_request(
            ...
            serde_json::to_value(HttpRequestParams {
                ...
                headers: vec![HttpHeader {
                    name: "x-agiworkforce-test".to_string(),
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-agiworkforce-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<agiworkforce_app_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
