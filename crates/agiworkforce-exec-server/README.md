# agiworkforce-exec-server

`agiworkforce-exec-server` is the library backing `codex exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`agiworkforce-utils-pty`.
...
filesystem operations and `agiworkforce-linux-sandbox`.

## Transport

The server speaks the shared `agiworkforce-app-server-protocol` message envelope on
the wire.
...
- `arg0`: optional argv0 override forwarded to `agiworkforce-utils-pty`.
