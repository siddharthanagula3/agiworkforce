...
use agiworkforce_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::AGIWORKFORCE_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.agiworkforce_self_exe;
        ...
        let command = SandboxCommand {
            program: helper.as_path().to_string_lossy().to_string(),
            args: vec![AGIWORKFORCE_FS_HELPER_ARG1.to_string()],
        ...
        sandbox_manager
            .transform(SandboxTransformRequest {
                ...
                agiworkforce_linux_sandbox_exe: self.runtime_paths.agiworkforce_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.agiworkforce_self_exe.as_path())
        .chain(runtime_paths.agiworkforce_linux_sandbox_exe.as_deref().into_iter())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let agiworkforce_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(agiworkforce_self_exe, /*agiworkforce_linux_sandbox_exe*/ None)
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let agiworkforce_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(agiworkforce_self_exe.clone(), Some(agiworkforce_self_exe))
                .expect("runtime paths");
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let agiworkforce_self_exe = root.path().join("bin").join("agiworkforce");
        let agiworkforce_linux_sandbox_exe = root.path().join("aliases").join("agiworkforce-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(agiworkforce_self_exe, Some(agiworkforce_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let agiworkforce_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
        ...
        assert!(policy.can_read_path_with_cwd(agiworkforce_parent.as_path(), cwd.as_path()));
