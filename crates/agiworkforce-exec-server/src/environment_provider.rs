...
use crate::environment::AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR;
use crate::environment::LOCAL_ENVIRONMENT_ID;
use crate::environment::REMOTE_ENVIRONMENT_ID;

/// Lists the concrete environments available to Agiworkforce.
///
...
/// Default provider backed by `AGIWORKFORCE_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `AGIWORKFORCE_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `AGIWORKFORCE_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR).ok())
    }
