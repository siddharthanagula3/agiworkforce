...
pub const AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR: &str = "AGIWORKFORCE_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the Agiworkforce runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `AGIWORKFORCE_EXEC_SERVER_URL` behavior
/// while provider-based construction accepts a provider-supplied snapshot.
///
/// Setting `AGIWORKFORCE_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `AGIWORKFORCE_EXEC_SERVER_URL` and local runtime paths
    /// used when creating local filesystem helpers.
    pub async fn new(args: EnvironmentManagerArgs) -> Self {
        let EnvironmentManagerArgs {
            local_runtime_paths,
        } = args;
        let exec_server_url = std::env::var(AGIWORKFORCE_EXEC_SERVER_URL_ENV_VAR).ok();
        Self::from_default_provider_url(exec_server_url, local_runtime_paths).await
    }
    ...
    async fn from_default_provider_url(
        ...
        if environment_disabled {
            // TODO: Remove this legacy `AGIWORKFORCE_EXEC_SERVER_URL=none` crutch once
...
impl Environment {
    /// Builds an environment from the raw `AGIWORKFORCE_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `AGIWORKFORCE_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = agiworkforce_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            agiworkforce_protocol::models::PermissionProfile::from_runtime_permissions(
                &agiworkforce_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
