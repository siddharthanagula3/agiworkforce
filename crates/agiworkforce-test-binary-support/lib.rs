    ...
    F: FnOnce(&str, Option<&str>) -> TestBinaryDispatchMode,
{
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let agiworkforce_home = match tempfile::Builder::new().prefix(agiworkforce_home_prefix).tempdir() {
                Ok(agiworkforce_home) => agiworkforce_home,
                Err(error) => panic!("failed to create test AGIWORKFORCE_HOME: {error}"),
            };
            let previous_agiworkforce_home = std::env::var_os("AGIWORKFORCE_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("AGIWORKFORCE_HOME", agiworkforce_home.path());
            }
            ...
            match previous_agiworkforce_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("AGIWORKFORCE_HOME", value);
                },
                None => unsafe {
            ...
            Some(TestBinaryDispatchGuard {
                ...
                arg0,
                _previous_agiworkforce_home: previous_agiworkforce_home,
            })
        }
    }
}
