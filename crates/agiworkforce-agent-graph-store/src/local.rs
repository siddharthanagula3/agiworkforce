...
impl std::fmt::Debug for LocalAgentGraphStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LocalAgentGraphStore")
            .field("agiworkforce_home", &self.state_db.agiworkforce_home())
...
fn to_state_status(status: ThreadSpawnEdgeStatus) -> agiworkforce_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => agiworkforce_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => agiworkforce_state::DirectionalThreadSpawnEdgeStatus::Closed,
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let agiworkforce_home = TempDir::new().expect("tempdir should be created");
        let state_db =
