...
fn auth_identity(auth: &AgiWorkforceAuth) -> (Option<String>, Option<String>) {
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: agiworkforce_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(agiworkforce_core::config_loader::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        agiworkforce_core::config_loader::AppRequirementToml {
                            enabled: Some(false),
                        },
                    )]),
                }),
