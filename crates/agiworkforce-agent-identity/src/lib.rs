...
use chrono::Utc;
use agiworkforce_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "agiworkforce-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/agiworkforce-backend/agent-identity";
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "agiworkforce-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "agiworkforce-cli".to_string(),
        },
...
mod tests {
    ...
    use agiworkforce_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    #[test]
    fn agent_identity_jwks_url_uses_agiworkforce_api_base_url() {
        assert_eq!(
