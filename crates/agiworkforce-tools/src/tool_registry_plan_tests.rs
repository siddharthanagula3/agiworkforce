...
use agiworkforce_protocol::models::VIEW_IMAGE_TOOL_NAME;
use agiworkforce_protocol::openai_models::InputModality;
use agiworkforce_protocol::openai_models::ModelInfo;
use agiworkforce_protocol::openai_models::WebSearchToolType;
...
const AGIWORKFORCE_APPS_MCP_SERVER_NAME: &str = "agiworkforce_apps";
...
#[test]
fn test_full_toolset_specs_for_gpt5_agiworkforce_unified_exec_web_search() {
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(agiworkforce_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(agiworkforce_protocol::config_types::WebSearchUserLocation {
            r#type: agiworkforce_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(agiworkforce_protocol::config_types::WebSearchContextSize::High),
    };
...
fn namespaced_dynamic_specs_are_hidden_when_namespace_tools_are_disabled() {
    ...
    assert_lacks_tool_name(&tools, "agiworkforce_app");
...
fn search_tool_description_lists_each_mcp_source_once() {
    ...
    let (tools, handlers) = build_specs(
        ...
        Some(HashMap::from([
            (
                ToolName::namespaced("mcp__agiworkforce_apps__calendar", "_create_event"),
                mcp_tool(
    ...
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("mcp__agiworkforce_apps__calendar", "_create_event"),
        kind: ToolHandlerKind::Mcp,
    }));
    assert!(handlers.contains(&ToolHandlerSpec {
...
fn search_tool_registers_for_deferred_dynamic_tools() {
    ...
    assert!(description.contains("- Dynamic tools: Tools provided by the current Agiworkforce thread."));
    assert_contains_tool_names(&tools, &[TOOL_SEARCH_TOOL_NAME, "agiworkforce_app"]);
    assert_eq!(
        tools
            .iter()
            .filter(|tool| tool.name() == "agiworkforce_app")
            .count(),
        1
    );
    assert_eq!(
        namespace_function_names(&tools, "agiworkforce_app"),
        vec![
    ...
    for tool_name in ["automation_update", "automation_list"] {
        let dynamic_tool = find_namespace_function_tool(&tools, "agiworkforce_app", tool_name);
    ...
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("agiworkforce_app", "automation_update"),
        kind: ToolHandlerKind::DynamicTool,
    }));
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("agiworkforce_app", "automation_list"),
        kind: ToolHandlerKind::DynamicTool,
    }));
}
...
fn model_info() -> ModelInfo {
    serde_json::from_value(json!({
        "slug": "gpt-5-codex",
        "display_name": "GPT-5 Agiworkforce",
        "description": null,
