...
use agiworkforce_protocol::openai_models::ApplyPatchToolType;
    ...
    if config.code_mode_enabled {
        let namespace_descriptions = params
            ...
            .map(|(namespace, detail)| {
                (
                    namespace.clone(),
                    agiworkforce_code_mode::ToolNamespaceDescription {
        ...
        plan.register_handler(
            agiworkforce_code_mode::WAIT_TOOL_NAME,
        ...
        && (deferred_mcp_tools_for_search.is_some() || !deferred_dynamic_tools.is_empty())
    {
        ...
        if !deferred_dynamic_tools.is_empty() {
            search_source_infos.push(ToolSearchSourceInfo {
                ...
                description: Some("Tools provided by the current Agiworkforce thread.".to_string()),
            });
        }
...
fn compare_code_mode_tools(
    left: &agiworkforce_code_mode::ToolDefinition,
    right: &agiworkforce_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &agiworkforce_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, agiworkforce_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
