...
use crate::ToolName;
use crate::ToolSpec;
use agiworkforce_code_mode::CodeModeToolKind;
use agiworkforce_code_mode::ToolDefinition as CodeModeToolDefinition;
            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            agiworkforce_code_mode::augment_tool_definition(definition).description;
                    }
                }
            }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    agiworkforce_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| agiworkforce_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| agiworkforce_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(agiworkforce_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();
...
pub fn create_wait_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: agiworkforce_code_mode::WAIT_TOOL_NAME.to_string(),
        description: format!(
            "Waits on a yielded `{}` cell and returns new output or completion.\n{}",
            agiworkforce_code_mode::PUBLIC_TOOL_NAME,
            agiworkforce_code_mode::build_wait_tool_description().trim()
        ),
        strict: false,
...
pub fn create_code_mode_tool(
    ...
    namespace_descriptions: &BTreeMap<String, agiworkforce_code_mode::ToolNamespaceDescription>,
    code_mode_only: bool,
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        name: agiworkforce_code_mode::PUBLIC_TOOL_NAME.to_string(),
        description: agiworkforce_code_mode::build_exec_tool_description(
            enabled_tools,
