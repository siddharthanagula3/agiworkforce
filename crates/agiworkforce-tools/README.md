...
`agiworkforce-tools` is intended to become the home for tool-related code that is
...
compatibility-sensitive orchestration in `agiworkforce-core` until the surrounding
...
- tool metadata and compatibility shims that do not depend on `agiworkforce-core`
- other narrowly scoped utility code that multiple crates need

The corresponding non-goals are just as important:

- do not move `agiworkforce-core` orchestration here prematurely
...
2. Switch non-core consumers to depend on `agiworkforce-tools` directly.
3. Leave compatibility-sensitive adapters in `agiworkforce-core` while downstream
   call sites are updated.
4. Only extract higher-level tool infrastructure after the crate boundaries are
   clear and independently testable.

That means it is normal for `agiworkforce-core` to temporarily re-export types or
helpers from `agiworkforce-tools` during the transition.
...
`agiworkforce-core`, that is a sign to revisit the extraction boundary before adding
more here.
