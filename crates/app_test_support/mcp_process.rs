...
use agiworkforce_app_server_protocol::ConfigBatchWriteParams;
use agiworkforce_app_server_protocol::ConfigReadParams;
use agiworkforce_app_server_protocol::ConfigValueWriteParams;
...
use agiworkforce_app_server_protocol::FsReadDirectoryParams;
use agiworkforce_app_server_protocol::FsReadFileParams;
use agiworkforce_app_server_protocol::FsRemoveParams;
use agiworkforce_app_server_protocol::FsUnwatchParams;
use agiworkforce_app_server_protocol::FsWatchParams;
...
use agiworkforce_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use agiworkforce_login::default_client::AGIWORKFORCE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "agiworkforce-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "AGIWORKFORCE_APP_SERVER_DISABLE_MANAGED_CONFIG";

impl McpProcess {
    pub async fn new(agiworkforce_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(agiworkforce_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(agiworkforce_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(agiworkforce_home, &[(DISABLE_MANAGED_CONFIG_ENV_VAR, Some("1"))]).await
    }
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(agiworkforce_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(agiworkforce_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(agiworkforce_home, &[], &[]).await
    }

    pub async fn new_with_args(agiworkforce_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(agiworkforce_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = agiworkforce_utils_cargo_bin::cargo_bin("agiworkforce-app-server")
            .context("should find binary for agiworkforce-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.current_dir(agiworkforce_home);
        cmd.env("AGIWORKFORCE_HOME", agiworkforce_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "AGIWORKFORCE_APP_SERVER_MANAGED_CONFIG_PATH",
            agiworkforce_home.join("managed_config.toml"),
        );
        cmd.env_remove(AGIWORKFORCE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("agiworkforce-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: agiworkforce_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `agiworkforce-app-server` child process.
        //
