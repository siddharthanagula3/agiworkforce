...
export const AGIWorkforceHooksPlugin = async ({ client, $, directory, worktree }: PluginInput) => {
  ...
  return {
    ...
    'tool.execute.before': async (input: { tool: string; args?: Record<string, unknown> }) => {
      ...
      if (
        ...
        log('info', '[AGI] Remember to review changes before pushing: git diff origin/main...HEAD');
      }
      ...
      if (hookEnabled('pre:bash:tmux-reminder', 'strict') && input.tool === 'bash') {
        ...
        if (
          ...
          log('info', '[AGI] Long-running command detected - consider using background execution');
        }
      }
    },
    ...
    'experimental.session.compacting': async () => {
      ...
      return {
        ...
        compaction_prompt:
          'Focus on preserving: 1) Current task status and progress, 2) Key decisions made, 3) Files created/modified, 4) Remaining work items, 5) Any security concerns flagged, 6) Tauri IPC correctness issues. Discard: verbose tool outputs, intermediate exploration, redundant file listings.',
      };
    },
    ...
    'permission.ask': async (event: { tool: string; args: unknown }) => {
      ...
      if (
        event.tool === 'bash' &&
        /^(pnpm typecheck|cargo check|cargo clippy|pnpm lint)/.test(cmd)
      ) {
