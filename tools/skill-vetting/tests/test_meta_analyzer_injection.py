"""A skill's own text may not delete the scanner's high-severity findings.

The meta-analyzer prompt embeds the verbatim skill file, so the model's
``is_vulnerability`` verdict is attacker-reachable. For CRITICAL/HIGH findings
that verdict must only lower confidence, never remove the finding, and the
untrusted content must sit inside a delimiter the file cannot close.
"""
...
def test_injected_rejection_cannot_delete_critical_or_high_findings() -> None:
    findings = [
    ...
    assert [(f.rule_id, f.severity) for f in kept] == [("E2", "CRITICAL"), ("P1", "HIGH")]
...
def test_medium_findings_are_still_filtered_when_unconfirmed() -> None:
