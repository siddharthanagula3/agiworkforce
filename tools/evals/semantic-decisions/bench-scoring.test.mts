...
describe('the confusion counts a gate is chosen on', () => {
  ...
  it('reports the fallback rate over every case, not only the scored ones', () => {
    expect(
      scoreBinary([binary({ id: 'a', used: false }), binary({ id: 'b' })]).fallbackRate,
    ).toBe(0.5);
  });
});
...
describe('the validation the runner relies on, against a fake provider', () => {
  ...
  it('scores a category the same way whether the gate acted or escalated', () => {
    const score = scoreCategory([
      { id: 'a', split: 'calibration', tags: [], scored: true, correct: true, used: true, confidence: 0.9 },
      { id: 'b', split: 'calibration', tags: [], scored: true, correct: false, used: false, confidence: 0.2 },
    ]);

    expect(score).toMatchObject({ scored: 2, accuracy: 0.5, accuracyWhenUsed: 1, escalationRate: 0.5 });
  });
});
