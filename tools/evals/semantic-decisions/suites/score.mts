...
export function loadLabels(suiteDir: string, cases: readonly EvalCase[]): LabelFile {
  ...
  if (extra.length > 0) throw new Error(`${file.suite}: labels for unknown cases ${extra.join(', ')}`);
  return file;
}
...
export function scoreRows(rows: readonly ScoredRow[]): Record<string, unknown> {
  ...
  return {
    ...
    byTag: Object.fromEntries(tags.map((tag) => [tag, tally(rows.filter((row) => row.tags.includes(tag)))])),
    ambiguous: {
...
export function writeBaseline(
  suiteDir: string,
  body: { suite: string; baseline: Record<string, unknown>; rows: readonly ScoredRow[]; extra?: Record<string, unknown> },
): void {
  const output = {
    suite: body.suite,
