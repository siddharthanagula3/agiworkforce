 ...
 */
const SKIPPABLE =
  /(^|\/)(docs?\/|CHANGELOG\.md$)|\.md$|__snapshots__\/|\.snap$|(^|\/)(pnpm-lock\.yaml|package-lock\.json|Cargo\.lock)$/;
...
const decidedByPath = rows.filter(
  (row) => (row.extra as { pathOnlyRuleSkips: boolean }).pathOnlyRuleSkips,
);
