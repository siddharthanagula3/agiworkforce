...
if (process.argv[1]?.endsWith('source-chunks.mts')) {
  ...
  for (const source of sources.sources) {
    ...
    if (digest) {
      console.log(`===== ${source.id} [${source.bucket}] ${source.path} @ ${source.sha.slice(0, 9)} (${chunk.bytes}B)`);
    ...
    cases.push({
      ...
      meta: { tags: [], provenance: { sha: source.sha, path: source.path }, chunkBytes: chunk.bytes },
    });
  }
  if (digest) {
