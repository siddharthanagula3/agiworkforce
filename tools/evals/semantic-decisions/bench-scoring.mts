...
export function scoreCategory(rows: readonly CategoryRow[]): CategoryScore {
  ...
  return {
    ...
    accuracy: scored.length > 0 ? round(scored.filter((r) => r.correct).length / scored.length) : null,
    accuracyWhenUsed:
...
export function reliability(
  ...
  return BIN_EDGES.map((lower, index) => {
    ...
    const bin = rows.filter(
      (row) => row.scored && row.confidence !== null && row.confidence >= lower && row.confidence < upper,
    );
    return {
      lower,
      upper: upper > 1 ? 1 : upper,
      count: bin.length,
      observedAccuracy: bin.length > 0 ? round(bin.filter((r) => r.correct).length / bin.length) : null,
    };
  });
}
...
export function sliceByTag<Row extends { tags: string[] }, Score>(
  ...
  return Object.fromEntries(tags.map((tag) => [tag, score(rows.filter((r) => r.tags.includes(tag)))]));
}
...
export function scoreSets(rows: readonly SetRow[]): SetScore {
  const scored = rows.filter((row) => row.scored);
  const sum = (pick: (row: SetRow) => number) => scored.reduce((total, row) => total + pick(row), 0);
  ...
  const keptWanted = sum(
    (row) => row.kept.filter((item) => row.core.includes(item) || row.optional.includes(item)).length,
  );
...
export function latencyAndCost(
  ...
  const usdPerCase = perCase === null ? null : (perCase * usdPerMillionInputTokens) / TOKENS_PER_MILLION;
  return {
    cases: runs.length,
