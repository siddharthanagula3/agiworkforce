...
function scoreWorthExtracting(runs: CaseRun[]): SuiteReport {
  ...
  const gate = chooseThreshold(thresholdLadder(0.1), (g) =>
    objective(rowsAt(extract.chosen, g)),
  );
...
function scoreSecurityGate(runs: CaseRun[]): SuiteReport {
  ...
  const skipped = (rows: readonly BinaryRow[]) => ({
    ...
    modelCallsPerChunk:
      rows.length > 0 ? round(1 + rows.filter((row) => row.predicted).length / rows.length, 3) : null,
  });
  ...
  return {
    ...
    byTag: sliceByTag(final, scoreBinary),
    extra: {
      falseNegativeIds: final.filter((row) => row.scored && row.expected && !row.predicted).map((row) => row.id),
...
function scoreElementResolution(runs: CaseRun[]): SuiteReport {
  ...
  return {
    ...
    fixtureSha256: fixtureHash('element_resolution', ['cases.json', 'labels.final.json', 'pages.json']),
...
function scoreToolShortlist(runs: CaseRun[], cases: EvalCase[]): SuiteReport {
  ...
  ).reduce((map, row) => map.set(row.id, row), new Map<string, { shortlist: string[]; selectedBytes?: number }>());
  ...
  const rowsAt = (keep: number, maybe: number): SetRow[] =>
    runs.map((run) => {
      const label = labels[run.id] as
        | { label: { core: string[]; optional: string[] } | null; scored: boolean }
        | undefined;
  ...
  const productionRecall = scoreSets(productionRows.filter((row) => row.split === 'calibration'))
    .coreRecall;
  ...
  for (const keep of ladder) for (const maybe of ladder) if (maybe <= keep) pairs.push([keep, maybe]);
  ...
  return {
    ...
    reliability: reliability(
      runs.flatMap((run) => {
        const label = labels[run.id] as
          | { label: { core: string[] } | null; scored: boolean }
          | undefined;
        const message = String(byId.get(run.id)?.input['message'] ?? '');
        const { asked } = partitionFor(message);
...
interface MemorySets {
  sets: Record<
    string,
    { memories: { id: string; content: string; category: string; pinned: boolean; updatedAt: string }[] }
  >;
}
...
function productionOrder(setId: string): MemorySets['sets'][string]['memories'] {
  ...
  rows.sort((a, b) => (a.pinned !== b.pinned ? (a.pinned ? -1 : 1) : b.updatedAt.localeCompare(a.updatedAt)));
...
function scoreMemoryRelevance(runs: CaseRun[], cases: EvalCase[]): SuiteReport {
  ...
  const rowsAt = (minimumConfidence: number): SetRow[] =>
    runs.map((run) => {
      const label = labels[run.id] as
        | { label: { core: string[]; optional: string[]; standing?: string[] } | null; scored: boolean }
        | undefined;
  ...
  return {
    ...
    reliability: reliability(
      runs.flatMap((run) => {
        const label = labels[run.id] as
          | { label: { core: string[]; standing?: string[] } | null; scored: boolean }
          | undefined;
        const { ids, candidates } = memoryRows(
          String(byId.get(run.id)?.input['memorySetId'] ?? ''),
        );
        return ids.flatMap((id, index): { confidence: number | null; correct: boolean; scored: boolean }[] => {
          if (candidates[index]?.pinned) return [];
          const answer = run.answers[`memory_${index}`];
          if (answer?.kind !== 'choice') return [{ confidence: null, correct: false, scored: false }];
...
function scoreTurnSignals(runs: CaseRun[]): SuiteReport {
  ...
  const booleanReport = (labelField: keyof typeof TURN_SIGNAL_FIELDS) => {
    ...
    return {
      ...
      reliability: reliability(
        final.map((row) => {
          const probability = noulOf(runs.find((run) => run.id === row.id)!, key);
          return {
            confidence: probability === null ? null : noulConfidence(probability),
            correct: row.predicted === row.expected,
            scored: row.scored,
  ...
  return {
    ...
    extra: {
      unscoredPerField: Object.fromEntries(
        ['needs_current_info', 'needs_external_tools', 'needs_code', 'task_family', 'complexity4'].map(
          (field) => [field, runs.filter((run) => fieldOf(run.id, field)?.['scored'] !== true).length],
        ),
      ),
...
process.stdout.write(
  `wrote ${reports.length} result file(s) and SUMMARY.md; $${round(totals.usd, 4)} of input tokens\n`,
);
