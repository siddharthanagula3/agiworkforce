...
// A second implementation of a sync primitive is worse than an unused one: the
// copies drift, and the two surfaces then disagree about which revision is
// newer while both look right.
//
// All four are `bigintGreater`, copied verbatim out of
// packages/client/sync/src/cursor.ts into apps/web/app/api/{chat,memory,
// projects,settings}/sync/route.ts. Deduping them is not a code change: apps/web
// does not depend on @agiworkforce/sync, so it needs that dependency edge added
// to apps/web/package.json and pnpm-lock.yaml regenerated by the package
// manager. Until that lands the count is pinned, not excused: four may become
// zero, and must never become five.
const MAX_DUPLICATED_SYNC_PRIMITIVES = 4;

function topLevelFunctionNames(relativePath) {
  const sourceFile = ts.createSourceFile(
    relativePath,
    readText(relativePath),
    ts.ScriptTarget.Latest,
  ...
  for (const statement of sourceFile.statements) {
    if (ts.isFunctionDeclaration(statement) && statement.name) {
      names.push({ name: statement.name.text, node: statement });
      ...
      for (const declaration of statement.declarationList.declarations) {
        if (
          ...
          names.push({ name: declaration.name.text, node: statement });
        }
      }
    }
  }
  return { sourceFile, names };
}

const syncPrimitiveNames = collectExportedNames(syncFiles.map((file) => `${syncPath}/src/${file}`));
const duplicatedSyncPrimitives = [];
for (const scanRoot of ['apps/web', 'apps/desktop', 'apps/mobile', 'apps/extension', 'apps/cli']) {
  for (const relativePath of walk(scanRoot)) {
    if (!sourceExtensions.has(path.extname(relativePath))) continue;
    if (/\.(test|spec)\.[jt]sx?$/.test(relativePath)) continue;
    const { sourceFile, names } = topLevelFunctionNames(relativePath);
    for (const { name, node } of names) {
      if (!syncPrimitiveNames.has(name)) continue;
      // A same-named adapter that calls the aliased core export is delegation,
      // not a fork, and mobile's engine is built that way on purpose.
      if (/Core\b/.test(node.getText(sourceFile))) continue;
      duplicatedSyncPrimitives.push(`${relativePath} declares ${name}`);
    }
  }
}
if (duplicatedSyncPrimitives.length > MAX_DUPLICATED_SYNC_PRIMITIVES) {
  errors.push(
    `${duplicatedSyncPrimitives.length} app-level re-implementations of ${syncPackage} exports, ` +
      `above the pinned ${MAX_DUPLICATED_SYNC_PRIMITIVES}: ${duplicatedSyncPrimitives.join('; ')}`,
  );
}
