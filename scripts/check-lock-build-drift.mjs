...
const MODEL_ID_RE = /\b(claude-[\w.-]+-[\d.]+[\w-]*|gpt-[\d.]+[\w.-]*|gemini-[\d.]+[\w.-]*|deepseek-[\w.-]+-[\w.-]+|qwen[\w.-]+-[\w.-]+|grok-[\d.]+[\w.-]*|kimi-[\w.]+-[\d.]+[\w-]*|mistral-[\w.]+-[\d]+[\w-]*|glm-[\d.]+[\w.-]*|sonar(?:-(?:pro|reasoning(?:-pro)?|deep-research))?)\b/g;
...
function main() {
  ...
  if (driftItems.length === 0) {
    console.log(`AP-09: aligned — all model refs in ${lockFiles.length} lock files match models.json.`);
    if (cargoVersion) {
  ...
  console.warn('These model IDs appear in lock files but are not in packages/types/src/models.json:');
