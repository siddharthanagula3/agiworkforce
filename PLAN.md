...
- Add file inclusion policy and per-file privacy labels. (Round 8 partial — per-file privacy chip rendered on Web composer attachments via the new `attachmentPrivacyShortLabel` prop on `ChatComposerNew`, sourced from `SendPreviewPresentation.privacyShortLabel`. Each image thumbnail + document chip stamps the outbound destination ('Local' / 'BYOK' / 'Managed') with a lock-icon chip. File-inclusion policy itself — rules controlling which files attach to which projects/turns — remains open as a separate slice.)
