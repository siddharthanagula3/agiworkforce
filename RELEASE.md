...
| ID      | Title                                                                                                                    | Surface        | Sev  | Status        | Auto?  | Evidence                                                                                                                                                  |
| ------- | ------------------------------------------------------------------------------------------------------------------------ | -------------- | ---- | ------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REL-025 | Non-streaming chat completions never persist the assistant turn to `web_messages` (silent history loss for stream:false) | web/api        | med  | TODO          | yes    | `app/api/llm/v1/chat/completions/lib/response-builder.ts` buildNonStreamResponse never inserts web_messages; route.ts:834 onSuccessfulTurn is memory-only |
| REL-020 | Excel edit tool silently no-ops DeleteRow/DeleteColumn/InsertColumn/UpdateStyle while reporting success                  | desktop        | high | TODO          | yes    | `apps/desktop/src-tauri/src/features/document/edit_excel.rs:210-251` return Ok without applying — false success                                           |
| REL-014 | Subagent spawn has no recursion depth limit — nested task-tool calls can spawn unbounded subagent trees                  | cli/core-agent | high | TODO          | yes    | `apps/cli/src/subagent.rs` run_subagent() ~492 no depth guard                                                                                             |
| REL-022 | No safety limit on subagent/task decomposition fan-out or size                                                           | core-agent     | med  | TODO          | yes    | `apps/desktop/src-tauri/src/core/swarm/task_decomposer.rs` build_decomposition_prompt — unbounded                                                         |
...
| REL-018 | Managed-cloud turns can silently drop with no assistant reply and no failure UI                                          | web/chat       | high | INVESTIGATING | manual | orphaned user message with no paired assistant row; transcript renders no error/retry affordance (code-confirmed absent)                                  |
