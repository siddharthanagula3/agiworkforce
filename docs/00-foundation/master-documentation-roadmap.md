...
- **Purpose / Dependencies / Prerequisites / Cross-References / Expected Inputs / Expected Outputs / Review Requirements:** Document AGI Code (coding/developer agent) as a cross-surface Experience across developer surfaces. Depends on VOL-08 (Connectors/MCP, Computer Use, Skills, Generated Files, Artifacts). Prereqs: none outstanding — D5 resolved 2026-08-04 (Desktop mounts AGI Code as CodeWorkspace, Local-only). Cross-refs: trust-mode-surface-matrix.md (CLI/VS Code/Desktop). Inputs: canonical definition, developer-surface roster. Outputs: code manifestation map. Review: capability-honesty (CodeWorkspace Local-only gating), trust-boundary, developer-session sync rules.
- **Chapters:**
  ...
  - **CH-07.02.05 — Code current-vs-target: Desktop mount status (UNBLOCKED — D5 resolved 2026-08-04)** — depends-on: owner-decision-register D5 (resolved) · references: apps/desktop/src/features/code/CodeWorkspace.tsx (mounted Local-only), owner-decision-register §3 D5 · related features: AGI Code mount · est pages: 6 · difficulty: med · review checklist: records the Local-only mount and its three gates (nav, navigate guard, eviction); no invented availability; capability-honesty
...
- VOL-07 BK-07.02 (AGI Code current-vs-target) — D5 (resolved 2026-08-04 — mounted as CodeWorkspace, Local-only; the CodeModeHome stub was deleted)
