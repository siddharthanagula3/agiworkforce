...
- **Billing plan tier** — Canonical set `'local-only' | 'byok' | 'free' | 'basic' | 'pro' | 'max' | 'max_15x' | 'team' | 'enterprise'` (`BillingPlanTier` in `packages/contracts/types/src/billing-catalog.ts`). Labels come from `BILLING_PLAN_PRICING` (`max` is "Max 5x", `max_15x` is "Max 15x"). `team` is the only per-seat tier (`perSeat: true`), so every surface that renders its price must say "/seat". **Avoid** `hobby` and `pro_plus`: both were removed from the catalog in `343457c8d` (2026-06-20, "no users, fresh start"), and the entry tier returned as `basic` in `c39eba06c`/`7a78ecbd0` (2026-07-16/19). They survive only as legacy-input aliases — `hobby` → `basic`, `pro_plus` → `max` — in `normalizeUIPlanTier` (`packages/contracts/types/src/design-system/user-identity.ts:44-45`) and the equivalent web normalizer (`apps/web/features/billing/hooks/use-billing-queries.ts:99`), never as tier names. The desktop pricing list no longer diverges: `apps/desktop/src/constants/pricing.ts` types its plan ids as `BillingPlanTier` and reads every price from this catalog.
