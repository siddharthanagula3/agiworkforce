...
A polyglot monorepo: **pnpm workspace** (`apps/*`, `packages/*`, `packages/providers/*`, `services/*`) + **Cargo workspace** (`apps/desktop/src-tauri`, `apps/cli`, `crates/*`). Node 22, Rust edition 2021.

- **Surfaces** (`apps/`): web, desktop, mobile, cli, extension (Chrome), extension-vscode, sandbox. (`AGI-SURF-0001`.)
- **Shared TypeScript** (`packages/`, 21): contracts (`types`), suite spine (`unified-chat`), provider adapters (`providers/*`), `llm-normalize`, `llm-runtime`, `routing`, `mcp`, `browser-tool`, `local-llm`, `compliance`, `runtime`, `data-layer`, `ui`, `stores`, `skills`, `api`, etc.
...
1. **Contracts** — `packages/types` (`suite-contracts.ts`, `models.json` SSOT), `crates/agiworkforce-protocol`.
2. **Mechanics** — provider adapters, `llm-normalize`, `llm-runtime`, `routing`, `mcp`, `browser-tool`; Rust `execpolicy`/`sandbox-policy`/`network-proxy`.
3. **Orchestration** — `packages/unified-chat` (suite spine) + per-surface runtimes.
...
The organizing axis is not classic DDD but a trust-mode state machine encoded in `packages/types/src/suite-contracts.ts` and `docs/current/trust-mode-surface-matrix.md`:
...
- **Local** — on-device inference (Ollama/LM Studio on desktop; Apple Foundation Models / ExecuTorch / llama.rn on mobile via `packages/local-llm`). Fails closed.
...
- **Streaming** — provider stream → 8 canonical `StreamChunk` variants → surface runtime → state. 90s idle watchdog; gateway-aware retry/fallback (`packages/llm-runtime`).

## 5. Provider architecture (Current)

- SSOT catalog `packages/types/src/models.json` (57 models / 15 populated providers; ~10 provider definitions with no entries yet — `docs/current/byok-open-model-provider-strategy.md`).
- 8 native adapters (`anthropic, openai, google, deepseek, xai, perplexity, ollama, lmstudio`); other providers via OpenAI-wire-compat path + `llm-normalize`. (`AGI-AI-0001`, `AGI-ARCH-0001`.)
...
- Compliance: EU AI Act Article 50 disclosure + provenance + Chinese-HQ-provider opt-in (`packages/compliance`, `AGI-COMP-0001`).
