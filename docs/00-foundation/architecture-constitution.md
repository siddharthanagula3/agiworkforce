...
Second, **the schema is owned by an auditable, sequential, append-only migration history, applied through a runner that records what it applied.** Migrations are immutable once landed; corrections are new migrations, not edits. **Current state:** the migration files are sequential and immutable, but production was historically applied by a self-described temporary script with no ledger table. That script was deleted on 2026-07-30 because it read production credentials from a temporary file; no replacement ledger-backed runner exists yet, so drift between the committed migrations and the live schema remains unverifiable from the repo. **Target state:** a runner that records applied migrations in a ledger, applied branch-first, with an RLS-activation probe as a gate. Access MUST flow through the vendor-neutral data-layer adapter (`packages/platform/data-layer`); feature code MUST NOT import a concrete driver, and all SQL MUST be parameterized.
...
| A11 | **Production migrations have no ledger-backed runner**; the former hand-rolled temporary script was deleted, so committed-vs-live schema drift remains unverifiable from the repo.                                                                                                                                     | §29                | Database Spec                                                  |
