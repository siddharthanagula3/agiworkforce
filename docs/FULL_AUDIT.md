...
|---|----------|------|-------|------------|
| 1 | HIGH | fallback_chain.rs | RateLimitTracker created but never consulted in router — no rate-limit awareness | M |
...
| 28 | HIGH | provider_adapter.rs | OpenAI Responses API detection may fail for new gpt-5 variants | M |
...
| 34 | MEDIUM | llm_router.rs | Streaming has no idle timeout — frozen UI if provider goes silent | M |
...
| 46 | HIGH | provider_adapter.rs | Google Gemini adapter passes raw ChatMessage instead of Gemini parts format — multimodal fails | M |
