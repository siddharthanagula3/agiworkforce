...
## Executive Summary
*(Written after all waves complete)*
...
| # | Severity | Tag | File | Issue | Fix Effort |
|---|----------|-----|------|-------|------------|
...
| 7 | LOW | [LIVE] | background_manager.rs | 500ms polling loop instead of tokio notify | S |
| 8 | LOW | [LIVE] | prompt_policy.rs | Searches all messages even after finding marker | S |
| 9 | CRITICAL | [HISTORICAL] | server/handlers.rs | Fixed: embedded MCP `tools/call` routes through `DesktopMcpServerExecutor` | — |
| 10 | CRITICAL | [HISTORICAL] | server/http_server.rs | Fixed: embedded MCP server receives `AppHandle` through `McpServerState` | — |
| 11 | HIGH | [LIVE] | extensions/manager.rs | Config encryption NOT implemented — API keys stored plaintext | M |
| 12 | HIGH | [LIVE] | extensions/installer.rs | Install progress callback not wired to frontend | M |
| 13 | HIGH | [LIVE] | server/handlers.rs | No ToolGuard/approval integration — bash runs without consent | L |
| 14 | HIGH | [STALE] | config.rs (MCP) | `open_mcp_settings_db` and `decrypt_oauth_token` now exist. Residual fallback issue tracked in #18 | — |
...
| 38 | MEDIUM | [LIVE] | approval.rs | Two approval systems (Manager + Controller) not integrated | M |
| 39 | MEDIUM | [LIVE] | code_generator.rs | Silent failure on malformed LLM response — returns empty Vec | S |
| 40 | MEDIUM | [LIVE] | rag_system.rs | Defines embedding fields but always sets None — no actual RAG | M |
| 41 | MEDIUM | [LIVE] | semantic_search.rs | Real embeddings exist in DB but not used — TF-IDF only | M |
| 42 | LOW | [LIVE] | undo_manager.rs | TOCTOU race condition on file existence check | S |
...
| 56 | MEDIUM | [HISTORICAL] | control.rs | Fixed: local database reset runs inside one transaction | — |
| 57 | MEDIUM | [HISTORICAL] | compaction.rs | Fixed: compaction persists a replacement summary message | — |
| 58 | MEDIUM | [HISTORICAL] | search.rs (chat) | Fixed: semantic search computes document magnitude correctly | — |
| 59 | CRITICAL | [HISTORICAL] | auth_db.rs | Fixed: `auth_sessions` rebuilt with hash/encrypted columns in migration `v59` | — |
| 60 | CRITICAL | [HISTORICAL] | oauth.rs | Fixed: `OAuthAuthorizationUrl` no longer exposes `pkce_verifier` | — |
| 61 | HIGH | [HISTORICAL] | auth_db.rs | Fixed: `update_session_tokens` rotates hash/encrypted fields | — |
| 62 | HIGH | [HISTORICAL] | auth_db.rs | Fixed: auth session reads require encrypted token payloads | — |
| 63 | HIGH | [HISTORICAL] | auth.rs | Fixed: token validation rate limiting keys on full-token digest | — |
| 64 | HIGH | [HISTORICAL] | permissions.rs | Fixed: `get_all_permissions` uses `updated_at` with regression coverage | — |
| 65 | HIGH | [HISTORICAL] | storage.rs | Fixed: `SecureStorage::lock()` uses volatile writes plus compiler fence | — |
| 66 | HIGH | [HISTORICAL] | audit_logger.rs | Fixed: `generate_signature` returns error on HMAC failure | — |
| 67 | MEDIUM | [HISTORICAL] | tool_guard.rs | Fixed: SQL validation normalizes comments, blocks injection | — |
| 68 | MEDIUM | [LIVE] | dm_protection.rs | All state in-memory only — pairing codes lost on restart | M |
| 69 | MEDIUM | [STALE] | machine_key.rs | `has_machine_only_secrets()` now returns `false` (safe default); `MasterPasswordManager` handles migration tracking. Original "always true" claim is stale | — |
| 70 | MEDIUM | [HISTORICAL] | updater.rs | Fixed: semantic version comparison rejects downgrades | — |
| 71 | MEDIUM | [HISTORICAL] | validator.rs + command_validator.rs | Fixed: `validator.rs` removed; `command_validator.rs` is sole surface | — |
| 72 | CRITICAL | [HISTORICAL] | chat/mod.rs + 6 submodules | Fixed: live chat runtime declares and uses real submodules | — |
| 73 | HIGH | [HISTORICAL] | chat/messages.rs vs mod.rs | Fixed: persistence/cloud-sync in canonical chat persistence path | — |
| 74 | HIGH | [STALE] | chat/mod.rs | `total_input_tokens`/`total_output_tokens` now computed from message data in `persistence.rs` and `conversation.rs`. Initial zeros in `send_message_execution.rs` are pre-message defaults, not a bug | — |
| 75 | HIGH | [HISTORICAL] | chat/mod.rs (context.rs) | Fixed: tool-definition ownership in canonical chat helper modules | — |
| 76 | CRITICAL | [HISTORICAL] | analytics.rs | Fixed: analytics reuse managed `AppDatabase` connection | — |
| 77 | CRITICAL | [HISTORICAL] | code_editing.rs | Fixed: `apply_changes` uses canonical `validate_path_security()` guard | — |
| 78 | CRITICAL | [HISTORICAL] | code_editing.rs | Fixed: `try_git_revert` routes through path-security guard | — |
| 79 | HIGH | [HISTORICAL] | cache.rs | Fixed: cache analytics read `last_used_at` correctly | — |
| 80 | HIGH | [HISTORICAL] | cache.rs | Fixed: cache stats derive hit/miss counts from percentage rates | — |
| 81 | HIGH | [HISTORICAL] | error_reporting.rs | Fixed: Sentry reporting preserves DSN auth | — |
| 82 | HIGH | [STALE] | analytics.rs | `new_users_today` now uses session-based detection (has sessions today + no prior sessions). Original UUID comparison claim is stale | — |
| 83 | HIGH | [HISTORICAL] | checkpoints.rs | Fixed: `checkpoint_restore` uses real transaction helper | — |
...
| 91 | HIGH | [HISTORICAL] | draft_manager.rs | Fixed: `message_drafts` created on startup | — |
| 92 | HIGH | [STALE] | database/postgres.rs | `get_client()` now reuses pooled connections with health checks. Original "new connection every call" claim is stale | — |
| 93 | CRITICAL | [HISTORICAL] | billing/webhooks.rs:252 | Fixed: subscription writes use `updated_at` | — |
| 94 | CRITICAL | [HISTORICAL] | billing/webhooks.rs:597 | Fixed: retry reuses stored payload | — |
| 95 | CRITICAL | [HISTORICAL] | permissions/manager.rs:102 | Fixed: policies written to `self.policies` during startup | — |
| 96 | HIGH | [HISTORICAL] | billing/webhooks.rs:441 | Fixed: payment-failed handling logs grace period without missing column | — |
| 97 | HIGH | [HISTORICAL] | billing/stripe_client.rs:790 | Fixed: payment-method persistence quotes `"type"` column | — |
| 98 | HIGH | [HISTORICAL] | permissions/audit.rs | Fixed: single owned SQLite connection | — |
| 99 | CRITICAL | [HISTORICAL] | lib.rs | Fixed: `AGICheckpointState` managed during startup with fallback | — |
| 100 | CRITICAL | [HISTORICAL] | lib.rs | Fixed: embeddings managed as exact `Arc<Mutex<EmbeddingService>>` type | — |
| 101 | HIGH | [HISTORICAL] | lib.rs | Fixed: `hooks_*` commands registered in `generate_handler!` | — |
| 102 | HIGH | [HISTORICAL] | lib.rs | Fixed: `thinking_*` commands registered in `generate_handler!` | — |
| 103 | HIGH | [HISTORICAL] | lib.rs | Fixed: `canvas_*` commands registered in `generate_handler!` | — |
| 104 | HIGH | [HISTORICAL] | lib.rs | Fixed: Gmail OAuth commands registered in `generate_handler!` | — |
| 105 | HIGH | [HISTORICAL] | lib.rs | Fixed: git merge/PR/conflict commands registered in `generate_handler!` | — |
