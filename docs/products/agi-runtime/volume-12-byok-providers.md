...
Provider and model identity are governed by the SSOT catalog `packages/contracts/types/src/models.json` (25 providers today). Model IDs are read from that file, never invented. Provider strategy (direct frontier, hosted-open-model, local runtimes) follows `docs/current/byok-open-model-provider-strategy.md`.
...
- **Desktop** — ✅ Built: `llm_configure_provider` and `llm_set_default_provider` Tauri commands (`apps/desktop/src-tauri/src/sys/commands/llm.rs`) configure and default a provider; the catalog is `packages/contracts/types/src/models.json`.
...
- `packages/contracts/types/src/models.json` — provider/model SSOT.
...
- [ ] **Build:** provider add/configure/default + direct dispatch works on Desktop and CLI; VS Code reaches parity or its gap is tracked; model IDs read only from `packages/contracts/types/src/models.json`.
