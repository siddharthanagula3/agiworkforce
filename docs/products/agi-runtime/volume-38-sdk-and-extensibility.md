...
**✅ Built (internal trait) / 🔭 Planned (public tool SDK)** — native embedders can plug into the generic `ToolDispatch` trait (`crates/agiworkforce-app-server/src/lib.rs`): `list_tools()` returns MCP-style `{name, description, inputSchema}` entries and `call_tool(name, args)` returns `{content, isError}`. The live CLI app-server instead supplies `CliDeveloperSessionHost`, keeping one full agent registry across stdio and WebSocket; the obsolete seven-tool `CliToolDispatch` duplicate was removed. Slash-command-style tools register through `RegistryCommand::builtin_slash` / `prompt` in `crates/agiworkforce-command-registry/src/lib.rs`. **✅ Built.**
