...
Authority: `AGENTS.md`; `docs/current/source-of-truth.md`; `docs/products/README.md` (canon); `services/AGENTS.md`; and the real code this volume grounds in — `crates/agiworkforce-protocol/src/mcp.rs`, `crates/agiworkforce-plugin-runtime/src/lib.rs`, `crates/agiworkforce-app-server/src/lib.rs`, `packages/mcp/src/{connect,transport,types}.ts`, `services/api-gateway/src/mcp/{mcpConfig,sharedClient,mcpRoutes}.ts`, `services/api-gateway/mcp-servers.json`.
...
- `crates/agiworkforce-plugin-runtime/src/lib.rs` — manifest MCP server discovery (`mcpServers`, `.mcp.json`).
- `crates/agiworkforce-app-server/src/lib.rs` — `run_mcp_server` (CLI-as-MCP-server) + `ToolDispatch::list_tools`.
- `packages/mcp/src/{connect,transport,types}.ts` — `@agiworkforce/mcp` SDK client, transport resolver, catalog builder.
...
- [ ] Build: `resources/list`+`resources/read`+`prompts/list`+`prompts/get` wired in `packages/mcp` and mirrored on the gateway; tool-list-changed notifications handled.
