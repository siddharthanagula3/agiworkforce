...
- `packages/client/client-runtime/src/events.ts` — cross-runtime `listen`/`once`/`emit` bus.
...
- `crates/agiworkforce-plugin-runtime (REMOVED 2026-07-08, zero dependents — no replacement crate exists yet; treat as an unbuilt gap, not a live path)/src/lib.rs` — plugin manifest load/discovery.
...
- [ ] Build: `packages/client/client-runtime` + protocol crate compile; `ToolStreamEvent` and `EventMsg` variants round-trip serialize; app-server WS requires a non-empty auth token.
...
- Do not hardcode or invent model IDs in reroute/verification events; read `packages/contracts/types/src/models.json`.
