...
This volume specifies how the AGI VS Code Extension discovers, indexes, and assembles the developer's repository into context for chat, inline, and agent requests. The surface is **workspace-scoped**: everything here reads the folders the user opened and stays there. Trust is **Local + BYOK + Managed Cloud** with explicit model selection and visible host/provider labels. Workspace context is built locally; when attached, the selected developer-session provider determines the destination. There is **no automatic sync** into Web/Mobile/Desktop app-chat history: Desktop handoff is an explicit, redacted command. Local sessions are never silently routed to BYOK or Cloud.

## Repository Discovery

The extension discovers the working set from three sources: (1) `vscode.workspace.workspaceFolders` plus a resolved active folder via `platform/workspaceFolders`; (2) open editor tabs, auto-detected by `contextPanelProvider.ts`; and (3) a filtered top-level tree via `ContextBuilder.getWorkspaceStructure()`, which skips common generated/vendor directories. Project instruction files (`CLAUDE.md`, `AGENTS.md`, `.cursorrules`) are discovered from the workspace root and injected as data-only context. Untrusted-workspace handling restricts endpoint/gateway/CLI/auto-apply/telemetry/tier overrides and disables agent-mode file writes until trust is granted. Multi-root ranking beyond the active-folder resolver remains planned.
...
`ContextBuilder.buildFullContext()` assembles the request payload from the active file, open files, prioritized diagnostics, redacted git status/diff summary, and workspace structure. The **Context Files tree** backs pinned files plus auto-detected open tabs. User-selected paths are canonicalized and rejected when outside the workspace, sensitive, unreadable, or a folder before they enter persistent context or the privileged runtime. Assembly is model-aware and budgeted through `getContextBudget()`, using catalog-derived context limits rather than hardcoded model IDs.
...
- Referencing Supabase, reintroducing removed tiers, or offering credit top-ups. The extension manifest exposes only current access modes.
