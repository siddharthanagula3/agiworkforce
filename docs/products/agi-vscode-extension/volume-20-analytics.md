...
- ✅ Built — Model selection reports a **normalized** model id via `normalizeConfiguredModelId` (`src/features/model-picker/modelConstants`), not a raw free-text string, so analytics never emits an invented or hardcoded model id. Model ids remain sourced from `packages/types/src/models.json`.
