...
Pricing is shared across the suite: Free, Basic ($8 · ₹399), Pro ($20), Max ($100 and $200), and Enterprise. There is no Plus or Hobby tier, and there are no top-ups. Local and BYOK are free access modes wherever the surface permits them, so developers can run the CLI against local or bring-your-own-key providers without a paid plan while managed inference remains gated by the shared tiers.
