...
✅ Built — `openai_provider()` (`OPENAI_API_KEY`), plus OAuth `chatgpt`/`copilot` login paths (`apps/cli/src/models/mod.rs`, `apps/cli/src/auth.rs`). Shared adapter `packages/ai/providers/openai`.

### Anthropic

✅ Built — native `Provider::Anthropic` (Messages API, `ANTHROPIC_API_KEY`); `agi login anthropic` OAuth supported. Extended-thinking budget is Anthropic-only (`thinking_budget_tokens`, `apps/cli/src/agent/mod.rs`). Adapter `packages/ai/providers/anthropic`.

### Google

✅ Built — native `Provider::Google` (Gemini, `GOOGLE_API_KEY`), `apps/cli/src/models/mod.rs`. Adapter `packages/ai/providers/google`.
...
✅ Built — `Provider::Ollama(OllamaMode::Local)`, default `http://localhost:11434`, keyless → `PrivacyMode::Local`; probed by `agi models scan` (`probe_ollama`, `apps/cli/src/local_models.rs`). Adapter `packages/ai/providers/ollama`. (Hosted `ollama-cloud` requires `OLLAMA_API_KEY` and is a non-local route.)

### LM Studio

✅ Built — `lmstudio_provider()`, keyless `http://localhost:1234/v1`, probed by `agi models scan` (`probe_openai_compatible_local`, `apps/cli/src/local_models.rs`). Adapter `packages/ai/providers/lmstudio`.
...
- `apps/cli/src/model_catalog.rs` — reads `packages/contracts/types/src/models.json` (SSOT).
...
- `packages/contracts/types/src/models.json`; `packages/ai/providers/{openai,anthropic,google,deepseek,xai,perplexity,lmstudio,ollama}`.
...
- Hardcoding or inventing model IDs — always read `packages/contracts/types/src/models.json`.
