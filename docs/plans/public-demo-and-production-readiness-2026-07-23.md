...
| Record a skill               | Wired in the current working tree: composer entry, privacy consent, macOS permission preflight, real global click/type/scroll capture, editable review, zero-step failure, and standard managed `SKILL.md` + `recording.json` persistence. | In the real Tauri app, record a multi-app workflow, remove one reviewed step, save the skill, invoke it from a new chat, approve replay, and confirm the expected result. | Per-step screenshots/app identity, semantic accessibility selectors instead of coordinate-first replay, optional narration, secret detection/redaction, and organization skill publishing. |
...
- 2026-07-23: Desktop “Record a skill” production wiring added. Shared command types now match the
  native recording and skill payloads; the recorder no longer drops legitimate actions inside a
  100 ms window; the visible flow checks macOS permissions, rejects empty captures, supports step
  removal, and creates a standard managed skill without overwriting an existing name. Focused
  proof passed across the shared composer menu and Desktop recorder/shell (3 files/26 tests), both
  command-client and consuming-surface typechecks, all three relevant linters, 6 native Rust tests,
  and the Desktop Rust build. A real native WDIO smoke passed the live shared-composer entry,
  privacy consent, permission preflight, recorder start, native action capture, review, managed
  skill creation, native skill-registry lookup, return to chat, and exact temporary-skill cleanup.
  The native runner still reports the repository's existing optional `tauri-driver` diagnostic
  while successfully using its configured embedded driver. Real multi-app capture, new-chat
  invocation, approved replay, and packaged-build proof remain open.
