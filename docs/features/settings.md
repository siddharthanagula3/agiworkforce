...
| Sub-stores | `appPreferencesStore.ts` (window/UI), `chatPreferencesStore.ts` (agent mode, auto-approve), `executionPreferencesStore.ts` (timeouts), `llmConfigStore.ts` (provider/model config). Security preferences (allowedDirectories, features) are now in `settingsStore.ts`. |
