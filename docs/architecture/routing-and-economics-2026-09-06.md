...
| Retained Tauri and CLI   | A second resolver in Rust over the same compiled policy, with runtime profiles per surface and trust mode. The CLI exposes Local/BYOK/Managed routes; retained Tauri code also carries those profiles but is not the public Desktop. A 1,412-case conformance fixture holds the resolvers to the same answers for the cases the Rust side can compute.                                                                                                                                                                                                                                  | `crates/agiworkforce-model-registry/src/lib.rs`, `apps/desktop/src-tauri/src/core/llm/llm_router.rs`, `apps/cli/src/model_catalog.rs`, `packages/ai/routing/src/__tests__/fixtures`                    |
