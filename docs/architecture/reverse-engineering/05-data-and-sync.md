...
| --------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------- |
| Projects                    | `packages/contracts/types` contract + per-surface persistence                         | Web, Desktop, Mobile                                  | synced app data                                                        |
...
| Developer sessions          | CLI-hosted Rust app-server session runtime                                            | CLI, VS Code                                          | **not** synced to app chats without explicit handoff                   |
| Browser conversations       | Chrome extension `chrome.storage.local` store                                         | Chrome                                                | browser-local; selected/redacted handoff only                          |
| Artifacts / generated files | `ComputeSession` / `GeneratedFile` / `ArtifactManifest` in `packages/contracts/types` | Desktop first, Web managed later, Mobile as requester | carry privacy mode, owner session, checksum, TTL                       |
...
Normal chat sync spans only Web/Mobile/Desktop. CLI/VS Code stay
workspace/session-scoped, while Chrome owns a separate browser-local
conversation history. Neither boundary joins app-chat sync implicitly.

## 5.2 The wire truth: `cloud-contracts` (Zod)

`packages/contracts/cloud-contracts/` is the **one managed-cloud wire truth** — canonical
per-endpoint Zod schemas, paths, stream deltas, and typed clients:
...
Signed organization policy is not a cloud endpoint. Its schema, verifier, and
fixture corpus are owned by the offline licensing boundary at
`packages/contracts/licensing/src/org-policy.ts` and
`packages/contracts/licensing/src/__fixtures__/org-policy/`.

**Enforcement anchor:** the web sync routes and the auth store _derive_ from these schemas, and web **route contract tests** are the enforcement point — a wire drift breaks a test, not production silently. Mobile consumes the same schemas at runtime. (Gotchas from `docs/agent-context` cloud-contracts notes: `mockReset` and vitest-glob quirks in the contract tests.)

## 5.3 The apply engine: `@agiworkforce/sync` (cross-language)

`packages/client/sync/src/` holds the **pure delta apply + bigint-cursor logic**, shared so every surface applies deltas identically:
...
- `web_conversations.id` / `web_messages.id` (uuid) **are** the canonical cloud IDs. Mobile already matches. Desktop maps its INTEGER PK to a UUIDv7 `cloud_id` on its own side (`packages/platform/utils` `./uuidv7`).
