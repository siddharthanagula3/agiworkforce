...
| **Badge fill (subtle gray chip)**                 | `var(--surface-elevated, rgba(0,0,0,0.06))`             | `background-color: rgba(0, 0, 0, 0)` (transparent) — Tailwind v4 drops the comma-containing rgba fallback; token also undefined globally              | **MISS** — letters float on page bg with no chip; this is the central visual divergence from Claude (see Open issue #1)                         |
...
| Badge letter color                                | `var(--text-muted)`                                     | resolves to body text color (`text-secondary`) because `--text-muted` undefined globally; letters render in default body fg                           | **partial** — letters visible but lack the muted-gray tone of Claude's reference                                                                |
| "Result" sub-label                                | small monospace below row                               | rendered, font-size 10px, `data-result-label=""` attr present                                                                                         | **MISS** — font-family resolves to `ui-sans-serif, system-ui, ...` instead of monospace; spec §4 line 17 requires monospace (see Open issue #2) |
...
| Color scheme (page bg) + body typography          | light bg `#f9f8f6`, body color near-black               | confirmed `rgb(249, 248, 246)` body bg, `rgb(2, 8, 23)` body color in light mode                                                                      | **match**                                                                                                                                       |

## Overall verdict

**NEEDS WORK.**

Geometry is correct everywhere — every measured dimension (badge 24px, row 28px, letter glyphs, group header,
web-search card structure) matches the R22 spec exactly. **However, the core visual deliverable of R22 lane 11
— Claude-parity round letter-badges with a subtle gray chip background — does NOT match the reference in the
running browser.** Two confirmed misses against the spec:

1. **Badge fill is transparent**, not the gray chip in Claude's reference. Letters float on the page
   background instead of sitting on a chip. (Root cause: Tailwind v4 arbitrary-value parsing of
   `var(--token, rgba(...))` plus missing global token definitions — see Open issue #1.)
2. **"Result" sub-label is rendered in the sans-serif body font**, not monospace. The component declares
   `font-mono` (`InlineToolCall.tsx:448`) but the computed style is `ui-sans-serif, system-ui, sans-serif, ...`.
   (See Open issue #2.)

Two additional issues are documented below (#3 — broader token-wiring gap; #4 — `mcp-custom` letter `M` vs.
Claude's search-glass glyph for `list_tools`-style discovery calls).

The R22 React-level wiring is otherwise complete and identical across both apps. The fixes are narrow:
either add the missing CSS tokens to `apps/{web,desktop}/styles/globals.css`, or migrate the component
classes to the `--chat-*` token namespace already shipped by `@agiworkforce/design-tokens`.
