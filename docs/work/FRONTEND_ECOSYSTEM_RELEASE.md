...
| F45 | Electron, `/code`: the window's traffic lights draw over the "AGI Code" wordmark. The hidden-title-bar inset is applied by `globals.css` to elements marked `data-sidebar-region` or `data-window-brand`, which the chat sidebar carries and the Code rail does not, so a second product surface put content under the traffic lights (seen in the desktop-code-1 item D captures). Fixed in 56fc1c7c2: the Code surface reads the host's strip tokens (still owned by globals.css) and reserves the inset itself, open and collapsed, both themes, `/chat` unmoved; a globals.css rule that does not assume the chat sidebar's padding stays a lead follow-up so every surface has one owner. | VERIFIED (fixed) | scratchpad desktop-code/itemD-models-dark.png; apps/web/app/globals.css |
