...
| F48 | Chrome: the store-zip smoke's signed-out check (`apps/extension/e2e/smoke.mjs`, added by dcf3dd434) never passed in CI; the lane is path-filtered and first ran it hours later, so it looked like a regression from the Chrome passes. After a reload the panel sits in its loading state showing the literal placeholder "Type / for commands", stale against the catalog's ready copy, and the account refresh waits for both Clerk clients to burn four retries each against the CI fixture host before it says signed out: 7.7 s after reload against an 8 s timeout, while the smoke samples at a fixed 1 s. Bisected across five commits (all fail) and measured with a state sampler and a timing probe (the background answer alone takes 3.1 s). Fix assigned as chrome-5 item 0: the catalog owns the placeholder for every state, signed-out is decided from the auth context alone, and the smoke waits for the gate to settle. | VERIFIED (defect; cause measured) | CI run 34827070106; lead probes 2026-09-14 |
...
(9912ba691); the panel's unpair reaching the shell and the store-zip smoke's
signed-out check (F48) are chrome-5's remaining items, in flight.
