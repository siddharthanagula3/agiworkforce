...
| Desktop sign-in → CLI and VS Code | One account per machine                                | VERIFIED (93cec8ec2): the shell signs each app-server in through the protocol's device flow, approving only the code it received from its own session; live in the real shell the CLI flips to the QA account, 15 of 19 plan models are reachable with the rest offering an upgrade, and a shell sign-out signs the CLI out | reports/desktop-identity-1.md; scratchpad desktop-identity captures |
