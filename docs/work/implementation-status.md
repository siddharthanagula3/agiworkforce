...
Public Desktop references in this matrix mean the Electron shell selected by
D-2026-09-15-04. Rows explicitly labeled retained Tauri describe internal code
only; they do not authorize a second public Desktop shell. Consumer Desktop
Cloud remains account-scoped. Desktop Code may expose Local/BYOK developer
sessions only through the host-owned runtime and credential broker shared with
CLI and VS Code; that does not turn consumer chat into a Local/BYOK surface.

## September 21 Documentation Rebaseline

The current competitor and ecosystem target is
`docs/research/chatgpt-claude-ecosystem-delta-2026-09-21.md`. It supersedes the
older "Competitor Deltas" section for current product behavior while preserving
that section as dated evidence.

The active product rules are:

- Website is the only active release surface. Work proceeds serially through
  Mobile, Desktop, Chrome, CLI, and VS Code.
- One account and one effective entitlement span the suite. Managed Free is
  available after sign-in; paid upgrades remain waitlist/access-code gated.
- Web, Mobile Cloud, Desktop Cloud, and eligible Chrome Managed Cloud share the
  Account Cloud objects defined in `docs/product/experience-contract.md`.
- Desktop Code, CLI, and VS Code share host-owned local sessions, tools,
  permissions, files, and credential references. Desktop is an explicit bridge,
  not an implicit sync path.
- Chat and durable work share one product home. Unified Cloud search, editable
  scoped memory, multiple OAuth identities per supported provider, reusable
  plugin/workflow packages, complete artifact lifecycle, and a first-class
  privacy center are current acceptance targets.

Every dated audit table below remains evidence for the revision and date it
names. It is not automatically a statement about the 2026-09-21 working tree.
...
| W13 | Voice: `app/api/voice/live/sessions/route.ts`, `features/chat/lib/live-voice-session.ts`, `app/api/llm/v1/audio/transcriptions/route.ts`                | Implemented: live voice is a GPT Live WebRTC session brokered by the live sessions route and driven by the web and mobile session drivers; dictation stays on the transcription route.                           | Live probe 2026-09-16: session.started 6.2 s after entry, first assistant transcript 1.6 s after the user finished; interruption is server-side full duplex. Remaining: mobile audio-session routing on a device (MOBILE-005). |
...
| Chat/history                                                         | Present              | Present in Managed Cloud through the public Electron shell; retained Tauri Local/BYOK paths are internal only              | Present across Local/Cloud | Host-owned developer sessions, not consumer history                                                                                  | Host-owned developer sessions through CLI app-server | Eligible Managed Cloud chat plus separate browser-local task history |
| AGI Work run (composer mode)                                         | Present              | Present                                                                                                                    | Present                    | N/A                                                                                                                                  | N/A                                                  | Workflow UI is not Cloud Work                                        |
| Standalone work session surface (historical separate-surface target) | Missing              | Missing                                                                                                                    | Missing                    | N/A                                                                                                                                  | N/A                                                  | N/A                                                                  |
...
- The public Desktop production shell is Electron (`apps/desktop/electron/main.ts`) with the hosted web renderer. The older `DesktopShellV3`, Tauri scheduler, Local/BYOK chat, and related pages remain internal implementation history after D-2026-09-15-04; they are not public Electron capabilities. AGI Work in the public Desktop follows the same Managed Cloud web contract as `apps/web`.
...
- **Web, autonomous Class-1 at zero** (theme toggle re-verified working.
  ...
  the shipped feature).
- **Retained Tauri desktop, batch-1 at zero** (honest local-whisper gating, dead speech
...
## Historical 2026-08-05 Founder Decisions

The ordering and separate-surface Work language in this section are superseded
by the September 21 documentation rebaseline at the top of this file. They are
preserved only to explain the dated implementation tables that follow.
...
| Model dropdown            | W, D, M, CLI, VSC      | ChatGPT/Claude/Codex expose model switchers and effort/thinking controls.                                                    | Dropdown reads catalog/capabilities and shows only routes the active surface exposes. Public Desktop is Managed Cloud only; Local availability and BYOK key state belong to developer surfaces. | Partial: shared model catalog exists; surface-specific admission remains guarded.                              |
...
| Component / option        | Surfaces                         | Competitive target                                                              | AGI requirement                                                                                                                                                                                                                                                            | Current AGI status                                                                 |
| ------------------------- | -------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Local model mode          | M, CLI, VSC; retained Tauri only | AGI differentiator; Claude/OpenAI do not make this first-class in consumer app. | Detect local providers such as Ollama/LM Studio/local runtime, show installed/running state, never require AGI account for local. The public Electron Desktop must continue to reject local inference.                                                                     | Released CLI supports Ollama and LM Studio; Mobile and VS Code remain unpublished. |
| BYOK mode                 | CLI, VSC; retained Tauri only    | AGI differentiator.                                                             | Provider key setup, OS credential storage where applicable, direct provider label, usage/cost disclosure, explicit Local-to-BYOK fork. Web, Mobile v1, and public Electron Desktop do not expose BYOK.                                                                     | Released CLI supports BYOK; VS Code remains unpublished.                           |
| Managed cloud mode        | W, D, M, CLI, VSC                | ChatGPT/Claude/Codex managed compute baseline.                                  | Managed Free is enabled after sign-in; paid-only routes require an existing paid entitlement and new paid acquisition remains waitlist/access-code gated. `AGI_MANAGED_COMPUTE_PRIVATE_BETA` is a kill-switch only. UI labels public alpha and does not over-claim GA/SLA. | Public alpha.                                                                      |
...
| Local MCP servers            | CLI, VSC               | Claude Desktop local MCP; Claude Code MCP.                              | Add/config/edit/view logs, status, OAuth where available, per-tool permissions, local path restrictions. Public Electron Desktop reports `localMcp: false`. | Partial in released CLI; VS Code remains unpublished.                     |
| Remote MCP/custom connectors | W, D, CLI, VSC         | Claude custom connectors using remote MCP; ChatGPT custom apps via MCP. | URL/auth headers, OAuth/bearer token, timeout, SSL, tool discovery, permission review, admin allow/deny.                                                    | Partial: public Desktop uses the hosted account-scoped connector service. |
...
## Scheduled Tasks, Automations, Dispatch, And Work
...
| Extensions    | Hosted connector and plugin controls in public Electron; local extension installation remains a retained Tauri/internal capability and is not public Desktop behavior.                                                                                                                                                                 | Partial.                                              |
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Chat mode            | Managed Cloud chat with unified composer, artifacts, files, model selector, settings, and sidebar. Public Electron exposes neither Local nor BYOK inference. | Present through the hosted web renderer; native-shell behavior still requires release and production smoke verification.                                                                                                                                                                                                                                                                   |
...
| Code mode / AGI Code | Repo/folder dashboard, local folder add, branch/worktree, permissions, model/effort, usage plan, sessions, PRs, routines, terminal/actions, diff review.     | Partial through the Electron developer-session and approved-root runtime. This device execution path is separate from conversation inference and does not make Desktop Local/BYOK chat.                                                                                                                                                                                                    |
...
| Local device host    | Approved files, native messaging, sandboxed developer commands, and browser/computer-use approvals; no local-model or local-MCP declaration.                 | Partial; `dispatcher.ts` is the executable capability owner.                                                                                                                                                                                                                                                                                                                               |
| Cloud onboarding     | Managed Cloud is the public Electron Desktop's only conversation mode.                                                                                       | Public-alpha web contract loaded in the shell; release smoke remains pending.                                                                                                                                                                                                                                                                                                              |

Primary paths: `apps/desktop/electron`, `apps/web`, `packages/contracts/local-runtime`, and `packages/ui/unified-chat`. `apps/desktop/src-tauri` is retained internal code, not a second public Desktop.
...
| Billing/usage      | Stripe/payment links, invoices, credits, limits. Managed Free is available after sign-in. New paid subscriptions and upgrades remain waitlist/access-code gated; existing paid entitlements continue to resolve normally across the suite. | Partial/Gated.     |
| Connectors/apps    | Directory, OAuth/custom apps, sync/search/write action permissions.                                                                                                                                                                        | Partial/Missing.   |
| AGI Code dashboard | Repo selector, activity heatmap, sessions, PRs, routines, run history; host-owned developer sessions may be projected to authorized cloud clients without converting them into account chat history.                                       | Partial/Missing.   |
...
| Component              | Required behavior                                                                                                                                                              | Current AGI status                             |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| Local-first onboarding | Choose Local or Cloud (Mobile v1 does not expose BYOK). Managed Free is available after sign-in; paid acquisition remains waitlist/access-code gated; Local stays fail-closed. | Partial: Local + Managed Free (sign-in gated). |
...
| Cloud/local continuation | Codex cloud delegation/follow-up, AGI Local/BYOK/Managed boundaries.                           | Managed Free tasks are available after sign-in; local sessions hand off only with provenance, scan, preview and consent. | Partial.           |
...
| Paid acquisition waitlist | New paid subscriptions and upgrades are waitlist/access-code gated until the founder opens acquisition. Managed Free remains available after sign-in; existing paid entitlements continue to resolve. | Partial/Gated.     |
| Access/promo codes        | Access codes authorize paid acquisition; promo codes may redeem credits or offers. Neither creates a separate per-surface subscription, and neither gates Managed Free access.                        | Partial/Gated.     |
