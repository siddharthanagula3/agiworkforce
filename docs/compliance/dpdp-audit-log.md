...
| `/subprocessors`: "We do not process your prompts; the request flows directly from your client" | True of provider-key requests from the released CLI. VS Code support is coming soon. **The public Electron Desktop and web app are cloud-only and have no user-supplied-key path**, so their model requests use Managed Cloud. The current surface boundary is named on both `/subprocessors` and `/privacy`. |
