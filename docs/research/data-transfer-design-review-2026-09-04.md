...
## Measured on 2026-09-04 after the fixes

A real client-side switch between two conversations (sidebar click, not a reload) made 12 to 13 requests before the caching work and 8 to 9 after: connector permissions and media availability went from 2 to 0, and the me, projects and skills routes were already cached and did not refetch, which corrects the blanket claim above. The remaining 8 are the page remount itself (usage 2, conversation list 2, conversation 2, sync 1, branches 1). The remount fix is a route merge of app/chat/page.tsx and app/chat/[sessionId]/page.tsx into an optional catch-all with an audit of every mount-scoped effect in WebChatPage.tsx; it is sized at several days and recorded as design debt.
