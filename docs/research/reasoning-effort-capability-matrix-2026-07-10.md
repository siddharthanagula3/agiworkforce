...
Amended 2026-07-25: the Opus row now reflects Claude Opus 5's official
contract. It is documentation-verified after this report's original probe
date; historical probe claims below refer only to the then-current predecessor.
...
- Some take **discrete effort levels** — and the allowed set differs by model
  ...
  `xhigh`; Anthropic Opus 5 adds `max`).
- The reigning example of drift: **Anthropic Opus 5 rejects the classic
...
| Anthropic                   | then-current Opus generation                        | 200 (adaptive+effort) / **400** (enabled+budget)      | historical probe; superseded by the 2026-07-25 row   |
...
| claude-opus-5                                                                                                               | anthropic   | yes                                     | effort_levels                                  | `low,medium,high,xhigh,max` (default `high`; disabled thinking permits through `high`)                           | messages · `output_config.effort` + `thinking:{type:"adaptive"}`; classic manual budgets are unsupported | Official Anthropic model, effort, and extended-thinking docs, verified 2026-07-25 |
...
| gemini-3.5-flash-lite                                                                                                       | google      | yes (default `minimal`)                 | effort_levels                                  | `minimal,low,medium,high`; no documented off level                                                               | gen · `thinkingConfig.thinkingLevel`                                                                     | ai.google.dev thinking docs (verified 2026-07-22)                                 |
...
- **Current (Opus 5, Sonnet 5)**: adaptive thinking + discrete effort.
...
budget_tokens}` is **rejected with 400** on Opus 5. Effort may be sent
...
```jsonc
// effort_levels — Anthropic Opus 5 (adaptive + output_config.effort; adds max)
"claude-opus-5": {
  "reasoning": {
    "capable": true,
...
## Historical flags and 2026-07-25 resolution status
...
budget_tokens}` — **which now 400s on Opus 5**; (b) caps OpenAI at `xhigh`
...
3. **Anthropic Opus 5 request path — resolved 2026-07-25.**
   Web, Desktop, and the direct Anthropic adapter now read the shared reasoning
   metadata. They emit adaptive thinking, preserve explicit disabled thinking,
   reject disabled `xhigh`/`max`, suppress forbidden sampling parameters, and
   cover the canonical provider ID with focused tests.
...
| Anthropic      | `claude-haiku-4.5` (budget floor)                                                   | `claude-opus-5` (flagship), `claude-sonnet-4.6` (pro) | prior-gen opus/sonnet when superseded                                                                                             |
| Google         | `gemini-3.5-flash-lite` (budget floor)                                              | `gemini-3.5-flash`, `gemini-3.1-pro-preview`          | prior-gen when superseded                                                                                                         |
