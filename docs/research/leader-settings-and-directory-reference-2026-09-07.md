...
## 7. Sidebar, composer and chat page against ours (code on 2026-09-07)

Our chrome: the shared sidebar in `packages/ui/ui/src/sidebar/` mounted by two hosts (`WebChatPage.tsx` for chat, `WebAppShell.tsx` for Projects, Library and Schedules), the composer in `apps/web/features/chat/components/Composer/`, and the chat header in `WebChatPage.tsx`.

| surface                 | claude.ai / chatgpt.com                                                                                                                                                                                                    | ours today                                                                                                                                                                       | gap                                                                                   | priority |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | -------- |
| Sidebar nav             | Claude: New, Quick task, Projects, Artifacts, Scheduled, Customize. ChatGPT: New chat, Library, Scheduled, Plugins                                                                                                         | Chat, Projects, Library, Schedules, Admin (hideable)                                                                                                                             | Done; Customize and Plugins are settings links the account menu already reaches       | done     |
| Sidebar search          | One search dialog on every page                                                                                                                                                                                            | `GlobalSearchDialog` on the chat page (search route, filters, history) but the package's client-only `SearchOverlay` on the app shell because that host passes no `onOpenSearch` | Pass the global dialog from the app shell so both shells search the same way          | P2       |
| Recents                 | Claude: Filter and group recents (Type, Status, Last activity, Group by, Sort by, Reset). ChatGPT: Organize chats (In one list, By project)                                                                                | Organize chats (In one list, By project), temporal groups, pin first                                                                                                             | Done for the ChatGPT shape; Claude's filter popover is P3                             | P3       |
| Conversation row menu   | Claude: Open as quick task, Copy session ID, Pin, Mark as read, Rename, Add to project, Move to group, Archive, Delete. ChatGPT: Share, Rename, Pin chat, Archive, Delete, Move to project                                 | Share, Rename, Pin, Mark as read, Archive, Delete, Move to project                                                                                                               | Done. `onStar` is plumbed through three files but never rendered; delete the plumbing | P2       |
| Project row menu        | ChatGPT: Share, Rename, Project settings, Project home, Pin, Delete. Claude: Unpin, Edit details, Archive, Delete                                                                                                          | Share, Rename, Settings, Home, Pin, Delete                                                                                                                                       | Done                                                                                  | done     |
| Usage meter             | ChatGPT rail: credits, 5-hour and weekly pills open Usage                                                                                                                                                                  | Meter on the chat shell only; the app shell never passes it                                                                                                                      | Pass the usage props from the app shell                                               | P3       |
| Account menu            | Claude: Settings, Language, Get help, plan action, Get apps, Academy, API keys, Log out. ChatGPT: Upgrade, Personalization, Profile, Settings, Help, Log out                                                               | Settings, Get help, Send feedback, Keyboard shortcuts, Upgrade, Get apps, legal links, Log out                                                                                   | Done                                                                                  | done     |
| Phone drawer            | One drawer                                                                                                                                                                                                                 | Chat shell uses `MOBILE_NAV_DRAWER_WIDTH`; app shell hardcodes 280px                                                                                                             | One constant                                                                          | P3       |
| Composer face           | Claude: plus, model with effort, Dictate, Voice; Chat and Cowork. ChatGPT: plus, model and effort pill, dictation                                                                                                          | Plus, Chat and AGI Work, Style, model, effort, dictation, voice                                                                                                                  | Done (Style on the face is the founder's call)                                        | done     |
| Plus menu               | Claude: files, screenshot, Add to project, Add from GitHub, Skills, Add connector, Plugins, Research, Web search, Memory. ChatGPT: files, Add from library, Create image, Web search, Deep research, plugins, search field | Files, image, video, screenshot, working folder, Deep Research, Office files, Skills, Connectors, Plugins submenus, temporary chat                                               | Add "Add from library" (our Library exists) as a P3; the rest is done                 | P3       |
| Connector toggles       | One place per product                                                                                                                                                                                                      | Two: the plus menu Connectors submenu (checkbox rows) and `ComposerPluginsMenu` on the AGI Work bar (switch rows), both writing the same state                                   | Keep one presentation; make the AGI Work bar popover reuse the submenu rows           | P2       |
| Slash and mention menus | Claude "/" groups own skills then plugins. ChatGPT "/" is a command palette, "@" is the plugin palette                                                                                                                     | "/" grouped by owner and plugin (853c99577); "@" lists skills only                                                                                                               | Done; "@" for connectors is P3                                                        | P3       |
| Model picker and effort | Model list with descriptions, effort submenu or slider, more models                                                                                                                                                        | Auto, recommended, favourites, full catalogue, effort slider with plan gating                                                                                                    | Done                                                                                  | done     |
| Chat header             | Claude: title, share, files or artifacts sheet. ChatGPT: title menu, share                                                                                                                                                 | Title menu (Rename, Move to project, Share, Print, Export, Fork, Delete), Share, work session, sources and artifacts toggles                                                     | Done                                                                                  | done     |
| Message actions         | See `leader-ui-reference-2026-09-04.md` section on turns                                                                                                                                                                   | Edit, Copy, thumbs, task feedback, variants, Regenerate with model, Branch, more menu (read aloud, pin, report, tokens, delete)                                                  | Done                                                                                  | done     |

## 8. Build order additions

1. (P2) App shell search: pass the global search dialog from `WebAppShell.tsx` the way `WebChatPage.tsx` does, and delete the package `SearchOverlay` fallback if no consumer remains.
2. (P2) Delete the unreachable `onStar` plumbing from `SessionItem.tsx`, `Sidebar.tsx` and both hosts.
3. (P2) One connector toggle presentation: the AGI Work bar popover renders the same rows as the plus menu submenu.
4. (P3) Usage meter and one drawer-width constant on the app shell; "Add from library" in the plus menu; Claude's recents filter popover; "@" for connectors.
