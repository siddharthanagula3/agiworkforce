...
2. Capability wiring gaps (grounded, from the gap board): **URL fetch** tool (none exists), sandbox (E2B fallback metered), file-creation/generated-file byte pipeline on web (unblocks the PDF/image byte path), tool-call/result rendering polish. Deep-research follow-ups: OpenAI path has 0 sources on chat-completions (`web_search_preview` is Responses-only — pair with the at-GA Responses work), google `legacy-web` usage reconciliation now feasible (usage chunks flow post-CRLF-fix).
