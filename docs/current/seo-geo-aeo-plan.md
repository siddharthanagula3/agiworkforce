...
- **Uniqueness gate**: ≥1 comparison table with ≥6 rows of *substantive difference* and ≥120 words of original prose per page; reject if the table is >70% identical to another comparison.

### 3.2 Glossary pages — AI/agent terms
- **URL**: `/glossary/<term>` · **Hub**: `/glossary`
- **Unique value**: a direct one-sentence definition (AEO-ready), then how the term works *in AGI*, with a short example. Feeds FAQPage/DefinedTerm structured data.
...
- **Source data**: `packages/types/src/models.json` + provider capability metadata + `docs/current/byok-open-model-provider-strategy.md`.
