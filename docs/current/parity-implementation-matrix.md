...
| Local/BYOK/Managed are separate trust boundaries | All                    | Local never silently routes to BYOK/Managed. Local to BYOK is explicit fork with selection, scan, preview, label, and consent. Managed stays waitlist/private beta.                                               | `packages/types/src/suite-contracts.ts`, `apps/cli/src/agent/mod.rs`, `apps/mobile/stores/chat` |
...
| BYOK mode                 | W, D, M, CLI, VSC | AGI differentiator.                                                             | Provider key setup, encrypted storage where applicable, direct provider label, usage/cost disclosure, explicit Local-to-BYOK fork.                              | Partial: Desktop Models/Keys and mobile consent exist; naming currently confusing in places. |
| Managed cloud mode        | W, D, M, CLI, VSC | ChatGPT/Claude/Codex managed compute baseline.                                  | Waitlist/private-beta/invite only until ledgering/abuse/billing controls are proven. UI must not imply general availability.                                    | Gated.                                                                                       |
...
| Cloud mode onboarding | Waitlist, invite code, private-beta unlock, never public by default.                                                                                     | Gated/Partial.                                                                         |
...
Primary paths: `apps/web/app`, `apps/web/features`, `apps/web/core`, `apps/web/stores`, `services/api-gateway`, `supabase/migrations`.
...
| Cloud/local continuation | Codex cloud delegation/follow-up, AGI Local/BYOK/Managed boundaries.                           | Cloud tasks private-beta; local conversation can hand off only with preview/consent.                      | Partial/Gated.     |
...
Primary paths: `apps/web/features`, `apps/mobile/app/(app)/billing`, `services/api-gateway`, `packages/types/src/enterprise`, `supabase/migrations`.
