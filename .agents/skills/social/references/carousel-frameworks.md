...
Five slide-by-slide narrative architectures for carousels — Instagram carousels and LinkedIn document posts. Each framework is a different _reason to keep swiping_; picking the right one for the content matters more than polishing individual slides.
...
- **One visual template per carousel.** Same layout, type scale, and palette on every interior slide. Variety between slides reads as clutter; consistency lets the _content_ change while the frame stays still.
...
_SaaS example:_ "12 ChatGPT prompts for SaaS onboarding emails" → one prompt per slide with the use case → "Comment PROMPTS and I'll send the full doc."
...
The "I did X, here's the system" carousel. The hook is a _result stated as fact_, not advice — and the final slide shows the receipt, closing the loop slide 1 opened.

| Slide                   | Job                                                                               | Pattern                                         |
| ----------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------- |
| 1 — Hook                | A specific personal claim with a number. A result, not a tip.                     | "[I/we did X]. [Specific result, with number]." |
| 2 — Reframe the problem | Name what's _actually_ going wrong so the reader recognizes themselves.           | "The real problem — [named issue]"              |
...
_SaaS example:_ "We cut churn 22% with one onboarding email." → the real problem (activation, not price) → the sequence structure → the literal email copy → the retention chart.
...
_SaaS example:_ "90% of trial emails never get opened." → why (they read like receipts, not messages) → Hack #1: The founder-from line, Hack #2: The one-question subject, ... → "Trial emails are conversations, not confirmations. Save this for your next sequence."
...
The "someone had to say it" carousel. Personality-led and polarizing by design — the structure exists to keep the heat _fair_ so it reads as conviction, not bitterness.
...
| N−1 — Fairness pivot             | "Don't get me wrong…" — clarify what you're _not_ attacking. Anti-laziness, not anti-tool; anti-practice, not anti-person. | "The problem isn't [X], it's [Y]"         |
...
_SaaS example:_ "Your AI-generated LinkedIn posts are costing you customers." → the specifics (same em-dash cadence, same 'game-changer' vocabulary, zero lived detail) → "Don't get me wrong — I use AI daily. The problem isn't the tool, it's publishing the first draft." → "Write like you talk. — [Name]"
...
| 3 — Process overview     | The numbered step list, shown _before_ the detail. Seeing the whole path lowers mid-swipe drop-off — readers commit when they know how long the road is. | "[N] steps to [outcome] in [timeframe]"  |
...
_SaaS example:_ "From CSV to live dashboard in 4 steps" → the pain (weekly manual reporting) → the 4-step overview → one screenshot per step → the finished dashboard + "Built for marketers, not analysts."
