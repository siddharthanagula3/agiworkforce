...
- **Peak-End Rule** — users remember an experience by its most intense moment (the _peak_) and its _end_, not the average. Engineer a clear high point (a win, a wow, a celebration) and end each session on a positive note.
- **Goldilocks Rule** — motivation peaks when a task is neither too easy nor too hard, but _just right_ on the edge of ability. Tune early steps so they're achievable but not trivial.
