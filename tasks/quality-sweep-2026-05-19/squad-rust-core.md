...
3. The `check-no-hardcoded-models.sh` script (`ci.yml:72-77`) is wired for both Rust surfaces (CLI + Tauri) but its current rule set is narrow: Gate 1 matches the literal `claude-opus-4-6-mini` (ghost-model guard) and Gate 2 matches `const FAST_*_MODEL = "..."` / `const DEFAULT_*_MODEL = "..."` assignment patterns. It does **not** match `.unwrap_or("literal-model-id")` fallbacks — which is why Finding #1 at `agent/mod.rs:488` is live despite the script existing. The "rule-models-json exception" comment in agent/mod.rs is informal; either Wave 1 should extend the script with a Gate 3 covering `unwrap_or\("[a-z]+-[a-z0-9-]+"\)` patterns, or the exception should be formally tracked with a ticket and the gate's coverage scope explicitly documented.
