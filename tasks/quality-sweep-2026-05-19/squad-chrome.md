...
| 4   | P2       | Cross-file schema drift                                | Protocol schema drift             | No             | 1            | Extension sends `selected_text_query` with camelCase field `selectedText`; desktop Rust struct `SelectedTextQuery` expects `#[serde(rename = "selectedText")]` — this particular field matches. However, `tabId` (camelCase in extension) vs `tab_id` (snake_case in Rust `SelectedTextQuery`) is **not** covered by any `#[serde(rename)]` annotation on the desktop side. This means `tab_id` will deserialise as `None` for every context-menu "Ask AGI Workforce" invocation. See detail in Native Messaging section. |
